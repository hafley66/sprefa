...
| 2026-09-18 | `Result` is a prelude constructor beside `Option`: `(: Result (* (: ok type) (: error type) (: return type)))`; `(Result Body str)` interns one sum node per pair and two rules hang `ok` and `error` on it. | `prelude/0_constructors.dl7:6`, `prelude/2_constructor_rules.dl7` |
