...
| 2026-09-18 | `Result` is a prelude constructor beside `Option`: the declaration `(: Result (* (: ok type) (: error type) (: return type)))` is the constructor's signature, a product like every callable; the node `(Result Body str)` mints is a different node, a sum, and two rules hang its `ok` and `error` variant edges and its `sum` row on it. | `prelude/0_constructors.dl7:6`, `prelude/2_constructor_rules.dl7` |
