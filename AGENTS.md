...
| 2026-09-17 | Emitters are userland std modules: `@std/dl6` (sqlite_ivm app emitter; declares `relation`, `key` as interned nodes), later `@std/ts`, `@std/cli`. An emitter is rules over `:` rows plus `str.cons` for text, `doc` rows for comments, and `fs.write` as its one effect. `product` and `sum` stay kernel so an emitter reads columns flat by index, never through `cons`. | `_3_check/_5_kernel.rs:19-20`, `_2_declare.rs:280-288` |
