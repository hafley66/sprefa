...
| 2026-09-17 | `^` read-back is fork A: `str.cons` is a two-way kernel op and the caret macro matches `(str.cons "^" ?Name ?Text)`. Forks B (delimiter token) and C (`(^ x)` reader form) are footnotes. | `plans/v8/2026-09-17-caret-macro.findings.md` |
