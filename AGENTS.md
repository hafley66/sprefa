...
| 2026-09-17 | Hosted namespaces are short and name the idea, never the tool: `fs.` for files (uses git when a repo is present, plain disk otherwise), `git.` for refs and history, `oai.` for OpenAPI, `env.` for process facts, `http.` for requests. No user-facing `soopy` name. | `src/_9_runtime/_3_executors/{soopy_refs,soopy_history,repo_at}.rs` RELATION consts |
...
| 2026-09-18 | Round 0 cadence, from code: Once = `fs.json`, `fs.at`, `fetch_json`, `extract`, `git.history`; Continuing = `timer`, `git.refs`. `git.refs` stays runtime-only. Comptime serves a Once relation only when its companion relations are declared. | `src/_9_runtime/_3_executors/git_refs.rs:176`, `mod.rs` `once_executors` |
| 2026-09-18 | An effect is a type constructor: `(effect Input Output)`, `Input` a product (one of its rows is one request), `Output` a sum (one variant is one answer shape). A host callable is declared `(: json (effect (* (: url str)) (Result type str)))`. Served = every edge whose target interns `effect`; `dl8 eval --serve` retires. Companion `_error` products (`fs.json_error`, `at_error`, `fetch_json_error`) retire into the output sum. The request row is the input product's own row, derived or seeded by the program; retracting it cancels. Cadence is the output's key shape: `return` on the answer keyed by the input gives one answer per request; no such key gives a stream (`timer`). No `Once`/`Continuing` method in Rust. | `src/_9_runtime/_2_reconcile.rs:13-18` retires, `_3_executors/mod.rs:48`, `std/fs.dl7:16`, `_6_eval/_6_json.rs:513` |
| 2026-09-18 | `Result` is a function on types: `(: Result (* (: ok type) (: error type) (: return (+ (: ok ok) (: error error)))))`. The declaration is a product because every callable's signature is one; the `return` column is a sum whose branch targets name the input columns, resolved by the existing `scoped()` walk. `(Result Body str)` interns a node whose return sum has `ok = Body`, `error = str`; that substitution at intern is generics and is not built (`docs/generics-wrapper-inspection.md`). | `prelude/0_constructors.dl7:6` (`Option`, the precedent), `src/_2_lower/_10_index.rs:170`, `_8_express.rs:297` |
| 2026-09-18 | A constructor is a product with no rows. Apply is `intern`, free, from lower; no rule is needed for allocation. A rule is needed only to hang edges on the minted node. `intern` is `Symbol.for`, never `Symbol()`: same constructor and arguments give the same node in every round, which is why the compiler may run it. | `src/_2_lower/_8_express.rs:297,453`, `_3_check/_6_strata.rs` |
| 2026-09-18 | `fetch_json` is `http.json`; the namespace is `http.`. | `src/_9_runtime/_3_executors/fetch_json.rs` |
| 2026-09-18 | Comptime and runtime answer effects through one contract, the IVM shape: request row in, answer rows in, re-evaluate to stable. One answer step (`Answerer`) held by both loops. Comptime answers every effect whose input row it can derive; runtime echoes the same requests, a fetch at compile time and again at eval time is the accepted cost until answered rows travel in the compiled program. | PR #793, `src/_4_comptime/_2_rounds.rs`, `tests/_17_reconcile.rs:220` |
| 2026-09-18 | `key` is `dl6.key`, declared in `std/dl6.dl7`; `keyed_edge`, `key_predecessor`, `key_has_predecessor`, `key_rank`, `composite_key`, `program_key`, `program_key_position` move out of the prelude with it. The `:` row's own two key sets stay, a separate uniqueness concept. | `prelude/3_derived_rules.dl7:61-87`, `prelude/1_declarations.dl7:75-100,255-262` |
| 2026-09-18 | Caret 3, the lower half of `^`: the macro emits `(: Name T)` and `(: Name () body)`, the body one anonymous edge it never looks inside. A `()` label mints its index as its name at declare time. Dot lookup and bare-name lookup share `scoped()`: own labels first, then anonymous product-valued edges in index order (skipping rule nodes), then the parent; first hit wins; a second hit on the walk is a `shadowed_member` diagnostic row. `Name.1` is a plain label probe, no int-segment arm. Arity stays the edges inside `(* ...)`. Statics, currying, overloading, generics are footnotes, not this lane. Not a metaclass: the walk runs over rows present at compile time. | `src/_2_lower/_10_index.rs:170-193`, `_1_forms.rs:80-86`, `_3_check/_1_graph.rs:60`, `plans/v8/2026-09-18-re-edging-two-walks.design.md` |
