...
| 2026-09-18 | Supersedes the row above on one point: `[a b]` is a list literal, read as a bracket form and rewritten at macrotime to `(cons a (cons b (nil)))`, the `^` shape. Kernel unchanged. It exists so `(intern Option [?Target] ?Node)` is one inline goal; the prelude read-back rule pairs for `Option`, `Partial`, `Key` retire with it. | `src/_0_read/_2_reader.rs:372` (dot arm precedent), `macrotime/`, `prelude/2_constructor_rules.dl7` |
