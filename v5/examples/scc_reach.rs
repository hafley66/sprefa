...
fn verify_small() {
    ...
    assert_eq!(
        count_pairs(&c),
        naive_reach_count(&adj),
        "SCC == naive on example"
    );
    assert_eq!(count_pairs(&c), 16, "running example has 16 reaches pairs");
    assert_eq!(
        reaches_from(&c, id("main")).len(),
        4,
        "main reaches run,parse,lex,log"
    );
    assert!(
        reaches_from(&c, id("run")).contains(&id("run")),
        "run reaches itself (on the cycle)"
    );
...
fn main() {
    ...
    let db = args.get(1).cloned().unwrap_or_else(|| {
        eprintln!("usage: scc_reach <db> [reaches <fn>]");
        std::process::exit(1);
    });
    ...
    {
        let mut stmt = conn
            .prepare("SELECT caller, callee FROM rel_calls")
            .unwrap();
        let rows = stmt
    ...
    if args.get(2).map(|s| s.as_str()) == Some("reaches") {
        if let Some(fname) = args.get(3) {
            match intern.get(fname) {
                Some(&fid) => {
                    let tq = Instant::now();
                    let mut hit: Vec<&str> = reaches_from(&cond, fid)
                        .iter()
