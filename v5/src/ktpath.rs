...
pub fn plan_move(old_path: &str, new_path: &str, content: &str) -> Result<KotlinMove> {
    ...
    let Some(root) = source_root(old_path, &old_pkg) else {
        bail!(
            "{old_path}'s directory does not match its package {old_pkg} \
    ...
    let decls: Vec<String> = kotlin_decl_re()
        .captures_iter(&clean)
...
impl KotlinMove {
    ...
    pub fn rewrite_import(&self, spec: &str) -> Option<String> {
        ...
        if !self.decls.iter().any(|d| d == name) {
            return None;
        }
    ...
    pub fn old_wildcard(&self) -> String {
        format!("{}.*", self.old_pkg)
    }
}
...
mod tests {
    ...
    fn root_and_package_math() {
        assert_eq!(
            source_root("lib/com/lib/Util.kt", "com.lib").as_deref(),
            Some("lib")
        );
        assert_eq!(
            source_root("com/lib/Util.kt", "com.lib").as_deref(),
            Some("")
        );
        assert_eq!(source_root("weird/spot/Util.kt", "com.lib"), None);
        assert_eq!(
            package_for("lib/com/core/Util.kt", "lib").as_deref(),
            Some("com.core")
        );
    ...
    fn plan_rewrites_decl_imports_only() {
        ...
        assert_eq!(
            mv.rewrite_import("com.lib.Util").as_deref(),
            Some("com.core.Util")
        );
        assert_eq!(
            mv.rewrite_import("com.lib.Util.Inner").as_deref(),
            Some("com.core.Util.Inner")
        );
        assert_eq!(
            mv.rewrite_import("com.lib.helper").as_deref(),
            Some("com.core.helper")
        );
    ...
    fn plan_is_loud_when_layout_disagrees_with_package() {
        ...
        assert!(
            err.to_string().contains("does not match its package"),
            "{err}"
        );
    }
}
