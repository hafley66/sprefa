...
pub fn count_sys_path_mutators(
    ...
    files
        ...
        .filter(|content| {
            content.contains("sys.path.insert") || content.contains("sys.path.append")
        })
        .count()
}
...
fn parse_py_import_list(text: &str) -> Vec<(String, Option<String>)> {
    ...
    for item in inner.split(',') {
        ...
        match item.split_once(" as ") {
            Some((name, alias)) => {
                out.push((name.trim().to_string(), Some(alias.trim().to_string())))
            }
...
fn py_resolve_absolute(dotted: &str, files: &HashSet<String>, roots: &[String]) -> Resolution {
    ...
    match hits.len() {
        ...
        n if n >= 2 => Resolution::Unresolved(format!(
            "{dotted}: ambiguous across {n} candidate import roots: {hits:?}"
        )),
        _ => {
            ...
            let known = roots.iter().any(|root| {
                ...
                files
                    .iter()
                    .any(|f| f == &top_file || f.starts_with(&top_dir))
            });
            if known {
                Resolution::Unresolved(format!(
                    "{dotted}: no matching module/package under any candidate import root"
                ))
            } else {
...
fn py_resolve_relative(
    ...
    let mut comps: Vec<&str> = if base_dir.is_empty() {
        vec![]
    } else {
    ...
    if pops > comps.len() {
        return Resolution::Unresolved(format!(
            "{spec_text}: relative import escapes the scanned tree"
        ));
    }
...
impl ModuleResolver for PyResolver {
    ...
    fn edges(&self, file: &str, content: &str, cx: &ProjectCx) -> Vec<ModuleRef> {
        ...
        for c in py_from_import_re().captures_iter(&clean) {
            ...
            // targets.
            let module_bindings: Vec<(String, String, &'static str)> = names
                .iter()
                .map(|(name, alias)| {
                    let local = alias.clone().unwrap_or_else(|| name.clone());
