...
fn observe_ref_tracks_advances_and_projects_query_rels() {
    ...
    assert_eq!(adv, Some((None, c1.clone())), "first sight is an advance with old=None");
    // Re-observe with no commit: no advance.
    assert_eq!(eng.observe_ref(&slug, &d, "HEAD").unwrap(), None, "unchanged HEAD must not advance");
    ...
    assert_eq!(adv2, Some((Some(c1.clone()), c2.clone())), "second observe is c1 -> c2");

    // The changed file (tracked in `_file`) shows in the diff.
    let changed = eng.files_changed_between(&slug, &d, &c1, &c2).unwrap();
    assert_eq!(changed, vec!["src/a.rs".to_string()], "only the edited tracked file: {changed:?}");
    ...
    assert!(log.iter().any(|r| r[0].as_str() == Some(c1.as_str())
        && r[1].as_str() == Some(c2.as_str())), "c1->c2 row present: {log:?}");
}
...
fn daemon_rels_are_reserved() {
    ...
    fs::write(d.join("p.dl"), "rel head(a: text).\nhead(\"x\").\n? head(a).\n").unwrap();
    let conn = db::open(Some(d.join("db").to_str().unwrap())).unwrap();
    let mut eng = Engine::new(conn, d.clone());
    let (prog, _diags, _) = prepare_paths(&[d.join("p.dl")]).unwrap();
    let err = eng.tick(&prog, true).unwrap_err().to_string();
    assert!(err.contains("built-in daemon-state relation"), "reserved-name guard: {err}");
}
