...
fn fixture(tag: &str) -> (PathBuf, PathBuf) {
    ...
    let store = fs::canonicalize(std::env::temp_dir())
        .unwrap()
        .join(format!("notouch_store_{tag}"));
...
fn run(dir: &Path, store: Option<&Path>) -> String {
    ...
    cmd.arg(PROG)
        ...
        .current_dir(dir);
    if let Some(s) = store {
        cmd.env("SPREFA_CLAUDE_PROJECTS", s);
    }
...
fn agent_edit_inside_protected_ranges_diags_both_kinds() {
    ...
    assert!(
        diags
            .iter()
            .any(|r| r.get(1).map(|s| s == "6").unwrap_or(false)),
    ...
    assert!(
        diags
            .iter()
            .any(|r| r.get(1).map(|s| s == "10").unwrap_or(false)),
    ...
    assert!(
        !diags
            .iter()
            .any(|r| r.get(1).map(|s| s == "8").unwrap_or(false)),
...
fn no_agent_store_means_no_diag() {
    ...
    assert!(
        diag_block(&out).is_empty(),
        "no agent => no no-touch diag:\n{out}"
    );
}
...
fn reverting_guarded_file_to_head_clears_diag() {
    ...
    assert!(
        !diag_block(&first).is_empty(),
        "initial guarded edit must diag:\n{first}"
    );
    fs::write(d.join("doc.md"), base).unwrap();
    let reverted = run(&d, Some(&store));
    assert!(
        diag_block(&reverted).is_empty(),
        "byte-identical HEAD revert must clear diag:\n{reverted}"
    );
}
