...
fn rail_flags_unregistered_literal_name() {
    ...
    assert_eq!(
        code, 2,
        "rail fails --check on an unregistered magic name: out={out} err={err}"
    );
    let text = format!("{out}{err}");
    assert!(
        text.contains("magic-rel-unregistered"),
        "reports the ban code: {text}"
    );
    assert!(
        text.contains("totally_unregistered_rel"),
...
fn rail_ignores_test_gated_code() {
    ...
    assert_eq!(
        code, 0,
        "rail is green on a cfg(test)-gated magic name: {err}"
    );
}
...
fn rail_ignores_test_file() {
    ...
    assert_eq!(
        code, 0,
        "rail is green on an unregistered name in a *_tests.rs file: {err}"
    );
}
