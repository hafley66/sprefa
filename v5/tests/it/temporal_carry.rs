...
fn rel_count(db_path: &Path, rel: &str) -> i64 {
    ...
    conn.query_row(&format!("SELECT COUNT(*) FROM rel_{rel}"), [], |r| r.get(0))
        .unwrap()
}
...
fn ints(db_path: &Path, rel: &str, col: &str) -> Vec<i64> {
    ...
    let mut s = conn
        .prepare(&format!(
            "SELECT \"{col}\" FROM rel_{rel} ORDER BY \"{col}\""
        ))
        .unwrap();
    let v: Vec<i64> = s
        .query_map([], |r| r.get(0))
...
fn next_carry_lags_by_one_tick() {
    ...
    assert_eq!(
        rel_count(&dbp, "echo"),
        0,
        "echo must be empty on the first tick"
    );
    ...
    assert_eq!(
        ints(&dbp, "echo", "v"),
        vec![7],
        "echo must carry ping's tick-1 value"
    );
...
fn next_carry_accumulates() {
    ...
    assert_eq!(
        ints(&dbp, "acc", "v"),
        vec![1, 2],
        "acc carries the prior acc_next"
    );
}
...
fn carry_rel_must_be_next_only() {
    ...
    assert!(
        err.to_string().contains("written only by @next"),
        "expected a carry-conflict bail: {err}"
    );
}
