...
fn run_failing_with_row_max(dir: &PathBuf, prog: &str, row_max: usize) -> String {
    ...
    assert!(
        !out.status.success(),
        "diverging program unexpectedly succeeded"
    );
...
fn growing_recursive_component_hits_row_budget_loudly() {
    ...
    let stderr = run_failing_with_row_max(
        &dir,
        r#"
...
"#,
    ...
    assert!(
        stderr.contains("fixpoint row budget exceeded"),
        "missing budget bail:\n{stderr}"
    );
    assert!(
        stderr.contains("reach"),
        "missing recursive relation name:\n{stderr}"
    );
    assert!(
        stderr.contains("4 rows (limit 3)"),
        "missing promoted-row count:\n{stderr}"
    );
...
fn tick_chain(dir: &PathBuf, n: usize, naive: bool) -> (usize, Vec<String>) {
    ...
    let rows: Vec<String> = eng
        .query_sql("SELECT x FROM rel_reach_txt ORDER BY x", &[])
        .unwrap()
        .into_iter()
...
fn naive_rerun_counter_is_deterministic() {
    ...
    assert_eq!(
        count1, 8,
        "naive full re-runs for a 4-chain: 2 stmts x 4 extra passes"
    );
...
fn seminaive_eliminates_full_reruns_at_scale() {
    ...
    assert_eq!(
        naive_count,
        2 * n,
        "naive re-runs scale with fixpoint depth"
    );
    let budget = 20;
    assert!(
        naive_count > budget,
    ...
    assert_eq!(
        naive_rows, semi_rows,
        "row sets are identical across evaluators"
    );
...
"#;
    ...
    assert!(
        !edges.is_empty(),
        "df_edge must be non-empty for a 4-let-binding chain:\n{stdout}"
    );
    ...
    for e in &edges {
        assert!(
            reach.contains(e),
            "direct edge {e:?} must appear in reach:\n{stdout}"
        );
    }
    assert!(
        reach.len() > edges.len(),
...
fn stmt_ms_rows(dir: &PathBuf, n: usize, naive: bool) -> Vec<(String, i64, i64)> {
    ...
    eng.force_naive_fixpoint.set(naive);
    eng.tick(&prog, true).unwrap();
    eng.query_sql("SELECT rel, ms, n FROM _stmt_ms ORDER BY rel", &[])
        .unwrap()
        .into_iter()
        .map(|r| {
            (
...
fn stmt_ms_records_exact_statement_count_seminaive() {
    ...
    assert_eq!(
        *count, 30,
        "semi-naive 4-chain: 3 setup + 5 iters x 3 + 4 promotes x 3: {rows:?}"
    );
...
fn stmt_ms_records_exact_statement_count_naive() {
    ...
    assert_eq!(
        *count, 11,
        "naive 4-chain: 1 wipe + 2 stmts x 5 passes: {rows:?}"
    );
    assert!(*ms >= 0, "ms is a wall-time sum, never negative: {rows:?}");
}
...
"#;
    ...
    let rows: Vec<(String, i64)> = eng
        .query_sql(
            "SELECT rel, n FROM _stmt_ms WHERE rel LIKE '%:edge' ORDER BY rel",
            &[],
        )
        .unwrap()
        .into_iter()
