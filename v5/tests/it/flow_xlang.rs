...
fn spec_op_resolves_across_languages() {
    ...
    let ops: Vec<String> = eng
        .query_sql("SELECT \"op\" FROM rel_spec_op_txt", &[])
        .unwrap()
        .into_iter()
        .map(|r| s(&r[0]))
        .collect();
    assert!(
        ops.contains(&"getPet".to_string()),
        "spec_op missing getPet: {ops:?}"
    );
    ...
    let ts_client = rows
        .iter()
        .any(|(op, role, f)| op == "getPet" && role == "client" && f.ends_with("ts/app/pages.ts"));
