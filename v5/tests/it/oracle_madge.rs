...
fn madge_json(madge: &Path, dir: &Path, args: &[&str]) -> serde_json::Value {
    ...
    let out = Command::new(madge)
        .args(&full_args)
        .current_dir(dir)
        .env("NO_COLOR", "1")
        .env("FORCE_COLOR", "0")
        .output()
        .unwrap_or_else(|e| panic!("run madge {args:?}: {e}"));
...
fn madge_warnings(madge: &Path, dir: &Path) -> BTreeSet<String> {
    let out = Command::new(madge)
        .args(["--extensions", "ts,js", "--warning", "."])
        .env("NO_COLOR", "1")
        .env("FORCE_COLOR", "0")
        .current_dir(dir)
        .output()
        .expect("run madge --warning");
    let stdout = strip_ansi(&String::from_utf8_lossy(&out.stdout));
    let after = stdout.split("Skipped").nth(1).unwrap_or("");
    after
        .lines()
        .skip(1) // the "N files" remainder of the "Skipped" line
        .map(str::trim)
        .filter(|l| !l.is_empty())
        .map(String::from)
        .collect()
...
fn dl_rows(dir: &Path) -> std::collections::HashMap<String, Vec<Vec<String>>> {
    ...
    let out = Command::new(DL)
        ...
        .args([
            "--no-daemon",
            "--db",
            dir.join("madge.db").to_str().unwrap(),
        ])
        .current_dir(dir)
        .output()
        .expect("run dl");
    ...
    for line in stdout.lines() {
        ...
        if let Some(rest) = line.strip_prefix("? ") {
            cur = rest.split([' ', '(']).next().map(String::from);
            continue;
        }
        let t = line.trim();
...
fn madge_oracle_dep_graph_and_cycles_agree() {
    ...
    let ours: HashSet<(String, String)> = rows
        .get("dep")
        .cloned()
        .unwrap_or_default()
        .into_iter()
    ...
    eprintln!(
        "[oracle:madge] ours={} madge={} matched={}",
        ours.len(),
        truth.len(),
        matched.len()
    );
    ...
    assert_eq!(
        ours, truth,
        "dep graph must agree edge-for-edge on this fixture"
    );
    assert!(
        ours.contains(&("src/withjson.ts".to_string(), "src/data.json".to_string())),
    ...
    let our_cycle: BTreeSet<String> = rows
        .get("cycle_member")
        .cloned()
        .unwrap_or_default()
        .into_iter()
    ...
    assert!(
        our_cycle.contains("src/b.ts") && our_cycle.contains("src/c.ts"),
    ...
    let truth_orphans: BTreeSet<String> = truth_orphans
        .as_array()
        .into_iter()
        .flatten()
        .filter_map(|f| f.as_str().map(String::from))
        .collect();
    let orphans: BTreeSet<String> = rows
        .get("orphan")
        .cloned()
        .unwrap_or_default()
        .into_iter()
        ...
        .collect();
    assert_eq!(
        orphans, truth_orphans,
        "orphans must agree with madge --orphans"
    );
    assert!(
        orphans.contains("src/index.ts") && orphans.contains("src/d.ts"),
        "orphans: {orphans:?}"
    );
    assert!(
        !orphans.contains("src/data.json"),
    ...
    let leaves: BTreeSet<String> = rows
        .get("leaf")
        .cloned()
        .unwrap_or_default()
        .into_iter()
    ...
    assert_eq!(
        leaves, truth_leaves,
        "leaves must agree with madge --leaves"
    );
    assert!(
        leaves.contains("src/leaf.ts"),
        "src/leaf.ts is the fixture's dedicated leaf: {leaves:?}"
    );
    assert!(
        leaves.contains("src/data.json"),
    ...
    let summary: std::collections::HashMap<String, i64> = rows
        .get("summary")
        .cloned()
        .unwrap_or_default()
        .into_iter()
    ...
    assert_eq!(
        ...
        "summary counts must agree with madge --summary"
    );
    assert_eq!(
        summary["src/index.ts"], 3,
        "index.ts imports a, b, leaf: {summary:?}"
    );
    assert_eq!(
        summary["src/leaf.ts"], 0,
        "leaf.ts has zero deps, must still appear: {summary:?}"
    );
    assert_eq!(
        summary["src/withjson.ts"], 2,
        "withjson.ts imports data.json + a: {summary:?}"
    );
    assert_eq!(
        summary["src/data.json"], 0,
    ...
    assert!(
        truth_depends.len() >= 2,
        "fixture must give a.ts multiple direct dependents: {truth_depends:?}"
    );
    let depends: BTreeSet<String> = rows
        .get("depends_on")
        .cloned()
        .unwrap_or_default()
        .into_iter()
        .filter(|r| r.len() == 2 && r[0] == "src/a.ts")
        .map(|r| r[1].clone())
        .collect();
    assert_eq!(
        depends, truth_depends,
        "depends_on(src/a.ts, _) must agree with madge --depends src/a.ts"
    );
    ...
    let npm_deps: BTreeSet<String> = rows
        .get("npm_dep")
        .cloned()
        .unwrap_or_default()
        .into_iter()
        .filter(|r| r.len() == 2 && r[0] == "src/npmuser.ts")
        .map(|r| r[1].clone())
        .collect();
    assert_eq!(
        npm_deps,
        BTreeSet::from(["lodash".to_string()]),
        "npm_dep must see the bare lodash import: {npm_deps:?}"
    );
    assert!(
        !truth.contains(&("src/npmuser.ts".to_string(), "lodash".to_string())),
    ...
    assert_eq!(
        truth_warnings,
        BTreeSet::from(["./does-not-exist".to_string(), "lodash".to_string()]),
        "madge --warning ground truth drifted: {truth_warnings:?}"
    );
    let skipped: BTreeSet<String> = rows
        .get("skipped")
        .cloned()
        .unwrap_or_default()
        .into_iter()
