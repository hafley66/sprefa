...
fn toggle_flips_a_row_and_double_toggle_restores() {
    ...
    // First toggle mutes.
    assert!(
        eng.toggle_diag_mute("dl-directive").unwrap(),
        "first toggle -> muted"
    );
    ...
    assert!(
        !eng.toggle_diag_mute("dl-directive").unwrap(),
        "second toggle -> un-muted"
    );
    assert!(
        eng.muted_codes().unwrap().is_empty(),
        "double toggle restores empty"
    );
    ...
    assert_eq!(
        eng.muted_codes().unwrap().len(),
        2,
        "two distinct codes muted"
    );
    assert!(!eng.toggle_diag_mute("a").unwrap());
    let m = eng.muted_codes().unwrap();
...
fn diag_code_states_includes_muted_with_no_finding() {
    ...
    assert!(
        states.iter().any(|(c, m)| c == "orphan-code" && *m),
...
fn check_output_is_unchanged_with_a_mute_row_present() {
    ...
    assert!(
        baseline.contains("broken-import"),
        "baseline reports broken-import:\n{baseline}"
    );
    ...
    // still-valid digest, would find nothing to report.)
    {
        ...
        assert!(
            eng.muted_codes().unwrap().contains("broken-import"),
            "mute seeded"
        );
    }
...
fn execute_command_mutes_a_code_and_republishes_without_it() {
    ...
    assert!(
        codes_for(&pubs, "src/a.rs").contains(&"chatty".to_string()),
        "cold publish carries chatty: {}",
    ...
    assert!(
        !codes_for(&pubs, "src/a.rs").contains(&"chatty".to_string()),
        "after mute, chatty is filtered from the republish: {:?}",
        codes_for(&pubs, "src/a.rs")
    );
