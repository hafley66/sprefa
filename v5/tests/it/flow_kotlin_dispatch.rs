...
fn build_index() -> Index {
    ...
    Index { documents: vec![api, imp, http], ..Default::default() }
}
...
fn interface_dispatch_finds_function_on_two_op_paths() {
    ...
    assert!(ops.iter().any(|r| r == &["getPet"]), "spec_op getPet: {ops:?}\n{stdout}");
    assert!(ops.iter().any(|r| r == &["createPet"]), "spec_op createPet: {ops:?}");

    // each op's entry is the INTERFACE method (the iface target of scip_impl).
    let entries = query_block(&stdout, "? op_entry");
    assert!(entries.iter().any(|r| r == &["getPet", "scip-java maven . . api/PetApi#getPet()."]),
    ...
    assert!(multi.iter().any(|r| r == &["scip-java maven . . api/HttpClient#exec()."]),
        "HttpClient#exec is on both op paths: {multi:?}\n{stdout}");
    // the per-op impl override is on exactly one path, so it is NOT multi_op.
    assert!(!multi.iter().any(|r| r.first() == Some(&"scip-java maven . . api/PetApiImpl#getPet().")),
