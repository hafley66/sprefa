...
fn count(eng: &Engine, rel: &str) -> i64 { eng.count_rows(rel).unwrap_or(-1) }
...
fn fixture(tag: &str, n: usize) -> PathBuf {
    ...
    for i in 0..n {
        fs::write(dir.join("src").join(format!("m{i}.rs")), format!("fn f{i}() {{}}\n")).unwrap();
        git(&dir, &["add", "-A"]);
...
fn created_tick_has_no_n1() {
    ...
    assert!(rows > 64, "fixture must exceed the N+1 threshold (got {rows})");
...
fn created_retick_is_noop() {
    ...
    assert!(first > 0, "created populated on the cold tick (got {first})");

    // Re-tick an unchanged repo: append-only history -> stored-set early-out.
    eng.tick(&prog, true).unwrap();
    assert_eq!(count(&eng, "created"), first, "re-tick changed the created set");
    assert!(eng.last_n1.is_none(), "re-tick tripped N+1: {:?}", eng.last_n1);
}
