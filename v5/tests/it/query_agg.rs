...
fn query_json_group_array_grouped_and_valid_json() {
    ...
    for row in &rows {
        ...
        assert!(
            arr.is_array(),
            "json_group_array yields an array: {}",
            row[1]
        );
        if row[0] == "food" {
...
fn query_whole_rel_aggregate_one_row() {
    ...
    assert_eq!(
        rows,
        vec![vec!["16".to_string()]],
        "one row, total price: {out}"
    );
}
...
fn query_json_group_object_two_column() {
    ...
    for row in &rows {
        ...
        assert!(
            obj.is_object(),
            "json_group_object yields an object: {}",
            row[1]
        );
        if row[0] == "food" {
...
"#;
    ...
    assert!(
        combined.contains("query `pair` failed"),
        "query reported as failed: {combined}"
    );
    assert!(
        combined.contains("consumes two columns") && combined.contains("put _ at the value column"),
...
"#;
    ...
    let combined = format!("{out}{err}");
    assert!(
        combined.contains("query `raw` failed"),
        "query reported as failed: {combined}"
    );
    assert!(
        combined.contains("function call not supported in a query head"),
        "existing message: {combined}"
    );
}
...
fn query_aggregate_over_daemon() {
    ...
    fs::write(
        dir.join("p.dl"),
        format!("{SALES}? sale(dept, count(n), _).\n"),
    )
    .unwrap();
    ...
    let sale = results
        .iter()
        .find(|r| r["rel"] == "sale")
        .expect("sale query result");
    let cols: Vec<String> = sale["columns"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert_eq!(
        cols,
        vec!["dept".to_string(), "n".to_string()],
        "aggregate header: {resp}"
    );
    let mut got: Vec<(String, i64)> = sale["rows"]
        .as_array()
        .unwrap()
        .iter()
        .map(|row| {
    ...
    assert_eq!(
        got,
        vec![("food".to_string(), 2), ("toy".to_string(), 1)],
        "grouped counts over daemon: {resp}"
    );
