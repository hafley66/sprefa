...
"#;
    ...
    // Live values: Tok is the unambiguous top (30 after the `Tok::ThinArrow`
    // effect-output token). Below it the counts sit in a tight band that drifts as src/
    ...
    assert_eq!(get("Tok"), 30, "Tok fan-out drifted again: {out}");
