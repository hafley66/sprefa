...
pub enum AnyDataNode {
    Json(super::_1_json::JsonNode),
    Yaml(super::_2_yaml::YamlNode),
    Toml(super::_3_toml::TomlNode),
}
...
pub fn parse_by_ext(ext: &str, src: Arc<Bytes>) -> Result<AnyDataNode, ParseError> {
    match ext {
        "json" => super::_1_json::JsonNode::parse(src).map(AnyDataNode::Json),
        "yaml" | "yml" => super::_2_yaml::YamlNode::parse(src).map(AnyDataNode::Yaml),
        "toml" => super::_3_toml::TomlNode::parse(src).map(AnyDataNode::Toml),
