...
// ---------------------------------------------------------------------------
// scan_partial — partial-evaluation driver for completion
// ---------------------------------------------------------------------------

/// Drive a single wildcard op with a minimal seed stream and collect every
/// cursor it emits.
///
/// Used by `DocSession::completions_at` to answer "what values does this op
/// slot accept" without asking the op for a separate completion API. The
/// framework treats the in-progress op as a wildcard instance, runs it, and
/// harvests witnesses from the output — same machinery the full pipeline
/// uses, stopped one op deep.
///
/// Seed rules: emits one cursor per `(repo, rev)` pair from `config`. When
/// either list is empty, one cursor with placeholder values is emitted so
/// ops like `repo` can still seed from elsewhere (config.repos for repo, for
/// instance — its `pipe` reads the config independently).
///
/// No ResultStore writes. No scan-loop growth. No xrefs. This is a one-shot
/// read that later gets cached by the caller keyed on upstream fingerprint,
/// and eventually moves behind an RPC to a daemon Runner.
pub fn scan_partial(
    wildcard_op: Arc<dyn crate::_5_op::Op>,
    reader:      Arc<dyn crate::_3_reader::Reader>,
    config:      Arc<Config>,
) -> Vec<crate::_0_types::Cursor> {
    use futures::executor::block_on;
    use futures_util::stream::StreamExt;
    use std::collections::HashSet;
    use std::sync::Mutex;

    use crate::_0_types::{Cursor, FilePath, OpId, RunId, SprfPath};
    use crate::_5_op::{DiagSink, EventSink, OpCtx};
    use crate::writers::MemWriter;

    let seeds: Vec<Cursor> = seed_cursors(&config);

    let (result_store, xref_seen) = OpCtx::fresh_xref_state();
    let _discard_diags: Arc<Mutex<Vec<Box<dyn Diagnostic>>>> = Arc::new(Mutex::new(Vec::new()));
    let _discard_ref = _discard_diags.clone();
    let ctx = OpCtx {
        run_id: RunId(0),
        op_id:  OpId(0),
        reader,
        ...
        diags:  DiagSink(Arc::new(move |d| {
            _discard_ref.lock().unwrap().push(d);
        })),
    ...
    let input: futures_core::stream::BoxStream<'static, Cursor> =
        futures_util::stream::iter(seeds).boxed();
    let out: Vec<Cursor> = block_on(wildcard_op.pipe(input, ctx).collect());

    // Silence unused bindings that exist only to keep the DiagSink alive.
    let _ = (FilePath(Arc::from(Path::new("x"))), SprfPath(Arc::from(Vec::new().into_boxed_slice())), HashSet::<()>::new());
    out
}

/// Build seed cursors for `scan_partial`. One cursor per (repo, rev) pair.
/// Empty lists on either axis fall back to a single placeholder so ops that
/// independently enumerate (e.g. `repo` reading `config.repos` directly in
/// its `pipe`) still receive an input to fan out from.
fn seed_cursors(config: &Config) -> Vec<crate::_0_types::Cursor> {
    use crate::_0_types::{Cursor, RunId, SprfPath};
    let placeholder: Arc<str> = Arc::from("");
    let repos: Vec<Arc<str>> = if config.repos.is_empty() {
        vec![placeholder.clone()]
    } else { config.repos.clone() };
    let revs: Vec<Arc<str>> = if config.revs.is_empty() {
        vec![placeholder.clone()]
    } else { config.revs.clone() };
    let mut out = Vec::with_capacity(repos.len() * revs.len());
    for r in &repos {
        for v in &revs {
            out.push(Cursor {
                run_id:   RunId(0),
                ...
                fks:      Default::default(),
                path:     SprfPath(Arc::from(Vec::new().into_boxed_slice())),
                evidence: Vec::new(),
                content:  None,
            });
        }
