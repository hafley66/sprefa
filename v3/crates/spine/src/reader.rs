//! Reader trait — the read-side boundary of the runtime.
//!
//! Every byte-reading op passes through here. The Reader abstracts away
//! whether a file's content lives in a git blob on disk, in a worktree
//! overlay, in a dirty editor buffer, or in a test-only in-memory map.
//!
//! **Who implements:** `GitBlobReader` (sprefa::readers::git),
//! `MemReader` (sprefa::readers::mem), `BufferOverlay` + `ParseCacheReader`.
//! LSP stacks them: ParseCache(BufferOverlay(GitBlob)).
//! **Who consumes:** every `Op::pipe` that needs bytes. Accessed via
//! `OpCtx.reader: Arc<dyn Reader>`.
//! **Cardinality:** one live Reader stack per run. `bytes` / `parsed`
//! calls can reach millions per run in a big scan.
//! **Stream-shaped:** every method returns `BoxStream` for one trait
//! signature across one-shot (`bytes`, `config`) and streaming (`files`,
//! `repos`) cases. Callers read the first item and drop the stream.
...
use crate::types::{FileId, FilePath};
...
///
/// Bounded set: Json / Toml / Yaml parse to `*DataNode` structures;
/// TsAst / RsAst parse to `ast_grep_core::AstGrep<StrDoc<SupportLang>>`.
///
/// **Cardinality:** five variants. Passed by value everywhere.
/// **When new ones arrive:** adding a language means adding a variant
/// here, plus a parser impl in sprefa's reader stack, plus the
/// downcast target type.
...
///
/// **Cardinality:** one per (file, parser-kind) pair actually asked
/// for. Cached in `ParseCacheReader` keyed by blob oid (or content
/// hash fallback), so repeated reads of the same file hit the cache.
/// **Allocation:** heavy — parsing a file takes N ms and allocates the
/// whole parse tree. That's why caching is the dominant win.
#[derive(Debug)]
...
///
/// **Cardinality:** three variants. Used as a scalar filter.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
///
/// **Cardinality:** one per distinct unscanned combo. Can reach tens of
/// thousands between scan-check passes on a large workspace.
/// **Allocation:** tiny — three Arc-shared strings. Live only long
/// enough for the scan loop to consume.
#[derive(Debug, Clone)]
...
///
/// **Cardinality:** one per cross-rule match. Can be heavy on
/// high-fan-out xrefs — `xref_cartesian_limit` in `RuntimeConfig`
/// bounds it.
#[derive(Debug, Clone)]
...
///
/// **Cardinality:** one per violation. Zero in a passing scan; grows
/// with failure count.
#[derive(Debug, Clone)]
