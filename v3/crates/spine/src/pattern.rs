...
//!
//! Ported from v1 `crates/rules/src/pattern.rs`. Logic is identical to
//! v1; internal `HashMap` uses std here because these are short-lived
//! scratch maps inside the recursive matcher.
//!
//! Shared by the walker (`walk::`) and top-level ops (`repo`, `rev`,
//! `fs`) so every pattern slot in the language uses one matcher:
//!   - `re:` regex
//!   - `$NAME` segment capture
//!   - glob with `|` alternation, `**` recursive, `?`, `[abc]`, `{a,b}`
//!     (all via `globset`)
//!
//! **Cardinality:** every pattern string in a .sprf file compiles to
//! one `CompiledPattern`, typically tens to hundreds per program.
//! **Hot path:** matchers are called per cursor per pattern check —
//! millions of `is_match` calls per scan. Compilation is one-shot at
//! lower time; runtime cost is dominated by the underlying
//! globset/regex engines.
...
///
/// **Cardinality:** a Vec of these per `SegmentCapture` matcher —
/// typically 2-6 segments per pattern.
/// **Allocation:** once at pattern compile; the `String` payloads own
/// their bytes (small, pattern-literal sized).
#[derive(Debug, Clone)]
pub enum Segment {
...
///
/// **Cardinality:** one per pattern literal in a .sprf file. A
/// `|`-alternation turns into multiple `Glob` variants inside one
/// `CompiledPattern.matchers` Vec.
/// **Hot path:** `is_match` called on every filename / rev name /
/// capture value evaluated against this pattern.
#[derive(Clone)]
...
///
/// **Wraps:** the Arc<str> pattern source + the compiled matcher set.
/// Keeping the source Arc lets downstream code show "which pattern
/// matched" in diagnostics and hover.
/// **Cardinality:** one per pattern slot in the language — O(tens) per
/// program. Held inside `RepoOp`, `RevOp`, `FsOp`, walker keys.
/// **Allocation:** one-shot at op lower time. Cloning a
/// `CompiledPattern` is cheap (Arc + Vec<PatternMatcher> clone).
#[derive(Clone)]
