//! `host_parse` and friends — source bytes in, AST out.
//!
//! The heart of this crate. Tokenizes, classifies (`TokenClass`), scans
//! balanced braces, emits `Pipe` trees of `OpInvocation`. No I/O, no
//! shared state, no threading. One `host_parse(src, file_path) -> ...`
//! call is idempotent and allocation-only.
//!
//! Two parse doors, one runner (see `project_v2_parse_modes` memory):
//!   - `host_parse`            — strict. Errors bubble as `ParseError`.
//!                                CLI path.
//!   - `host_parse_tolerant`   — recovers past errors, accumulates diags.
//!                                LSP path. Same tree shape, partial OK.
//!
//! Who calls into this module:
//!   - `sprefa::registry` — `lower_rules` runs `host_parse` over a
//!     source file, then walks every `Pipe` to drive per-op lowering.
//!   - `sprefa::ops::rule::parse` — reparses a rule's brace body with
//!     `host_parse_arm_brace_abs` to pick up nested ops.
//!   - `sprefa::ops::{repo, rev, fs}` — each imports `classify_token` /
//!     `TokenClass` to validate their atom/capture args.
//!   - `sprefa::analysis` — tolerant parse for hover/completion on a
//!     partially-broken buffer.
//!   - `sprefa::server::run` — strict parse in the server's CLI mode,
//!     tolerant in the LSP layer.
//!
//! Invariants to preserve:
//!   - No allocation per-byte; `scan_balanced` is O(n) over source.
//!   - All byte ranges in emitted AST are absolute over the original
//!     source (not relative to a slot body).
//!   - Parse errors carry a `ParseSite`; LSP surfaces them as-is.
...
use crate::ast::{BraceSlot, BracketSlot, CrossRefOccurrence, OpInvocation, ParenSlot};
...
/// `:name` atom literal. v3 value type — the only shape in which a bare
/// identifier reaches an op. No unprefixed idents are values.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AtomRef { pub name: Arc<str> }
...
pub fn parse_atom(s: &str) -> Option<AtomRef> {
    let rest = s.strip_prefix(':')?;
    if rest.is_empty() { return None; }
    if !is_ident_start(rest.as_bytes()[0]) { return None; }
    if !rest.bytes().all(is_ident_byte) { return None; }
    Some(AtomRef { name: Arc::from(rest) })
}
...
pub fn classify_token(s: &str) -> TokenClass {
    ...
    if let Some(a) = parse_atom(s)         { return TokenClass::Atom(a); }
    TokenClass::Literal
}
