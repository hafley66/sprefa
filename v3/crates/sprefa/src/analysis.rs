...
use crate::types::{Cursor, CursorExpr, ParseSite, RunId, Severity};
use crate::diagnostic::{Diagnostic, Renderer};
use crate::op::{
    CompletionCtx, CompletionSuggestion, Op, ParenRole,
    Pipeline, ProgramCtx,
};
use crate::op::OpInvocation;
use crate::init_cursors::{collect_run_report, init_cursors, InitInputs};
use crate::parse::{host_parse_tolerant, host_parse_arm_brace, parse_capture, parse_cross_ref, Pipe};
...
impl DocSession {
    ...
    pub fn run_pipelines(
        ...
        let writer: Arc<dyn crate::writer::Writer> = Arc::new(MemWriter::new());
        let store:  Arc<dyn crate::store::Store>      = Arc::new(crate::store::NoopStore::new());
        let shared_result_store = Arc::new(crate::result_store::ResultStore::new());
        ...
        let reader_dyn: Arc<dyn crate::reader::Reader> = self.reader.clone();
        let cancel = tokio_util::sync::CancellationToken::new();
        let (mtx, _mrx) = tokio::sync::mpsc::channel::<crate::mutations::MutationRequest>(32);

        let result = crate::scan_loop::run_scan_loop(
            shared_result_store.clone(),
            initial_config,
            crate::scan_loop::DEFAULT_DEPTH,
            |config, shared_store| {
    ...
    pub fn completions_at(&mut self, pos: usize) -> Vec<CompletionSuggestion> {
        ...
        let reader: Arc<dyn crate::reader::Reader> = self.reader.clone();
    ...
    fn complete_head(
        &self,
        config: Arc<Config>,
        reader: Arc<dyn crate::reader::Reader>,
    ...
    fn complete_in_paren(
        ...
        pos: usize,
        _config: Arc<Config>,
        _reader: Arc<dyn crate::reader::Reader>,
        ...
        let rewritten_body = crate::pipeline_rewrite::truncate_and_substitute(
            body,
            &adapted_path,
            wildcard_op.clone(),
        );
        ...
        let rewritten_pipeline = Pipeline::Op(
            crate::op::LoweredOp::bare(new_rule_op),
        );
    ...
    fn reparse(&mut self, source: String) {
        ...
        if let crate::dag::RuleDag::Cycle { path } =
            crate::dag::build(&outcome.pctx.rules)
        {
...
impl Diagnostic for ParseDiagWrapper {
    ...
    fn render(&self, out: &mut dyn crate::diagnostic::Renderer) {
