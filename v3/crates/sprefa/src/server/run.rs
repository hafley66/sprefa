...
use crate::types::{CursorExpr, RunEvent, RunId};
use crate::init_cursors::{init_cursors, InitInputs};
use crate::parse::host_parse;
use crate::registry::lower_rules;
...
pub async fn run_pipeline(state: Arc<ServerState>, req: RunRequest) -> RunStart {
    ...
    let reader: Arc<dyn crate::reader::Reader> = {
    ...
    let stream: BoxStream<'static, RunEvent> = if timing {
        ...
        stream.inspect(move |ev| {
            ...
            RunEvent::Done => {
                let (pn, pns, pb) = crate::readers::parse_cache::parse_stats_snapshot();
...
fn build_expr_levels(
    exprs: &[CursorExpr],
    rules: &std::collections::HashMap<Arc<str>, crate::op::RuleHandle>,
) -> Vec<Vec<usize>> {
    use crate::dag;
    ...
    match dag::build(rules) {
        dag::RuleDag::Cycle { .. } => {
            (0..exprs.len()).map(|i| vec![i]).collect()
        }
        dag::RuleDag::Ordered { mut levels } => {
...
struct WithHandler<S> {
    inner:   S,
    _guard:  crate::task_guard::TaskGuard,
...
use crate::types::{ParseSite, Severity};
