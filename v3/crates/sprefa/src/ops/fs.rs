...
use crate::parallel::rayon_map;

use crate::types::{Capture, Cursor, FilePath, ParseSite, Tri};
...
use crate::parse::{classify_token, TokenClass};
    ...
    fn hover_match(&self, site: &crate::types::ParseSite, cursors: &[Cursor]) -> Option<String> {
        ...
        for c in cursors {
            ...
            let line = if single_rev {
                format!("- {}", crate::op::path_link_or_code(&path))
            } else {
                format!("- {} (rev: {})", crate::op::path_link_or_code(&path), c.rev)
            };
            lines.push(line);
...
impl Diagnostic for FsDiag {
    ...
    fn severity(&self) -> crate::types::Severity {
        match self {
            FsDiag::NoMatch { .. } => crate::types::Severity::Warn,
            _                      => crate::types::Severity::Error,
        }
    }
...
mod tests {
    ...
    async fn no_match_emits_warn_diag() {
        ...
        let reader: Arc<dyn crate::reader::Reader + Send + Sync> = Arc::new(
        ...
        let diags = DiagSink(Arc::new(move |d: Box<dyn crate::diagnostic::Diagnostic>| {
    ...
    fn base_cursor_hover(rev: &str, fs: Option<&str>) -> Cursor {
        Cursor {
            run_id: crate::types::RunId(0),
            repo:   Arc::from("org/repo"),
            rev:    Arc::from(rev),
            fs:     fs.map(|p| crate::types::FilePath(Arc::from(Path::new(p)))),
            ..Default::default()
        }
    }
    ...
    async fn collect_paths(
        op: FsOp,
        reader: Arc<dyn crate::reader::Reader + Send + Sync>,
    ) -> Vec<String> {
        let config = make_config();
        let writer: Arc<dyn crate::writer::Writer + Send + Sync> =
            Arc::new(MemWriter::new());
    ...
    fn mem_reader_with_files(paths: &[&str])
        -> Arc<dyn crate::reader::Reader + Send + Sync>
    {
        let config = make_config();
        Arc::new(
