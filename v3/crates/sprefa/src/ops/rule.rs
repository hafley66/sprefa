...
use crate::types::{Cursor, ParseSeg, ParseSite};
use crate::diagnostic::{Diagnostic, Renderer};
use crate::writer::{ColumnKind, ColumnSpec, ExtractionRow, RuleTableSpec};
use crate::op::{
    BraceMode, CompletionItem, GrammarRef, Op, OpCtx, OpInvocation, Operator, Pipeline,
    ProgramCtx, RuleHandle,
};
use crate::op::ForkBranch;
use crate::parse::Pipe;
use crate::registry::lower_chain;
...
impl Operator for RuleFactory {
        ...
        -> Result<Pipeline, Vec<Box<dyn Diagnostic>>>
    {
        ...
        let body_pipes: Vec<Pipe> = crate::parse::host_parse_arm_brace_abs(
            &brace.src,
...
impl Op for RuleOp {
    ...
    fn body_pipeline(&self) -> Option<&crate::op::Pipeline> { Some(&self.body) }
    ...
    fn hover_capture(&self, cap: &str, cursors: &[crate::types::Cursor]) -> Option<String> {
        ...
        -> BoxStream<'static, Arc<[Cursor]>>
    {
        ...
        // v3: rule is a named Pipeline. Seed is a single empty cursor;
        // the body owns its own enumeration (repo/rev/fs/... ops). Runner-fed
        // input is ignored — a rule call as a stage in a chain re-seeds.
        //
        // The rule op's remaining responsibility is manipulating the global
        // sprf path table: at parse time it derives a `RuleTableSpec` from
        // body captures; at run time it snapshots each emitted cursor's
        // captures into `result_store` under the rule name (the xref-
        // addressable table) and into the `writer` (SQL row form).
        let run_id = ctx.run_id;
        let seed_batch: Arc<[Cursor]> = Arc::from(
            vec![Cursor { run_id, ..Default::default() }].into_boxed_slice()
        );
...
/// v3: rule name is an atom. `rule(:my_rule)` — the leading `:` is
/// required. Unprefixed identifiers are not a value type in v3. This op
/// delegates to the language-core token classifier (`classify_token`)
/// and extracts the atom name; any other token class is a diagnostic.
fn rule_name(inv: &OpInvocation) -> Result<Arc<str>, Box<dyn Diagnostic>> {
    ...
    match crate::parse::classify_token(raw) {
        crate::parse::TokenClass::Atom(a) => Ok(a.name),
        _ => Err(Box::new(RuleDiag::NameMustBeAtom {
            site: (*inv.parse_site).clone(),
            got:  Arc::from(raw),
        })),
    }
...
enum RuleDiag {
    ...
    NameMustBeAtom { site: ParseSite, got: Arc<str> },
}
...
impl Diagnostic for RuleDiag {
    fn code(&self) -> &str {
        match self {
            ...
            RuleDiag::NameMustBeAtom { .. } => "rule/name-must-be-atom",
        }
    }
    fn severity(&self) -> crate::types::Severity { crate::types::Severity::Error }
    fn primary(&self) -> &ParseSite {
        match self {
            ...
            RuleDiag::NameMustBeAtom { site, .. }    => site,
        }
    }
    fn render(&self, out: &mut dyn Renderer) {
        match self {
            ...
            RuleDiag::NameMustBeAtom { site, got } => {
                out.header(self.code(), self.severity(),
                    &format!("rule name must be an atom (e.g. `:{got}`); got `{got}`"));
                out.primary(site);
            }
        }
    }
...
mod tests {
    ...
    use crate::registry::{lower_rules, OperatorRegistry};
    use crate::op::ProgramCtx;
    use crate::parse::host_parse;
