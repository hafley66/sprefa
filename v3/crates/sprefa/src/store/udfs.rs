//! Scalar UDFs registered on every sqlite connection.
//!
//! Ported verbatim from `crates/schema/src/udfs.rs` with one change: the
//! `sprf_norm` body inlines the normalize function instead of calling
//! `sprefa_config::normalize` (v2 has no config crate dependency).
//!
//! The five UDFs (`re_extract`, `split_part`, `fzy_score`, `sprf_norm`,
//! plus `repo_name` / `file_path` carried from v1) are registered through
//! raw `sqlite3_create_function_v2` calls. `register_all` is invoked from
//! the sqlx `after_connect` hook on each pool connection.
...
// ---------------------------------------------------------------------------
// fzy_score(haystack, needle) -> REAL
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// sprf_norm(text) -> TEXT
// Strip non-ASCII-alphanumeric, lowercase. Must match the in-process
// normalize used at ingest so cross-repo joins line up.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// repo_name(repo_id) / file_path(file_id)
// ---------------------------------------------------------------------------
...
unsafe extern "C" fn xfunc_repo_name(
    ...
    lookup_by_id(ctx, argc, argv, b"SELECT name FROM repos WHERE id = ?\0");
}
...
unsafe extern "C" fn xfunc_file_path(
    ...
    lookup_by_id(ctx, argc, argv, b"SELECT path FROM files WHERE id = ?\0");
}

unsafe fn lookup_by_id(
    ctx: *mut ffi::sqlite3_context,
    argc: c_int,
    argv: *mut *mut ffi::sqlite3_value,
    sql: &[u8],
...
#[cfg(test)]
mod tests {
    use super::{fzy_score_impl, normalize};
    ...
    #[test]
    fn fzy_subsequence() {
        let s = fzy_score_impl("hello world", "hwd");
        assert!(s > 0.0 && s < 1.0);
    }
