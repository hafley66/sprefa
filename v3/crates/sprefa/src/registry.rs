...
use crate::op::{BraceMode, ForkBranch, Operator, OpInvocation, Pipeline, ProgramCtx};
use crate::parse::{scan_slot_scan_pointers, Pipe, ScanPointerOccurrence};
            ...
            Ok(mut p) => {
                ...
                if op.brace_mode() == BraceMode::DefaultFork {
                    if let Some(brace) = &inv.brace_src {
                        match crate::parse::host_parse_arm_brace_abs(
                            &brace.src,
...
mod tests {
    use super::*;
    use crate::op::{GrammarRef, ScanPointer};
    ...
    fn site_at(range: std::ops::Range<usize>) -> Arc<crate::types::ParseSite> {
        use std::path::Path;
        Arc::new(crate::types::ParseSite {
