...
//!
//! Covers (post-stream-runner port):
...
//!   5. Object-safe Op storage in Pipeline::Op(Arc<dyn Op>).
...
impl Op for StampLenOp {
    ...
    fn pipe<'a>(&'a self, _ctx: &'a RtCtx, batch: Arc<[Cursor]>) -> BoxFuture<'a, Arc<[Cursor]>> {
        Box::pin(async move {
            let mut out: Vec<Cursor> = batch.iter().cloned().collect();
            for c in &mut out {
                let n = c.active().len();
...
impl Op for HalveOp {
    ...
    fn pipe<'a>(&'a self, _ctx: &'a RtCtx, batch: Arc<[Cursor]>) -> BoxFuture<'a, Arc<[Cursor]>> {
        Box::pin(async move {
            let mut out: Vec<Cursor> = Vec::with_capacity(batch.len() * 2);
            for c in batch.iter() {
                let n = c.active().len();
                let mid = n / 2;
...
fn upstream_of(cs: Vec<Cursor>) -> BoxStream<'static, Arc<[Cursor]>> {
    Box::pin(stream::iter(vec![Arc::<[Cursor]>::from(cs)]))
}

async fn run_collect(pipe: &Pipeline, ctx: &RtCtx, inputs: Vec<Cursor>) -> Vec<Cursor> {
    let mut s = pipe.run(ctx, upstream_of(inputs));
...
async fn seq_threads_slot_between_ops() {
    ...
    let pipe = Pipeline::Seq(vec![
        Pipeline::Op(Arc::new(StampLenOp)),
        Pipeline::Op(Arc::new(ReadStampOp)),
    ]);
    let out = run_collect(&pipe, &ctx, vec![cursor_from("hello world")]).await;
    assert_eq!(out.len(), 1);
...
async fn fork_distributes_and_tags_arms() {
    ...
    let mut out = run_collect(&pipe, &ctx, vec![cursor_from("abcd")]).await;
    assert_eq!(out.len(), 2, "two arms, one input cursor each = 2 outputs");
    // Order across arms is non-deterministic under stream::select_all; sort by index.
    out.sort_by_key(|c| match c.path.last().unwrap() {
        PathSeg::ForkArm { index } => *index,
        _ => usize::MAX,
    });
...
async fn op_calls_effect_and_stashes_response() {
    ...
    let pipe = Pipeline::Op(Arc::new(LenEffectOp));
    let out = run_collect(&pipe, &ctx, vec![cursor_from("abc")]).await;
    assert_eq!(out.len(), 1);
...
async fn rebase_clears_slots_preserves_captures_and_path() {
    let ctx = RtCtx::default();
    let pipe = Pipeline::Op(Arc::new(StampLenOp));
    let out = run_collect(&pipe, &ctx, vec![cursor_from("xyz")]).await;
    let c = &out[0];
...
async fn halve_multiplies_cursors_downstream_sees_all() {
    let ctx = RtCtx::default();
    let pipe = Pipeline::Seq(vec![
        Pipeline::Op(Arc::new(HalveOp)),
        Pipeline::Op(Arc::new(StampLenOp)),
    ]);
    let out = run_collect(&pipe, &ctx, vec![cursor_from("abcdef")]).await;
    assert_eq!(out.len(), 2);
...
async fn path_tags_leaf_first_op_then_nothing() {
    let ctx = RtCtx::default();
    let pipe = Pipeline::Op(Arc::new(StampLenOp));
    let out = run_collect(&pipe, &ctx, vec![cursor_from("a")]).await;
...
#[tokio::test]
async fn pipeline_streams_multiple_input_batches() {
    // Verify Pipeline::run preserves multiple upstream emissions — not
    // collected upfront, processed as they arrive.
    let ctx = RtCtx::default();
    let pipe = Pipeline::Op(Arc::new(StampLenOp));

    let upstream: BoxStream<'static, Arc<[Cursor]>> = Box::pin(stream::iter(vec![
        Arc::<[Cursor]>::from(vec![cursor_from("a")]),
        Arc::<[Cursor]>::from(vec![cursor_from("bb")]),
        Arc::<[Cursor]>::from(vec![cursor_from("ccc")]),
    ]));

    let mut s = pipe.run(&ctx, upstream);
    let mut all: Vec<Cursor> = Vec::new();
    while let Some(b) = s.next().await {
        all.extend(b.iter().cloned());
    }
    assert_eq!(all.len(), 3);
    let lens: Vec<usize> = all
        .iter()
        .map(|c| c.get_slot::<Stamped>().unwrap().0)
        .collect();
