//! `tag(:name, ...)` — facade lowering to relation ops (parse.md §14.5).
//!
//! tag/rule unification (chat_log/20260426.7): tag.rs no longer carries
//! runtime logic. It classifies the paren args at lower time and emits a
//! [`TagOp`] enum that delegates to the appropriate op in
//! [`crate::ops::relation`].
//!
...
//! | Spelling                                | Lowers to            |
//! |-----------------------------------------|----------------------|
//! | `tag(:r, $X, $Y)` all-bound, raw        | [`WriteOp`]          |
//! | `tag(:r, ${X?}, ${Y?})` all-unbound,raw | [`QueryOp`]          |
//! | `tag?(...)` all-bound                   | [`ProbeOp`] (kcl)    |
//! | `tag?(...)` mixed                       | [`JoinOp`] (kcl)     |
//! | `tag?(...)` all-unbound                 | [`QueryOp`]          |
//!
//! Today: only the `?`-less surface is parsed; `tag?` is reserved for
//! step kcl (probe/join wire-up). Mixed bound/unbound on the raw side is
//! a lower error (`tag/mixed-mode`) — `?` makes it valid by routing to
//! join.
...
/// Re-export so prior `use crate::ops::tag::TagArg` callers still resolve.
pub type TagArg = RelationArg;
    ...
    fn from_values(values: Vec<Value>) -> Result<Self, Vec<PatternDiagnostic>> {
        ...
        if have_unbound {
            let holes: Vec<Arc<str>> = rest
                ...
                .collect();
            return Ok(TagOp::Query(QueryOp::new(name, Arc::from(holes))));
        }
...
mod tests {
    ...
    async fn write_then_read_via_seq_pipeline() {
        ...
        let pipe = Pipeline::Seq(vec![
            Pipeline::Op(Arc::new(TagOp::from_values(vec![atom("a"), s("x")]).unwrap())),
            Pipeline::Op(Arc::new(TagOp::from_values(vec![atom("a"), s("y")]).unwrap())),
            Pipeline::Op(Arc::new(
