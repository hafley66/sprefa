...
Companion: `IMPACT.visual.human.unga.md` (plain words, no citations).
...
| `readBoundary` | one boundary select per relation | `1_incremental.ts:1030-1063` |
...
(`fixtures/4_struct_values.pl:757`). They carry them because the fixture
...
- **O1 the same cone, computed by the same predicate.** The cone lives in
  `analyze.pl` (section 4), which `engine.pl` does not currently import.
  It does import `1_expansion.pl` and `1_host_expand.pl`, which are the
  shared pre-passes (`engine.pl:504-508`), so a third shared module is
  consistent with the existing door split.
...
path. `tsv2` imports exactly one thing from the store package,
`stmt_counter` (`v6/tsv2/serve/3_engine.ts:49`). The store's own engine
header says v6/dl's tick loop does not reach its cascade
(`sprefa-store/js/src/engine/engine.ts:35-38`).
...
the OPPOSITE of the user's share-no-reset ruling. Copying its shape would
re-cold every cone on every reader.
...
### 2.3 After first demand: share-no-reset, and the defect it names

The user's ruling is share with NO reset: once a cone is warm it never goes
cold. In this engine that is: a query's demand row is a STANDING row, and
teardown is demand-row deletion and nothing else. That is already ruled
(`subscription_kernel`, `rulings.pl:170-190`: teardown = ordinary IVM
retraction, demand-row deletion IS the abort signal).

The refCount plumbing exists per row: `__support_count` is a real column on
every derived table, with a five-statement reconcile
(`gen_emitted/native_ts_query_term.ts:134` for the column,
`1_incremental.ts:500-531` for the reconcile).

**Concrete defect found, worth its own row:** `LiveEngine.ticks$` uses bare
`share()` (`v6/tsv2/serve/3_engine.ts:102-113`). Under rxjs 7
(`v6/tsv2/package.json:26`, `"rxjs": "^7.8.2"`) bare `share()` defaults to
`resetOnRefCountZero: true`. When the last subscriber leaves, the `tap`
finalize at `3_engine.ts:108-110` flips `running` false, and the next
`submit` errors with "tsv2 engine is not running"
(`3_engine.ts:118-121`). That is a reset-on-refcount-zero in the exact place
the user's ruling says there must not be one. Today it is masked because
`runProgram$` holds a permanent inner subscription (`4_http.ts:164`); under
a query-as-subscribe design it stops being masked.
Fix: `share({ resetOnRefCountZero: false })`, one line, with a test that
drops every reader and submits again.
...
section 3.4 shows why it is a separate hazard that lives in the composition,
not in the ingress.
...
### 3.3 The user's composition, in dl and in rx

The shape: first `pre_commit`, then switch to a merge of `interval(1000)`
and the pre-commit edge again, shared with no reset.

```
source pre_commit(repo: text, sha: text, at_ms: int).
...
```
rx lowering, the shape the user wrote:
```
const scanDue$ = preCommit$.pipe(
  take(1),                                              // "first pre_commit"
  switchMap(() => merge(interval(1000), preCommit$)),    // the merge after it
  share({ resetOnRefCountZero: false }),                 // lazy init one global
);
```

Two things to read off the dl:

- The `interval` leg and the `pre_commit` leg are two RULES with the same
  head. Multiple rules on one head are ordinary datalog union, which is what
  `merge` is. No new construct.
- `scan_gate` is the `take(1)`, and it is a `set` rel, so the second
  pre-commit writes an identical row and is silent
  (`rulings.pl:69` `r_equal_row_write = noop`). That is correct for the gate
  and it is the source of the hazard below.

### 3.4 What the clock checker must prove: the second-event hazard

Trace the existing checker over the program above.

`clock_dependencies/2` yields, among others
(`3_clock_check.pl:41-49`, roles from `registry.pl:207-213`):

| from | to | role | grade | receipt |
|---|---|---|---|---|
| `pre_commit/3` | `scan_gate/1` | `trigger` | 0 (source is not edge-headed) | `3_clock_check.pl:136-142` |
| `scan_gate/1` | `scan_due/2` | `level_read` | 0 | `3_clock_check.pl:92-95`, `registry.pl:207` |
| `interval/2` | `scan_due/2` | `level_read` | 0 | same |
| `pre_commit/3` | `scan_due/2` | `level_read` | 0 | same |

`clock_origin/3` returns TWO origins (`pre_commit/3` and `interval/2`,
`3_clock_check.pl:192-195`). From origin `pre_commit`, `scan_due` is
reachable at offset 0 by two paths, so `clock_path_conflict`
(`3_clock_check.pl:328-339`) does not fire. **The program compiles today.**
That is the good news and it is worth stating: the composition is already
expressible.

**Now delete the third rule** (the direct `pre_commit` leg), leaving only
the gate path. The rx reading is `exhaustMap`, not `switchMap`: the gate
opened once and a second pre-commit changes nothing, because `scan_gate` is
a set rel and the identical write is silent. `scan_due` re-fires only when
`interval` advances. **Nothing in the checker says a word about this.** The
program compiles, the clock offsets are consistent, and the second commit
does not fire.

**The obligation this creates, stated as a decidable property:**

> For a head `H` and an event origin `E`, if every causal path from `E` to
> `H` passes through a set-kind rel whose write from `E` is idempotent, then
> a second occurrence at `E` does not reach `H`.

That is decidable from facts the checker already computes:
`causal_dependency/4` gives the paths, `relation_kind/3`
(`0_program_check.pl`, imported at `3_clock_check.pl:27`) gives the kind,
and `edge_headed_refs/2` gives which rels are edge-written. The missing
piece is only the walk that asks it.

**Fork, priced:**
- **D1 a `clock_boundary/2` label**, in the idiom of the two existing labels
  (`3_clock_check.pl:349-364` `multi_trigger_batch_invariance`,
  `:386-393` `arm_absence_batch_invariance`). Name:
  `not_provable(second_event_refires(Head, Origin))`.
  Price: ~20 lines in `3_clock_check.pl`, one row in the replay gate
  (`compile/test/3_clock_history.pl`, which the TICK-MODEL calls the
  completeness evidence, `TICK-MODEL.md:26-30`), one plunit test.
  Buys: the hazard is named and queryable and never silent.
  Costs: it does not stop the program.
- **D2 a refusal.** Price: same code, plus the risk of rejecting a program
  that WANTS first-only semantics (`exhaustMap` is a legitimate shape and
  the ruling vocabulary has no other spelling for it).
  Both existing batch-invariance facts were labels for exactly this reason
...
- Recommendation: **D1 now, D2 only after a corpus measurement shows zero
  legitimate first-only programs.** That measurement is one grep over the 31
  `.dl6` files plus 281 fixtures.

**Second obligation, and this one the checker cannot prove:** liveness of
the shared inner. "share with no reset" is a runtime property. What the
compiler CAN do is pin it in the plan: `snapshot(current)` today has one
value (`emit_ts.pl:419-422`); a `snapshot(standing)` alternative would be
the field that carries the user's ruling into the emitted module, where the
runtime can honor it. That is a one-field widening of `IQueryPlanData`
(`emit_ts.pl:288`).
...
- **The graph walk belongs in `analyze.pl`**, because every other rel-graph
  predicate is already there: `program_refs/2`, `derived_refs/2`,
  `body_ref_uses/2` (`analyze.pl:104-107`), `edge_headed_refs/2`,
  `level_headed_refs/2` (`analyze.pl:8-13` exports). A new export
  `query_cone/3` sits beside them.
- **The call site is `compile.pl:program_plan/2`**, one line after the refs
  ...
  (`lower.pl:3359-3418`).
- **The graph machinery is already bought**: `0_graph.pl` gives
  `graph_from_edges/3` (`0_graph.pl:41-44`) and a reachability walk
  (`collect_reachable/6` at `0_graph.pl:170-181`). Build the edge list
  REVERSED (body ref -> head ref) and the cone is one `collect_reachable`
  seeded with the query rels. `3_clock_check.pl:247-254` already builds an
  edge list this way; copy that shape.

Sketch, signatures first per the planning protocol:

```prolog
%% query_cone(+Rules, +QueryRefs, -ConeRefs) is det.
%  Every ref that can contribute a row to some ref in QueryRefs.
%  Body: edges are BodyRef-HeadRef reversed to HeadRef-BodyRef, seeded with
%  QueryRefs, closed by 0_graph:collect_reachable/6. Edge kinds included:
%  positive uses, negated uses, latest/pre/finalize wrapper refs (a `pre`
%  read is a cone edge -- see section 1.13), and every probe's generated
%  demand and response ref.
query_cone(Rules, QueryRefs, ConeRefs).
```
Instance lifetime: none, it is a pure function of the expanded program,
computed once per compile inside `program_plan/2`.
...
Estimated size: ~15 lines in `analyze.pl`, ~4 in `compile.pl`.
...
(`1_incremental.ts:1030-1063` reads the boundary for every relation passed
...
**Step 1. Compute the cone; consume nothing.** `analyze.pl:query_cone/3`
plus the call in `compile.pl:program_plan/2`, emitting a
`demandedRels` constant. Under the compat rule the constant equals every rel
when the program has no query.
...
(`engine.pl:503-531`), so the referee agrees.
...
**Step 5. The share-no-reset fix.** `share({ resetOnRefCountZero: false })`
at `3_engine.ts:112`, with a test that drops every `ticks$` subscriber and
then submits.
Gate: `tsv2-test`, `serve-leak-soak`. This is one line and it is independent
of every other step; it can land first.
...
**Step 8. The second-event label** (section 3.4, D1).
...
|---|---|---|
| conformance fixtures | 276 | 5 (`2_hosts_wiring.pl:66,138,179,234`; `4_struct_values.pl:757`) |
...
3. **Second-event hazard: label or refusal?** Section 3.4, D1 vs D2.
