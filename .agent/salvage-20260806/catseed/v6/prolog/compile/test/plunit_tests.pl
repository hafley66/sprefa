...
% A program that names __catalog_rel gets the two catalog DDL statements,
% byte for byte: the keyed table plus the child-walk index on parent.
test(catalog_table_shape) :-
    Prog = prog([], [ (source_row(A, B) <- '__catalog_rel'(A, B)) ]),
    Term = fixture(catalog_shape, Prog, [ '__catalog_rel'(foo, bar) ], [], []),
    once(( program_plan(Term-[], Plan),
           lower_program(Plan, lowered(_, Ddl, _, _, _, _, _, _)) )),
    memberchk('CREATE TABLE "__catalog_rel" ("rel_id" INTEGER NOT NULL, "parent_id" INTEGER NOT NULL, "ordinal" INTEGER NOT NULL, "local_name" TEXT NOT NULL, "kind" TEXT NOT NULL, "type_id" INTEGER NOT NULL, PRIMARY KEY ("rel_id")) WITHOUT ROWID', Ddl),
...
test(catalog_rows_are_one_statement) :-
    Prog = prog([], [ (source_row(A, B) <- '__catalog_rel'(A, B)) ]),
    Term = fixture(catalog_rows, Prog, [ '__catalog_rel'(foo, bar) ], [], []),
    once(( program_plan(Term-[], Plan),
           lower_program(Plan, lowered(_, Ddl, _, _, _, _, _, _)) )),
    findall(Seed,
            ( member(Seed, Ddl),
            ...
            [CatalogSeed]),
    sub_atom(CatalogSeed, 0, _, _, 'INSERT OR IGNORE INTO "__catalog_rel"').

% Ids are positional: primitives 1..5, first rel at 6 with columns at 7 and
% 8 (ordinals 1,2), second rel at 9 with columns at 10 and 11.
test(catalog_ids_are_positional) :-
    Prog = prog([], [ (source_row(A, B) <- '__catalog_rel'(A, B)) ]),
    Term = fixture(catalog_ids, Prog, [ '__catalog_rel'(foo, bar) ], [], []),
    once(( program_plan(Term-[], Plan),
           lower_program(Plan, lowered(_, Ddl, _, _, _, _, _, _)) )),
    findall(CatalogSeed,
            ( member(CatalogSeed, Ddl),
              sub_atom(CatalogSeed, 0, _, _, 'INSERT OR IGNORE INTO "__catalog_rel"') ),
            [Seed]),
    sub_atom(Seed, 0, _, _, 'INSERT OR IGNORE INTO "__catalog_rel"'),
    forall(member(Expected, [
        ...
        "(7,6,1,'col1','column',1)",
        "(8,6,2,'col2','column',1)",
        "(9,0,0,'source_row','rel',0)",
        "(10,9,1,'col1','column',1)",
        "(11,9,2,'col2','column',1)"]),
        sub_atom(Seed, _, _, _, Expected)).
