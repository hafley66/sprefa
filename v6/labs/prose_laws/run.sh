...
# Boots tsv2 over prose-laws.dl6 and folds violation/3 by rule_id. Mode
# `fixture` feeds the probe sentences, `corpus` (default) the transcript set.
...
# Sentence total comes from the feed in both modes, so the fixture receipt is
# computed, never hardcoded. Sub-mode is feed|fixture, not the corpus label.
...
# Idle-based quiescence fires too early: a chunk host takes seconds to walk.
# Wait for the sentence count to reach the total, then settle the drain tick.
last_size=-1
idle_slept=0
step_ms=50
