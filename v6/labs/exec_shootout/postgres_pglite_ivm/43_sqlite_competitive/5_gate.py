...
for index,command in enumerate(commands):
    with (run/f'{index}.log').open('wb') as log:
        try:rc=subprocess.run(command,env=dict(env,TAKE2_SOURCE_VIEWS='1') if index in [4,6,8] else env,stdout=log,stderr=subprocess.STDOUT,timeout=120).returncode
...
if rc==0:
    ...
    if len(paired)!=11 or not all(r.get('all_input_output_states_match') and r.get('state_count_per_arm')==13 and len(r['arms'])==3 and not r['excluded_arms'] for r in paired):rc=1
