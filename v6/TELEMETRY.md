...
The tsv2 serve layer's wire keys come from `v6/prolog/compile/registry.pl`'s
`trace_event/2` table via `runtime/0_traceSchema.ts`, generated by
`compile/3_emit_trace_schema.pl`. A tsv2 wire key that is not declared there is
a key the rust emitter cannot reproduce, so `actor`/`seam` are declared on
`trace_event(tick_line, ...)` rather than added at the sink:

| standard | trace_event/2 key | note |
|---|---|---|
| `actor` | `actor` | package.component, declared on `tick_line` |
| `seam` | `seam` | where the line happened, declared on `tick_line` |
...
Nested records (`rules[]`, `effects[]`, `binds[]`, `watches[]`) carry no
envelope: the line they sit on already answers who and where, and `unit`/`ms` on
a rule record would restate `rule`/`wall_ms` byte for byte.
...
|---|---|---|
| `unit` | `rule` | tsv2 rule record (nested, no envelope) |
...
1. tsv2 `sprefa:tick` and `sprefa:effect` CHANNEL events are pinned to exactly
   ...
   schema's keys, in order"). The envelope goes on the JSONL LINE, never on the
   channel event; the tick event is the `tick_line` prefix before `actor`.
   `v6/tsv2/goldens/trace-line.jsonl` is re-pinned whenever a declared field is
   added (`TRACE_GOLDEN_WRITE=1`).
