...
pub fn run(
    ...
    for rel in rels {
        state.insert(rel.name.clone(), Vec::new());
    }
    for row in initial {
...
fn change(state: &mut Relations, row: &Row, sign: i8) -> Result<()> {
    ...
    if sign > 0 {
        if !target.contains(&row.values) {
            target.push(row.values.clone());
        }
    } else {
        target.retain(|tuple| tuple != &row.values);
    }
    Ok(())
}
...
fn settle(
    ...
    for head in &heads {
        state.insert(head.clone(), Vec::new());
    }
    for _ in 0..10_000 {
        let old = state.clone();
        let mut next = base.clone();
        for head in &heads {
            next.insert(head.clone(), Vec::new());
        }
        for op in operators {
            if op.kind == "map"
                && !operators
    ...
    for (alias, relation) in bindings {
        ...
        for prior in &out {
            for tuple in tuples {
                ...
                for (column, value) in fields.iter().zip(tuple) {
