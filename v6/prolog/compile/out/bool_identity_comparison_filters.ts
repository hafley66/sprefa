...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    enabled_name: selectRows(seam, `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name" FROM "enabled_name"`, relColumns.enabled_name!, relColumnTypes.enabled_name!),
...
const finalSelect: Record<string, string> = {
  enabled_name: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name" FROM "enabled_name"`,
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "enabled_name", kind: "set", tableName: "enabled_name", deltaTableName: "__delta_enabled_name", frontierTableName: "__frontier_enabled_name", nextFrontierTableName: "__next_frontier_enabled_name", columns: ["name"], columnTypes: ["text"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_enabled_name" WHERE "_sign" IN (-1, 1) GROUP BY "name", "_sign"` },
