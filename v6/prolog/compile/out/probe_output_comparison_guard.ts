...
const finalSelect: Record<string, string> = {
  __host_demand_score: `SELECT CASE WHEN json_valid("identity_digest") AND json_type("identity_digest") = 'object' THEN json_extract("identity_digest", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("identity_digest", '$.args')) || ')' ELSE "identity_digest" END AS "identity_digest", CASE WHEN json_valid("witness_digest") AND json_type("witness_digest") = 'object' THEN json_extract("witness_digest", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("witness_digest", '$.args')) || ')' ELSE "witness_digest" END AS "witness_digest", CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path" FROM "__host_demand_score"`,
  __host_response_score: `SELECT CASE WHEN json_valid("witness_digest") AND json_type("witness_digest") = 'object' THEN json_extract("witness_digest", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("witness_digest", '$.args')) || ')' ELSE "witness_digest" END AS "witness_digest", "ordinal", CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", "score" FROM "__host_response_score"`,
  accepted: `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", "score" FROM "accepted"`,
  input: `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path" FROM "input"`,
};

const ARRIVAL_STATEMENTS: Record<string, { kind: "log" | "set"; addSql: string; delSql: string | null }> = {
  __host_response_score: { kind: "set", addSql: `INSERT OR REPLACE INTO "__host_response_score" ("witness_digest", "ordinal", "path", "score") VALUES (?, ?, ?, ?)`, delSql: `DELETE FROM "__host_response_score" WHERE "witness_digest" = ? AND "ordinal" = ? AND "path" = ? AND "score" = ?` },
