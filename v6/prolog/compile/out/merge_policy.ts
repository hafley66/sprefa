...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "closed", kind: "log", tableName: "closed", deltaTableName: "__delta_closed", frontierTableName: "__frontier_closed", nextFrontierTableName: "__next_frontier_closed", columns: ["session_id", "tab_id"], arrivalAddSql: null, boundarySql: `SELECT CASE WHEN json_valid("session_id") AND json_type("session_id") = 'object' THEN json_extract("session_id", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("session_id", '$.args')) || ')' ELSE "session_id" END AS "session_id", CASE WHEN json_valid("tab_id") AND json_type("tab_id") = 'object' THEN json_extract("tab_id", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("tab_id", '$.args')) || ')' ELSE "tab_id" END AS "tab_id", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_closed" WHERE "_sign" IN (-1, 1) GROUP BY "session_id", "tab_id", "_sign"` },
