...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    adjusted: selectRows(seam, `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "value" FROM "adjusted"`, relColumns.adjusted!, relColumnTypes.adjusted!),
...
const finalSelect: Record<string, string> = {
  adjusted: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "value" FROM "adjusted"`,
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "adjusted", kind: "set", tableName: "adjusted", deltaTableName: "__delta_adjusted", frontierTableName: "__frontier_adjusted", nextFrontierTableName: "__next_frontier_adjusted", columns: ["name", "value"], columnTypes: ["text", "float"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "value", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_adjusted" WHERE "_sign" IN (-1, 1) GROUP BY "name", "value", "_sign"` },
