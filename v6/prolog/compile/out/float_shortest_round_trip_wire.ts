...
const relCatalog: readonly IRelCatalogRow[] = [
  ...
  { relId: 6, parentId: 0, ordinal: 0, localName: "float_shortest_round_trip_wire", kind: "module", typeId: 0, arity: 0, moduleId: 6, hId: "e9e391a6d100076c", hSchema: "", hRule: "" },
  { relId: 7, parentId: 6, ordinal: 0, localName: "sample", kind: "rel", typeId: 0, arity: 1, moduleId: 6, hId: "5dbccfad58303c3f", hSchema: "bec07f8b9884a74a", hRule: "" },
  { relId: 8, parentId: 7, ordinal: 1, localName: "value", kind: "column", typeId: 3, arity: 0, moduleId: 6, hId: "8971bcef2ebbf0a2", hSchema: "", hRule: "" },
];
...
const arrivalTargets: readonly string[] = ["sample"];
...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    sample: selectRows(seam, `SELECT "value" FROM "sample"`, relColumns.sample!, relColumnTypes.sample!),
  });
}
...
const ARRIVAL_STATEMENTS: Record<string, { kind: "log" | "set"; addSql: string; delSql: string | null }> = {
  sample: { kind: "set", addSql: `INSERT OR IGNORE INTO "sample" ("value") VALUES (?)`, delSql: `DELETE FROM "sample" WHERE "value" = ?` },
};
...
function buildDeltas(before: Snapshot, after: Snapshot): ITickDeltas {
  const sample = multisetDiff(before.sample, after.sample);
  return {
    rels: [
