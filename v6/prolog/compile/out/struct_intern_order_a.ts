...
const relCatalog: readonly IRelCatalogRow[] = [
  ...
  { relId: 6, parentId: 0, ordinal: 0, localName: "struct_intern_order_a", kind: "module", typeId: 0, arity: 0, moduleId: 6, hId: "3534320a047f041c", hSchema: "", hRule: "" },
  { relId: 7, parentId: 6, ordinal: 0, localName: "mark", kind: "rel", typeId: 0, arity: 1, moduleId: 6, hId: "b5f3ea202ac83f50", hSchema: "54514ddea6393695", hRule: "" },
  { relId: 8, parentId: 7, ordinal: 1, localName: "at", kind: "column", typeId: 0, arity: 0, moduleId: 6, hId: "d4f93625c5c58df1", hSchema: "", hRule: "" },
  { relId: 9, parentId: 6, ordinal: 0, localName: "span", kind: "rel", typeId: 0, arity: 2, moduleId: 6, hId: "d944561fe217805b", hSchema: "302755ba572df023", hRule: "" },
  { relId: 10, parentId: 9, ordinal: 1, localName: "start", kind: "column", typeId: 2, arity: 0, moduleId: 6, hId: "583b6868a0c51735", hSchema: "", hRule: "" },
  { relId: 11, parentId: 9, ordinal: 2, localName: "end", kind: "column", typeId: 2, arity: 0, moduleId: 6, hId: "c7e994cdc5c61ede", hSchema: "", hRule: "" },
];
...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    mark: selectRows(seam, `SELECT (SELECT d."__rendered" FROM "__ref_span" d WHERE d."__id" = "at") AS "at" FROM "mark"`, relColumns.mark!, relColumnTypes.mark!),
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "mark", kind: "set", tableName: "mark", deltaTableName: "__delta_mark", frontierTableName: "__frontier_mark", nextFrontierTableName: "__next_frontier_mark", columns: ["at"], columnTypes: ["ref"], keyIndices: [], arrivalAddSql: `INSERT OR IGNORE INTO "mark" ("at") SELECT json_extract(value, '$[0]') FROM json_each(?) RETURNING "at"`, arrivalDelSql: `DELETE FROM "mark" WHERE ("at") IN (SELECT json_extract(value, '$[0]') FROM json_each(?)) RETURNING "at"`, boundarySql: `SELECT (SELECT d."__rendered" FROM "__ref_span" d WHERE d."__id" = "at") AS "at", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_mark" WHERE "_sign" IN (-1, 1) GROUP BY "at", "_sign"`, ruleObservers: [] },
