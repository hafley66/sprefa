...
const ddl: readonly string[] = [
  ...
  `CREATE TABLE "hot" ("name" TEXT NOT NULL, "__refcount" INTEGER NOT NULL DEFAULT 1, PRIMARY KEY ("name")) WITHOUT ROWID`,
...
const relCatalog: readonly IRelCatalogRow[] = [
  ...
  { relId: 6, parentId: 0, ordinal: 0, localName: "counter_fold_matches_hand_computation", kind: "module", typeId: 0, arity: 0, moduleId: 6, hId: "d81798e7265b73f8", hSchema: "", hRule: "" },
  { relId: 7, parentId: 6, ordinal: 0, localName: "counter", kind: "rel", typeId: 0, arity: 2, moduleId: 6, hId: "944a70b98ec92175", hSchema: "6106d6fe97f57493", hRule: "ea98ffc7c6cdd41b" },
  { relId: 8, parentId: 7, ordinal: 1, localName: "name", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "137e3e253918d3cc", hSchema: "", hRule: "" },
  { relId: 9, parentId: 7, ordinal: 2, localName: "next", kind: "column", typeId: 2, arity: 0, moduleId: 6, hId: "90a13c2042db6e44", hSchema: "", hRule: "" },
  { relId: 10, parentId: 6, ordinal: 0, localName: "decrement", kind: "rel", typeId: 0, arity: 2, moduleId: 6, hId: "7c5178e8bd37f551", hSchema: "bd656f1e3e3dad1a", hRule: "" },
  { relId: 11, parentId: 10, ordinal: 1, localName: "name", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "191b49a0ab159837", hSchema: "", hRule: "" },
  { relId: 12, parentId: 10, ordinal: 2, localName: "col2", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "054f9d8a24d280e4", hSchema: "", hRule: "" },
  { relId: 13, parentId: 6, ordinal: 0, localName: "hot", kind: "rel", typeId: 0, arity: 1, moduleId: 6, hId: "afa50c404c0f1f4b", hSchema: "a30b139c04a632dd", hRule: "fdf8d9a58ed9ca92" },
  { relId: 14, parentId: 13, ordinal: 1, localName: "name", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "c372af05f34c9cf5", hSchema: "", hRule: "" },
  { relId: 15, parentId: 6, ordinal: 0, localName: "increment", kind: "rel", typeId: 0, arity: 2, moduleId: 6, hId: "8dc17267cd9dda0d", hSchema: "bd656f1e3e3dad1a", hRule: "" },
  { relId: 16, parentId: 15, ordinal: 1, localName: "name", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "d8d3f9ae7ecf06ed", hSchema: "", hRule: "" },
  { relId: 17, parentId: 15, ordinal: 2, localName: "col2", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "3adb60dfa09c462f", hSchema: "", hRule: "" },
];

const relDeclaredColumnTypes: Record<string, readonly string[]> = {
};

const arrivalTargets: readonly string[] = ["decrement", "increment"];
...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    ...
    decrement: selectRows(seam, `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' AND json_type("name", '$.fn') = 'text' AND json_type("name", '$.args') = 'array' THEN json_extract("name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("name", '$.args')), '') || ')' ELSE "name" END AS "name", CASE WHEN json_valid("col2") AND json_type("col2") = 'object' AND json_type("col2", '$.fn') = 'text' AND json_type("col2", '$.args') = 'array' THEN json_extract("col2", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("col2", '$.args')), '') || ')' ELSE "col2" END AS "col2" FROM "decrement"`, relColumns.decrement!, relColumnTypes.decrement!),
    hot: selectRows(seam, `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' AND json_type("name", '$.fn') = 'text' AND json_type("name", '$.args') = 'array' THEN json_extract("name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("name", '$.args')), '') || ')' ELSE "name" END AS "name" FROM "hot"`, relColumns.hot!, relColumnTypes.hot!),
    increment: selectRows(seam, `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' AND json_type("name", '$.fn') = 'text' AND json_type("name", '$.args') = 'array' THEN json_extract("name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("name", '$.args')), '') || ')' ELSE "name" END AS "name", CASE WHEN json_valid("col2") AND json_type("col2") = 'object' AND json_type("col2", '$.fn') = 'text' AND json_type("col2", '$.args') = 'array' THEN json_extract("col2", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("col2", '$.args')), '') || ')' ELSE "col2" END AS "col2" FROM "increment"`, relColumns.increment!, relColumnTypes.increment!),
  });
}
...
const finalSelect: Record<string, string> = {
  ...
  hot: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' AND json_type("name", '$.fn') = 'text' AND json_type("name", '$.args') = 'array' THEN json_extract("name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("name", '$.args')), '') || ')' ELSE "name" END AS "name" FROM "hot"`,
...
const ARRIVAL_STATEMENTS: Record<string, { kind: "log" | "set"; addSql: string; delSql: string | null }> = {
  decrement: { kind: "log", addSql: `INSERT INTO "decrement" ("name", "col2") VALUES (?, ?)`, delSql: null },
  increment: { kind: "log", addSql: `INSERT INTO "increment" ("name", "col2") VALUES (?, ?)`, delSql: null },
};
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "counter", kind: "set", tableName: "counter", deltaTableName: "__delta_counter", frontierTableName: "__frontier_counter", nextFrontierTableName: "__next_frontier_counter", columns: ["name", "next"], columnTypes: ["text", "int"], keyIndices: [0], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' AND json_type("name", '$.fn') = 'text' AND json_type("name", '$.args') = 'array' THEN json_extract("name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("name", '$.args')), '') || ')' ELSE "name" END AS "name", "next", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_counter" WHERE "_sign" IN (-1, 1) GROUP BY "name", "next", "_sign"`, ruleObservers: ["hot/1"] },
  { rel: "decrement", kind: "log", tableName: "decrement", deltaTableName: "__delta_decrement", frontierTableName: "__frontier_decrement", nextFrontierTableName: "__next_frontier_decrement", columns: ["name", "col2"], columnTypes: ["text", "text"], keyIndices: [], arrivalAddSql: `INSERT INTO "decrement" ("name", "col2") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "name", "col2"`, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' AND json_type("name", '$.fn') = 'text' AND json_type("name", '$.args') = 'array' THEN json_extract("name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("name", '$.args')), '') || ')' ELSE "name" END AS "name", CASE WHEN json_valid("col2") AND json_type("col2") = 'object' AND json_type("col2", '$.fn') = 'text' AND json_type("col2", '$.args') = 'array' THEN json_extract("col2", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("col2", '$.args')), '') || ')' ELSE "col2" END AS "col2", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_decrement" WHERE "_sign" IN (-1, 1) GROUP BY "name", "col2", "_sign"`, ruleObservers: ["counter/2"] },
  { rel: "hot", kind: "set", tableName: "hot", deltaTableName: "__delta_hot", frontierTableName: "__frontier_hot", nextFrontierTableName: "__next_frontier_hot", columns: ["name"], columnTypes: ["text"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' AND json_type("name", '$.fn') = 'text' AND json_type("name", '$.args') = 'array' THEN json_extract("name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("name", '$.args')), '') || ')' ELSE "name" END AS "name", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_hot" WHERE "_sign" IN (-1, 1) GROUP BY "name", "_sign"`, ruleObservers: [] },
...
const INCREMENTAL_EDGE_STATEMENTS: readonly IIncrementalEdgeStatement[] = [
  { headRel: "counter", ruleId: "counter_fold_matches_hand_computation:counter/2#1", headKind: "set", headTableName: "counter", headDeltaTableName: "__delta_counter", headColumns: ["name", "next"], keyIndices: [0], projectSql: `SELECT d0."name" AS "name", (b0."next" + 1) AS "next" FROM "__frontier_increment" d0, "__pre_counter" b0 WHERE d0."_phase" >= 0 AND b0."name" = d0."name" ORDER BY d0."_phase", d0."_sequence"` },
  { headRel: "counter", ruleId: "counter_fold_matches_hand_computation:counter/2#2", headKind: "set", headTableName: "counter", headDeltaTableName: "__delta_counter", headColumns: ["name", "next"], keyIndices: [0], projectSql: `SELECT d0."name" AS "name", (b0."next" - 1) AS "next" FROM "__frontier_decrement" d0, "__pre_counter" b0 WHERE d0."_phase" >= 0 AND b0."name" = d0."name" ORDER BY d0."_phase", d0."_sequence"` },
];
...
function resolveCounter_1Writes(seam: ISqlSeam, before: Snapshot, arrivals: IArrivalBatch): Observable<readonly SqlStatement[]> {
  const triggerRows = triggerOccurrences("log", "decrement", before.decrement, arrivals);
  if (triggerRows.length === 0) return of([]);
  return forkJoin(triggerRows.map((arrival) => seam.runner.execute(seam.db, { sql: EDGE_COUNTER_1_PROJECT_SQL, args: bindArgs(arrival.row) }))).pipe(
    map((results) => {
      const resolved = new Map<string, IRow>();
      for (const result of results) {
        const projectedRows = result.rows.map((row) => EDGE_COUNTER_1_HEAD_COLUMNS.map((column) => row[column] as IRowValue) as IRow);
        for (const projectedRow of projectedRows) {
          const key = JSON.stringify(EDGE_COUNTER_1_KEY_INDICES.map((index) => projectedRow[index]));
          resolved.set(key, projectedRow);
        }
      }
      return [...resolved.values()].map((row): SqlStatement => ({ sql: EDGE_COUNTER_1_WRITE_SQL, args: bindArgs(row) }));
    }),
  );
}
...
const ORDERED_EDGE_ARMS: readonly IOrderedEdgeArm[] = [
  ...
  { triggerRel: "decrement", triggerKind: "arrival", headRel: "counter", headKind: "set", headColumns: ["name", "next"], keyIndices: [0], projectSql: `SELECT ?1 AS "name", (b0."next" - 1) AS "next" FROM "__pre_counter" b0 WHERE b0."name" = ?1`, writeSql: `INSERT INTO "counter" ("name", "next") VALUES (?, ?) ON CONFLICT("name") DO UPDATE SET "next" = excluded."next"`, evolvesPre: true },
];
...
function orderedOutsideOccurrences(before: Snapshot, arrivals: IArrivalBatch): readonly IOrderedOccurrence[] {
  ...
  for (const arrival of triggerOccurrences("log", "decrement", before["decrement"], arrivals)) accepted.add(arrival);
...
function orderedCarryAdditions(mid: Snapshot, after: Snapshot, boundary: ITickDeltas, written: readonly IOrderedWrite[]): readonly IRelDelta[] {
  ...
  for (const row of multisetDiff(mid["hot"], after["hot"]).add) { const rowText = JSON.stringify(row); const exact = JSON.stringify(["hot", row]); if (seen.has(exact) || !(boundaryAdds.get("hot")?.has(rowText) ?? false)) continue; seen.add(exact); additions.push({ rel: "hot", add: [row], del: [] }); }
...
function buildDeltas(before: Snapshot, after: Snapshot): ITickDeltas {
  ...
  const decrement = multisetDiff(before.decrement, after.decrement);
  const hot = multisetDiff(before.hot, after.hot);
