...
export const demandedRels: readonly string[] = ["pull_request/3", "resp/1"];
...
const ddl: readonly string[] = [
  `CREATE TABLE "pull_request" ("number" INTEGER NOT NULL, "title" TEXT NOT NULL, "author" TEXT NOT NULL, "__support_count" INTEGER NOT NULL DEFAULT 1, PRIMARY KEY ("number", "title", "author")) WITHOUT ROWID`,
  ...
  `CREATE TEMP TABLE "__support_next_pull_request" ("number" INTEGER NOT NULL, "title" TEXT NOT NULL, "author" TEXT NOT NULL, "__support_count" INTEGER NOT NULL, PRIMARY KEY ("number", "title", "author")) WITHOUT ROWID`,
];
...
const boot: readonly IBootStatement[] = [
  { sql: `INSERT OR IGNORE INTO "resp" ("body") VALUES (?)`, params: ["[{\"number\":1,\"title\":\"first\",\"user\":{\"login\":\"octo\"}},{\"number\":2,\"title\":\"second\",\"user\":{\"login\":\"hubot\"}}]"] },
  { sql: `DELETE FROM "pull_request"`, params: [] },
  { sql: `INSERT OR IGNORE INTO "pull_request" ("number", "title", "author") SELECT json_extract(j0.value, '$."number"'), json_extract(j0.value, '$."title"'), json_extract(j0.value, '$."user"."login"') FROM "resp" b0, json_each(b0."body") j0 WHERE json_type(b0."body", '$') = 'array' AND j0.type = 'object' AND json_extract(j0.value, '$."number"') IS NOT NULL AND json_extract(j0.value, '$."title"') IS NOT NULL AND json_type(j0.value, '$."user"') = 'object' AND json_extract(j0.value, '$."user"."login"') IS NOT NULL`, params: [] },
];
...
INSERT OR IGNORE INTO "pull_request" ("number", "title", "author") SELECT json_extract(j0.value, '$."number"'), json_extract(j0.value, '$."title"'), json_extract(j0.value, '$."user"."login"') FROM "resp" b0, json_each(b0."body") j0 WHERE json_type(b0."body", '$') = 'array' AND j0.type = 'object' AND json_extract(j0.value, '$."number"') IS NOT NULL AND json_extract(j0.value, '$."title"') IS NOT NULL AND json_type(j0.value, '$."user"') = 'object' AND json_extract(j0.value, '$."user"."login"') IS NOT NULL`, supportSql: [`DELETE FROM "__support_next_pull_request"`, `INSERT INTO "__support_next_pull_request" ("number", "title", "author", "__support_count") SELECT "number", "title", "author", sum("__support_count") FROM (SELECT json_extract(j0.value, '$."number"') AS "number", json_extract(j0.value, '$."title"') AS "title", json_extract(j0.value, '$."user"."login"') AS "author", count(*) AS "__support_count" FROM "resp" b0, json_each(b0."body") j0 WHERE json_type(b0."body", '$') = 'array' AND j0.type = 'object' AND json_extract(j0.value, '$."number"') IS NOT NULL AND json_extract(j0.value, '$."title"') IS NOT NULL AND json_type(j0.value, '$."user"') = 'object' AND json_extract(j0.value, '$."user"."login"') IS NOT NULL GROUP BY json_extract(j0.value, '$."number"'), json_extract(j0.value, '$."title"'), json_extract(j0.value, '$."user"."login"')) GROUP BY "number", "title", "author"`, `UPDATE "pull_request" AS h SET "__support_count" = "__support_count" - ("__support_count" - COALESCE((SELECT n."__support_count" FROM "__support_next_pull_request" n WHERE n."number" = h."number" AND n."title" = h."title" AND n."author" = h."author"), 0))`, `DELETE FROM "pull_request" WHERE "__support_count" <= 0 RETURNING "number", "title", "author"`, `INSERT INTO "pull_request" ("number", "title", "author", "__support_count") SELECT "number", "title", "author", n."__support_count" FROM "__support_next_pull_request" n WHERE NOT EXISTS (SELECT 1 FROM "pull_request" h WHERE n."number" = h."number" AND n."title" = h."title" AND n."author" = h."author") RETURNING "number", "title", "author"`], aggregateSql: null },
];
