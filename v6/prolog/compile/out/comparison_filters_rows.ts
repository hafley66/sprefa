...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "jaccard", kind: "set", tableName: "jaccard", deltaTableName: "__delta_jaccard", frontierTableName: "__frontier_jaccard", nextFrontierTableName: "__next_frontier_jaccard", columns: ["left", "right", "col3"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("left") AND json_type("left") = 'object' THEN json_extract("left", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("left", '$.args')) || ')' ELSE "left" END AS "left", CASE WHEN json_valid("right") AND json_type("right") = 'object' THEN json_extract("right", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("right", '$.args')) || ')' ELSE "right" END AS "right", "col3", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_jaccard" WHERE "_sign" IN (-1, 1) GROUP BY "left", "right", "col3", "_sign"` },
  ...
  { rel: "union_size", kind: "set", tableName: "union_size", deltaTableName: "__delta_union_size", frontierTableName: "__frontier_union_size", nextFrontierTableName: "__next_frontier_union_size", columns: ["left", "right", "union"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("left") AND json_type("left") = 'object' THEN json_extract("left", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("left", '$.args')) || ')' ELSE "left" END AS "left", CASE WHEN json_valid("right") AND json_type("right") = 'object' THEN json_extract("right", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("right", '$.args')) || ')' ELSE "right" END AS "right", "union", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_union_size" WHERE "_sign" IN (-1, 1) GROUP BY "left", "right", "union", "_sign"` },
];
