...
function read_snapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    group_rels: select_rows(seam, `SELECT CASE WHEN json_valid("group_name") AND json_type("group_name") = 'object' AND json_type("group_name", '$.fn') = 'text' AND json_type("group_name", '$.args') = 'array' THEN json_extract("group_name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("group_name", '$.args')), '') || ')' ELSE "group_name" END AS "group_name", "col2" FROM "__txt_group_rels" t`, rel_columns.group_rels!, rel_column_types.group_rels!),
...
const final_select: Record<string, string> = {
  group_rels: `SELECT CASE WHEN json_valid("group_name") AND json_type("group_name") = 'object' AND json_type("group_name", '$.fn') = 'text' AND json_type("group_name", '$.args') = 'array' THEN json_extract("group_name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("group_name", '$.args')), '') || ')' ELSE "group_name" END AS "group_name", "col2" FROM "__txt_group_rels" t`,
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "group_rels", kind: "set", table_name: "group_rels", delta_table_name: "__delta_group_rels", frontier_table_name: "__frontier_group_rels", next_frontier_table_name: "__next_frontier_group_rels", columns: ["group_name", "col2"], column_types: ["text", "json"], key_indices: [], arrival_add_sql: null, arrival_del_sql: null, boundary_sql: `SELECT CASE WHEN json_valid("group_name") AND json_type("group_name") = 'object' AND json_type("group_name", '$.fn') = 'text' AND json_type("group_name", '$.args') = 'array' THEN json_extract("group_name", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("group_name", '$.args')), '') || ')' ELSE "group_name" END AS "group_name", "col2", "_sign" AS "__sign", count(*) AS "__count" FROM "__txt___delta_group_rels" t WHERE "_sign" IN (-1, 1) GROUP BY "group_name", "col2", "_sign"`, rule_observers: [] },
