...
const ddl: readonly string[] = [
  ...
  `CREATE TEMP TABLE "__frontier_identical_increments_stack_as_log_deltas_counter" ("_phase" INTEGER NOT NULL, "_sequence" INTEGER NOT NULL, "name" INTEGER NOT NULL, "next" INTEGER NOT NULL)`,
...
const boot: readonly IBootStatement[] = [
  ...
  { rel: "counter", sql: `INSERT OR IGNORE INTO "identical_increments_stack_as_log_deltas_counter" ("name", "next") VALUES ((SELECT "__id" FROM "__str" WHERE "content" = ?), ?)`, params: ["clicks", 0] },
];
...
function snapshot_ordered_pre(seam: ISqlSeam): Observable<void> {
  return seam.runner.executeMultiple(seam.db, `DELETE FROM "__pre_identical_increments_stack_as_log_deltas_counter";
INSERT INTO "__pre_identical_increments_stack_as_log_deltas_counter" ("name", "next") SELECT "name", "next" FROM "identical_increments_stack_as_log_deltas_counter"`);
}
...
const ORDERED_EDGE_ARMS: readonly IOrderedEdgeArm[] = [
  { trigger_rel: "increment", trigger_kind: "arrival", head_rel: "counter", head_table_name: "identical_increments_stack_as_log_deltas_counter", head_kind: "set", head_columns: ["name", "next"], key_indices: [0], project_sql: `SELECT ?1 AS "name", (b0."next" + 1) AS "next" FROM "__pre_identical_increments_stack_as_log_deltas_counter" b0 WHERE b0."name" = ?1`, write_sql: `INSERT INTO "identical_increments_stack_as_log_deltas_counter" ("name", "next") VALUES (?, ?) ON CONFLICT("name") DO UPDATE SET "next" = excluded."next"`, evolves_pre: true },
];
