...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    append_line: selectRows(seam, `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' THEN json_extract("channel", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("channel", '$.args')) || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("piece") AND json_type("piece") = 'object' THEN json_extract("piece", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("piece", '$.args')) || ')' ELSE "piece" END AS "piece" FROM "append_line"`, relColumns.append_line!, relColumnTypes.append_line!),
    log_text: selectRows(seam, `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' THEN json_extract("channel", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("channel", '$.args')) || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("next") AND json_type("next") = 'object' THEN json_extract("next", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("next", '$.args')) || ')' ELSE "next" END AS "next" FROM "log_text"`, relColumns.log_text!, relColumnTypes.log_text!),
  });
}

const finalSelect: Record<string, string> = {
  append_line: `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' THEN json_extract("channel", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("channel", '$.args')) || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("piece") AND json_type("piece") = 'object' THEN json_extract("piece", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("piece", '$.args')) || ')' ELSE "piece" END AS "piece" FROM "append_line"`,
  log_text: `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' THEN json_extract("channel", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("channel", '$.args')) || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("next") AND json_type("next") = 'object' THEN json_extract("next", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("next", '$.args')) || ')' ELSE "next" END AS "next" FROM "log_text"`,
};
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "append_line", kind: "log", tableName: "append_line", deltaTableName: "__delta_append_line", frontierTableName: "__frontier_append_line", nextFrontierTableName: "__next_frontier_append_line", columns: ["channel", "piece"], columnTypes: ["text", "text"], keyIndices: [], arrivalAddSql: `INSERT INTO "append_line" ("channel", "piece") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "channel", "piece"`, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' THEN json_extract("channel", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("channel", '$.args')) || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("piece") AND json_type("piece") = 'object' THEN json_extract("piece", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("piece", '$.args')) || ')' ELSE "piece" END AS "piece", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_append_line" WHERE "_sign" IN (-1, 1) GROUP BY "channel", "piece", "_sign"` },
  { rel: "log_text", kind: "set", tableName: "log_text", deltaTableName: "__delta_log_text", frontierTableName: "__frontier_log_text", nextFrontierTableName: "__next_frontier_log_text", columns: ["channel", "next"], columnTypes: ["text", "text"], keyIndices: [0], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' THEN json_extract("channel", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("channel", '$.args')) || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("next") AND json_type("next") = 'object' THEN json_extract("next", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("next", '$.args')) || ')' ELSE "next" END AS "next", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_log_text" WHERE "_sign" IN (-1, 1) GROUP BY "channel", "next", "_sign"` },
];
