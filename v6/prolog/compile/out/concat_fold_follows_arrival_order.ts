...
function read_snapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    ...
    log_text: select_rows(seam, `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' AND json_type("channel", '$.fn') = 'text' AND json_type("channel", '$.args') = 'array' THEN json_extract("channel", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("channel", '$.args')), '') || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("next") AND json_type("next") = 'object' AND json_type("next", '$.fn') = 'text' AND json_type("next", '$.args') = 'array' THEN json_extract("next", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("next", '$.args')), '') || ')' ELSE "next" END AS "next" FROM "__txt_log_text" t`, rel_columns.log_text!, rel_column_types.log_text!),
  });
}
...
const final_select: Record<string, string> = {
  ...
  log_text: `SELECT CASE WHEN json_valid("channel") AND json_type("channel") = 'object' AND json_type("channel", '$.fn') = 'text' AND json_type("channel", '$.args') = 'array' THEN json_extract("channel", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("channel", '$.args')), '') || ')' ELSE "channel" END AS "channel", CASE WHEN json_valid("next") AND json_type("next") = 'object' AND json_type("next", '$.fn') = 'text' AND json_type("next", '$.args') = 'array' THEN json_extract("next", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("next", '$.args')), '') || ')' ELSE "next" END AS "next" FROM "__txt_log_text" t`,
};
