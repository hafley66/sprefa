...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "body_page", kind: "set", tableName: "body_page", deltaTableName: "__delta_body_page", frontierTableName: "__frontier_body_page", nextFrontierTableName: "__next_frontier_body_page", columns: ["id", "view"], arrivalAddSql: `INSERT OR IGNORE INTO "body_page" ("id", "view") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "id", "view"`, boundarySql: `SELECT "id", "view", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_body_page" WHERE "_sign" IN (-1, 1) GROUP BY "id", "view", "_sign"` },
  { rel: "body_redirect", kind: "set", tableName: "body_redirect", deltaTableName: "__delta_body_redirect", frontierTableName: "__frontier_body_redirect", nextFrontierTableName: "__next_frontier_body_redirect", columns: ["id", "to"], arrivalAddSql: `INSERT OR IGNORE INTO "body_redirect" ("id", "to") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "id", "to"`, boundarySql: `SELECT "id", CASE WHEN json_valid("to") AND json_type("to") = 'object' THEN json_extract("to", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("to", '$.args')) || ')' ELSE "to" END AS "to", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_body_redirect" WHERE "_sign" IN (-1, 1) GROUP BY "id", "to", "_sign"` },
  { rel: "body_tag", kind: "set", tableName: "body_tag", deltaTableName: "__delta_body_tag", frontierTableName: "__frontier_body_tag", nextFrontierTableName: "__next_frontier_body_tag", columns: ["id", "tag"], arrivalAddSql: null, boundarySql: `SELECT "id", CASE WHEN json_valid("tag") AND json_type("tag") = 'object' THEN json_extract("tag", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("tag", '$.args')) || ')' ELSE "tag" END AS "tag", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_body_tag" WHERE "_sign" IN (-1, 1) GROUP BY "id", "tag", "_sign"` },
];
