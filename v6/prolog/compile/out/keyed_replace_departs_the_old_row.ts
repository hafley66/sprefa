...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "replaced_value", kind: "log", tableName: "replaced_value", deltaTableName: "__delta_replaced_value", frontierTableName: "__frontier_replaced_value", nextFrontierTableName: "__next_frontier_replaced_value", columns: ["key", "old_value"], columnTypes: ["text", "text"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("key") AND json_type("key") = 'object' THEN json_extract("key", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("key", '$.args')) || ')' ELSE "key" END AS "key", CASE WHEN json_valid("old_value") AND json_type("old_value") = 'object' THEN json_extract("old_value", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("old_value", '$.args')) || ')' ELSE "old_value" END AS "old_value", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_replaced_value" WHERE "_sign" IN (-1, 1) GROUP BY "key", "old_value", "_sign"` },
];
