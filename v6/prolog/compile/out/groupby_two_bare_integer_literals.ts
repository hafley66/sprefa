...
const boot: readonly IBootStatement[] = [
  { sql: `DELETE FROM "classified"`, params: [] },
  { sql: `INSERT OR IGNORE INTO "classified" ("name", "line", "column") SELECT b0."name", 0, 0 FROM "source" b0`, params: [] },
];
...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    classified: selectRows(seam, `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "line", "column" FROM "classified"`, relColumns.classified!, relColumnTypes.classified!),
    source: selectRows(seam, `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name" FROM "source"`, relColumns.source!, relColumnTypes.source!),
  });
}

const finalSelect: Record<string, string> = {
  classified: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "line", "column" FROM "classified"`,
  source: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name" FROM "source"`,
};
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "classified", kind: "set", tableName: "classified", deltaTableName: "__delta_classified", frontierTableName: "__frontier_classified", nextFrontierTableName: "__next_frontier_classified", columns: ["name", "line", "column"], columnTypes: ["text", "int", "int"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "line", "column", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_classified" WHERE "_sign" IN (-1, 1) GROUP BY "name", "line", "column", "_sign"` },
  { rel: "source", kind: "set", tableName: "source", deltaTableName: "__delta_source", frontierTableName: "__frontier_source", nextFrontierTableName: "__next_frontier_source", columns: ["name"], columnTypes: ["text"], keyIndices: [], arrivalAddSql: `INSERT OR IGNORE INTO "source" ("name") SELECT json_extract(value, '$[0]') FROM json_each(?) RETURNING "name"`, arrivalDelSql: `DELETE FROM "source" WHERE ("name") IN (SELECT json_extract(value, '$[0]') FROM json_each(?)) RETURNING "name"`, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_source" WHERE "_sign" IN (-1, 1) GROUP BY "name", "_sign"` },
];
