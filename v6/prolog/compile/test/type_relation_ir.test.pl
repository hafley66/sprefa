...
test(type_application_absent_from_frozen_rows_is_discovered_on_refreeze) :-
    Constructor = named(local, relation, box),
    Application = application(Constructor, [primitive(int)]),
    Decls = [ type_decl(holder, [col(value, box(int))]),
              semantic_type_rows([
                  declaration(Constructor, root, box, relation, compile_time),
                  declaration(named(local, relation, holder), root, holder,
                              relation, materialized),
                  member(named(local, relation, holder),
                         named(local, relation, holder), 1, value,
                         type_ref(Application)) ]) ],
    freeze_type_rows(Decls, Frozen),
    memberchk(semantic_type_rows(Rows), Frozen),
...
test(type_application_duplicate_requests_have_one_application_row) :-
    ...
    freeze_type_rows([semantic_type_rows(Rows)], Frozen),
    memberchk(semantic_type_rows(FrozenRows), Frozen),
    findall(Row, member(application(Application, _), FrozenRows), RowsForApp),
    RowsForApp = [application(Application, Constructor)].

test(type_application_recursive_constructor_is_refused,
     [throws(unsupported_construct(_))]) :-
    string_codes("rel Box(T)(value: Box(T)).\nrel Holder(value: Box(int)).\n",
                 Codes),
    parse_dl(Codes, Program, Bindings, []),
