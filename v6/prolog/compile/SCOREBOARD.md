...
Regenerated by the EXPRESSION + AGGREGATE LIFT arc (2026-07-29). The prose
sections below this one are historical and were written against the 110-fixture
corpus; the numbers here and in the two tables that follow come from
`out/manifest.json` + `out/run-results.json` as of that arc.

| bucket | count |
|---|---|
| fixtures swept | 120 |
| UNSUPPORTED (compiler refuses, named construct) | 60 |
| compiled (lowering + emission succeeded) | 60 |
| — of which IDENTICAL (tick log byte-identical to oracle) | 57 |
| — of which WRONG (diff vs oracle) | 0 |
| — of which run_error / no_oracle_log (rejection-path fixtures) | 3 |

IDENTICAL + run_error/no_oracle + UNSUPPORTED = 57 + 3 + 60 = 120.

Both emitter modes agree row for row: the incremental default and
`SPREFA_TSV2_EMITTER_MODE=naive` produce the same 57/0/3.
...
All three `final_wrong` rows are **pre-existing and unrelated to expressions**:
`retention_count_prunes_oldest` (keep(count) is not lowered at all — Finding 1,
which the tick log could never see), plus the two rejection-path fixtures that
throw before a final state exists.
