...
Regenerated by the doc-truth wave (2026-07-29). The prose sections below this
one are historical; the numbers here and in the two tables that follow come
from `out/manifest.json` + `out/run-results.json` after the new refusal
fixture.
...
IDENTICAL + run_error/no_oracle + UNSUPPORTED = 67 + 3 + 66 = 136.

Both emitter modes agree row for row: the incremental default and
`SPREFA_TSV2_EMITTER_MODE=naive` produce the same 67/0/3.

The doc-truth fixture is `keep_on_non_log_rel_rejected`: it adds one named
UNSUPPORTED result and leaves the compiled set and both emitter buckets
unchanged.
...
The two `final_wrong` rows are rejection-path fixtures that throw before a
final state exists. `retention_count_prunes_oldest` is final-identical in the
current sweep.
