...
| retention_count_prunes_oldest | engine_core.pl | identical | final_wrong |
| log_without_retention_rejected | engine_core.pl | no_oracle_log | no_oracle_final |
...
Regenerated from `out/manifest.json`. The comparison / arithmetic-bind /
head-arithmetic / aggregate_head buckets that dominated this table through
phase C are gone: they are lowered now (see the arc section below). What is
left is the `pre`/`now`/`latest`/`decode` edge-body family, which is its own
construct and explicitly out of the expression arc's scope.
...
| `edge_body_needs_bind` | 1 |

## EXPRESSION + AGGREGATE LIFT (2026-07-29)

Ruling `expression_residency` (fuse to SQL deltas, TS deopt last) executed.

Lifted, each fixture compiling AND grading byte-identical in both modes:
comparisons (`< =< > >= == \==`), `:=` / `is` binds, head arithmetic,
`concat/1`, and the `count`/`sum`/`min`/`max` aggregate heads.

Movement: compiled 34 -> 60, IDENTICAL 31 -> 57, WRONG 0 -> 0.

Three miscompiles were caught ON THE WAY IN and are the reason the arc has
receipts rather than just a bigger number:

1. **Guard dropped from the delta arm.** The guard walk went into the
   recompute insert, the refCount arm and the recursive-CTE arm, but not
   `level_delta_select_arm/7`. `dirty_retracts_on_matching_commit` retracted
   correctly at tick 2 and the tick-3 drain re-inserted the row off the
   frontier with `WorktreeDigest \== TreeDigest` simply not applied.
   `compile_body_guards/4` is now the single place any statement family turns
   body guards into SQL.
2. **`none` resolved to TEXT before the type fixpoint settled.** A rel with no
   literal witness anywhere entered the environment as text, text dominates the
   merge, and a genuinely-integer column typed by a SIBLING clause collapsed to
   text — taking all nine `timeless_rail.pl` fixtures out with
   `comparison_operand_not_int`.
3. **Cross-type join under affinity conversion** (found by the Q4
   reconciliation against `plans/2026-07-29-sqlite-udf-graft-verdict.md`).
   engine.pl joins by UNIFICATION, where `'1'` and `1` never unify. SQLite
   applies affinity conversion and joins them. Measured through the real
   driver: one row where the oracle derives none. Refused by name
   (`join_column_type_mismatch`), with two new fixtures pinning the oracle
   side.

Aggregates are maintained by **group-scoped** delete-then-recompute, never a
whole-table recompute: `count`/`sum` are decomposable and `min`/`max` are not
(the match-frontier lab records incremental min/max over a retractable set as
IMPOSSIBLE), so all four share one statement family and the scope table is
what keeps it cheap. `tests/aggregateScope.test.ts` is the COUNT/PLAN receipt
— 5,000 groups x 4 rows, one row retracted, `SEARCH ... USING PRIMARY KEY`
on both the scoped DELETE and the scoped re-derive, 1 row touched of 5,000.

Still refused, crack named rather than papered over: `json_array` /
`json_object`. A Prolog list value renders through the SHARED tick-log encoder
(`ticklog.pl:term_text/2`) as right-nested cons text
`[|](4,[|](4,[|](9,[])))`, and `json_object` as `obj([|](-(k,v),[]))`. Neither
is `json_group_array`/`json_group_object`'s own output text, so no ORDER BY
pinning reaches byte identity. The ordering rule itself is settled and
recorded (`msort/2` = Prolog standard order, duplicates kept); it is the
ENCODING, not the order, that blocks these two.
