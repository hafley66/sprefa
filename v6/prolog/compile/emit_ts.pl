...
header_lines(Name, Lines) :-
    format(atom(TitleLine), '// GENERATED by v6/prolog/compile (tsv2 emitter P2). Do not', []),
...
incremental_relation_entry_line(RelPlans, ArrivalStatements,
    ...
    format(atom(Line),
           '  { rel: "~w", kind: "~w", tableName: "~w", deltaTableName: "~w", frontierTableName: "~w", nextFrontierTableName: "~w", columns: ~w, arrivalAddSql: ~w, boundarySql: ~w },',
           [Name, Kind, Name, DeltaTable, FrontierTable, NextFrontierTable,
            ColumnsText, ArrivalAddTemplate, BoundaryTemplate]).
...
incremental_level_statement_entry_line(RelPlans,
        levelstmt(HeadRef, DeleteSql, InsertSqls, DeltaInsertSql), Line) :-
    ref_name(HeadRef, HeadName),
    format(atom(DeltaTable), '__delta_~w', [HeadName]),
    ...
    format(atom(Line),
           '  { headRel: "~w", headDeltaTableName: "~w", headColumns: ~w, insertSql: ~w, selectSql: ~w, recomputeSql: ~w },',
           [HeadName, DeltaTable, ColumnsText, DeltaInsertTemplate,
            SelectTemplate, RecomputeTemplate]).
...
run_incremental_tick_fn_lines(EdgeStatements, Lines) :-
    ...
    -> MergeLine = '    concatMap(() => of(undefined)),',
       ...
       RecomputeLine = '    concatMap(() => of(undefined)),'
    ;  MergeLine = '    concatMap(() => IncrementalRuntime.mergeNextIntoCurrent(seam, INCREMENTAL_RELATIONS)),',
       ...
       RecomputeLine = '    concatMap(() => IncrementalRuntime.recomputeLevelsAfterEdges(seam, INCREMENTAL_LEVEL_STATEMENTS, INCREMENTAL_RELATIONS)),'
    ),
    Lines =
