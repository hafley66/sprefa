...
% SPREFA_TSV2_EMITTER_MODE=naive and remains the fallback for P1-unsafe
% shapes and retraction ticks.
%
...
header_lines(Name, Lines) :-
    format(atom(TitleLine), '// GENERATED by v6/prolog/compile (tsv2 emitter P1). Do not', []),
...
incremental_relation_entry_line(RelPlans, ArrivalStatements,
    ...
    format(atom(Line),
           '  { rel: "~w", kind: "~w", tableName: "~w", deltaTableName: "~w", columns: ~w, arrivalAddSql: ~w, boundarySql: ~w },',
           [Name, Kind, Name, DeltaTable, ColumnsText, ArrivalAddTemplate,
            BoundaryTemplate]).
...
incremental_level_statement_entry_line(RelPlans,
        levelstmt(HeadRef, _DeleteSql, _InsertSqls, DeltaInsertSql), Line) :-
    ref_name(HeadRef, HeadName),
    format(atom(DeltaTable), '__delta_~w', [HeadName]),
    ...
    format(atom(Line),
           '  { headRel: "~w", headDeltaTableName: "~w", headColumns: ~w, insertSql: ~w },',
...
recompute_levels_fn_lines(LevelStatements, Lines) :-
    ...
    % single-clause case.
    findall(Sql, ( member(levelstmt(_, DeleteSql, InsertSqls, _), LevelStatements), ( Sql = DeleteSql ; member(Sql, InsertSqls) ) ), Sqls),
...
run_incremental_tick_fn_lines(EdgeStatements, Lines) :-
    incremental_carry_expr(EdgeStatements, CarryExpr),
    format(atom(CarryLine),
           '      carryPending: rels.some((delta) => ~w),',
           [CarryExpr]),
    Lines =
...
incremental_program_safe(plan(_, prog(_, Rules), _, _, _, _),
                         EdgeStatements, LevelStatements, Safe) :-
    ( EdgeStatements \== [], LevelStatements \== []
    -> Safe = false
    ; rules_have_only_positive_level_bodies(Rules)
    -> Safe = true
    ; Safe = false
    ).

rules_have_only_positive_level_bodies([]).
rules_have_only_positive_level_bodies([Rule | Rest]) :-
    ( Rule = (_ <+ _)
    -> true
    ; Rule = (_ <- Body),
    ...
    rules_have_only_positive_level_bodies(Rest).
...
emit_program(Name, Plan, Lowered, BootStatements, Text) :-
    ...
    incremental_mode_lines(IncrementalSafe, IncrementalModeLines),
    run_incremental_tick_fn_lines(EdgeStatements, RunIncrementalTickFnLines),
    incremental_plan_export_lines(IncrementalPlanExportLines),
