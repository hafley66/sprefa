...
% A list(T)-typed scalar: the expression answers the interned list id, and the
% caller owes the intern statements (content + members) that make the id real.
% Encoding carries the intern request; Encoding == direct means a plain scalar.
...
% ═══ list(T) in expression position: content-intern the list (slice 2) ═══════
% A list(T)-typed head position owes its entity and member rels the interned
% rows BEFORE the main insert reads a list id back out. Four statements per
% list, in order: the content json text into __str, the entity row keyed on
% that __str id, each member value into __str, then the member rows themselves
% keyed (list_id, idx). Content-interned, so the member insert is ON CONFLICT
% DO NOTHING -- rows are immutable.
