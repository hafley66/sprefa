% Lowers the two-rule reachability program below to a specialized main.rs.
% Run from this directory: swipl -g main -t halt emit_rust.pl
...
% Rule 1 seeds the set relation from the index relation; rule 2 joins the
% delta side to it on the shared column. Both drive every emit below.
...
emit_lines(SetRel, IndexRel, Source, JoinKey, Target) -->
    ...
    [ "// GENERATED by v6/prolog/labs/emit_rust_shootout/emit_rust.pl. Edits are lost.",
      "",
      "use std::fs::File;",
      ...
      "",
      "// Seen set sharded per source node, one FxHashSet of targets each: a",
      "// membership test touches one bucket, never the whole derived set.",
      S0,
      S1,
      "",
      "struct State {",
...
% The recursive rule's delta join. Lines are listed in emitted order: the
% trace line sits after the delta loop closes, so it fires once per round.
loop_lines(Source, JoinKey, Target) -->
    { format(string(ForDelta), '        for &(~w, ~w) in &state.delta {', [Source, JoinKey]),
      format(string(Lookup), '            if let Some(outgoing) = state.index.get(&~w) {', [JoinKey]),
      format(string(ForTarget), '                for &~w in outgoing {', [Target]),
      format(string(Insert), '                    if insert_pair(&mut state.seen, ~w, ~w) {', [Source, Target]),
      format(string(Push), '                        next.push((~w, ~w));', [Source, Target]) },
