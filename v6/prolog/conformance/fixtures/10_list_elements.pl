...
%   list_of_json_documents_round_trips    list(json) is spellable and survives a tick
%   nested_list_of_text_round_trips       list(list(text)) compiles, stores, renders byte-identically
...
fixture(list_of_json_documents_round_trips,
  prog([ col_type(batch/2, id, int),
         col_type(batch/2, payloads, list(json)) ],
...
fixture(non_array_value_at_list_column_is_refused,
  ...
  [ batch(1, 42) ],
  [],
  [ throws(type_arrival_shape_mismatch(batch/2, payloads, list(json),
                                       field_not_array(42))) ]).

fixture(wrong_element_type_is_refused,
  prog([ col_type(batch/2, id, int),
         col_type(batch/2, payloads, list(text)) ],
       []),
  [ batch(1, [alpha, 42]) ],
  [],
  [ throws(type_arrival_shape_mismatch(
             batch/2, payloads, list(text),
...
fixture(list_of_relation_refs_still_refused,
  prog([ type_decl(span, [col(start, int), col(end, int)]),
         ...
         col_type(doc/2, spans, list(span)) ],
       []),
