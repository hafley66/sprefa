...
fixture(switch_as_keyed_replace,
  prog([ kind(route_change/2, log), keep(route_change/2, all),
       ...
       [ (open_scope(SessionId, route_data(RouteId)) <+ only(route_change(SessionId, RouteId))),
...
fixture(merge_policy,
  prog([ kind(open_request/2, log), keep(open_request/2, all),
       ...
       [ (open_tab(SessionId, TabId) <+ only(open_request(SessionId, TabId))),
         (closed(SessionId, TabId) <+ only(close_request(SessionId, TabId))),
...
fixture(exhaust_policy,
  prog([ kind(open_request/2, log), keep(open_request/2, all),
         ...
         kind(tab_row/2, set),
         keyed(open_tab/2, [1]) ],
       [ (open_tab(SessionId, TabId) <+ only(open_request(SessionId, TabId)), not(live_tab(SessionId, _))),
         (closed(SessionId, TabId) <+ only(close_request(SessionId, TabId))),
         (live_tab(SessionId, TabId) <- open_tab(SessionId, TabId), not(closed(SessionId, TabId))),
         (demanded(tab(TabId), SessionId) <- live_tab(SessionId, TabId)),
         (tab_view(TabId, Body) <- demanded(tab(TabId), _), tab_row(TabId, Body)) ]),
...
fixture(concat_program_queue,
  prog([ kind(open_request/2, log), keep(open_request/2, all),
       ...
       [ (open_tab(SessionId, TabId) <+ only(open_request(SessionId, TabId)), not(live_tab(SessionId, _))),
         (queue_next(SessionId, Next) <+ only(open_request(SessionId, TabId)), live_tab(SessionId, _),
            pre(queue_next(SessionId, SoFar)), Next := SoFar + 1),
         (queue_slot(SessionId, Next, TabId) <+ only(open_request(SessionId, TabId)), live_tab(SessionId, _),
            pre(queue_next(SessionId, SoFar)), Next := SoFar + 1),
         (closed(SessionId, TabId) <+ only(close_request(SessionId, TabId))),
         (live_tab(SessionId, TabId) <- open_tab(SessionId, TabId), not(closed(SessionId, TabId))),
         ...
         (drained(SessionId, Ordinal) <+ only(departed(live_tab(SessionId, _))), pre(queue_head(SessionId, Ordinal))),
         (open_tab(SessionId, TabId) <+ only(departed(live_tab(SessionId, _))), pre(queue_head_tab(SessionId, TabId))),
         (demanded(tab(TabId), SessionId) <- live_tab(SessionId, TabId)),
         (tab_view(TabId, Body) <- demanded(tab(TabId), _), tab_row(TabId, Body)) ]),
...
fixture(scope_done_three_spellings,
  prog([ kind(stream_event/2, log), keep(stream_event/2, all),
       ...
       [ (closed_a(ScopeId) <+ only(stream_event(Target, done)), open_scope_a(ScopeId, Target)),
         ...
         (closed_c(ScopeId) <+ only(close_signal(ScopeId)), open_scope_c(ScopeId)),
...
fixture(completion_propagation_lattice_tick,
  prog([ kind(end_a_signal/1, log), keep(end_a_signal/1, all),
       ...
       [ (closed_outer(OuterId) <+ end_a_signal(OuterId), end_b_signal(OuterId), open_outer(OuterId)),
         ...
         (closed_inner(OuterId, InnerId) <+ only(end_c_signal(InnerId)), open_inner(OuterId, InnerId)),
...
fixture(fill_as_cache_update_swr,
  prog([ kind(fill_arrived/2, log), keep(fill_arrived/2, all),
       ...
       [ (demanded(Target, SessionId) <- open_feed(SessionId, Target)),
         (cache_row(Target, Body) <+ only(fill_arrived(Target, Body))),
