...
fixture(split_value_is_the_interned_list_id,
  ...
  [],
  [ final(row_parts/2, [ row_parts(path, [usr, local, bin]) ]),
...
fixture(two_producers_share_one_interned_list_id,
  ...
  [],
  [ final(left_parts/2, [ left_parts(one, [alpha, beta]) ]),
    final(right_parts/2, [ right_parts(two, [alpha, beta]) ]),
    final('__gen__list_text_df210f232c1299bd'/1,
...
fixture(list_element_type_flows_through_spread,
  ...
  [ symbol(emit_rust) ],
  [],
  [ final(symbol_parts/2, [ symbol_parts(emit_rust, [emit, rust]) ]),
    final(symbol_word/2,
...
fixture(empty_separator_interns_a_one_element_list,
  ...
  [ line(word, 'a,b') ],
  [],
  [ final(line_parts/2, [ line_parts(word, ['a,b']) ]),
    final('__gen__list_text_df210f232c1299bd'/1,
