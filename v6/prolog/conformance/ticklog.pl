...
json_value_term(json_object(Pairs)) :- is_list(Pairs).
json_value_term(json_array(Values)) :- is_list(Values).
json_value_term(json_null).

json_value_json('{}', '{}') :- !.
json_value_json({}(Fields), Json) :- !,
    json_canon({}(Fields), Canon),
    json_value_json(Canon, Json).
json_value_json(json_object(Pairs), Json) :- !,
    json_canon(json_object(Pairs), Canon), json_value_json(Canon, Json).
json_value_json(json_array(Values), Json) :- !,
    json_value_json(Values, Json).
json_value_json(json_null, null) :- !.
