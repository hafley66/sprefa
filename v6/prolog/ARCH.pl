...
task(ordered_aggregate_arc, unbuilt, [ordered_aggregate_lab]). % UNBLOCKED 2026-07-30 (lab verdict in, json-potholes registry/lower fence lifted at merge ddffb8b6): wire json_group_array(value) / json_group_array(value, ordinal) / group_concat(value, sep ORDER BY ordinal) per the verdict's 6-step arc list (parser+registry rows, both-axis lowering, scoped delete/seed/ordered insert beside the min/max plan, canonical tick-log fixtures incl EXECUTED minus-delta + empty-group + nested, the 4 sighting programs promoted). Deletes FOUR sightings: python mermaid assembly (self-map), v5-collect, json agg head refusals, extract-t2 rebuild.
...
task(golden_oracle_arrival_fix, unbuilt, [json_wiring]). % golden_oracle.pl carries a second copy of potholes-(b) (schedule_value/2, its own door); golden-flex.dl6 header documents the exact fix; needs the TS half too (out of the potholes lane fence)
