...
task(json_flex_lab,        unbuilt, []). % IN FLIGHT (opus lab, base a116e3e9, header plans/2026-07-30-json-flex-lab-header.md): grade-then-harden the json plane against the real json state machine. Seeded because coverage was measured THIN: 23 fixture entries, zero touching json null/floats/escapes/unicode, 2 live known defects (top-level scalar encodeValue sniff; json_extract null-collapse = one doc silently lost, option lab receipt). Q1-Q7 incl JSONTestSuite external oracle; slots incl json float fate + bignum contract
