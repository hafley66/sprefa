...
task(struct_host_output_seam, unbuilt, [struct_as_rows]).  % ESCALATED debt->BLOCKER 2026-07-29: terra's flow-parity value-plane rewrite is COMPLETE and HELD on branch codex/flow-parity (ccfe53ec, worktree removed) behind exactly this seam -- span-typed host outputs refuse as unsupported_surface(column_type_wrapper(Host, Col, none)) at the compiler AND serve/1_hosts.ts coerce stringifies struct outputs. Seam arc = compiler wrapper acceptance (WAITS on cq-bundle lane, shared files) + serve union widening; flow-parity merges right after.
task(probe_output_guard,   in_flight, []).                    % flagship gap 1: comparison guard over a probe OUTPUT var in a level rule refused as unsupported_construct(unbound_head_var(_G)) -- wrong name, no location (review-B4 in the wild; lower.pl:383 compile_guard_goals bound-set lacks response atom output columns). Shipped workaround = route through own rel. Fix = widen bound set + name the real reason + location
