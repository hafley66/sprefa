...
task(extract_one_hash_per_file, unbuilt, [extract_trail]). % FOUND by the hash span (PR #668): go and ts hash every file twice (the extract cache key, then the door's parse cache key); #664 removed a third. The pinned counts in 31_tracing fail on any change, so the fix moves the pin to 1 with a sabotage receipt. % phase spans (hash, parse, family, bind_plan, chain, tsi_syntax, tsi_semantic, flatten, write, resolve_leg), load average beside every timing, run + phase rows in ~/.agent/dl6.db, extract --trail, and the load-independent rail hash.calls == files. Brief plans/2026-09-03-extract-trail.BRIEF.md.
task(extract_tier_decline_is_a_diagnostic, unbuilt, [tsi_a2_multi_witness]). % FOUND 2026-09-03 by hand use: --ts-checker with node off PATH emits a plain syntax stream and says nothing (the reason 'ts checker tier off: no node driver: node is not on PATH' is an info log, silent by default). Under --witness a declined tier must be a `diagnostic` record (run=syntax, relation=tier.tsc, detail=<reason>) in the stream itself. Same for --rust-checker.
task(extract_resolve_carries_syntax_tsi_rows, unbuilt, [tsi_a4_syntax_rows]). % FOUND 2026-09-03: A4's syntax-tier tsi rows ride the per-file --family stream only; `--witness --resolve` emits run 0 with resolved_type_edge rows and zero tsi.* facts, so with the checker declined the stream has no type graph at all. project.rs should flatten aux.tsi under --witness on the resolve path too.
task(v7_loader_skips_foreign_records_by_name, unbuilt, [tsi_a7_v7_loader]). % FOUND 2026-09-03: load_tsi_stream/3 files one tsi_line(...malformed_record(resolved_type_edge)) diagnostic per non-TSI extract row (node, edge, sig, resolved_edge, ...). Known non-TSI records are skipped without a diagnostic; only an unknown `record` value is malformed.
