...
fork('2026-07-31', repo_extraction_executor, distinct_sprefa_extract_repo_executor, fall_through_to_the_generic_shell_executor,
     "the brief's second fork, resolved the same way ruling A resolved the first. A repo-scoped extraction host's inputs are three columns wide, which host_executor_contract(sprefa_extract, ...) refuses by exact positional list (measured: host_executor_mismatch). Widening that row was ruled out by the brief; the remaining two were a DISTINCT executor name with its own exact contract, or letting the declaration fall to `shell`. Falling through needs no row at all -- writing the template as '{repo}/{path}' already fails the match -- and costs the applicative fold (N named projections over one file become N subprocesses) while making a QUOTING CHARACTER decide which executor runs, which is the silence class this repo files as a defect. The distinct row is six lines of prolog and three of TS, and the fold is real value on a crawl. Backtrack = deleting the row falls straight through to shell with no other edit").
...
task(simplify_wave,        unbuilt, []).   % 19 deduped items from four read-only opus reviewers over 934dcc4d..HEAD (reuse/simplification/efficiency/altitude), brief plans/2026-07-29-simplify-wave-brief.md. P0 THREE ARE CORRECTNESS-ADJACENT, not cleanup: (1) lower.pl:1767 sniffs a `__dict_` NAME PREFIX to find dictionary relplans -- the banned magic-name pattern, and a user rel called __dict_x silently loses its delta arms; (2) 0_unsupported_messages.pl's umbrella renders unsupported_construct/1 only, so 1_host_expand.pl's 15 bare throws still print "Unknown message" (the B4 complaint, half-closed); (3) watch and enumerate write two DIFFERENT hash functions into one column named `digest` (2_binds.ts sha256 vs git hash-object) and nothing asserts they agree. Item 1 of the altitude set already landed at 6522f848
...
task(type_matrix_lab, labbed, [golden_flex_e2e]). % LANDED 2026-07-30 (merged, coordinator regraded on merged main: 422 cells = 79 identical / 156 DIVERGENT / 71 silent coercion / 116 unsupported construct; verdict plans/2026-07-30-type-matrix-verdict.md + addendum; lab dir ALIVE as fix-wave verification tool, symlinks committed). Slot recs: decl conflict = widen edge_head_column_type_mismatch to all positions (2 vs 155 divergences, the loud position is the working one); float/int boundary already collapsed, contract owes the sentence; undeclared text default fine, its two paths must agree. AWAITING USER: unsupported construct-widening blast radius, int_widens_to_float, wide_int_fate, bool_storage.
