...
task(csp_idioms_lab,       unbuilt, []). % IN FLIGHT (opus lab, base c1518683, header plans/2026-07-30-csp-idioms-lab-header.md): 9 CSP idioms cold-authored with Go referent comments, both doors, determinism double-runs; deliverable = boilerplate census (the evidence the seq(name) ruling asked for) + error census + slots (select spelling, rendezvous capacity-0, worker-pool fairness). Fixes nothing; silent-wrongs = fail-first findings
task(oracle_body_gate,     unbuilt, []). % IN FLIGHT (opus lane, base 3de711f3): surface/5-driven load-time body-goal gate on the oracle (prolog-review F1/F2 -- combine/next have no solve/2 clause while registry-LIVE, zip family silently derives nothing oracle-side), print_dl combine erasure, + booked defects group_concat_silent_miscompile and host_column_shadows_runtime as load refusals. edb_definition boundary preserved: bare relation atoms stay legal EDB
task(extract_t2_lab,       unbuilt, []). % IN FLIGHT (opus lab, base 3de711f3, header plans/2026-07-30-extract-t2-lab-header.md): schema reification = the diet-typecheck t2 rung; buy analysis first (quicktype/protoc-descriptor/avro/graphql), reify real openapi + proto through the json plane as dl6 programs, cross-repo who-calls-what-shape join vs t1 edges, 800-repo price projection
task(self_map_rail,        unbuilt, []). % IN FLIGHT (opus lane, base 3de711f3): the system describes its own arch in dl6 -- registry.pl/ARCH.pl/1_expansion.pl facts through sh hosts + json plane into rels, derived dataflow edges, v6/ARCH-MAP.md with mermaid fences; string assembly host-side (no string aggregate = named gap); manual `just self-map` byte-stable first, watch-bind live mode if clean; production rail class, never graded
