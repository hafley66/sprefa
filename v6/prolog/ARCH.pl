...
task(refusal_messages_arc, done, []). % LANDED 2026-07-31 (merge 3ffc1074, codex luna): beta gate 1 -- 107 specific refusal message clauses 0 fallback, parse errors line:col furthest-failure, text door file:line at/3; serve-path exit classifier three-shape fix by coordinator.
...
task(manifest_reason_diff, done, []). % PROPOSED by the fork_join lane 2026-07-31: a refusal-reason-level manifest diff as a standing check -- the lane's own first draft silently rewrote 2 edge fixtures' refusal reason (same bucket, same counts, sweep green) and only a manifest diff caught it. Small: sweep already writes the manifest; the check is a git-diff classifier over (name, bucket, reason-functor) triples.
...
task(type_ruling_round, unbuilt, [type_matrix_lab]). % the user-ruling half of the matrix findings: (a) widen decl-type refusal to level/agg heads + arrivals (needs blast-radius sweep), (b) int_widens_to_float (36 cells hang on it), (c) wide_int_fate (refuse / TEXT / driver change), (d) bool_storage (32 of 60 cells divergent; matrix = evidence for the golden plan bool-is-not-a-column-type rec). Present, do not improvise.
