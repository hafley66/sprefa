...
task(ordered_aggregate_arc, unbuilt, [json_flex_lab]). % QUEUED BY USER WORD 2026-07-30 ("yes"): one ordered string/array aggregate head construct deletes FOUR sightings in one day -- python mermaid assembly (self-map), v5-collect expressibility (effect-chain), json agg head refusals (json-wiring measured blocker: where the inner ORDER BY lives in the flat aggregate SELECT), extract-t2 Q5 python rebuild. Stream-lab ordinal machinery supplies the ORDER BY column naturally. Dispatch when json-potholes lane lands (registry/lower fence)
