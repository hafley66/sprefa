...
task(get_else_wiring,      unbuilt, []). % RULED 2026-07-30 (rulings.pl null_design = get_else_use_site_never_storage): candidate B (T? nullable columns) DEAD, null never enters storage or the type system, plans/2026-07-30-null-implementation-plan.md superseded. get_else/2 body operator: get_else(rel_atom(Args), Default) = LEFT JOIN + coalesce in SQL, ?? in rx, Datomic get-else precedent. Wiring = registry row + parse/print + oracle clause + lowering in every level-statement family + fixtures both doors. some/none stays the existing enum machinery (coverage counting), stacks with get_else; generic Option<T> rides monomorphization
