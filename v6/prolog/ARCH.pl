...
task(struct_as_rows,       in_flight, [tick_phase_alignment]). % RULED (rulings.pl compound_storage, user "lol d"): types-as-rels lab design EXECUTED -- struct value = rel row + content id ref; dictionary rows carry memoized rendered_text canonical JSON written at intern time (tick log prints VALUES never ids, the lab's rendered_text_stable_under_both_policies as a real grade); dictionaries boundary-invisible (storage plane, frontier-TEMP class); GC = refCount on the value DAG; json1 = untyped only; per-column ref(Type) coexistence. Header = plans/2026-07-29-struct-as-rows-header.md (4 slots). Unlocks: 20 json fixtures, byte spans, LSP line numbers. DISPATCHED 2026-07-29 morning, opus worktree, user go
