...
task(rust_checker_all_features, done, [tsi_a6_rust_semantic]). % LANDED 2026-09-03 (PR #680): rust_checker_ra.rs CargoConfig features: CargoFeatures::All; a supplied file owning no module in the loaded crate graph is a tier.rust-analyzer diagnostic on the semantic run; tests/107_rust_checker_features.rs 3, fixture rust_probe gains a gated module and an orphan file.
task(rust_checker_walk_scales_with_crate, done, [rust_checker_all_features]). % LANDED 2026-09-03 (PR #683): Impl::all_for_type over the supplied modules' adts and Impl::all_for_trait over their traits replace Impl::all_in_crate; src/trace.rs rust.impl 1139 -> 7; tests/108_rust_checker_walk_by_file.rs 2 + 1 ignored wall probe. The measurement showed walk_ms unchanged (~138s) with the item walk off: the cost is walk_file, row rust_checker_site_resolve_cost.
task(rust_checker_site_resolve_cost, done, [rust_checker_walk_scales_with_crate]). % LANDED 2026-09-03 (PR #686): a span per resolution kind (checker_type_path / checker_nav / checker_method / checker_call_path); resolve_path only on type-position paths (605 -> 165 calls), navs memoized per definition (355 -> 113); tests/109_rust_checker_site_cost.rs pins the counts. Measured: walk_ms on src/trace.rs is 81% sema.parse_guess_edition (crate_def_map_query over 508 crates, 143 of them this crate's own test-target crates sharing the source root), 19% first-touch body inference; release walk 5.0s, debug 96s. failure-modes 108.
task(rust_checker_def_map_over_sibling_targets, unbuilt, [rust_checker_site_resolve_cost]). % FORK for Chris (from PR #686): the first parse_guess_edition on a supplied file computes the def map of every crate in the loaded workspace, including every test target of the crate under analysis (cargo_workspace.rs:116 CargoConfig.all_targets defaults true). Options measured only in outline: (1) all_targets: false, drops the 143 test-target crates, loses #[cfg(test)] modules the set_test flag was added for; (2) narrow the loaded workspace to the package owning the supplied files; (3) accept: release walk is 5.0s on this crate, under the law, and this crate is the pathological case (its rust-checker feature pulls the rust-analyzer crate graph, 508 crates).
...
task(ts_syntax_type_graph_parity, done, [rust_syntax_type_graph]). % LANDED 2026-09-03 (PR #694, coordinator by hand: opus/fable 529, codex-acp and glm53f down): ts.rs tsi_type_id recursion, applications as tsi.called once per text with the origin on the head, tuples/literals/unions/function types anonymous shapes, keyword primitives without origin, has_type for typed const/let; tests/111_ts_syntax_graph.rs 7; 99 two assertions moved; 100/101 untouched and green. Was FOLLOW-UP: ts.rs tsi_target mints one id per written text with no tsi.called outside alias bodies, no tsi.primitive for string/number/boolean, no tsi.has_type for const; the rust lane fixes D2/D5/D6 on the rust door only.
task(tsi_name_row, done, [rust_syntax_type_graph, tsi_a6_rust_semantic]). % LANDED 2026-09-03 (user: "send the name"): tsi.name(Id, Text) in the registry; syntax tiers name every keyed written text, primitive class, parameter and callable (TsiNames::name), semantic tiers name nominal defs, symbols and rendered types (rust_checker_ra.rs name/1, ts_checker.mjs typeToString/symbol.name); tuples and impl blocks nameless. tests/110_tsi_name.rs 3. v7 loader side (tsi_relation('tsi.name', 2)) is codex-tsi's.
task(go_syntax_type_graph, done, [tsi_name_row]). % LANDED 2026-09-03 (PR #698, claude-280): src/lang/go_type_edges.rs beside project_types, structs/interfaces/type sets/generics/methods/aliases/consts/primitives per the brief table, fixture probe_graph.go, tests/112_go_syntax_graph.rs 12; v7 load reports the Go primitive classes absent until the prelude block lands (dl7 takeover). Was: go.rs emits zero tsi rows at 555869ed7; new go_type_edges.rs beside project_types, rows per the brief table, fixture probe_graph.go, tests/112. v7 prelude has no Go primitive block (codex-tsi).
task(dl7_tsi_render_takeover, unbuilt, [tsi_name_row]). % DISPATCHED 2026-09-03 (brief plans/2026-09-03-dl7-tsi-render-takeover.BRIEF.md, fable lane, codex-tsi stood down at f24ece60e / PR #689): U1 () as the empty product type, U2 dotted atoms User.T in reader+lowerer+emitter, U3 one type emitter over ts/rust/go, U4 Go primitive block in the prelude.
task(python_syntax_type_graph, done, [go_syntax_type_graph]). % LANDED 2026-09-03 (PR #700, fable lane grind-python-tsi): src/lang/python/_1_type_edges.rs, the go twin; 202 rows over tests/fixtures/tsi/probe_graph.py (name 38, type 35, origin 29, edge 23, has_type 23), --ingest rc=0, tests/117_python_syntax_graph.rs 12. v7 prelude lacks python primitive classes None/bytes/float/int (dl7 side); the v7 loader reports tsi.name unknown for every language.
task(go_type_call_module_grind, done, [go_syntax_type_graph]). % LANDED 2026-09-03 (PR #701, fable lane grind-go-graph2, coordinator re-measured): go.type.syntax.oracle-typedecl 73.37/97.04 -> 85.64/97.22 (go_edge_candidates emits uses for func/slice/map/alias specs and interface method specs, 792 rows); go.module.syntax.oracle precision 15.18 -> 100.00 (Projection::GoScope, the oracle loads no _test.go packages); x = f() on an unbound name records a bind-plan row (348 agreed-missed, was 351); review fix: assignment and := arms pair named children only. tests/114_go_graph_grind.rs 7. Forks in OPEN-PROBLEMS rows 15-16: implements rows 891 = 14.4 pt typedecl ceiling; vta closure $n rows 6,440.
task(ts_module_edges_grind, done, [extract_emit_throughput_budget]). % LANDED 2026-09-03 (PR #702, fable lane grind-ts-module, coordinator re-measured): ts5.module.{syntax,checker}.madge 50.57/32.85 -> 99.95/100.00, new depcruise Case rows equal; resolved_import kind=module (one row per literal specifier: static, export-from, side-effect, import x = require, import(), require()), bench normal_form keeps hops <= 1 rows only; ModuleFacts.requested_modules + RuntimeModuleRequests visitor in ts_resolve.rs; tests/113_ts_module_edges.rs 3. OPEN-PROBLEMS row 4 closed.
task(rust_call_syntax_grind, done, [rust_checker_site_resolve_cost]). % LANDED 2026-09-03 (PR #703, fable lane grind-rust-call, coordinator re-measured): rust.call.syntax codeql 64.96/76.17 -> 65.27/77.15, oracle 75.66/49.78 -> 76.19/50.56, scip_override 78.89/45.25 -> 79.36/45.85; Enum::Variant { .. } literals mint no call site (rust.rs is_variant_literal_path, 431 contradicted rows); init-chain receiver typing in rust_receivers.rs (assoc_rets keyed (self type, method), init_type/expr_type, T::new() -> T, call-result receivers). tests/116_rust_call_grind.rs 5 + ignored syntax dump. Rejected and measured: clone/unwrap passthrough (scip -0.28), let-else Some(x). Next lever: carry -> T per ImplEntry method and per field into RustModuleIndex so phase-1 receivers type across files (bare-ident receivers 4,253 of 6,134 sites). Pre-existing: rust.type.checker.oracle-typedecl reads 65.04/98.05 against floor 64.93/98.26 on origin/main (precision -0.21), owner grind-rust-type.
task(extract_more_data_research, done, [analysis_oracle_exam]). % LANDED 2026-09-03 (PR #705, fable lane research-more-data): plans/2026-09-03-extract-more-data/PLAN.md + PLAN.visual.human.unga.md, eight axes with receipts, top-8 ranking sec 10.2 (A heldout scip call oracle is callable-only + --indexer flag; B python/kotlin module plane; C cfg roles python/prolog/dl6; D kotlin tsi rows; E scip relationship -> tsi.conforms; F python trace oracle via sys.monitoring; G markdown link/fence doc_node rows; H go checker tier via go/types sidecar). Surprises: heldout SCORES rows scored against scip_fn_edge built from every occurrence (scip_v5_rels.rs:73-117), so the 8-41 recall rows are not an overfit finding; tsi.value/value_argument/scip_symbol registered with zero emitters; MarkdownSource and DataSource are in the roster (lang/mod.rs:87-100), CLAUDE.md's `source_for returns None for .md` is stale. Dispatched same day: D (grind-kotlin-tsi), A, C, G.
task(kotlin_syntax_type_graph, done, [python_syntax_type_graph]). % LANDED 2026-09-03 (PR #706, fable lane grind-kotlin-tsi): src/lang/kotlin_type_edges.rs, 16 tsi relations over tests/fixtures/tsi/probe_graph.kt (type 53, name 56, edge 40, has_type 25), T? as an anonymous sum of T and a null primitive, sealed/enum classes as sums, extension receivers own the callable and fill no slot; --ingest rc=0; tests/119_kotlin_syntax_graph.rs 15. Not covered: variance atoms, secondary constructors, companion objects, star projections, declarations inside bodies. v7 prelude has no kotlin primitive block (dl7 side).
task(markdown_link_fence_rows, done, [doc_format_extraction]). % LANDED 2026-09-03 (PR #707, fable lane grind-markdown-rows, arc G of extract_more_data_research): doc_node kinds link (target, title; reference links resolve through link_reference_definition, undefined labels mint nothing; autolinks) and image, code_block gains a body span (indented blocks: empty name); README.md 35 links / 26 fences; wire keys serialize only when present so heading rows are byte-identical; tests/125_markdown_links_fences.rs 4, 11_markdown +1, 22_doc_node strings gain body. CLAUDE.md markdown line corrected in the same ARCH PR.
task(python_call_pycg_grind, done, [python_syntax_type_graph]). % LANDED 2026-09-03 (PR #708, fable lane grind-python-call, coordinator re-scored): PyCG micro-suite recall 69.49 -> 86.86 at precision 100.00 (builtins 25 -> 100, classes 77 -> 100, lambdas 36 -> 86, dicts 37 -> 74, lists 46 -> 77, mro 71 -> 100); same-file rules in python/_0_source.rs: container literal slots with #-tagged int keys, <lambdaN> def names, call-result binds (PyCallBind aux row), attribute values by trailing name, param rule through bound and forwarded callees, map/filter call their args, __init__ through same-file bases, update() rebinds and mutators KILL; tests/118_python_call_grind.rs 10. Stops: cross-file param/return flow (ProjectCx carries no other file's aux rows), __iter__/__next__ dispatch, unions vs uniqueness, computed keys, eval. REGRESSION: extract --resolve --family call on click/core.py hangs past 15 s (0.36 s at 42ca51ff6), stack overflow over 17 files, frames PyResolver::param_target/callee_def/param_args; fix lane dispatched, PR #711 (+1.28, call results as values) HELD behind it.
task(heldout_scip_call_oracle_repair, done, [extract_more_data_research]). % LANDED 2026-09-03 (PR #710, fable lane grind-heldout-repair, arc A): heldout run.py scores against callable callees only (scip descriptor `().`), the parked lab harness landed (plans/extract-eval-2026-08-31/heldout/{run.py,SCORES.tsv,HELDOUT.tsv,REPORT.md,FINDINGS.md}), --indexer flag cherry-picked (failure-modes 109, tests/scip_indexer_pick.rs 6). Tuning controls vs scip: go 35.27 -> 97.12 (receipt met), ts 1.61 -> 61.97 (26 pt short: 57.2% of misses are caller attribution, nested `local N` callables dropped from fn_defs at scip_v5_rels.rs:340/:361), rust 26.01 -> 66.38 syntax / 90.06 checker. Held-out: go 92.96/79.29/93.53, python 52.57/61.66/63.93, ts umami 96.24, vite 58.91, trpc 44.70 with a tier.tsc decline (tsc RangeError max call stack, ts_checker.mjs; node --stack-size is the shape), rust rspack 63.29/83.74, clippy 91.64/92.32. The earlier 8-41 held-out rows were the reference-graph artifact, not overfit.
task(cfg_roles_python_prolog_dl6, done, [extract_more_data_research]). % LANDED 2026-09-03 (PR #712, fable lane grind-cfg-roles, arc C): cfg.rs PYTHON_ROLES/PROLOG_ROLES/DL6_ROLES, roles Clause/Cases/Fork/Negation/Try/Seq/SelfCall/Aggregate; probe_graph.py 0 -> 30 cfg rows, corpus_2_meta_use.pl 0 -> 11, 2_callee.dl6 0 -> 5; tests/124_cfg_python_prolog_dl6.rs 5, hand-derived edge sets. No arm wiring needed (cst already filled). Gaps: rust match / ts switch arms link as a sequence (Cases fixes python only), python for/while-else, conditional_expression role-free.
task(python_trace_oracle, done, [python_call_pycg_grind]). % LANDED 2026-09-03 (PR #713, fable lane grind-trace-oracle, arc F): python-oracle/trace/run.py (sys.monitoring, python 3.14.6), 105 of 119 mains ran (0 timeouts, 5 suite defects, 6 ext absent, 3 raise by design), 220 trace edges every one a PyCG row, 16 PyCG-only disagreements (12 past a crash line, 2 ext, 2 PyCG over-approximations); recall-of-covered 87.27 vs static 86.86, misses identical; pycg_score.py --oracle trace, default path byte-identical.
task(rust_type_edges_grind, done, [rust_call_syntax_grind]). % LANDED 2026-09-03 (PR #704, fable lane grind-rust-type, coordinator re-measured both legs): rust.type.syntax.oracle-typedecl 59.25/92.31 -> 76.99/93.29, rust.type.checker.oracle-typedecl 64.93/98.26 -> 84.19/98.36 (main read 65.04/98.05 before, precision 0.21 under its floor). Mechanisms: a bare impl self-type head is a uses candidate from the owner (rust.rs impl_self_type_candidates, classes X2+X2b 1,680 oracle rows); checker type answers keep every reference and a (file, name) collision narrows by written spelling then owner distance (rust_checker.rs type_at, CheckerRef.written from rust_checker_ra.rs type_ref; closes OPEN-PROBLEMS row 14). tests/115_rust_type_grind.rs 4; 94 pin renamed to ..._still_binds_both_declarations; golden_parity grader skips the v6-only self uses row. Residual 1,319: A1 assoc-type bodies 204, X3 qualifier owner 202, X1 outside corpus 192, E1b implements declared in another file 180, K field 178, B variant payload 150, row 11 macros 255 (user-gated).
task(python_kotlin_module_plane, done, [extract_more_data_research]). % LANDED 2026-09-03 (PR #714, fable lane grind-module-plane, arc B): python/_2_modules.rs PyModuleIndex (absolute, relative, __init__ packages, as, star) and kotlin_modules.rs KtModuleIndex (package -> files index, import a.b.C / a.b.* / as); resolved_import rows flask 312 (module 156), requests 381, kotlin fixture 12; tests/126_python_modules.rs 10, 127_kotlin_modules.rs 6; 54/57/62 module planes untouched. Not landed: python kind=import_resolve call edges through the plane; corpus-stats STATS.tsv rows_module not regenerated. Python files parse a second time for module facts (ts uses a handoff slot); 45_emit_throughput is the rail.
task(python_resolve_recursion_cycle, done, [python_call_pycg_grind]). % LANDED 2026-09-03 (PR #716, fable lane, coordinator re-timed): PyResolver memo per (callee, site) + per-site scratch for cut answers + PY_RESOLVE_DEPTH 12; click/core.py 0.16 s (was past 15 s at e08866c82), 17 click files 0.14 s, suite 86.86 unchanged; tests/118 +1 forwarding_ring_resolves_under_the_wall. failure-modes 110. Open: project.rs:1106 rayon workers run resolve on 2 MB stacks, a deep finite recursion aborts the whole run.
task(scip_relationship_conforms, done, [extract_more_data_research]). % LANDED 2026-09-03 (PR #718, opus lane grind-scip-conforms, arc E, coordinator re-ran the corpus): scip_relationship.is_implementation becomes resolved_type_edge kind implements (type satisfies interface) and kind overrides (callable satisfies callable), resolution_origin scip, and tsi.conforms(Impl, Iface, scip) under --witness; typescript-go 5,097 files: 967 implements / 2,025 overrides in 7.25 s, go implements recall 1.2 -> 92.36 at precision 88.73 (oracle 929 pairs); TypeScript-5.9 289 / 768. Fixed on the way: a scip-go index names build-cache documents by a path climbing out of the root and one such path failed the whole batched read, so every scip-informed run on typescript-go errored before (project.rs inside_project). Stated deviation: the implements target may sit outside the supplied paths. tests/128_scip_relationship_conforms.rs 10, fixture index.scip force-added past .gitignore:26 (follow-up: a negation line). Open: OPEN-PROBLEMS row 15, the parse-only go leg still has zero implements rows.
task(python_call_results_as_values, done, [python_resolve_recursion_cycle]). % LANDED 2026-09-03 (PR #719, the rebase of #711 after #716): call results spelled <call>@start in bind/arg/slot/return rows; PyCG 86.86 -> 88.14 at precision 100.00 (coordinator re-scored 208/236, click 17 files 0.16 s).
task(extract_ingest_test_flakes_under_load, unbuilt, []). % SEEN twice 2026-09-03: tests/97_ingest.rs an_impl_row_declares_its_own_symbol FAILED once inside a full --release battery under load 10-13 (coordinator, PR #702 gate, and the prior session), 16/16 x3 isolated both times. ingest() spawns extract --ingest /dev/stdin with a piped stdin; suspect the pipe path. Reproduce under load before fixing; failure-modes entry pending.
