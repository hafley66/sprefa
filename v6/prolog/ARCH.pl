...
task(depth2_ref_fix,       unbuilt, []). % IN FLIGHT (opus, lane/depth2-ref-fix): relation patterns at depth>=2 silently empty in emitter (json_extract on INTEGER __id), chained decode depth-2 dead in oracle. Fail-first at depth 2+3, cardinality 0/1/many, EXPLAIN SEARCH receipts, shared named refusal for out-of-scope shapes
task(golden_flex_e2e,      unbuilt, []). % IN FLIGHT (opus, lane/golden-e2e): USER DIRECTIVE -- one golden-flex.dl6 exercising EVERY live registry construct, registry-driven coverage gate (new construct fails golden until exercised = the language-kept-in-check mechanism), 0/1/many + perturbed cardinality grading, served-HTTP e2e leg; owns the serve-exit /idb race + hardcoded-port collision fixes. Motivated by the unit-maxing diagnosis: every bug this session was a composition bug under all-green single-construct suites
task(json_syntax_arc,      unbuilt, []). % IN FLIGHT lab (opus, lane/json-syntax-lab): USER DIRECTIVES -- json rel type lowering to sqlite json1, native json5-ish {} literals with v3/v4-style holes ({ means json for now), list types + generics folded in. Lab returns exact spelling CARDS; nothing lands without user sign-off (no-unsighted-syntax law)
task(openapi_codegen_spine, unbuilt, []). % IN FLIGHT lab (opus, lane/openapi-codegen): USER DIRECTIVE -- one openapi spec generated from prolog facts drives ts server now + rust axum post-flip + CLI; generalizes the cli_command/3 inventory-parity mechanism to HTTP routes; buy research first; emit_openapi.pl prototype + parity gate red/green receipt
task(v5_parity_spelunk,    unbuilt, []). % IN FLIGHT (opus, lane/v5-parity-spelunk): USER IDEA -- dl6 programs mechanically extract v5's op/builtin inventory + examples construct usage (regex/comment-mark hosts) and join against the v6 registry into parity_row(thing, count, covered|partial|absent); plus the git-fact diags rail class (enumerate vs enumerate_at antijoin = new-file diags, ratchet via count vs baseline)
