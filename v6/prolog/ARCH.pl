...
task(struct_host_output_seam, unbuilt, [struct_as_rows]).  % the LSP-line-number blocker, named seam not language gap: serve/1_hosts.ts coerce/3 JSON-stringifies nested objects before the arrival row and IHostColumnPlan.type is closed union int|text|json -- struct-typed host outputs cannot arrive. Fix = widen the union + pass objects through OR StructPlane parses canonical JSON for ref columns; plus diag-rail.dl6 + flagship-callgraph.dl6 rewrites. serve/ ownership was outside the struct arc
