...
task(assign_composition_lab, in_flight, []). % opus worktree lab, RUNNING: `:=` composes from match/scan/slice, assign = shorthand at most; ruling cards on landing; verdict lands at plans/2026-07-29-assign-composition-verdict.md. ITS BRIEFING IS KNOWN-WRONG ON ONE WORD: "slice" was briefed as destructuring and is sub-SPAN projection (corrected by plans/2026-07-29-file-span-design.md after dispatch). Lab protocol applies on landing
task(finish_the_job_epic,  in_flight, []). % opus worktree, RUNNING: plans/2026-07-29-finish-the-job-epic.md; instructed codex-driveable (sol/luna/terra owner-shapes), carries schema_import_epic + the bool/precision rulings + the decl-legibility cluster (type/rel indistinguishable, entity-extras spelling, plane visibility)
