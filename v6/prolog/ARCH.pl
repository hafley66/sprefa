...
task(rust_checker_walk_scales_with_crate, labbed, [rust_checker_all_features]). % FOUND 2026-09-03 by hand probe: --rust-checker over ONE ungated file (src/trace.rs, 1 file answered) walked 187s and emitted 1129 rust.impl / 1129 tsi.conforms / 983 tsi.callable / 1501 tsi.type: rust_checker_ra.rs:392 Impl::all_in_crate walks every impl of every crate a supplied file touches, so the walk is priced by the crate, never by the file. Under the 10-second law the walk is a defect (only the workspace LOAD carries the SCIP exception, project.rs CHECKER_BUDGET comment). Fix shape: impls whose self type or trait is declared in a supplied module, plus impls the supplied declarations name; everything else stays a leaf. Dispatch after rust_checker_all_features lands (same file). Brief plans/2026-09-03-rust-checker-walk-by-file.BRIEF.md.
task(rust_checker_site_resolve_cost, labbed, [rust_checker_walk_scales_with_crate]). % FOUND 2026-09-03 by PR #683 measurement: with the item walk OFF, --rust-checker over src/trace.rs still reports walk_ms 138310; the time is walk_file (rust_checker_ra.rs:168), resolve_method_call / resolve_path / try_to_nav over every syntax node of the file, 113 method sites, unmeasured per site. Brief plans/2026-09-03-rust-checker-site-resolve-cost.BRIEF.md: span each resolution kind first, then fix only what the spans name.
