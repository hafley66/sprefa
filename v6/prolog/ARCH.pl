...
task(ts_syntax_type_graph_parity, unbuilt, [rust_syntax_type_graph]). % DISPATCHED 2026-09-03 (brief plans/2026-09-03-ts-syntax-type-graph.BRIEF.md). FOLLOW-UP: ts.rs tsi_target mints one id per written text with no tsi.called outside alias bodies, no tsi.primitive for string/number/boolean, no tsi.has_type for const; the rust lane fixes D2/D5/D6 on the rust door only.
