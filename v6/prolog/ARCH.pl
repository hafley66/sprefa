...
task(golden_oracle_arrival_fix, active, [json_wiring]). % IN FLIGHT 2026-07-30 as the golden_json_seam codex lane (base 5ce31a24, brief plans/2026-07-30-golden-json-seam-brief.md): golden_oracle.pl schedule_value gets the dl6_oracle type-directed arrival treatment (shared impl) + golden-schedules.ts canonicalizes json-column arrivals; then the json axis (spread/$/**/{}/typed capture/list(T)) flexes in golden-flex.dl6 and coverage bookkeeping moves those rows to exercised.
task(type_matrix_lab, labbing, [golden_flex_e2e]). % IN FLIGHT 2026-07-30 (opus worktree, base 5ce31a24, header plans/2026-07-30-type-matrix-lab-header.md): decl-type x value-type x position cross product as a RUNNING regenerable matrix, both doors, 4-way mechanical classification; divergent cells (the oracle-4-vs-emitter-"4" class) become fail-first fixture candidates + a recommended refusal set. Slots: decl_source_conflict_fate, float_int_boundary, undeclared_column_default.
