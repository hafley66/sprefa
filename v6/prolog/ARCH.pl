...
task(scan_surface_composition_lab, queued, [scan_match_reconciliation]). % USER-DIRECTED 2026-07-30, SOL LAB ONLY. Find a tolerable nestable scan+match form with multiple keyed demand instances and multiple subscribers. Prove definition sharing versus state-instance identity, init/reset, demand add/remove, cancellation, subscriber fanout, 0/1/N reducer rows, clocks, table counts, and exact expansion to ordinary named rels, pre, keys, and edge rules. No production syntax or semantics until user rules on priced cards.
