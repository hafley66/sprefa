...
task(ordered_aggregate_lab, labbing, [json_flex_lab]). % LAB IN FLIGHT 2026-07-30 (user: "dont trust arch go back into a lab please with luna mid think codex"): codex gpt-5.6-luna medium, worktree ../sprefa-codex-ordagg base 855da45f, header plans/2026-07-30-ordered-aggregate-lab-header.md. Re-derives the ordered string/array aggregate design from v5 evidence (json_group_array ORDER BY the VALUE itself = v5 answer, no group_concat exists there); grades value-sort vs explicit-ordinal axes, string join, incremental group-scoped SQL drafts, oracle byte-equality. Fenced to labs/ + verdict doc; wiring = the follow-up arc.
task(ordered_aggregate_arc, unbuilt, [ordered_aggregate_lab]). % the WIRING arc, now gated on the lab verdict (was gated on json-potholes only): one ordered string/array aggregate head construct deletes FOUR sightings in one day -- python mermaid assembly (self-map), v5-collect expressibility (effect-chain), json agg head refusals (json-wiring measured blocker: where the inner ORDER BY lives in the flat aggregate SELECT), extract-t2 Q5 python rebuild. Stream-lab ordinal machinery supplies the ORDER BY column naturally. Still respects the json-potholes registry/lower fence
