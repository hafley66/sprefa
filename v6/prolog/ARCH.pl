...
task(incremental_affinity_drop, active, [type_matrix_lab]). % P0, FIX LANE IN FLIGHT (codex luna high, base 0b08ee75, brief plans/2026-07-30-incremental-affinity-drop-brief.md): DEFAULT emitter drops arrival deltas when sqlite affinity rewrites the value (1_incremental.ts RETURNING-vs-wire JSON.stringify; row stored, delta gone, downstream blind, ~50 matrix cells). Fix = delta carries the STORED value from RETURNING. Fail-first fixture + matrix before/after = the receipts.
