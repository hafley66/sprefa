...
impl Harness for Codex {
    ...
    /// `token_count` snapshots carry no per-call id; `message_id` below uses
    /// the line's byte offset, so re-syncing a range dedups but never merges.
...
fn project_line(
    ...
    match record_type {
        ...
        "token_count" => {
            ...
            let message_id = line.start.to_string();
            let usage = UsageRow {
                ts,
