 ...
 */
function golden_oracle_log(program_source: string, schedule: readonly IArrivalBatch[]): string {
  const work_dir = mkdtempSync(join(tmpdir(), "golden-flex-oracle-"));
  ...
  const run = spawnSync(
    "swipl",
    ["-q", "-l", GOLDEN_ORACLE_PL, "-g", `oracle_ticklog('${program_path}', '${schedule_path}')`, "-g", "halt"],
    { encoding: "utf8" },
  );
 ...
 */
function revive_struct_columns(schedule: readonly IArrivalBatch[]): readonly IArrivalBatch[] {
...
function tree_row(index: number): (string | number | object)[] {
...
async function wait_until(predicate: () => Promise<boolean>, what: string, timeout_ms = 20_000): Promise<void> {
...
test("golden-flex served: the live host runs, and the served tick log matches the oracle replayed on the served schedule", async () => {
  ...
  try {
    ...
    await post_arrivals(served.port, [{ rel: "quarantined", sign: "add", row: ["weed"] }]);
    await post_arrivals(served.port, [
      { rel: "tree", sign: "add", row: tree_row(1) as never },
      { rel: "tree", sign: "add", row: tree_row(2) as never },
      { rel: "tree", sign: "add", row: tree_row(3) as never },
    ...
    await wait_until(async () => (await rows_of(served.port, "weighed")).length === 2, "both weigh answers to land");

    // The value plane, two hops deep, read back over http.
    const trees = await rows_of(served.port, "tree");
    ...
    const patches = await rows_of(served.port, "patch");
    const plots = await rows_of(served.port, "plot");
    ...
    assert.deepEqual((await rows_of(served.port, "tree_label")).find((row) => row[0] === 1), [1, "patch-1"]);
    ...
    const weighed = [...(await rows_of(served.port, "weighed"))].sort((left, right) => Number(left[0]) - Number(right[0]));
    ...
    assert.deepEqual((await rows_of(served.port, "pick_stats")).find((row) => row[0] === 1), [1, 1, 11, 11, 11]);
    assert.deepEqual((await rows_of(served.port, "picked_by_ada")), [[1]]);
    ...
    // produce at all.
    assert.deepEqual(await rows_of(served.port, "last_picker"), [[1, "ada"]]);
    assert.deepEqual(
      [...(await rows_of(served.port, "pickable"))].map((row) => row[0]).sort(),
      [1, 3],
    ...
    const replayed = revive_struct_columns(schedule_from_ticks(outcomes, plans.arrival_targets));
    assert.equal(log_of_ticks(outcomes), golden_oracle_log(source, replayed));
  } finally {
    await served.stop();
  }
});
