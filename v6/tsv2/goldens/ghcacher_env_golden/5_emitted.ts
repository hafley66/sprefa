...
function finalStateLine(
  ...
  relColumnTypes: Readonly<Record<string, readonly IRowColumnType[]>> | undefined,
): string {
  const parts: string[] = [];
...
async function readFinal(program: IServedProgram, seam: ReturnType<typeof ScratchStore.open>): Promise<string> {
  const entries = await Promise.all(
    Object.keys(program.finalSelect).map(async (rel) => {
      const result = await firstValueFrom(seam.runner.execute(seam.db, program.finalSelect[rel]!));
      const columns = program.relColumns[rel] ?? [];
      const types = program.relColumnTypes?.[rel];
      const rows = result.rows.map((row) =>
        columns.map((column, index) => rowValueFromSql(types?.[index], row[column])),
      );
      return { rel, rows };
    }),
  );
...
async function main(): Promise<void> {
  ...
  if (moduleFile === undefined || scheduleFile === undefined) {
    process.stderr.write(
      "usage: node --experimental-transform-types 5_emitted.ts <module.ts> <schedule.json>\n",
    );
    process.exitCode = 2;
    return;
  }
  const loaded = (await import(pathToFileURL(resolve(moduleFile)).href)) as { readonly program: IServedProgram };
  const schedule = JSON.parse(readFileSync(scheduleFile, "utf8")) as readonly IArrivalBatch[];
