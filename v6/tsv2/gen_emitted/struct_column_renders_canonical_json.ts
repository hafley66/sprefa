...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    ...
    touched: selectRows(seam, `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' AND json_type("path", '$.fn') = 'text' AND json_type("path", '$.args') = 'array' THEN json_extract("path", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("path", '$.args')), '') || ')' ELSE "path" END AS "path" FROM "touched"`, relColumns.touched!, relColumnTypes.touched!),
  });
}
...
INSERT OR IGNORE INTO "touched" ("path") SELECT b0."path" FROM "finding" b0`, supportSql: [`DELETE FROM "__support_next_touched"`, `INSERT INTO "__support_next_touched" ("path", "__refcount") SELECT "path", sum("__refcount") FROM (SELECT b0."path" AS "path", count(*) AS "__refcount" FROM "finding" b0 GROUP BY b0."path") GROUP BY "path"`, `UPDATE "touched" AS h SET "__refcount" = "__refcount" - ("__refcount" - COALESCE((SELECT n."__refcount" FROM "__support_next_touched" n WHERE n."path" = h."path"), 0))`, `INSERT INTO "__delta_touched" ("_sign", "_sequence", "path") SELECT -1, row_number() OVER () - 1, "path" FROM "touched" WHERE "__refcount" <= 0`, `DELETE FROM "touched" WHERE "__refcount" <= 0`, `INSERT INTO "__delta_touched" ("_sign", "_sequence", "path") SELECT 1, row_number() OVER () - 1, n."path" FROM "__support_next_touched" n WHERE NOT EXISTS (SELECT 1 FROM "touched" h WHERE n."path" = h."path")`, `INSERT INTO "__frontier_touched" ("_phase", "_sequence", "path") SELECT ?, row_number() OVER () - 1, n."path" FROM "__support_next_touched" n WHERE NOT EXISTS (SELECT 1 FROM "touched" h WHERE n."path" = h."path")`, `INSERT INTO "__next_frontier_touched" ("_phase", "_sequence", "path") SELECT ?, row_number() OVER () - 1, n."path" FROM "__support_next_touched" n WHERE NOT EXISTS (SELECT 1 FROM "touched" h WHERE n."path" = h."path")`, `INSERT INTO "touched" ("path", "__refcount") SELECT n."path", n."__refcount" FROM "__support_next_touched" n WHERE NOT EXISTS (SELECT 1 FROM "touched" h WHERE n."path" = h."path")`], aggregateSql: null },
];
