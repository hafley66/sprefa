...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    finding: selectRows(seam, `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", (SELECT d."__rendered" FROM "__ref_span" d WHERE d."__id" = "at") AS "at" FROM "finding"`, relColumns.finding!, relColumnTypes.finding!),
    span: selectRows(seam, `SELECT "start", "end" FROM "span"`, relColumns.span!, relColumnTypes.span!),
    touched: selectRows(seam, `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path" FROM "touched"`, relColumns.touched!, relColumnTypes.touched!),
  });
}

const finalSelect: Record<string, string> = {
  finding: `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", (SELECT d."__rendered" FROM "__ref_span" d WHERE d."__id" = "at") AS "at" FROM "finding"`,
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "touched", kind: "set", tableName: "touched", deltaTableName: "__delta_touched", frontierTableName: "__frontier_touched", nextFrontierTableName: "__next_frontier_touched", columns: ["path"], columnTypes: ["text"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_touched" WHERE "_sign" IN (-1, 1) GROUP BY "path", "_sign"` },
];
