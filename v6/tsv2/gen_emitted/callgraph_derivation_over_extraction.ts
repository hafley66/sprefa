...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    ...
    calls: selectRows(seam, `SELECT CASE WHEN json_valid("caller") AND json_type("caller") = 'object' THEN json_extract("caller", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("caller", '$.args')) || ')' ELSE "caller" END AS "caller", CASE WHEN json_valid("callee") AND json_type("callee") = 'object' THEN json_extract("callee", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("callee", '$.args')) || ')' ELSE "callee" END AS "callee" FROM "calls"`, relColumns.calls!, relColumnTypes.calls!),
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "calls", kind: "set", tableName: "calls", deltaTableName: "__delta_calls", frontierTableName: "__frontier_calls", nextFrontierTableName: "__next_frontier_calls", columns: ["caller", "callee"], columnTypes: ["text", "text"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("caller") AND json_type("caller") = 'object' THEN json_extract("caller", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("caller", '$.args')) || ')' ELSE "caller" END AS "caller", CASE WHEN json_valid("callee") AND json_type("callee") = 'object' THEN json_extract("callee", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("callee", '$.args')) || ')' ELSE "callee" END AS "callee", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_calls" WHERE "_sign" IN (-1, 1) GROUP BY "caller", "callee", "_sign"` },
