...
const ddl: readonly string[] = [
  `CREATE TABLE "mean" ("group" TEXT NOT NULL, "value" REAL NOT NULL CHECK (typeof("value") = 'real' AND "value" BETWEEN -1.7976931348623157e308 AND 1.7976931348623157e308), "__support_count" INTEGER NOT NULL DEFAULT 1, PRIMARY KEY ("group", "value")) WITHOUT ROWID`,
...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    mean: selectRows(seam, `SELECT CASE WHEN json_valid("group") AND json_type("group") = 'object' THEN json_extract("group", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("group", '$.args')) || ')' ELSE "group" END AS "group", "value" FROM "mean"`, relColumns.mean!, relColumnTypes.mean!),
    score: selectRows(seam, `SELECT CASE WHEN json_valid("group") AND json_type("group") = 'object' THEN json_extract("group", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("group", '$.args')) || ')' ELSE "group" END AS "group", "value" FROM "score"`, relColumns.score!, relColumnTypes.score!),
  });
}

const finalSelect: Record<string, string> = {
  mean: `SELECT CASE WHEN json_valid("group") AND json_type("group") = 'object' THEN json_extract("group", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("group", '$.args')) || ')' ELSE "group" END AS "group", "value" FROM "mean"`,
  score: `SELECT CASE WHEN json_valid("group") AND json_type("group") = 'object' THEN json_extract("group", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("group", '$.args')) || ')' ELSE "group" END AS "group", "value" FROM "score"`,
};
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "mean", kind: "set", tableName: "mean", deltaTableName: "__delta_mean", frontierTableName: "__frontier_mean", nextFrontierTableName: "__next_frontier_mean", columns: ["group", "value"], columnTypes: ["text", "float"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("group") AND json_type("group") = 'object' THEN json_extract("group", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("group", '$.args')) || ')' ELSE "group" END AS "group", "value", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_mean" WHERE "_sign" IN (-1, 1) GROUP BY "group", "value", "_sign"` },
  { rel: "score", kind: "set", tableName: "score", deltaTableName: "__delta_score", frontierTableName: "__frontier_score", nextFrontierTableName: "__next_frontier_score", columns: ["group", "value"], columnTypes: ["text", "float"], keyIndices: [], arrivalAddSql: `INSERT OR IGNORE INTO "score" ("group", "value") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "group", "value"`, arrivalDelSql: `DELETE FROM "score" WHERE ("group", "value") IN (SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?)) RETURNING "group", "value"`, boundarySql: `SELECT CASE WHEN json_valid("group") AND json_type("group") = 'object' THEN json_extract("group", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("group", '$.args')) || ')' ELSE "group" END AS "group", "value", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_score" WHERE "_sign" IN (-1, 1) GROUP BY "group", "value", "_sign"` },
];
