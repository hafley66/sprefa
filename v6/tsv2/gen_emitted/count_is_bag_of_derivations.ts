...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    hit: selectRows(seam, `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", "line", "col3" FROM "hit"`, relColumns.hit!, relColumnTypes.hit!),
    hits: selectRows(seam, `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", "col2" FROM "hits"`, relColumns.hits!, relColumnTypes.hits!),
  });
}
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "hits", kind: "set", tableName: "hits", deltaTableName: "__delta_hits", frontierTableName: "__frontier_hits", nextFrontierTableName: "__next_frontier_hits", columns: ["path", "col2"], columnTypes: ["text", "int"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", "col2", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_hits" WHERE "_sign" IN (-1, 1) GROUP BY "path", "col2", "_sign"` },
];
