...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    labelled: selectRows(seam, `SELECT "tree_id", CASE WHEN json_valid("label") AND json_type("label") = 'object' THEN json_extract("label", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("label", '$.args')) || ')' ELSE "label" END AS "label" FROM "labelled"`, relColumns.labelled!, relColumnTypes.labelled!),
    name: selectRows(seam, `SELECT "tree_id", CASE WHEN json_valid("label") AND json_type("label") = 'object' THEN json_extract("label", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("label", '$.args')) || ')' ELSE "label" END AS "label" FROM "name"`, relColumns.name!, relColumnTypes.name!),
    ping: selectRows(seam, `SELECT "tree_id" FROM "ping"`, relColumns.ping!, relColumnTypes.ping!),
  });
}

const finalSelect: Record<string, string> = {
  labelled: `SELECT "tree_id", CASE WHEN json_valid("label") AND json_type("label") = 'object' THEN json_extract("label", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("label", '$.args')) || ')' ELSE "label" END AS "label" FROM "labelled"`,
  name: `SELECT "tree_id", CASE WHEN json_valid("label") AND json_type("label") = 'object' THEN json_extract("label", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("label", '$.args')) || ')' ELSE "label" END AS "label" FROM "name"`,
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "labelled", kind: "log", tableName: "labelled", deltaTableName: "__delta_labelled", frontierTableName: "__frontier_labelled", nextFrontierTableName: "__next_frontier_labelled", columns: ["tree_id", "label"], columnTypes: ["int", "text"], keyIndices: [], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT "tree_id", CASE WHEN json_valid("label") AND json_type("label") = 'object' THEN json_extract("label", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("label", '$.args')) || ')' ELSE "label" END AS "label", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_labelled" WHERE "_sign" IN (-1, 1) GROUP BY "tree_id", "label", "_sign"` },
  { rel: "name", kind: "set", tableName: "name", deltaTableName: "__delta_name", frontierTableName: "__frontier_name", nextFrontierTableName: "__next_frontier_name", columns: ["tree_id", "label"], columnTypes: ["int", "text"], keyIndices: [], arrivalAddSql: `INSERT OR IGNORE INTO "name" ("tree_id", "label") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "tree_id", "label"`, arrivalDelSql: `DELETE FROM "name" WHERE ("tree_id", "label") IN (SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?)) RETURNING "tree_id", "label"`, boundarySql: `SELECT "tree_id", CASE WHEN json_valid("label") AND json_type("label") = 'object' THEN json_extract("label", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("label", '$.args')) || ')' ELSE "label" END AS "label", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_name" WHERE "_sign" IN (-1, 1) GROUP BY "tree_id", "label", "_sign"` },
