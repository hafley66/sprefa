...
const boot: readonly IBootStatement[] = [
  { sql: `INSERT OR IGNORE INTO "left" ("name", "value") VALUES (?, ?)`, params: ["alpha", 0.3] },
  { sql: `INSERT OR IGNORE INTO "left" ("name", "value") VALUES (?, ?)`, params: ["beta", 0.30000000000000004] },
  { sql: `INSERT OR IGNORE INTO "right" ("name", "value") VALUES (?, ?)`, params: ["alpha", 0.30000000000000004] },
  { sql: `INSERT OR IGNORE INTO "right" ("name", "value") VALUES (?, ?)`, params: ["beta", 0.30000000000000004] },
  { sql: `DELETE FROM "matched"`, params: [] },
  { sql: `INSERT OR IGNORE INTO "matched" ("name") SELECT b0."name" FROM "left" b0, "right" b1 WHERE b1."name" = b0."name" AND b1."value" = b0."value"`, params: [] },
];
...
const INCREMENTAL_LEVEL_STATEMENTS: readonly IIncrementalLevelStatement[] = [
  { headRel: "matched", headDeltaTableName: "__delta_matched", headColumns: ["name"], insertSql: `INSERT OR IGNORE INTO "matched" ("name") SELECT DISTINCT d0."name" FROM "__frontier_left" d0, "right" b0 WHERE d0."_phase" >= 0 AND b0."name" = d0."name" AND b0."value" = d0."value" UNION ALL SELECT DISTINCT d0."name" FROM "__frontier_right" d0, "left" b0 WHERE d0."_phase" >= 0 AND b0."name" = d0."name" AND b0."value" = d0."value" RETURNING "name"`, selectSql: `SELECT "name" FROM "matched"`, recomputeSql: `DELETE FROM "matched";
