...
// hand-edit; recompile. Program: relation_pattern_wrong_target_rejected.
...
const boot: readonly IBootStatement[] = [
  ...
  { sql: `INSERT OR IGNORE INTO "seen" ("start") SELECT b0."start" FROM "span" b0 WHERE json_extract(b0."file", '$.fn') = 'fpath' AND json_extract(b0."file", '$.args[0]') = 'a.rs'`, params: [] },
];
...
function arrivalStatement(arrival: IArrivalRow): SqlStatement {
  ...
  if (template === undefined) {
    throw new Error(`relation_pattern_wrong_target_rejected: tick received an arrival for undeclared rel '${arrival.rel}'`);
  }
  if (arrival.sign === "del") {
    if (template.kind === "log") {
      throw new Error(`relation_pattern_wrong_target_rejected: retract from log rel '${arrival.rel}' (engine.pl retract_from_log)`);
    }
    if (template.delSql === null) {
      throw new Error(`relation_pattern_wrong_target_rejected: rel '${arrival.rel}' has no delete statement`);
    }
...
const INCREMENTAL_LEVEL_STATEMENTS: readonly IIncrementalLevelStatement[] = [
  { headRel: "seen", headDeltaTableName: "__delta_seen", headColumns: ["start"], insertSql: `INSERT OR IGNORE INTO "seen" ("start") SELECT DISTINCT d0."start" FROM "__frontier_span" d0 WHERE d0."_phase" >= 0 AND json_extract(d0."file", '$.fn') = 'fpath' AND json_extract(d0."file", '$.args[0]') = 'a.rs' RETURNING "start"`, selectSql: `SELECT "start" FROM "seen"`, recomputeSql: `DELETE FROM "seen";\nINSERT OR IGNORE INTO "seen" ("start") SELECT b0."start" FROM "span" b0 WHERE json_extract(b0."file", '$.fn') = 'fpath' AND json_extract(b0."file", '$.args[0]') = 'a.rs'`, supportSql: [`DELETE FROM "__support_next_seen"`, `INSERT INTO "__support_next_seen" ("start", "__support_count") SELECT "start", sum("__support_count") FROM (SELECT b0."start" AS "start", count(*) AS "__support_count" FROM "span" b0 WHERE json_extract(b0."file", '$.fn') = 'fpath' AND json_extract(b0."file", '$.args[0]') = 'a.rs' GROUP BY b0."start") GROUP BY "start"`, `UPDATE "seen" AS h SET "__support_count" = "__support_count" - ("__support_count" - COALESCE((SELECT n."__support_count" FROM "__support_next_seen" n WHERE n."start" = h."start"), 0))`, `DELETE FROM "seen" WHERE "__support_count" <= 0 RETURNING "start"`, `INSERT INTO "seen" ("start", "__support_count") SELECT "start", n."__support_count" FROM "__support_next_seen" n WHERE NOT EXISTS (SELECT 1 FROM "seen" h WHERE n."start" = h."start") RETURNING "start"`], aggregateSql: null },
];

function recomputeLevels(seam: ISqlSeam): Observable<void> {
  const sql = `DELETE FROM "seen";\nINSERT OR IGNORE INTO "seen" ("start") SELECT b0."start" FROM "span" b0 WHERE json_extract(b0."file", '$.fn') = 'fpath' AND json_extract(b0."file", '$.args[0]') = 'a.rs'`;
...
function runNaiveTick(seam: ISqlSeam, arrivals: IArrivalBatch): Observable<ITickDeltas> {
  ...
  // relation_pattern_wrong_target_rejected: no edge rules -- absorb arrivals, recompute levels, diff.
}
...
export const program: IGenProgramWithBoot = {
  name: "relation_pattern_wrong_target_rejected",
  ddl,
  relColumns,
  relColumnTypes,
  arrivalTargets,
