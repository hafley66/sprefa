...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "doubled", kind: "set", tableName: "doubled", deltaTableName: "__delta_doubled", frontierTableName: "__frontier_doubled", nextFrontierTableName: "__next_frontier_doubled", columns: ["name", "out"], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "out", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_doubled" WHERE "_sign" IN (-1, 1) GROUP BY "name", "out", "_sign"` },
  { rel: "reading", kind: "set", tableName: "reading", deltaTableName: "__delta_reading", frontierTableName: "__frontier_reading", nextFrontierTableName: "__next_frontier_reading", columns: ["name", "value"], arrivalAddSql: `INSERT OR IGNORE INTO "reading" ("name", "value") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "name", "value"`, arrivalDelSql: `DELETE FROM "reading" WHERE ("name", "value") IN (SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?)) RETURNING "name", "value"`, boundarySql: `SELECT CASE WHEN json_valid("name") AND json_type("name") = 'object' THEN json_extract("name", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("name", '$.args')) || ')' ELSE "name" END AS "name", "value", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_reading" WHERE "_sign" IN (-1, 1) GROUP BY "name", "value", "_sign"` },
];
