...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "route_view", kind: "set", tableName: "route_view", deltaTableName: "__delta_route_view", frontierTableName: "__frontier_route_view", nextFrontierTableName: "__next_frontier_route_view", columns: ["route_id", "body"], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT CASE WHEN json_valid("route_id") AND json_type("route_id") = 'object' THEN json_extract("route_id", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("route_id", '$.args')) || ')' ELSE "route_id" END AS "route_id", CASE WHEN json_valid("body") AND json_type("body") = 'object' THEN json_extract("body", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("body", '$.args')) || ')' ELSE "body" END AS "body", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_route_view" WHERE "_sign" IN (-1, 1) GROUP BY "route_id", "body", "_sign"` },
];
