...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    current_tree: selectRows(seam, `SELECT CASE WHEN json_valid("path") AND json_type("path") = 'object' AND json_type("path", '$.fn') = 'text' AND json_type("path", '$.args') = 'array' THEN json_extract("path", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("path", '$.args')), '') || ')' ELSE "path" END AS "path", CASE WHEN json_valid("digest") AND json_type("digest") = 'object' AND json_type("digest", '$.fn') = 'text' AND json_type("digest", '$.args') = 'array' THEN json_extract("digest", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("digest", '$.args')), '') || ')' ELSE "digest" END AS "digest" FROM "current_tree"`, relColumns.current_tree!, relColumnTypes.current_tree!),
...
const ARRIVAL_STATEMENTS: Record<string, { kind: "log" | "set"; addSql: string; delSql: string | null }> = {
  ...
  tree_file: { kind: "set", addSql: `INSERT INTO "tree_file" ("rev_id", "path", "digest") VALUES (?, ?, ?) ON CONFLICT ("rev_id", "path") DO UPDATE SET "digest" = excluded."digest"`, delSql: `DELETE FROM "tree_file" WHERE "rev_id" = ? AND "path" = ? AND "digest" = ?` },
};
...
const INCREMENTAL_EDGE_STATEMENTS: readonly IIncrementalEdgeStatement[] = [
  { headRel: "head", ruleId: "head_move_flips_current_tree_in_one_tick:head/2#1", headKind: "set", headTableName: "head", headDeltaTableName: "__delta_head", headColumns: ["repo_id", "rev_id"], keyIndices: [0], projectSql: `SELECT d0."repo_id" AS "repo_id", d0."rev_id" AS "rev_id" FROM "__frontier_head_move" d0 WHERE d0."_phase" >= 0 ORDER BY d0."_phase", d0."_sequence"` },
];
...
INSERT OR IGNORE INTO "current_tree" ("path", "digest") SELECT b1."path", b1."digest" FROM "head" b0, "tree_file" b1 WHERE b1."rev_id" = b0."rev_id"`, supportSql: [`DELETE FROM "__support_next_current_tree"`, `INSERT INTO "__support_next_current_tree" ("path", "digest", "__refcount") SELECT "path", "digest", sum("__refcount") FROM (SELECT b1."path" AS "path", b1."digest" AS "digest", count(*) AS "__refcount" FROM "head" b0, "tree_file" b1 WHERE b1."rev_id" = b0."rev_id" GROUP BY b1."path", b1."digest") GROUP BY "path", "digest"`, `UPDATE "current_tree" AS h SET "__refcount" = "__refcount" - ("__refcount" - COALESCE((SELECT n."__refcount" FROM "__support_next_current_tree" n WHERE n."path" = h."path" AND n."digest" = h."digest"), 0))`, `INSERT INTO "__delta_current_tree" ("_sign", "_sequence", "path", "digest") SELECT -1, row_number() OVER () - 1, "path", "digest" FROM "current_tree" WHERE "__refcount" <= 0`, `DELETE FROM "current_tree" WHERE "__refcount" <= 0`, `INSERT INTO "__delta_current_tree" ("_sign", "_sequence", "path", "digest") SELECT 1, row_number() OVER () - 1, n."path", n."digest" FROM "__support_next_current_tree" n WHERE NOT EXISTS (SELECT 1 FROM "current_tree" h WHERE n."path" = h."path" AND n."digest" = h."digest")`, `INSERT INTO "__frontier_current_tree" ("_phase", "_sequence", "path", "digest") SELECT ?, row_number() OVER () - 1, n."path", n."digest" FROM "__support_next_current_tree" n WHERE NOT EXISTS (SELECT 1 FROM "current_tree" h WHERE n."path" = h."path" AND n."digest" = h."digest")`, `INSERT INTO "__next_frontier_current_tree" ("_phase", "_sequence", "path", "digest") SELECT ?, row_number() OVER () - 1, n."path", n."digest" FROM "__support_next_current_tree" n WHERE NOT EXISTS (SELECT 1 FROM "current_tree" h WHERE n."path" = h."path" AND n."digest" = h."digest")`, `INSERT INTO "current_tree" ("path", "digest", "__refcount") SELECT n."path", n."digest", n."__refcount" FROM "__support_next_current_tree" n WHERE NOT EXISTS (SELECT 1 FROM "current_tree" h WHERE n."path" = h."path" AND n."digest" = h."digest")`], aggregateSql: null },
];
