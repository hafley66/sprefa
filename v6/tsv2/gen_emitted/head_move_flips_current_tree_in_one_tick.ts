...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    ...
    tree_file: selectRows(seam, `SELECT CASE WHEN json_valid("rev_id") AND json_type("rev_id") = 'object' THEN json_extract("rev_id", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("rev_id", '$.args')) || ')' ELSE "rev_id" END AS "rev_id", CASE WHEN json_valid("path") AND json_type("path") = 'object' THEN json_extract("path", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("path", '$.args')) || ')' ELSE "path" END AS "path", CASE WHEN json_valid("digest") AND json_type("digest") = 'object' THEN json_extract("digest", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("digest", '$.args')) || ')' ELSE "digest" END AS "digest" FROM "tree_file"`, relColumns.tree_file!),
  });
}
