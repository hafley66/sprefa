...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    hit: selectRows(seam, `SELECT CASE WHEN json_valid("owner") AND json_type("owner") = 'object' THEN json_extract("owner", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("owner", '$.args')) || ')' ELSE "owner" END AS "owner", (SELECT d."__rendered" FROM "__dict_span" d WHERE d."__id" = "at") AS "at" FROM "hit"`, relColumns.hit!),
  });
}

const finalSelect: Record<string, string> = {
  hit: `SELECT CASE WHEN json_valid("owner") AND json_type("owner") = 'object' THEN json_extract("owner", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("owner", '$.args')) || ')' ELSE "owner" END AS "owner", (SELECT d."__rendered" FROM "__dict_span" d WHERE d."__id" = "at") AS "at" FROM "hit"`,
};
