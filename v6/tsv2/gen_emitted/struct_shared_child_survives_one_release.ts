...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    hit: selectRows(seam, `SELECT CASE WHEN json_valid("owner") AND json_type("owner") = 'object' THEN json_extract("owner", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("owner", '$.args')) || ')' ELSE "owner" END AS "owner", "at" FROM "hit"`, relColumns.hit!),
    span: selectRows(seam, `SELECT "start", "end" FROM "span"`, relColumns.span!),
  });
}

const finalSelect: Record<string, string> = {
  hit: `SELECT CASE WHEN json_valid("owner") AND json_type("owner") = 'object' THEN json_extract("owner", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("owner", '$.args')) || ')' ELSE "owner" END AS "owner", "at" FROM "hit"`,
