...
export const demandedRels: readonly string[] = ["event/1", "star_event/2", "total/2"];
...
const ddl: readonly string[] = [
  ...
  `CREATE TABLE "star_event" ("repo" TEXT NOT NULL, "stars" INTEGER NOT NULL, "__support_count" INTEGER NOT NULL DEFAULT 1, PRIMARY KEY ("repo", "stars")) WITHOUT ROWID`,
  ...
  `CREATE TEMP TABLE "__support_next_star_event" ("repo" TEXT NOT NULL, "stars" INTEGER NOT NULL, "__support_count" INTEGER NOT NULL, PRIMARY KEY ("repo", "stars")) WITHOUT ROWID`,
...
const boot: readonly IBootStatement[] = [
  { sql: `DELETE FROM "star_event"`, params: [] },
  { sql: `INSERT OR IGNORE INTO "star_event" ("repo", "stars") SELECT json_extract(b0."payload", '$."repo"'), json_extract(b0."payload", '$."stars"') FROM "event" b0 WHERE json_type(b0."payload", '$') = 'object' AND json_type(b0."payload", '$."repo"') = 'text' AND json_type(b0."payload", '$."stars"') = 'integer'`, params: [] },
];
...
INSERT OR IGNORE INTO "star_event" ("repo", "stars") SELECT json_extract(b0."payload", '$."repo"'), json_extract(b0."payload", '$."stars"') FROM "event" b0 WHERE json_type(b0."payload", '$') = 'object' AND json_type(b0."payload", '$."repo"') = 'text' AND json_type(b0."payload", '$."stars"') = 'integer'`, supportSql: [`DELETE FROM "__support_next_star_event"`, `INSERT INTO "__support_next_star_event" ("repo", "stars", "__support_count") SELECT "repo", "stars", sum("__support_count") FROM (SELECT json_extract(b0."payload", '$."repo"') AS "repo", json_extract(b0."payload", '$."stars"') AS "stars", count(*) AS "__support_count" FROM "event" b0 WHERE json_type(b0."payload", '$') = 'object' AND json_type(b0."payload", '$."repo"') = 'text' AND json_type(b0."payload", '$."stars"') = 'integer' GROUP BY json_extract(b0."payload", '$."repo"'), json_extract(b0."payload", '$."stars"')) GROUP BY "repo", "stars"`, `UPDATE "star_event" AS h SET "__support_count" = "__support_count" - ("__support_count" - COALESCE((SELECT n."__support_count" FROM "__support_next_star_event" n WHERE n."repo" = h."repo" AND n."stars" = h."stars"), 0))`, `DELETE FROM "star_event" WHERE "__support_count" <= 0 RETURNING "repo", "stars"`, `INSERT INTO "star_event" ("repo", "stars", "__support_count") SELECT "repo", "stars", n."__support_count" FROM "__support_next_star_event" n WHERE NOT EXISTS (SELECT 1 FROM "star_event" h WHERE n."repo" = h."repo" AND n."stars" = h."stars") RETURNING "repo", "stars"`], aggregateSql: null },
];
