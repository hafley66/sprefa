...
const finalSelect: Record<string, string> = {
  current: `SELECT "id", CASE WHEN json_valid("kind") AND json_type("kind") = 'object' THEN json_extract("kind", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("kind", '$.args')) || ')' ELSE "kind" END AS "kind" FROM "current"`,
  event: `SELECT "id", CASE WHEN json_valid("kind") AND json_type("kind") = 'object' THEN json_extract("kind", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("kind", '$.args')) || ')' ELSE "kind" END AS "kind" FROM "event"`,
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  ...
  { rel: "result_ok", kind: "set", tableName: "result_ok", deltaTableName: "__delta_result_ok", frontierTableName: "__frontier_result_ok", nextFrontierTableName: "__next_frontier_result_ok", columns: ["id", "value"], arrivalAddSql: `INSERT OR IGNORE INTO "result_ok" ("id", "value") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "id", "value"`, arrivalDelSql: `DELETE FROM "result_ok" WHERE ("id", "value") IN (SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?)) RETURNING "id", "value"`, boundarySql: `SELECT "id", CASE WHEN json_valid("value") AND json_type("value") = 'object' THEN json_extract("value", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("value", '$.args')) || ')' ELSE "value" END AS "value", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_result_ok" WHERE "_sign" IN (-1, 1) GROUP BY "id", "value", "_sign"` },
  { rel: "result_tag", kind: "set", tableName: "result_tag", deltaTableName: "__delta_result_tag", frontierTableName: "__frontier_result_tag", nextFrontierTableName: "__next_frontier_result_tag", columns: ["id", "tag"], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT "id", CASE WHEN json_valid("tag") AND json_type("tag") = 'object' THEN json_extract("tag", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("tag", '$.args')) || ')' ELSE "tag" END AS "tag", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_result_tag" WHERE "_sign" IN (-1, 1) GROUP BY "id", "tag", "_sign"` },
];
