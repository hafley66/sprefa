...
const relCatalog: readonly IRelCatalogRow[] = [
  ...
  { relId: 6, parentId: 0, ordinal: 0, localName: "created_at_pinned_updated_at_advances", kind: "module", typeId: 0, arity: 0, moduleId: 6, hId: "17cf1a5e0b03187b", hSchema: "", hRule: "" },
  { relId: 7, parentId: 6, ordinal: 0, localName: "arrive", kind: "rel", typeId: 0, arity: 2, moduleId: 6, hId: "3bd8b2aa486a9b60", hSchema: "a1949cdd1cb051b0", hRule: "" },
  { relId: 8, parentId: 7, ordinal: 1, localName: "id", kind: "column", typeId: 2, arity: 0, moduleId: 6, hId: "1f14d3a4da12f284", hSchema: "", hRule: "" },
  { relId: 9, parentId: 7, ordinal: 2, localName: "payload", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "b40f0a485ab36d7c", hSchema: "", hRule: "" },
  { relId: 10, parentId: 6, ordinal: 0, localName: "thing", kind: "rel", typeId: 0, arity: 4, moduleId: 6, hId: "37de4e5c855f07d0", hSchema: "34da9fe5520c333d", hRule: "55e0b8ae3c2235d8" },
  { relId: 11, parentId: 10, ordinal: 1, localName: "id", kind: "column", typeId: 2, arity: 0, moduleId: 6, hId: "8821581ff851576f", hSchema: "", hRule: "" },
  { relId: 12, parentId: 10, ordinal: 2, localName: "payload", kind: "column", typeId: 1, arity: 0, moduleId: 6, hId: "4a66d8620f2b7ac6", hSchema: "", hRule: "" },
  { relId: 13, parentId: 10, ordinal: 3, localName: "born", kind: "column", typeId: 2, arity: 0, moduleId: 6, hId: "df304c07c8b5ed87", hSchema: "", hRule: "" },
  { relId: 14, parentId: 10, ordinal: 4, localName: "tick", kind: "column", typeId: 2, arity: 0, moduleId: 6, hId: "f992ac7b965716d8", hSchema: "", hRule: "" },
];

const relDeclaredColumnTypes: Record<string, readonly string[]> = {
};

const arrivalTargets: readonly string[] = ["arrive"];
...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    arrive: selectRows(seam, `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' AND json_type("payload", '$.fn') = 'text' AND json_type("payload", '$.args') = 'array' THEN json_extract("payload", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("payload", '$.args')), '') || ')' ELSE "payload" END AS "payload" FROM "arrive"`, relColumns.arrive!, relColumnTypes.arrive!),
    thing: selectRows(seam, `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' AND json_type("payload", '$.fn') = 'text' AND json_type("payload", '$.args') = 'array' THEN json_extract("payload", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("payload", '$.args')), '') || ')' ELSE "payload" END AS "payload", "born", "tick" FROM "thing"`, relColumns.thing!, relColumnTypes.thing!),
  });
}
...
const ARRIVAL_STATEMENTS: Record<string, { kind: "log" | "set"; addSql: string; delSql: string | null }> = {
  arrive: { kind: "log", addSql: `INSERT INTO "arrive" ("id", "payload") VALUES (?, ?)`, delSql: null },
};
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "arrive", kind: "log", tableName: "arrive", deltaTableName: "__delta_arrive", frontierTableName: "__frontier_arrive", nextFrontierTableName: "__next_frontier_arrive", columns: ["id", "payload"], columnTypes: ["int", "text"], keyIndices: [], arrivalAddSql: `INSERT INTO "arrive" ("id", "payload") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "id", "payload"`, arrivalDelSql: null, boundarySql: `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' AND json_type("payload", '$.fn') = 'text' AND json_type("payload", '$.args') = 'array' THEN json_extract("payload", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("payload", '$.args')), '') || ')' ELSE "payload" END AS "payload", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_arrive" WHERE "_sign" IN (-1, 1) GROUP BY "id", "payload", "_sign"`, ruleObservers: ["thing/4"] },
  { rel: "thing", kind: "set", tableName: "thing", deltaTableName: "__delta_thing", frontierTableName: "__frontier_thing", nextFrontierTableName: "__next_frontier_thing", columns: ["id", "payload", "born", "tick"], columnTypes: ["int", "text", "int", "int"], keyIndices: [0], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' AND json_type("payload", '$.fn') = 'text' AND json_type("payload", '$.args') = 'array' THEN json_extract("payload", '$.fn') || '(' || coalesce((SELECT group_concat(value, ',') FROM json_each("payload", '$.args')), '') || ')' ELSE "payload" END AS "payload", "born", "tick", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_thing" WHERE "_sign" IN (-1, 1) GROUP BY "id", "payload", "born", "tick", "_sign"`, ruleObservers: [] },
];
...
function resolveThing_0Writes(seam: ISqlSeam, before: Snapshot, arrivals: IArrivalBatch): Observable<readonly SqlStatement[]> {
  const triggerRows = triggerOccurrences("log", "arrive", before.arrive, arrivals);
  if (triggerRows.length === 0) return of([]);
  return forkJoin(triggerRows.map((arrival) => seam.runner.execute(seam.db, { sql: EDGE_THING_0_PROJECT_SQL, args: bindArgs(arrival.row) }))).pipe(
    map((results) => {
      const resolved = new Map<string, IRow>();
      for (const result of results) {
        const projectedRows = result.rows.map((row) => EDGE_THING_0_HEAD_COLUMNS.map((column) => row[column] as IRowValue) as IRow);
        for (const projectedRow of projectedRows) {
          const key = JSON.stringify(EDGE_THING_0_KEY_INDICES.map((index) => projectedRow[index]));
          resolved.set(key, projectedRow);
        }
      }
      return [...resolved.values()].map((row): SqlStatement => ({ sql: EDGE_THING_0_WRITE_SQL, args: bindArgs(row) }));
    }),
  );
}
...
function resolveThing_1Writes(seam: ISqlSeam, before: Snapshot, arrivals: IArrivalBatch): Observable<readonly SqlStatement[]> {
  const triggerRows = triggerOccurrences("log", "arrive", before.arrive, arrivals);
  if (triggerRows.length === 0) return of([]);
  return forkJoin(triggerRows.map((arrival) => seam.runner.execute(seam.db, { sql: EDGE_THING_1_PROJECT_SQL, args: bindArgs(arrival.row) }))).pipe(
    map((results) => {
      const resolved = new Map<string, IRow>();
      for (const result of results) {
        const projectedRows = result.rows.map((row) => EDGE_THING_1_HEAD_COLUMNS.map((column) => row[column] as IRowValue) as IRow);
        for (const projectedRow of projectedRows) {
          const key = JSON.stringify(EDGE_THING_1_KEY_INDICES.map((index) => projectedRow[index]));
          resolved.set(key, projectedRow);
        }
      }
      return [...resolved.values()].map((row): SqlStatement => ({ sql: EDGE_THING_1_WRITE_SQL, args: bindArgs(row) }));
    }),
  );
}
...
const ORDERED_EDGE_ARMS: readonly IOrderedEdgeArm[] = [
  { triggerRel: "arrive", triggerKind: "arrival", headRel: "thing", headKind: "set", headColumns: ["id", "payload", "born", "tick"], keyIndices: [0], projectSql: `SELECT ?1 AS "id", ?2 AS "payload", b0."born" AS "born", (SELECT "n" FROM "__tick") AS "tick" FROM "__pre_thing" b0 WHERE b0."id" = ?1`, writeSql: `INSERT INTO "thing" ("id", "payload", "born", "tick") VALUES (?, ?, ?, ?) ON CONFLICT("id") DO UPDATE SET "payload" = excluded."payload", "born" = excluded."born", "tick" = excluded."tick"`, evolvesPre: true },
  { triggerRel: "arrive", triggerKind: "arrival", headRel: "thing", headKind: "set", headColumns: ["id", "payload", "born", "tick"], keyIndices: [0], projectSql: `SELECT ?1 AS "id", ?2 AS "payload", (SELECT "n" FROM "__tick") AS "born", (SELECT "n" FROM "__tick") AS "tick" WHERE NOT EXISTS (SELECT 1 FROM "thing" n0 WHERE n0."id" = ?1)`, writeSql: `INSERT INTO "thing" ("id", "payload", "born", "tick") VALUES (?, ?, ?, ?) ON CONFLICT("id") DO UPDATE SET "payload" = excluded."payload", "born" = excluded."born", "tick" = excluded."tick"`, evolvesPre: true },
];
...
function orderedOutsideOccurrences(before: Snapshot, arrivals: IArrivalBatch): readonly IOrderedOccurrence[] {
  ...
  for (const arrival of triggerOccurrences("log", "arrive", before["arrive"], arrivals)) accepted.add(arrival);
...
function buildDeltas(before: Snapshot, after: Snapshot): ITickDeltas {
  const arrive = multisetDiff(before.arrive, after.arrive);
  const thing = multisetDiff(before.thing, after.thing);
  return {
    rels: [
      { rel: "arrive", add: arrive.add, del: arrive.del },
      { rel: "thing", add: thing.add, del: thing.del },
    ],
    carryPending: thing.add.length > 0 || thing.del.length > 0,
  };
}
