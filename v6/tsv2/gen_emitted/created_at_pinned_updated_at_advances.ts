...
const ddl: readonly string[] = [
  ...
  `CREATE INDEX "__next_frontier_arrive_phase" ON "__next_frontier_arrive" ("_phase")`,
  ...
  `CREATE INDEX "__next_frontier_thing_phase" ON "__next_frontier_thing" ("_phase")`,
...
function readSnapshot(seam: ISqlSeam): Observable<Snapshot> {
  return forkJoin({
    arrive: selectRows(seam, `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' THEN json_extract("payload", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("payload", '$.args')) || ')' ELSE "payload" END AS "payload" FROM "arrive"`, relColumns.arrive!, relColumnTypes.arrive!),
    thing: selectRows(seam, `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' THEN json_extract("payload", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("payload", '$.args')) || ')' ELSE "payload" END AS "payload", "born", "tick" FROM "thing"`, relColumns.thing!, relColumnTypes.thing!),
  });
}

const finalSelect: Record<string, string> = {
  arrive: `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' THEN json_extract("payload", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("payload", '$.args')) || ')' ELSE "payload" END AS "payload" FROM "arrive"`,
  thing: `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' THEN json_extract("payload", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("payload", '$.args')) || ')' ELSE "payload" END AS "payload", "born", "tick" FROM "thing"`,
};
...
const INCREMENTAL_RELATIONS: readonly IIncrementalRelationPlan[] = [
  { rel: "arrive", kind: "log", tableName: "arrive", deltaTableName: "__delta_arrive", frontierTableName: "__frontier_arrive", nextFrontierTableName: "__next_frontier_arrive", columns: ["id", "payload"], columnTypes: ["int", "text"], keyIndices: [], arrivalAddSql: `INSERT INTO "arrive" ("id", "payload") SELECT json_extract(value, '$[0]'), json_extract(value, '$[1]') FROM json_each(?) RETURNING "id", "payload"`, arrivalDelSql: null, boundarySql: `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' THEN json_extract("payload", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("payload", '$.args')) || ')' ELSE "payload" END AS "payload", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_arrive" WHERE "_sign" IN (-1, 1) GROUP BY "id", "payload", "_sign"` },
  { rel: "thing", kind: "set", tableName: "thing", deltaTableName: "__delta_thing", frontierTableName: "__frontier_thing", nextFrontierTableName: "__next_frontier_thing", columns: ["id", "payload", "born", "tick"], columnTypes: ["int", "text", "int", "int"], keyIndices: [0], arrivalAddSql: null, arrivalDelSql: null, boundarySql: `SELECT "id", CASE WHEN json_valid("payload") AND json_type("payload") = 'object' THEN json_extract("payload", '$.fn') || '(' || (SELECT group_concat(value, ',') FROM json_each("payload", '$.args')) || ')' ELSE "payload" END AS "payload", "born", "tick", "_sign" AS "__sign", count(*) AS "__count" FROM "__delta_thing" WHERE "_sign" IN (-1, 1) GROUP BY "id", "payload", "born", "tick", "_sign"` },
];
