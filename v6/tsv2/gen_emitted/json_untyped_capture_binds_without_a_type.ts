...
const INCREMENTAL_LEVEL_STATEMENTS: readonly IIncrementalLevelStatement[] = [
  { headRel: "seen", headDeltaTableName: "__delta_seen", headColumns: ["value"], insertSql: `INSERT OR IGNORE INTO "seen" ("value") SELECT DISTINCT json_extract(d0."payload", '$."stars"') FROM "__frontier_event" d0 WHERE d0."_phase" >= 0 AND json_type(d0."payload", '$') = 'object' AND json_extract(d0."payload", '$."stars"') IS NOT NULL RETURNING "value"`, selectSql: `SELECT "value" FROM "seen"`, recomputeSql: `DELETE FROM "seen";
