...
INSERT OR IGNORE INTO "stars" ("ep", "n") SELECT b0."ep", b1."stargazers_count" FROM "current_body" b0, "__ref_repo_body" b1 WHERE b1."__id" = b0."body"`, supportSql: [`DELETE FROM "__support_next_stars"`, `INSERT INTO "__support_next_stars" ("ep", "n", "__refcount") SELECT "ep", "n", sum("__refcount") FROM (SELECT b0."ep" AS "ep", b1."stargazers_count" AS "n", count(*) AS "__refcount" FROM "current_body" b0, "__ref_repo_body" b1 WHERE b1."__id" = b0."body" GROUP BY b0."ep", b1."stargazers_count") GROUP BY "ep", "n"`, `UPDATE "stars" AS h SET "__refcount" = "__refcount" - ("__refcount" - COALESCE((SELECT n."__refcount" FROM "__support_next_stars" n WHERE n."ep" = h."ep" AND n."n" = h."n"), 0))`, `INSERT INTO "__delta_stars" ("_sign", "_sequence", "ep", "n") SELECT -1, row_number() OVER () - 1, "ep", "n" FROM "stars" WHERE "__refcount" <= 0`, `DELETE FROM "stars" WHERE "__refcount" <= 0`, `INSERT INTO "__delta_stars" ("_sign", "_sequence", "ep", "n") SELECT 1, row_number() OVER () - 1, n."ep", n."n" FROM "__support_next_stars" n WHERE NOT EXISTS (SELECT 1 FROM "stars" h WHERE n."ep" = h."ep" AND n."n" = h."n")`, `INSERT INTO "__frontier_stars" ("_phase", "_sequence", "ep", "n") SELECT ?, row_number() OVER () - 1, n."ep", n."n" FROM "__support_next_stars" n WHERE NOT EXISTS (SELECT 1 FROM "stars" h WHERE n."ep" = h."ep" AND n."n" = h."n")`, `INSERT INTO "__next_frontier_stars" ("_phase", "_sequence", "ep", "n") SELECT ?, row_number() OVER () - 1, n."ep", n."n" FROM "__support_next_stars" n WHERE NOT EXISTS (SELECT 1 FROM "stars" h WHERE n."ep" = h."ep" AND n."n" = h."n")`, `INSERT INTO "stars" ("ep", "n", "__refcount") SELECT n."ep", n."n", n."__refcount" FROM "__support_next_stars" n WHERE NOT EXISTS (SELECT 1 FROM "stars" h WHERE n."ep" = h."ep" AND n."n" = h."n")`], aggregateSql: null },
];
