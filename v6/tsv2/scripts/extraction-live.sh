...
#   3  touch a.ts with IDENTICAL bytes  -> ZERO new ticks (content-addressed:
...
# a.ts/eval after the eval was edited out", because the stale digest's row keeps
...
EOF
await_rows banned_call '"a.ts","eval"' present \
  || fail "phase 1: banned_call never gained a.ts/eval (rows: $(rows_of banned_call))"
...
cat >"$CORPUS/.a.ts.swap" <<'EOF'
export function danger(source: string): unknown {
  return JSON.parse(source);
}
EOF
mv "$CORPUS/.a.ts.swap" "$CORPUS/a.ts"
await_rows banned_call '"a.ts","eval"' absent \
  || fail "phase 2: banned_call still holds a.ts/eval after the eval was edited out (rows: $(rows_of banned_call))"
say "PASS  phase 2  atomic save (write temp + rename) -> retick -> finding retracted"

# ── phase 3: identical bytes are not a change ───────────────────────────────
before="$(tick_count)"
touch "$CORPUS/a.ts"
cp "$CORPUS/a.ts" "$WORK/same.ts" && cp "$WORK/same.ts" "$CORPUS/a.ts"
sleep 1
after="$(tick_count)"
[ "$before" = "$after" ] \
...
EOF
await_rows banned_call '"b.ts","eval"' present \
  || fail "phase 4: banned_call never gained b.ts/eval (rows: $(rows_of banned_call))"
say "PASS  phase 4  second file -> finding: $(rows_of banned_call)"

# ── phase 5: deletion retracts through the real emitted SQL ─────────────────
rm "$CORPUS/b.ts"
await_rows banned_call '"b.ts","eval"' absent \
  || fail "phase 5: banned_call still holds b.ts/eval after the file was deleted (rows: $(rows_of banned_call))"
say "PASS  phase 5  file deleted -> '-' arrival -> finding retracted: $(rows_of banned_call)"

# The extractor really ran. After phase 5 the only file left is a.ts in its
