...
| H5 | **cache_size knob trade curve under 1GB**: quantify RSS vs disk read/write vs speed as `PRAGMA cache_size` is squeezed, find the smallest cache that finishes each scale under 1GB, then drive it down. | 2026-07-22, 1GB-budget reframe | in-flight (G10) | the golden-data bench (`v6/AGENTS.md` → `just perf`). |
