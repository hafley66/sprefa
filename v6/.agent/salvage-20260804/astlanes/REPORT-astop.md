...
```text
ee6a39eff05e77bb201cfb4aeb658f43e46beb18
```

The dl6 `ast/4` expansion is in `v6/prolog/0_ast_expand.pl`. It creates one
host per language and query pair, emits demand and response relation shape,
rewrites the rule body, threads capture bindings through the expansion fold,
and registers the four named refusals. Expansion tests cover the minted host,
rewritten rule, host reuse, and refusals.
...
TEXT_DOOR compiled=417 byte_identical=416 failures=1
  TEXT_DOOR_FAIL json_nfc_and_nfd_keys_stay_distinct error(io_error(write,<stream>(0x600002fd7c00)),context(system:format/3,'Encoding cannot represent character'))
error: recipe `text-door` failed on line 48 with exit code 1
REQUIRED_GATE_EXIT=1
```

The `&&` chain stopped at `text-door`. Separate `just plunit` output:

```text
ERROR: [Thread main] /Users/chrishafley/projects/sprefa-codex-astop/v6/prolog/compile/test/diag.test.pl:113:
...
The `expansion_order` suite passed 22/22 tests, including the AST tests. The
two gate failures are Unicode encoding errors in existing text-door and
diagnostic fixtures.
