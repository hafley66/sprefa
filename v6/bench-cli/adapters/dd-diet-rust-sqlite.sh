#!/usr/bin/env bash
# dd-diet-rust-sqlite bench-cli adapter: compile a .dl6 program through the
# dd_plan emitter option, then drive dd-runner's sqlite arm. stdout is the tick
# log and nothing else (contract clause 2.1: stdout is what gets byte-diffed).
# The compiler's own stdout and the dd-runner result are routed around it.
#
...
# ── phase 2: run dd-runner's sqlite arm ─────────────────────────────────────
...
"$RUNNER" "$PLAN" --dd-diet-rust-sqlite > "$TICKLOG" 2> "$BENCH_CLI/out/$NAME.dd.err"
run_status=$?
...
cat > "$PERF_OUT" <<JSON
{"engine":"dd-diet-rust-sqlite","wall_ms":${WALL_MS},"compile_ms":${COMPILE_MS},"ticks":${TICKS},"statements":"N/A","db_bytes":"N/A","notes":{"statements":"statements: this arm issues sql only through the dd-runner kernel's fixed projection, not per-adapter; not counted here","db_bytes":"db_bytes: run used an in-memory database ($DB), so there is no file to size","final_state":"final_state: dd-runner emits the tick log but no final-state line, so the 2.7 final-state hash is not produced (CONTRACT.md Priced and not taken)"}}
JSON
