#!/usr/bin/env bash
# dd-diet-rust-rust bench-cli adapter: refuses under contract clause 2.1 (the arm takes a dd_plan JSON with embedded schedule; no .dl6-text-to-dd_plan door exists, 6_emit_dd_plan.pl:33 / compile.pl:328).
...
REASON="contract clause 2.1: --program is a .dl6 TEXT file and the schedule is external to it, but the dd-diet-rust-rust arm takes a dd_plan JSON whose initial + schedule the emitter embeds (6_emit_dd_plan.pl:33); no .dl6-text-to-dd_plan door exists (compile.pl:328 emits emit_ts only). See CONTRACT.md Priced and not taken"

cat > "$PERF_OUT" <<JSON
{"engine":"dd-diet-rust-rust","wall_ms":"N/A","compile_ms":"N/A","ticks":"N/A","statements":"N/A","db_bytes":"N/A","notes":{"refusal":"$REASON"}}
JSON

echo "dd-diet-rust-rust.sh: refused: $REASON" >&2
exit 2
