/**
 * schedule-gen.ts — writes the scale schedules for the s1/s2/s3 programs at a
 * given row count.
 *
 ...
 * TickCount = Rows / 100. The one deliberate difference is s2's link rows,
 ...
 * programs/s2_two_hop_join.dl6's header.
 *
...
/** s1: 100 keys, every tick rewrites all of them with a fresh value. */
function s1Schedule(rows: number): IArrivalRow[][] {
...
/**
 * s2: one seeding tick of 100 b_link + 100 a_link rows, then `rows`/100 ticks
 ...
 */
function s2Schedule(rows: number): IArrivalRow[][] {
...
/** s3: all left arrivals, then all right arrivals. Quadratic on purpose. */
function s3Schedule(rows: number): IArrivalRow[][] {
  const ticks: IArrivalRow[][] = [];
...
function main(): void {
  ...
  if (build === undefined || !Number.isInteger(rows) || rows < BATCH || rows % BATCH !== 0 || outPath === undefined) {
    process.stderr.write("usage: schedule-gen.ts <s1|s2|s3> <rows: multiple of 100, >=100> <outPath>\n");
    process.exitCode = 2;
    return;
  }
