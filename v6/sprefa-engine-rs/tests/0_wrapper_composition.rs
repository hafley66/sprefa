...
fn nested_option_serde_roundtrips_every_presence_state() {
    let states = [
        ...
        (DlOption::Some(DlOption::None), r#"{"tag":"some","value":{"tag":"none"}}"#),
        (DlOption::Some(DlOption::Some(7)), r#"{"tag":"some","value":{"tag":"some","value":7}}"#),
    ];

    for (state, json) in states {
        assert_eq!(serde_json::to_string(&state).unwrap(), json);
        assert_eq!(serde_json::from_str::<DlOption<DlOption<i64>>>(json).unwrap(), state);
    }
}
