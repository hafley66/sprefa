...
fn sqlite_hw_mb(reset: bool) -> f64 {
    unsafe {
        libsqlite3_sys::sqlite3_memory_highwater(if reset { 1 } else { 0 }) as f64 / 1048576.0
    }
}
...
async fn open_store() -> (RelStore, std::path::PathBuf) {
    let path =
        std::env::temp_dir().join(format!("perf_{}_{}.sqlite", std::process::id(), rand_tag()));
    ...
    if let Ok(page_size) = std::env::var("DL_LAB_PAGE_SIZE") {
        conn.execute_unprepared(&format!("PRAGMA page_size={page_size};"))
            .await
            .unwrap();
    }
    ...
    if sqlite_ram_probe() {
        store
            .conn()
            .execute_unprepared("PRAGMA cache_size=-1048576;")
            .await
            .unwrap();
    }
    (store, path)
...
fn sqlite_ram_probe() -> bool {
    std::env::var("DL_SQLITE_RAM_PROBE")
        .map(|value| value == "1")
        .unwrap_or(false)
}
fn rand_tag() -> u64 {
...
impl Engine for Oracle {
    ...
    async fn measure(g: MultiGraph) -> Outcome {
        ...
        Outcome {
            retract_ms: ms,
            rust_peak_mb: memcap::peak_bytes() as f64 / 1048576.0,
            peak_rss_mb: peak_rss_mb(),
            survivors,
            ..Default::default()
        }
...
impl Engine for CountingScc {
    ...
    async fn measure(g: MultiGraph) -> Outcome {
        store_measure(g, |s, seed| {
            Box::pin(async move {
                s.retract_scc(&[seed]).await.unwrap();
            })
        })
        .await
    }
...
impl Engine for SignedDelta {
    ...
    async fn measure(g: MultiGraph) -> Outcome {
        store_measure(g, |s, seed| {
            Box::pin(async move {
                sprefa_store::cascade::retract_signed_delta_v2(s.conn(), s.ns(), &[seed])
                    .await
                    .unwrap();
            })
        })
...
async fn run_engine(name: &str, g: MultiGraph) -> Option<Outcome> {
    match name {
        ...
        "sqlite-signed-delta-v2" => Some(SignedDelta::measure(g).await),
...
async fn main() {
    ...
    let report_path = std::env::var("DL_PERF_REPORT")
        .unwrap_or_else(|_| format!("{}/PERF-REPORT.md", env!("CARGO_MANIFEST_DIR")));
    ...
    md.push_str(
        "Columns: **ms** = measured retract wall time; **stmts** = SQL statements in \
    ...
    md.push_str(
        "**Reading the memory columns (important):** the store engines show \
    ...
    for (label, l, w, bs) in &scales {
        ...
        md.push_str(&format!(
            ...
            if *bs == 0 {
                ...
                format!("cyclic stride={bs}")
            }
        ));
        md.push_str(&format!(
            "_input hash `{ref_in}` (all engines must match)_\n\n"
        ));
        ...
        for name in ENGINES {
            let cell = if name == "oracle" {
                Cell {
                    ok: oracle.ok,
                    correct: true,
                    out: oracle.out.clone(),
                    out_hash: oracle.out_hash.clone(),
                    in_hash: oracle.in_hash.clone(),
                }
            } else {
            ...
            md.push_str(&format!(
                "| {name} | {} | {mark} | {:.1} | {} | {:.2} | {:.2} | {:.1} | {:.2} |\n",
                cell.out.survivors.len(),
                cell.out.retract_ms,
                cell.out.statements,
                cell.out.rust_peak_mb,
                cell.out.sqlite_hw_mb,
                cell.out.peak_rss_mb,
                cell.out.db_mb
            ));
            json_rows.push(format!(
    ...
    if !broke.is_empty() {
        ...
        md.push_str(&format!(
            "Under the {cap} MB gun in the matrix above, these aborted (SIGABRT):\n\n"
        ));
        for b in &broke {
    ...
    let break_cap: u64 = std::env::var("DL_BREAK_CAP")
        ...
        .unwrap_or(700);
    md.push_str(&format!(
        "## Breakpoint ramp — tight gun {break_cap} MB\n\n"
    ));
    md.push_str(&format!(
    ...
    for &w in &[
        ...
        for e in ramp_engines {
            ...
            if c.ok {
                cells.push(format!(
                    "{:.0} ms / {:.0} MB rss",
                    c.out.retract_ms, c.out.peak_rss_mb
                ));
            } else {
        ...
        eprintln!(
            "[perf] ramp nodes={nodes} done ({:.0}s)",
            started.elapsed().as_secs_f64()
        );
        if still.is_empty() {
            break;
    ...
    for e in ramp_engines {
        if still.contains(e) {
            md.push_str(&format!(
                "- `{e}` never aborted in the ramp (RAM bounded at this cap).\n"
            ));
        }
    }
    ...
    let json_path = std::env::var("DL_PERF_JSON")
        .unwrap_or_else(|_| format!("{}/perf.json", env!("CARGO_MANIFEST_DIR")));
    let json = format!(
