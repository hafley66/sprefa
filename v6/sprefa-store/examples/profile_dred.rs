...
async fn main() {
    ...
    println!(
        "PROFILE cycle-safe retract  nodes={nodes} edges={edges} seed={seed:?}  memcap={cap_mb}MB"
    );
    println!(
        "network: NONE (embedded SQLite in-process; the only fd is the db file, zero sockets)\n"
    );
    ...
    memcap::reset_peak();
    let t = Instant::now();
    let rounds = cascade::retract_dred(&db, &ns, &[seed]).await.unwrap();
    ...
    println!(
        "  db file delta               {} -> {} bytes ({:+} KB)",
        db_bytes0,
        db_bytes1,
        (db_bytes1 as i64 - db_bytes0 as i64) / 1024
    );
    println!();
    println!("MEMORY");
    ...
    println!();
    println!(
        "per-statement breakdown: re-run with DL_CASCADE_TRACE=1 and aggregate (see profile note)."
    );
