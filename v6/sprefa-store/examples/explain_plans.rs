...
async fn main() {
    ...
    db.execute_unprepared(sprefa_store::unfuck_sqlite::OPEN_PRAGMAS)
        .await
        .unwrap();
    ...
    println!(
        "EXPLAIN QUERY PLAN — every DML statement in the cascade (no ANALYZE, matches runtime)"
    );
    println!(
        "graph: nodes={} edges={}  |  legend: SCAN=full table read, SEARCH ... USING = index seek",
        g.rows.len(),
        g.edges.len()
    );
    ...
    explain(
        &db,
        "retract seed decrement",
        false,
        "UPDATE cx_row SET weight = weight - 1 WHERE key IN (1,2,3)",
    )
    .await;
    explain(
        &db,
        "retract seed frontier fill",
        false,
        "INSERT INTO cx_frontier SELECT key FROM cx_row WHERE key IN (1,2,3) AND weight <= 0",
    )
    .await;
    explain(
        &db,
        "retract round: hits = children + lost-refCount count",
        false,
        "INSERT INTO cx_hits(key,dec) SELECT d.child_key, count(*) FROM cx_frontier f \
         CROSS JOIN cx_dep d ON d.parent_key = f.key GROUP BY d.child_key",
    )
    .await;
    explain(
        &db,
        "retract round: decrement each hit child",
        true,
    ...
    explain(
        &db,
        "assert seed frontier",
        false,
        "INSERT INTO cx_frontier SELECT key FROM cx_row WHERE key IN (1,2,3)",
    )
    .await;
    explain(
        &db,
        "assert round: next = dead children of frontier",
        false,
    ...
    explain(
        &db,
        "od round: next = alive children of frontier (THE HOT JOIN)",
        false,
        "INSERT OR IGNORE INTO cx_next(key) SELECT d.child_key FROM cx_frontier f \
         CROSS JOIN cx_dep d ON d.parent_key = f.key CROSS JOIN cx_row r ON r.key = d.child_key \
         WHERE r.weight > 0",
    ...
    explain(
        &db,
        "rd base: cone nodes with a surviving parent (REVERSE JOIN)",
        false,
    ...
    explain(
        &db,
        "rd round: alive children still in cone",
        false,
        "INSERT OR IGNORE INTO cx_next(key) SELECT d.child_key FROM cx_frontier f \
         CROSS JOIN cx_dep d ON d.parent_key = f.key CROSS JOIN cx_row r ON r.key = d.child_key \
         CROSS JOIN cx_cone c ON c.key = d.child_key WHERE r.weight = 0",
    )
    .await;
    ...
    explain(
        &db,
        "cte phase 1: over-delete cone walk",
        false,
