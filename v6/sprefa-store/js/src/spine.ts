 ...
 */
export function create_all_tables(db: SqliteDb): void {
  db.exec(`CREATE TABLE IF NOT EXISTS strings (string_id INTEGER PRIMARY KEY, content TEXT NOT NULL UNIQUE);
  ...
  for (const sql of secondary_indexes()) db.exec(sql);
}
...
export function drop_rel_table(db: SqliteDb, name: string): void {
