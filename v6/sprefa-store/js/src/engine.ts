 ...
 * db.transaction(|txn| async {...})). TS uses ONE `better-sqlite3` sync connection:
 * `async fn` -> sync `function`; `Result<T,DbErr>` -> throw on error; the Rust txn scope
 * maps to `db.transaction(() => { ... })()`. `db.exec(sql)` runs multi-statement strings
 * (the Rust `execute_unprepared` path also runs the combined `DELETE;INSERT;UPDATE`
 * blocks); `db.prepare(sql).pluck().get()/all()` and `.safeIntegers(true)` read scalars
 * and full-precision 64-bit digests. Every SQL string is the Rust string unchanged; only
 * the `format!` interpolation became template-literal `${ns.x}`.
 */
 ...
 * lock-free (sync better-sqlite3 is single-threaded by construction).
 */
...
function scalar(db: Db, sql: string): number {
  stmt_counter.incr();
  const v = db.prepare(sql).pluck(true).get();
 ...
 */
export function create_schema(db: Db, ns: GraphNs): void {
  // 256 MB page cache (cache_size negative = KiB) + 1 GB read mmap.
  db.exec("PRAGMA cache_size=-262144; PRAGMA mmap_size=1073741824;");
  db.exec(`CREATE TABLE ${ns.row} (
...
/** Batch-insert rows `(tag, id, weight)`. One transaction = one WAL commit for the load. */
export function insert_rows(db: Db, ns: GraphNs, rows: ReadonlyArray<readonly [number, number, number]>): void {
  db.transaction(() => {
    for (let i = 0; i < rows.length; i += CHUNK) {
      const chunk = rows.slice(i, i + CHUNK);
      const vals = chunk.map(([t, id, w]) => `(${key(t, id)},${w})`).join(",");
      exec(db, `INSERT INTO ${ns.row}(key,weight) VALUES ${vals}`);
    }
  })();
}
...
export function insert_deps(
  ...
  db.transaction(() => {
    for (let i = 0; i < edges.length; i += CHUNK) {
      ...
      exec(db, `INSERT INTO ${ns.dep}(parent_key,child_key) VALUES ${vals}`);
    }
  })();
}
 ...
 */
export function retract(db: Db, ns: GraphNs, seeds: ReadonlyArray<readonly [number, number]>): number {
  // The WHOLE cascade runs in ONE transaction.
  return db.transaction(() => {
    exec(db, `DELETE FROM ${ns.frontier}`);
    exec(db, `DELETE FROM ${ns.next}`);
    ...
    exec(db, `UPDATE ${ns.row} SET weight = weight - 1 WHERE key IN ${seed_in}`);
    exec(
      db,
    ...
    while (true) {
      if (scalar(db, `SELECT count(*) FROM ${ns.frontier}`) === 0) break;
      rounds++;

      // 1. hits = the frontier's children + how many supports each loses now.
      exec(db, `DELETE FROM ${ns.hits}`);
      exec(
        db,
      ...
      exec(db, `DELETE FROM ${ns.frontier}`);
      exec(db, `INSERT INTO ${ns.frontier} SELECT key FROM ${ns.next}`);
    }
    return rounds;
  })();
}
...
export function retract_scc(db: Db, ns: GraphNs, seeds: ReadonlyArray<readonly [number, number]>): number {
  return retract_scc_two_pass(db, ns, seeds);
}

function retract_scc_two_pass(db: Db, ns: GraphNs, seeds: ReadonlyArray<readonly [number, number]>): number {
  return db.transaction(() => {
    const seed_vals = seeds.map(([t, id]) => key(t, id).toString()).join(",");
    const seed_in = `(${seed_vals})`;
    exec(
    ...
    while (true) {
      ...
      if (scalar(db, `SELECT count(*) FROM ${ns.next}`) === 0) break;
      rounds++;
      exec(
        db,
 ...
 */
export function assert(db: Db, ns: GraphNs, seeds: ReadonlyArray<readonly [number, number]>): number {
  return db.transaction(() => {
    exec(db, `DELETE FROM ${ns.frontier}`);
    exec(db, `DELETE FROM ${ns.next}`);
    const seed_in = `(${seeds.map(([t, id]) => key(t, id).toString()).join(",")})`;
    exec(db, `INSERT INTO ${ns.frontier} SELECT key FROM ${ns.row} WHERE key IN ${seed_in}`);
    exec(db, `UPDATE ${ns.row} SET weight=1 WHERE key IN ${seed_in}`);
    let rounds = 0;
    // eslint-disable-next-line no-constant-condition
    while (true) {
      exec(
      ...
      if (scalar(db, `SELECT count(*) FROM ${ns.next}`) === 0) break;
      rounds++;
      exec(db, `UPDATE ${ns.row} SET weight=1 WHERE key IN (SELECT key FROM ${ns.next})`);
      exec(db, `DELETE FROM ${ns.frontier}`);
      exec(db, `INSERT INTO ${ns.frontier} SELECT key FROM ${ns.next}`);
    }
    return rounds;
 ...
 */
export function retract_dred(db: Db, ns: GraphNs, seeds: ReadonlyArray<readonly [number, number]>): number {
  return db.transaction(() => {
    exec(db, `DELETE FROM ${ns.frontier}`);
    exec(db, `DELETE FROM ${ns.next}`);
    exec(db, `DELETE FROM ${ns.cone}`);

    const seed_in = `(${seeds.map(([t, id]) => key(t, id).toString()).join(",")})`;
    exec(db, `INSERT INTO ${ns.frontier} SELECT key FROM ${ns.row} WHERE key IN ${seed_in} AND weight>0`);
    exec(db, `UPDATE ${ns.row} SET weight=0 WHERE key IN (SELECT key FROM ${ns.frontier})`);
    exec(db, `INSERT INTO ${ns.cone} SELECT key FROM ${ns.frontier}`);
    let rounds = 0;
    // eslint-disable-next-line no-constant-condition
    while (true) {
      exec(
      ...
      if (scalar(db, `SELECT count(*) FROM ${ns.next}`) === 0) break;
      rounds++;
      exec(db, `UPDATE ${ns.row} SET weight=0 WHERE key IN (SELECT key FROM ${ns.next})`);
      exec(db, `INSERT OR IGNORE INTO ${ns.cone} SELECT key FROM ${ns.next}`);
      exec(db, `DELETE FROM ${ns.frontier}`);
      exec(db, `INSERT INTO ${ns.frontier} SELECT key FROM ${ns.next}`);
    }
 ...
 * for the set-at-once CTE form).
 */
export function retract_dred_cte(db: Db, ns: GraphNs, seeds: ReadonlyArray<readonly [number, number]>): number {
  db.transaction(() => {
    const seed_in = `(${seeds.map(([t, id]) => key(t, id).toString()).join(",")})`;
    exec(db, `DELETE FROM ${ns.cone}`);
    // Phase 1 — over-delete.
    ...
    exec(db, `UPDATE ${ns.row} SET weight=0 WHERE key IN (SELECT key FROM ${ns.cone})`);
    // Phase 2 — rederive.
    exec(db, `DELETE FROM ${ns.frontier}`); // reused as the alive-set sink
    exec(
      db,
      `INSERT INTO ${ns.frontier}(key)
...
export function reaches_from(db: Db, ns: GraphNs, start: number): number[] {
  ...
  stmt_counter.incr();
  return (db.prepare(sql).pluck(true).all() as number[]).slice();
}

/** Reverse transitive closure into `target` (rides ix_*_dep_child). */
export function reached_by(db: Db, ns: GraphNs, target: number): number[] {
  const sql = `WITH RECURSIVE reach(key) AS (\
...
export function multi_source_walk(
  ...
  const result = db.transaction((): [number, number, number][] => {
    db.exec(`CREATE TEMP TABLE ${reached_table} (\
            tag INTEGER NOT NULL, node INTEGER NOT NULL, depth INTEGER NOT NULL, round INTEGER NOT NULL,\
            PRIMARY KEY(tag,node)\
        )`);
    db.exec(`CREATE TEMP TABLE ${halt_table} (node INTEGER PRIMARY KEY)`);

    if (halt) {
      for (const node of halt) {
        db.exec(`INSERT OR IGNORE INTO ${halt_table} VALUES (${node})`);
      }
    }
    ...
    // eslint-disable-next-line no-constant-condition
    while (true) {
      db.exec(`INSERT OR IGNORE INTO ${reached_table}(tag,node,depth,round) \
      ...
      const inserted =
        (db.prepare(`SELECT count(*) FROM ${reached_table} WHERE round = ${round + 1}`).pluck(true).get() as
          | number
          | undefined) ?? 0;
      if (inserted === 0) break;
      round++;
    ...
    const rows = db.prepare(`SELECT tag,node,depth FROM ${reached_table} ORDER BY tag,node`).all() as {
      tag: number;
      node: number;
      depth: number;
    }[];
    db.exec(`DROP TABLE ${reached_table}; DROP TABLE ${halt_table}`);
    return rows.map((r) => [r.tag, r.node, r.depth] as [number, number, number]);
  })();
  return result;
}
...
export function multi_source_halt_bfs(
  ...
  return multi_source_walk(db, ns, starts3, halt, null).map(([t, n]) => [t, n]);
}

/**
 * SCC partition as (node_key, comp_repr = MIN member key). Compare on the partition.
 *
 * ORM seam: Rust reads the unaliased COALESCE column by index; better-sqlite3 reads by
 * column name, so the computed column carries an explicit `AS repr` alias. The SQL
 * semantics (GROUP BY/ORDER BY on node.key, the COALESCE expression) are unchanged.
 */
export function scc_labels(db: Db, ns: GraphNs): [number, number][] {
  ...
  stmt_counter.incr();
  const rows = db.prepare(sql).all() as { key: number; repr: number }[];
  return rows.map((r) => [r.key, r.repr]);
}

/** Condensation derived from `scc_labels` + cx_dep group-bys. */
export function build_condensed(db: Db, ns: GraphNs): Condensed {
  const comp_of = scc_labels(db, ns);
  ...
  stmt_counter.incr();
  const edges = db.prepare(`SELECT parent_key,child_key FROM ${ns.dep}`).all() as {
    parent_key: number;
    child_key: number;
  }[];
  ...
  for (const { parent_key, child_key } of edges) {
 ...
 */
export function count_pairs(db: Db, ns: GraphNs): bigint {
  const cond = build_condensed(db, ns);
...
//!
//! `digest` is a full i64; it flows as bigint here. The seed/verify SQL interpolates the
//! integer literal (its `.toString()`); the SELECTs that read digests use `.safeIntegers`
//! so the 64-bit value round-trips without Number precision loss.
...
function query_ids(db: Db, sql: string): number[] {
  stmt_counter.incr();
  return db.prepare(sql).pluck(true).all() as number[];
}

/** Query bigint ids/digests (full 64-bit fidelity). */
function query_bigints(db: Db, sql: string): bigint[] {
  stmt_counter.incr();
  return db.prepare(sql).pluck(true).safeIntegers(true).all() as unknown as bigint[];
}

export function create_schema(db: Db, ns: GraphNs): void {
  db.exec(`CREATE TABLE ${ns.memo} (
...
export function seed(
  ...
  db.transaction(() => {
    exec(db, `INSERT INTO ${ns.memo}(id,digest,changed_at,verified_at) VALUES (${id},${digest},${rev},${rev})`);
    for (const d of deps) {
      exec(db, `INSERT OR IGNORE INTO ${ns.rdep}(reader,read) VALUES (${id},${d})`);
    }
  })();
}

/** An input's digest moved at `rev`: bump its changed_at so the CTE sees it stale. */
export function mark_changed(db: Db, ns: GraphNs, ids: ReadonlyArray<number>, rev: number): void {
  const in_list = ids.map((i) => i.toString()).join(",");
  exec(db, `UPDATE ${ns.memo} SET changed_at=${rev} WHERE id IN (${in_list})`);
}
 ...
 * driver is `propagate`.)
 */
export function dirty(db: Db, ns: GraphNs): number[] {
  return query_ids(
    db,
 ...
 */
export function verify(db: Db, ns: GraphNs, id: number, new_digest: bigint, rev: number): boolean {
  stmt_counter.incr();
  const old_row = db
    .prepare(`SELECT digest FROM ${ns.memo} WHERE id=${id}`)
    .safeIntegers(true)
    .get() as { digest: bigint } | undefined;
...
export function propagate(
  ...
  const dirty_set = new Set<number>(seeds);
  let count = 0;
  while (dirty_set.size > 0) {
    // smallest id (BTreeSet.iter().next()); ascending = topo here.
    let id = Infinity;
    for (const v of dirty_set) if (v < id) id = v;
    dirty_set.delete(id);

    const dep_digests = query_bigints(
      db,
    ...
    const moved = verify(db, ns, id, new_digest, rev);
    count++;
    if (moved) {
      const readers = query_ids(db, `SELECT reader FROM ${ns.rdep} WHERE read = ${id}`);
...
export function answer(db: Db, ns: GraphNs): bigint {
  const digests = query_bigints(db, `SELECT digest FROM ${ns.memo}`);
  let acc = 0n;
...
// =============================================================================
// temporal — append-only bitemporal fact storage (substrate, not a parity trait)
// =============================================================================
export namespace temporal {
//! fact(key, tt_from, tt_to, weight) WITHOUT ROWID; partial index ix_live WHERE
//! tt_to IS NULL. commit(deltas) = one JSON-batched txn: insert new live facts, weight +=
//! dw, close (tt_to = rev) at weight <= 0. live()/digest() read the tt_to IS NULL set.
//! Role: the versioned substrate UNDER the graph; no parity trait.
...
function create_schema(db: Db): void {
  db.exec(`CREATE TABLE fact(key INTEGER NOT NULL, tt_from INTEGER NOT NULL,
...
export class TemporalStore {
  ...
  static attach(db: Db): TemporalStore {
    db.exec(SOFT_HEAP_LIMIT);
    create_schema(db);
    const store = new TemporalStore(db);
    store.revision = Number(
      (db.prepare("SELECT COALESCE(MAX(tt_from), 0)").pluck(true).get() as number | undefined) ?? 0,
    );
    return store;
  }

  commit(deltas: ReadonlyArray<readonly [number, number]>): void {
    if (deltas.length === 0) return;
    this.revision += 1;
    const revision = this.revision;
    const delta_json_val = delta_json(deltas);
    db_transaction(this.db, () => {
      this.db.exec("DELETE FROM d");
      this.db
        .prepare(
          "INSERT INTO d(key,dw) SELECT json_extract(value,'$[0]'), sum(json_extract(value,'$[1]')) FROM json_each(?) GROUP BY 1",
        )
        .run(delta_json_val);
      this.db
        .prepare(
          "INSERT INTO fact(key,tt_from,tt_to,weight) SELECT d.key, ?, NULL, 0 FROM d LEFT JOIN fact f ON f.key=d.key AND f.tt_to IS NULL WHERE f.key IS NULL AND d.dw>0",
        )
        .run(revision);
      this.db.exec(
        "UPDATE fact SET weight = weight + (SELECT dw FROM d WHERE d.key=fact.key) WHERE tt_to IS NULL AND key IN (SELECT key FROM d)",
      );
      this.db
        .prepare("UPDATE fact SET tt_to=? WHERE tt_to IS NULL AND weight<=0 AND key IN (SELECT key FROM d)")
        .run(revision);
    });
  }
  ...
  live(): number {
    stmt_counter.incr();
    return Number((this.db.prepare("SELECT count(*) FROM fact WHERE tt_to IS NULL").pluck(true).get() as number) ?? 0);
  }

  total_rows(): number {
    stmt_counter.incr();
    return Number((this.db.prepare("SELECT count(*) FROM fact").pluck(true).get() as number) ?? 0);
  }

  digest(): number {
    stmt_counter.incr();
    const rows = this.db.prepare("SELECT key FROM fact WHERE tt_to IS NULL").pluck(true).all() as number[];
    return rows.reduce((digest, key) => (digest ^ mix_key(key)) | 0, 0);
  }
...
/** Run a fn under a better-sqlite3 transaction (BEGIN/COMMIT), re-throwing on error. */
function db_transaction<T>(db: Db, fn: () => T): T {
