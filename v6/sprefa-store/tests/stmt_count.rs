...
async fn commit_statement_count_is_constant_for_a_delta_batch() {
    ...
    let path =
        std::env::temp_dir().join(format!("temporal_stmtcount_{}.sqlite", std::process::id()));
    ...
    assert_eq!(
        stmt_counter::get(),
        5,
        "delta commit must be a fixed set-based batch, not N+1"
    );
...
async fn signed_delta_v2_retraction_uses_three_dispatches() {
    ...
    assert_eq!(
        stmt_counter::get(),
        3,
        "v2 must remain clear, recursive walk, publish"
    );
