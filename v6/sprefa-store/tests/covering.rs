...
mod oracle {
    ...
    pub fn multi_source_halt_bfs(
        ...
        let starts3: Vec<(u32, u32, i64)> =
            starts.iter().map(|&(tag, node)| (tag, node, 0)).collect();
...
fn dense(edges: &[(u32, u32)]) -> (Vec<Vec<u32>>, usize) {
    ...
    let remap: BTreeMap<u32, u32> = nodes
        .iter()
        .enumerate()
        .map(|(i, &n)| (n, i as u32))
        .collect();
    ...
    for &(a, b) in edges {
        adj[remap[&a] as usize].push(remap[&b]);
    }
...
async fn load_dense(adj: &[Vec<u32>]) -> (RelStore, std::path::PathBuf) {
    ...
    for (u, succ) in adj.iter().enumerate() {
        for &w in succ {
            deps.push((0i64, u as i64, 0i64, w as i64));
        }
    }
...
fn oracle_partition(cond: &oracle::Cond) -> BTreeSet<Vec<i64>> {
    ...
    for (node, &c) in cond.comp.iter().enumerate() {
        groups.entry(c).or_default().push(node as i64);
    }
...
fn shapes() -> Vec<(&'static str, Vec<(u32, u32)>)> {
    vec![
        ...
        (
            "dag_wide",
            vec![(0, 2), (0, 3), (1, 3), (1, 4), (2, 5), (3, 5), (4, 5)],
        ),
    ]
}
...
async fn scc_condensed_and_count_pairs_agree() {
    for (name, edges) in shapes() {
        ...
        assert_eq!(
            store_partition(&labels),
            oracle_partition(&cond),
        ...
        let cd = reach::build_condensed(store.conn(), store.ns())
            .await
            .unwrap();
...
async fn reaches_from_and_reached_by_agree() {
    for (name, edges) in shapes() {
        ...
        for start in 0..k as u32 {
            let want = sorted(
                oracle::reaches_from(&cond, start)
                    .into_iter()
                    .map(|n| n as i64)
                    .collect(),
            );
            let got = sorted(
                reach::reaches_from(store.conn(), store.ns(), start as i64)
                    .await
                    .unwrap(),
            );
            ...
            let want_r = sorted(
                oracle::reached_by(&cond, start)
                    .into_iter()
                    .map(|n| n as i64)
                    .collect(),
            );
            let got_r = sorted(
                reach::reached_by(store.conn(), store.ns(), start as i64)
                    .await
                    .unwrap(),
            );
...
async fn multi_source_walk_and_halt_bfs_agree() {
    ...
    for (name, edges, starts, halt_nodes, cap) in cases {
        ...
        for &h in &halt_nodes {
            halt_mask[h as usize] = true;
        }

        let want = oracle::multi_source_walk(
            &adj,
            &starts,
            if halt_nodes.is_empty() {
                None
            } else {
                Some(&halt_mask)
            },
        ...
        store.add_rows(&rows).await.unwrap();
        let deps: Vec<(i64, i64, i64, i64)> = edges
            .iter()
            .map(|&(a, b)| (0, a as i64, 0, b as i64))
            .collect();
        store.add_deps(&deps).await.unwrap();

        let starts_i: Vec<(i64, i64, i64)> = starts
            .iter()
            .map(|&(t, nd, d)| (t as i64, nd as i64, d))
            .collect();
        let halt_i: Vec<i64> = halt_nodes.iter().map(|&h| h as i64).collect();
        let got = reach::multi_source_walk(
            store.conn(),
            store.ns(),
            &starts_i,
            if halt_i.is_empty() {
        ...
        .await
        .unwrap();
        let want_i: Vec<(i64, i64, i64)> = want
            .iter()
            .map(|&(t, nd, d)| (t as i64, nd as i64, d))
            .collect();
        ...
        if cap.is_none() {
            ...
            let starts2_i: Vec<(i64, i64)> = starts2
                .iter()
                .map(|&(t, nd)| (t as i64, nd as i64))
                .collect();
            let got_h = reach::multi_source_halt_bfs(store.conn(), store.ns(), &starts2_i, &halt_i)
                .await
                .unwrap();
            let want_h_i: Vec<(i64, i64)> = want_h
                .iter()
                .map(|&(t, nd)| (t as i64, nd as i64))
                .collect();
