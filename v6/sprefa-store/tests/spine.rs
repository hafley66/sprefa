...
async fn golden_path_at_scale() {
    ...
    let files: Vec<([u8; 16], i64, i64)> = (0..N_FILES)
        .map(|i| {
            (
                content_hash(format!("file body {i}").as_bytes()),
                i as i64,
                10,
            )
        })
    ...
    assert_eq!(
        scalar_i64(&store, "SELECT count(*) FROM node").await,
        N_NODES as i64
    );
    // all four families landed in the ONE table
    assert_eq!(
        scalar_i64(&store, "SELECT count(DISTINCT family) FROM node").await,
        4
    );
    ...
    eprintln!(
        "[1c] inserted {N_NODES} nodes in {node_stmts} statements (not {N_NODES}) in {:?}",
        t.elapsed()
    );
    ...
    assert_eq!(
        scalar_i64(&store, "SELECT count(*) FROM edge").await,
        N_EDGES as i64
    );
    eprintln!(
        "[1d] inserted {N_EDGES} cross-family edges in {:?}",
        t.elapsed()
    );
    ...
    eprintln!(
        "[1e] reached {reached} nodes via one cross-family CTE in {:?}",
        t.elapsed()
    );
    ...
    let repo = store
        .repo_upsert("acme/widgets", "/repo", "https://x/y")
        .await
        .unwrap();
    ...
    let head = store
        .rev_committed(
            repo,
            sprefa_store::transforms::git_sha_bytes(&"ab".repeat(20)).unwrap(),
        )
        .await
        .unwrap();
    ...
    store
        .files_insert_batch(&[(shared_hash, 18, 1)])
        .await
        .unwrap();
    ...
    assert_eq!(
        max_sid,
        cnt_sid - 1,
        "string ids are dense (Codd + varint happy)"
    );
    ...
    let repo2 = store
        .repo_upsert("acme/scale", "/s", "https://s")
        .await
        .unwrap();
    let head2 = store
        .rev_committed(
            repo2,
            sprefa_store::transforms::git_sha_bytes(&"cd".repeat(20)).unwrap(),
        )
        .await
        .unwrap();
    let root2 = {
    ...
    let paths: Vec<i64> = (0..N_WORK)
        .map(|i| store.intern(&format!("s/f{i}.rs")))
        .collect();
    store.flush_strings().await.unwrap();
    let head_hashes: Vec<[u8; 16]> = (0..N_WORK)
        .map(|i| content_hash(format!("head body {i}").as_bytes()))
        .collect();
    ...
    let mut all_content: Vec<([u8; 16], i64, i64)> =
        head_hashes.iter().map(|h| (*h, 1, 1)).collect();
    all_content.extend(
        work_hashes
            .iter()
            .filter(|h| !head_hashes.contains(h))
            .map(|h| (*h, 1, 1)),
    );
    ...
    let head_place: Vec<(i64, i64, i64)> = (0..N_WORK)
        .map(|i| (head2, paths[i], idmap[&head_hashes[i]]))
        .collect();
    let work_place: Vec<(i64, i64, i64)> = (0..N_WORK)
        .map(|i| (work2, paths[i], idmap[&work_hashes[i]]))
        .collect();
    ...
    assert_eq!(
        dirty.len(),
        DIRTY,
        "exactly the diverged files report unstaged changes"
    );
    assert_eq!(
        dirty_stmts, 1,
        "unstaged detection is ONE join, not one query per file"
    );
    ...
    assert_eq!(
        got, expect,
        "the reported paths are exactly the edited ones"
    );
    eprintln!(
