...
async fn dred_survivors(g: &MultiGraph) -> Vec<i64> {
    ...
    store
        .retract_dred(&[(g.seed.0 as i64, g.seed.1)])
        .await
        .unwrap();
...
async fn signed_delta_survivors(g: &MultiGraph) -> Vec<i64> {
    ...
    store.add_rows(&rows).await.unwrap();
    store.add_deps(&deps).await.unwrap();
    sprefa_store::cascade::retract_signed_delta(
        store.conn(),
        store.ns(),
        &[(g.seed.0 as i64, g.seed.1)],
    )
    .await
    .unwrap();
    let keys = store.alive_keys().await.unwrap();
...
async fn delta_fold_survivors(g: &MultiGraph) -> Vec<i64> {
    ...
    store.add_deps(&deps).await.unwrap();
    store
        .retract_delta_fold(&[(g.seed.0 as i64, g.seed.1)])
        .await
        .unwrap();
    let keys = store.alive_keys().await.unwrap();
    drop(store);
...
async fn count_scc_survivors(g: &MultiGraph) -> Vec<i64> {
    ...
    store.add_deps(&deps).await.unwrap();
    store
        .retract_scc(&[(g.seed.0 as i64, g.seed.1)])
        .await
        .unwrap();
    let keys = store.alive_keys().await.unwrap();
    drop(store);
...
async fn assert_agreement(g: &MultiGraph, label: &str) {
    ...
    let dd = dd_survivors(
        &edges,
        &roots_from(g),
        benchgraph::encode(g.seed.0, g.seed.1),
    );
    assert_eq!(
        dred, oracle,
        "{label}: sqlite-DRed disagreed with the oracle"
    );
    assert_eq!(dd, oracle, "{label}: dd disagreed with the oracle");
    assert_eq!(
        dred, dd,
        "{label}: sqlite-DRed and dd disagreed with each other"
    );
    assert_eq!(
        signed, oracle,
        "{label}: signed-delta disagreed with the oracle"
    );
    assert_eq!(
        signed, dred,
        "{label}: signed-delta and sqlite-DRed disagreed with each other"
    );
    assert_eq!(
        signed_cte, oracle,
        "{label}: signed-delta-CTE disagreed with the oracle"
    );
    assert_eq!(
        signed_cte, dred,
        "{label}: signed-delta-CTE and sqlite-DRed disagreed with each other"
    );
    assert_eq!(
        delta_fold, oracle,
        "{label}: delta-fold disagreed with the oracle"
    );
    assert_eq!(
        delta_fold, dred,
        "{label}: delta-fold and sqlite-DRed disagreed with each other"
    );
}
...
async fn dag_all_engines_agree() {
    for &(layers, width) in &[(2usize, 200usize), (5, 500), (8, 1500)] {
        ...
        let oracle: Vec<i64> = benchgraph::oracle_survivors(&g, g.seed)
            ...
            .collect();
        assert_eq!(
            count_survivors(&g).await,
            oracle,
            "{label}: counting must be correct on a DAG"
        );
        assert_eq!(
            count_scc_survivors(&g).await,
            oracle,
            "{label}: SCC counting must be correct on a DAG"
        );
    }
}
    ...
    for &(layers, width) in &[(3usize, 300usize), (6, 800), (8, 1500)] {
        for &stride in &[2usize, 3, 5, 7, 11] {
            ...
            assert_eq!(
                count_scc_survivors(&g).await,
                oracle,
                "{label}: SCC counting must match the oracle"
            );
        }
    }
}
...
async fn counting_is_wrong_on_a_phantom_cycle() {
    ...
    // DRed stays correct.
    assert_eq!(
        dred_survivors(&g).await,
        oracle,
        "DRed must stay correct on the phantom-cycle graph"
    );
    ...
    for k in &oracle {
        assert!(
            count_set.contains(k),
            "counting dropped a real survivor {k}"
        );
    }
}
...
async fn signed_delta_kills_a_cut_cycle() {
    ...
    assert_eq!(
        signed, oracle,
        "{label}: signed-delta must kill the whole cut cycle"
    );
    assert_eq!(
        signed, dred,
        "{label}: signed-delta and DRed must agree on a cut cycle"
    );
