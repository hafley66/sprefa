...
async fn rows(db: &DatabaseConnection, sql: &str, cols: usize) -> Vec<Vec<i64>> {
    ...
    let mut v: Vec<Vec<i64>> = out
        ...
        .map(|r| {
            (0..cols)
                .map(|i| r.try_get_by_index::<i64>(i).unwrap())
                .collect()
        })
        .collect();
    v.sort();
...
async fn two_way_join() {
    ...
    exec(
        &db,
        "CREATE TABLE call(caller INTEGER, callee INTEGER);
               CREATE TABLE def(fn INTEGER, file INTEGER);
               INSERT INTO call VALUES (1,2),(1,3),(2,3);
               INSERT INTO def  VALUES (2,10),(3,11);",
    )
    .await;
    let got = rows(
        &db,
        "SELECT c.caller, c.callee, d.file FROM call c JOIN def d ON c.callee = d.fn",
        3,
    )
    .await;
    ...
    for (caller, callee) in call {
        for (fnn, file) in def {
            if callee == fnn {
                oracle.push(vec![caller, callee, file]);
            }
        }
    }
...
async fn union_distinct() {
    ...
    exec(
        &db,
        "CREATE TABLE e1(a INTEGER, b INTEGER);
               CREATE TABLE e2(a INTEGER, b INTEGER);
               INSERT INTO e1 VALUES (1,2),(2,3);
               INSERT INTO e2 VALUES (2,3),(3,4);",
    )
    .await;
...
async fn antijoin_negation() {
    ...
    exec(
        ...
        "CREATE TABLE call(caller INTEGER, callee INTEGER);
               ...
               INSERT INTO def  VALUES (1),(2),(3),(4);",
    )
    .await;
    let got = rows(
        &db,
        "SELECT fn FROM def WHERE fn NOT IN (SELECT callee FROM call) ORDER BY fn",
        1,
    )
    .await;
    ...
    let oracle: Vec<Vec<i64>> = [1, 2, 3, 4]
        .into_iter()
...
async fn aggregation_group_count() {
    ...
    exec(
        &db,
        "CREATE TABLE call(caller INTEGER, callee INTEGER);
               INSERT INTO call VALUES (1,3),(2,3),(1,4),(5,3);",
    )
    .await;
    let got = rows(
        &db,
        "SELECT callee, count(*) FROM call GROUP BY callee ORDER BY callee",
        2,
    )
    .await;
    ...
    for (_caller, callee) in [(1, 3), (2, 3), (1, 4), (5, 3)] {
        *m.entry(callee).or_insert(0) += 1;
    }
...
async fn three_way_join() {
    ...
    exec(
        ...
        "CREATE TABLE call(caller INTEGER, callee INTEGER);
               ...
               INSERT INTO owns VALUES (20,200),(30,300);",
    )
    .await;
    let got = rows(
        &db,
        "SELECT c.caller, o.team FROM call c \
         JOIN in_mod m ON c.callee=m.fn JOIN owns o ON m.module=o.module",
        2,
    )
    .await;
