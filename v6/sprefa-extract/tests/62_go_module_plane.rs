...
fn run(families: &str) -> Vec<Value> {
    let mut args: Vec<String> =
        vec!["--resolve".to_string(), "--family".to_string(), families.to_string()];
    args.extend(FILES.iter().map(|name| format!("tests/fixtures/go_modules/{name}")));
...
fn an_unexported_name_binds_no_edge() {
    assert!(!call_edges("call").iter().any(|(caller, ..)| caller == "UseUnexported"));
}
...
fn a_blank_import_mints_no_row_at_all() {
    assert!(!imports("call").iter().any(|(_, _, target, ..)| target == "blankpkg"));
    assert!(!unresolved("call").iter().any(|(_, _, detail)| detail.contains("blankpkg")));
}
...
fn an_external_import_drops_with_reason_external() {
    assert!(!imports("call").iter().any(|(_, _, _, target_name, _)| target_name == "errors"));
