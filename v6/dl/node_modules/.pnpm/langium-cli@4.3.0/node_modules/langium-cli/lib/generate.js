...
import { generateAstMultiLanguageProject, generateAstSingleLanguageProject, getLanguageIdentifier } from './generator/ast-generator.js';
 ...
 */
function mapGrammarElements(grammars, grammarServices, visited = new Set(), map = new Map()) {
    for (const grammar of grammars) {
        const doc = AstUtils.getDocument(grammar);
        const uriString = doc.uri.toString();
        if (!visited.has(uriString)) {
            visited.add(uriString);
            map.set(grammar, [...grammar.rules, ...grammar.types, ...grammar.interfaces]);
 ...
 */
function embedReferencedGrammar(grammar, map, grammarServices) {
...
export async function runGenerator(config, options) {
    ...
    for (const languageConfig of config.languages) {
        ...
        if (document) {
            const entryGrammar = document.parseResult.value;
    ...
    const configMap = languages.reduce((prev, curr) => { prev.set(curr.embeddedGrammar, curr.languageConfig); return prev; }, new Map());
    const embeddedGrammars = languages.map(l => l.embeddedGrammar);
    // grammar.ts
...
export async function embedGrammars(languages, config, options, grammarServices) {
