...
# Argument 3 is a comma-separated root list; with none, a Cargo.toml at the
# target makes the PROGRAM derive its roots from `cargo metadata`.
...
# Seeds are harness flags. With no roots named and no Cargo.toml the crawl
...
( cd "$TARGET" && DL_ADAPTERS_DIR="$HERE" \
    ...
    "$ENGINE/target/release/emit_rust_harness" "$WORK/rail.rs" "${SEEDS[@]}" \
