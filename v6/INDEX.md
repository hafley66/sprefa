...
134:## 3 · Graph-theory insights (the covering set on-disk)
157:## APPENDIX — FROZEN CONTRACT (do not drift; all three jobs code to THIS)
159:### A. `src/reach.rs` public API (graph = `cx_dep`, keys are i64)
203:### B. node ↔ key bridge (for the agreement test)
210:### C. uniform measurement (`src/measure.rs`) — the anti-guess rule
235:### D. ownership (disjoint, this wave)
```
...
### labs/exec_shootout/dl6/FACTS.dredland.md
```
1:# In-place recursive-head maintenance: banked numbers
10:## TOC
16:## 1. Incremental ticks — `incbench.ts`, grid 45x45, head 1,069,200 rows
35:## 2. The single build tick — `bench.sh`, min of 2 runs each
61:## 3. Deleting 100 scattered edges at once
77:## 4. Regenerate
```
...
### labs/exec_shootout/sqlite_raw/REPORT-TAIL.md
```
1:# REPORT-TAIL: phase-1 tail alone, measured on real data with no walk
15:## Method, in one line
30:## Results
59:## Verdict
66:## Which statement dominates
80:## Reproducing
```
