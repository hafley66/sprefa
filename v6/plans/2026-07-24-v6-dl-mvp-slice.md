...
9. **M8 · identity-vs-witness (owner escalation 2026-07-24 PM) — DONE** (63/63; curl-session honest and PASS x2+1; merged dl/m8-witness 3b68ca55 + dl/m8-super 366a513e..000b8e7f)
   1. ~~alpha: witness plumbing~~ ✓ file(path, content_hash) sha-256 at ingest (one read); bridge salt-arg law (extra probe args between inputs/outputs = positional-only digest salt, minted salt_<i> cols on __req/__resp, never reach the executor); fixture witness spelling; bridge+ingest goldens re-pinned
   2. ~~beta: supersession~~ ✓ effect_cache(full_digest PK, identity_digest + index); fire-once per FULL digest; new witness = ONE commit {insert new resp, retract superseded resp} through the weights plane; superseded cache rows DELETED (orchestrator latest-wins interpretation, flagged in code: A->B->A re-fires)
...
10. **M9 · engine-core storage (owner escalation, SEVERITY MAX) — DONE** (codex/m9-storage-plane d238bc8d..3047d6cb, merged 72fb7f48; 251-file rxjs corpus: db 40.9MB -> 9.2MB, rel-plane index share 45.9% -> 1.24% against the <=15% gate, zero TEXT join columns; review gate re-run at merge: 65/65 dl, 75/75 store, curl golden PASS x2 outside the sandbox, schema proof by sqlite_master dump). Original escalation text kept below as the record: the dl rel plane rebuilt v5's amplification disease (strings-as-keys, rowid full-tuple PKs, 48%-index file, absolute paths per row) beside its cure; audit found the runtime uses ONE store engine function (with_txn) and M3 built a parallel string-keyed ingest against the plan's ingestJsonl line. Law pinned in tasks.d.ts EngineStorageLaw + amendment.
    1. ~~M9-before~~ ✓ (dbstat harness + baseline; also landed the depth-ceiling fix): dbstat harness + scaled-corpus baseline on the string-keyed schema (rxjs src corpus, hundreds of files)
    2. ~~M9-core + codex execution~~ ✓ (columnTypes flow landed by the sonnet worker, storage rewrite executed by gpt-5.6-luna from a committed brief): wire the actual store — Store/GraphNs attach, interned spine = fact plane, per-file-diff as additive store extension, derived/host rels re-keyed to dictionary ids WITHOUT ROWID with affinity, read views keep the surface byte-identical, DL_ROOT-relative paths with absolute-resolve at the http boundary
    3. INCIDENT mid-arc (found by the baseline run, ruled same day): sqlAnyRowMatch's OR-of-row-predicates blows SQLite's compile-baked MAX_EXPR_DEPTH=1000 on any >~200-row single-rel commit batch (rxjs/src/index.ts: 1783 span_line rows, hard crash; same bomb on retract). Ruled fix: set-oriented TEMP-table matching (batched insert chunked 500, one NULL-safe JOIN pre-check, one EXISTS DELETE), depth O(columns); regression test ingests the exact blocking file; M9-before ownership expanded to land it; M9-core adopts the same form in id-space. Failure-ledger note owed: behavior-golden blindness (no golden corpus exceeded 41 nodes) — the stress-as-golden directive is the rail.
    4. ORCHESTRATOR ARBITRATION (flagged for owner veto at wrap): M9-core verified the store's literal node/edge tables cannot host the dl fact plane (INTEGER-ordinal family/kind vs extract's string kinds; node-id edge pairs vs span endpoints not guaranteed to be nodes; ux_node_identity kind-participation makes span->node_id ambiguous; store tests assert family=1). Confirmed HORN (C) under non-negotiable #8's own escape (additive extension with recorded reason): dl fact rels = interned/typed WITHOUT-ROWID tables minted via the store's create_rel_table, riding the store's ONE strings dictionary + files spine. Ruling essence intact (one dictionary, surrogate ids, zero string FKs); only literal node/edge shape reuse falls. Also pinned: derived//query column types by head-var-to-source-column tracing (declared types for user rels; agg tie-breaks count/sum->int, min/max->arg type).
    5. ~~after-dbstat vs baseline~~ ✓ 45.9% -> 1.24% index share on the same 251-file corpus and driver; churn stress green in-suite (p50 1.04ms/commit, p95 1.74ms, delta rows 216 exact). STILL OPEN: the stress run is not yet PINNED as a suite golden (amendment 3), and PERF-REPORT.md/perf.json now understate the loop engines.
11. **M10 · contract headers (owner directive: "extract the lib classes as interfaces, i want .h files style" -> "call them IGraphNs etc" -> "i want interfaces to express dataflow") — DONE** (c350e88e, c18efcf3, 2ded157e, 45b3225a)
    1. ~~store header~~ ✓ src/engine/types.ts: I-prefixed contract per class + I*Statics + DATAFLOW section (ICascadeApi/IReachApi/IReconcileApi/IRelTableApi + OpenDb/Stamp/CreateAllTables/IngestJsonl)
    2. ~~dl header~~ ✓ IDlRuntime + IHostRunner + Ddl/IngestFile/ShHost/StartServer; 4_ingest.ts stopped importing the concrete DlRuntime class from 3_runtime.ts (an upward import past the numbering law) and takes the contract it always used
    3. ~~import direction flipped~~ ✓ implementations import types from the header, never from each other; the header imports nothing package-local, so its one outside import (type Client) erases at build and lib.ts stays the sole runtime @libsql/client site
    4. ~~enforcement~~ ✓ AssertTrue<Holds extends true> with a **false** branch (never extends true is TRUE, so a never branch proves nothing); every rail broken on purpose before being trusted
    5. found by the rail: IngestFile had been declared without its first parameter since M3. Two dead `export import` aliases (TS's pub use) deleted, zero readers.
12. **M7 · types header consolidation — DONE** (dl/m7-types 5a39592a, merged; zero behavior change: 52/52, tsgo clean, curl-session PASS, fixtures byte-identical, store 75/75)
