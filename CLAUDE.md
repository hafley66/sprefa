...
- **What the language accepts**: `v6/prolog/compile/parse_dl.pl` is the real
  ...
  per fixture (390 rows as of 2026-08-12). `v6/dl/grammar/dl.langium` is a
  NARROW MVP SLICE of the same language, not the language; a construct absent
  there is usually present in the real one. Regenerate the manifest with
  `cd v6/tsv2 && bash scripts/sweep.sh`.
- **TWO `.dl6` renders of the corpus exist and only one is faithful.**
  `v6/prolog/compile/dl_view/*.dl6` (397 files, COMMITTED) DROPS `: type`
  annotations and whole `rel` declarations; `v6/prolog/compile/out/text-door/*.dl6`
  (gitignored, `.gitignore:66`, regenerated by sweep) carries them. Measured
  2026-08-12: three bench-cli cells "would not compile" purely because
  `v6/bench-cli/cases.json` points at the lossy render. Never conclude a program
  is invalid from a `dl_view` file without diffing it against the text-door one.
...
- **`just green-all` is RED and has been since at least 2026-08-11.** It is the
  gate in name; the truth is `.github/CI-KNOWN-RED.md`, which allowlists the
  failing legs and is what CI actually judges against. READ THAT FILE before
  reporting any leg as broken, and before claiming the tree is green. A leg that
  fails and is NOT allowlisted is the real signal. Measured 2026-08-12: two
  back-to-back whole-gate runs on one tree gave DIFFERENT failing sets under
  lane load, so measure a leg with `just <leg>` three times, never once, and
  never from the whole gate.
- Battery numbers as of 2026-08-12 (re-measure, do not trust this line):
  conformance 392/0, sweep `total=286 identical=283 wrong=0 rejection=3`,
  TEXT_DOOR `compiled=288 byte_identical=288 failures=0`, manifest 390 rows
  (286 compiled / 104 unsupported), dl_view corpus 397 files.
...
- **Labs run on flash4** (user-set 2026-08-04, supersedes the 2026-07-29
  ...
  opencode-orchestration skill. Opus stays for diagnosis/mid-task trade-offs
  ...
  **Every lane spawn goes through `boop beep lane create`**, never a bare tmux
  spawn and never `bus dispatch` (that spelling is dead):
  `boop beep lane create --branch <kind>/<name> --brief <ABS path> --goal "..."
  --preset <flash4|terra> --base-sha <origin/main sha>`.
  Presets live in `~/Library/Application Support/boop/config.json`:
  `flash4` = deepseek-v4-flash-0731, `terra` = `gpt-5.6-terra@medium` pinned to
  the codex harness. The `model@effort` suffix is how a codex lane sets
  reasoning effort (`v6/boop/src/harness/codex.rs:169-181`, accepts
  low|medium|high only). `boop beep lane create ... --dry-run` prints the exact
  spawn line; use it before dispatching a shape you have not used before.
...
## Open items
**User decisions 2026-08-12, verbatim, act on these without re-asking:**
- "no coercions". An untyped column does not silently take part in a comparison
  or a numeric aggregate.
- "TAKE THE CORRECT AND MOST CONSISTENT ONE EVEN IF IT MEANS MORE WORK". Applied
  to the two set-rel DDL shapes: ONE shape,
  ...
  the surrogate-keys law that was already on this page.
- "I DO NOT WANT TO RUN V5 ANYTHING ANYMORE". No design may end in "keep the v5
  binary running". Measured cost is one feature: diagnostics is the ONLY v6
  editor feature that reaches an editor through v5, and hover never worked at
  all (`plans/2026-08-12-v6-native-lsp.PLAN.md`).
- "i only want emitters man no more dd-rust-dd on its own". `dd-rust-dd` as a
  ...
  emitter, after `emit_rust`.
- **Generics need a written inspection, in docs, before any generics work.**
  Parked by the user 2026-08-12 ("no dont go chasing that yet but note we need
  to inspect how to make generics as part of docs"). The open question it came
  from: could one generic surface make every optional / undefinable / wrapper
  constructor lower into the SAME companion-table scheme, instead of a bespoke
  desugar each. Do not implement; document the current wrapper set and how each
  lowers first. Starting points: `0_type_plane.pl:145-151` (wrapper inventory),
  `0_generic_expand.pl:125-176` (collection artifacts), `0_option_expand.pl:39-49`
  (the scalar-vs-reference split).
- **A top-level enum rel IS a reusable named type.** Probed 2026-08-12:
  `rel body(page(view: text) ; redirect(to: text)).` plus
  `rel response(id: int, payload: body).` compiles rc=0 and emits `body_page`,
  `body_redirect`, `body_tag` (the discriminator) and `response.payload` holding
  the enum id. It works BECAUSE an enum rel declares no `key(...)`, so it is a
  pure type declaration wearing the `rel` keyword. A named three-variant type
  (a `MaybeUndefined`-shaped absence type, a `Result`-shaped wrapper) therefore
  needs NO new declaration form. Still stopped: `option(<enum>)`, at
  `0_option_expand.pl:43`. A coordinator claimed the opposite this session and
  was corrected by the user; probe before pricing a type as impossible.
- **Absence must be expressible.** `option(T)` today spells value-or-`none` and
  cannot say "key absent" as distinct from "key present, null". The JSON VALUE
  plane already makes that distinction and is conformance-tested
  (`conformance/fixtures/json_patch_fold.pl:132-145`, RFC 7396: a patch key set
  to `none` REMOVES the key). The COLUMN plane cannot. Design open;
  `4_emit_jsonschema.pl:121` currently papers over it by dropping option columns
  from `required`, which emits a schema looser than the IR.
