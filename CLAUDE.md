...
### Bugs / gaps
- [ ] **2026-07-18 kill-respawn read-storm incident** (failure-modes classes 16+17): external agent's doubled `dl --check` + user kills = 6 singleton generations in 7min, each blank-slate ("first-run"; deferred digest saves persist nothing on kill), each reading 400-500MB against a 979MB root db (7.3MB corpus, ~140x). Done: VACUUM (979→895MB — bloat is live rows: `_strings` 187MB w/ index, 463 tables autoindex-doubled), 1.9GB per-root fossil cache.db deleted. Open rails: client autostart-once+backoff, mid-cold digest persistence + kill-mid-cold resume it-test, db-size ratio verdict+ceiling, storage diet.
- [ ] **obs-logging arc IN FLIGHT** (sonnet agent, branch obs-logging): global invocation log (SQL row per CLI entry, ancestry, open row = killed, why joins spawner), tracing file layers dl.log/error.log w/ rotation, per-request ids on HTTP+socket surfaces w/ access log + jobq attribution plumbing, docs/engine-loops.md loop inventory.
