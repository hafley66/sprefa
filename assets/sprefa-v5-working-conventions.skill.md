...
| Mixed source/derived heads auto-desugar | A rel headed by both a source rule (`scan`/`match`/`ast`/`sg`/`json`/`cmd`/`comment`) and a derived rule — or by a term-extract rule (`json`/`jsonp` over a bound string) and a derived rule — used to bail: `rebuild_derived`'s full `DELETE FROM rel` would drop the reconciled source rows. The engine (`src/engine/desugar.rs`) now splits it itself into hidden `<rel>__src`/`<rel>__drv` twins plus a synthesized union, invisibly; every other rule/`?`/the panel keeps reading the original name. Two combinations still bail: a `key(...)`/`merge(...)` lattice rel mixed this way (the upsert winner is undecidable), and an `@in`/`@out` port rel headed by anything but its own serving loop. |
