...
///
/// Pattern compiles at construction (panic on syntax error to fail
/// fast at lower-time). Matching is over the full `cursor.value`
/// string; for path-shaped values, patterns like `**/*.rs` compose as
/// expected against absolute or relative paths.
pub struct GlobComponent {
    pub pattern: Arc<str>,
    matcher:     GlobMatcher,
    /// Layer 0c.2 — content-derived intern store. Reserved for future
    /// capture-bind coord stamping; the basic filter doesn't read it
    /// yet.
...
impl GlobComponent {
    pub fn new(pattern: Arc<str>) -> Self {
        let matcher = Glob::new(&pattern)
            ...
            .compile_matcher();
        Self { pattern, matcher, store: None }
    }
...
impl Component for GlobComponent {
    ...
    fn render(&self, _ctx: &RenderCtx, c: &Cursor) -> Node<Cursor> {
        ...
        if !self.matcher.is_match(hay) { return Node::Done; }
        // Filter pass-through: clone the cursor, leave value unchanged.
        // Capture bindings (`<NAME>` sigils via scan_glob_captures) will
        // attach here when the rule lands; today it's a pure filter.
