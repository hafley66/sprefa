...
impl FsComponent {
    ...
    pub fn with_include_exts(mut self, exts: Vec<String>) -> Self {
        self.include_exts = Some(Arc::new(exts));
        self
    }
    pub fn with_source_filter(mut self, f: Arc<dyn Fn(&str) -> bool + Send + Sync>) -> Self {
        self.source_filter = Some(f);
        self
    }
    ...
    fn include_path(&self, path: impl AsRef<std::path::Path>) -> bool {
        let Some(exts) = &self.include_exts else {
            return true;
        };
...
impl Component for FsComponent {
    ...
    fn dispatch(
        ...
        let any_rev = self
            ...
            .map(|s| {
                rows.iter()
                    .any(|r| s.coord_of(r.value.at).map(|c| c.rev != 0).unwrap_or(false))
            })
            .unwrap_or(false);
        if any_rev {
            for parent in rows {
                ...
                let Some((slug, _remote)) = store.lookup_repo(parent_coord.repo) else {
                    continue;
                };
                let slug_str: &str = slug.as_ref();
                let Some(root_pb) = config
                    .repos
                    .iter()
                    .find(|r| r.slug == slug_str)
                    .map(|r| r.root.clone())
                else {
                    continue;
                };
                let Some((_repo_id, oid_arc, _ts)) = store.lookup_rev(parent_coord.rev) else {
                    continue;
                };
...
impl Component for AstNmComponent {
    ...
    fn render_batch(&self, _ctx: &RenderCtx, batch: &[&Cursor]) -> Vec<Node<Cursor>> {
        ...
        par_render(batch, move |c| {
            ...
            let (cur_pat, cur_fixed): (Arc<Pattern<SupportLang>>, Arc<str>) = ast_timed(
                &telemetry,
                |t| &t.pattern_ns,
                || match &dyn_body {
                    ...
                    Some((body, interps)) => {
                        let pat_src =
                            crate::template::render_segments_no_diag(body.as_ref(), interps, c);
            ...
            let (src, file_id, repo, rev, source_path, source_bytes, prefiltered_bytes): (
                String,
                crate::FileId,
                crate::RepoId,
                crate::RevId,
                ...
                if let Some(t) = &telemetry {
                    ...
                    t.source_read_bytes
                        .fetch_add(source.bytes.len() as u64, Ordering::Relaxed);
                }
                ...
                if let Some(t) = &telemetry {
                    ...
                    t.source_utf8_bytes
                        .fetch_add(source_bytes.len() as u64, Ordering::Relaxed);
                }
                (
                    src,
                    source.file,
                ...
                ast_timed(
                    ...
                    || {
                        ...
                        let (repo, rev) = parent_coord.map(|c| (c.repo, c.rev)).unwrap_or((0, 0));
            ...
            let ranges: Vec<std::ops::Range<usize>> = ast_timed(
                &telemetry,
                |t| &t.match_ns,
                || {
                    grep.root()
            ...
            let hits: Vec<Node<Cursor>> = ranges
                .into_iter()
                .map(|r| {
                    let mut child = c.clone();
                    // Plain string mirror — keeps legacy consumers that
                    // expect LO/HI as text values intact.
            ...
            let hits: Vec<Node<Cursor>> = grep
                ...
                .map(|nm| {
                    ...
                    if let Some(store) = &store {
                        ...
                        store.observe_string(child.value_id, "ast_yaml_match", where_bytes, "", 0);
                        child.set_at("MATCH", slice, coord, store);
                        child.set("MATCH_LO", coord.lo.to_string());
                        child.set("MATCH_HI", coord.hi.to_string());
...
impl Component for MultiAstNmComponent {
    ...
    fn render_batch(&self, _ctx: &RenderCtx, batch: &[&Cursor]) -> Vec<Node<Cursor>> {
        ...
        par_render(batch, move |c| {
            ...
            let any_fires = pats
                .iter()
                .any(|(_, _, fx)| fx.is_empty() || src.contains(&**fx));
            if !any_fires {
                return Node::Done;
            }
            let grep: AstGrep<StrDoc<SupportLang>> = lang.ast_grep(&src);
            ...
            for (label, pat, fx) in pats.iter() {
                if !fx.is_empty() && !src.contains(&**fx) {
                    continue;
                }
...
impl SplitComponent {
    ...
    pub fn from_term(mut self, name: &str) -> Self {
        self.from = Some(name.into());
        self
    }
    pub fn into_term(mut self, name: &str) -> Self {
        self.into = Some(name.into());
        self
    }
}
impl Component for SplitComponent {
    ...
    fn render(&self, _ctx: &RenderCtx, c: &Cursor) -> Node<Cursor> {
        let src: &str = match &self.from {
            Some(name) => match c.get(name) {
                Some(s) => s,
                None => return Node::Done,
            },
            None => &c.value,
        };
        let out: Vec<Cursor> = src
...
impl Component for TrimComponent {
    type Next = Cursor;
    fn render(&self, _ctx: &RenderCtx, c: &Cursor) -> Node<Cursor> {
        let Some(s) = c.get(&self.from) else {
...
impl Component for LineComponent {
    type Next = Cursor;
    fn render(&self, _ctx: &RenderCtx, c: &Cursor) -> Node<Cursor> {
        let Some(text) = c.get(&self.source).map(|s| s.to_string()) else {
            return Node::Done;
        };
        let out: Vec<Cursor> = text
            .split('\n')
            .filter(|l| !l.is_empty())
            .map(|line| {
                let mut child = c.clone();
                child.set(&self.target, line);
                child
            })
            .collect();
        nodes_from(out)
...
impl ReComponent {
    ...
    pub fn on_term(mut self, term: &str) -> Self {
        self.on = term.to_string();
        self
    }
    pub fn with_match_text(mut self, on: bool) -> Self {
        self.want_match = on;
        self
    }
    ...
    fn render_batch(&self, _ctx: &RenderCtx, batch: &[&Cursor]) -> Vec<Node<Cursor>> {
        ...
        par_render(batch, move |c| {
            ...
            let cur_re: std::borrow::Cow<'_, regex::Regex> = match &dyn_body {
                ...
                Some((body, interps)) => {
                    let pat = crate::template::render_segments_no_diag(body.as_ref(), interps, c);
            ...
            for caps in re.captures_iter(&haystack) {
                ...
                if let Some(store) = &store {
                    let match_coord = Coord {
                        repo: source_coord.repo,
                        rev: source_coord.rev,
                        fs: file_id,
                        lo: m0.start() as u32,
                        hi: m0.end() as u32,
                    };
...
fn render_sh_template(
    ...
    for interp in interps.iter() {
        ...
        let value: String = match &interp.kind {
            ...
            InterpKind::SubPipe { lowered, .. } => match lowered {
...
impl ShComponent {
    pub fn emit_lines(cmd: &str) -> Self {
        Self {
            cmd_template: cmd.into(),
            interps: Arc::new(Vec::new()),
            mode: ShMode::EmitLines,
            bind: None,
        }
    }
    ...
    pub fn with_interps(mut self, interps: Vec<crate::compile::lower::op_def::DslInterp>) -> Self {
        self.interps = Arc::new(interps);
        self
    }
}
        ...
        par_render(batch, move |c| {
            ...
            match mode {
                ShMode::EmitLines => {
                    ...
                    let hits: Vec<Node<Cursor>> = stdout
                        .split('\n')
                        .filter(|l| !l.is_empty())
                        .map(|line| {
                            let mut child = c.clone();
...
impl SeedComponent {
    pub fn one() -> Self {
        Self {
            rows: vec![Cursor::default()],
        }
    }
    pub fn new(rows: Vec<Cursor>) -> Self {
        Self { rows }
    }
}
...
impl RepoComponent {
    ...
    pub fn auto(roots: &[PathBuf], exts: Vec<String>) -> Self {
        let labelled: Vec<(String, PathBuf)> = roots
            .iter()
            .map(|p| {
                let slug = p
                    .file_name()
...
impl Component for RepoComponent {
    ...
    fn dispatch(
        ...
        let RepoMode::Roots(roots) = &self.mode else {
            unreachable!()
        };
...
impl Component for RevComponent {
    ...
    fn dispatch(
        ...
        for parent in rows {
            ...
            let Some(repo_arc) = self.open(slug_str, &root) else {
                continue;
            };
...
impl Component for RepoFromTermComponent {
    type Next = Cursor;
    fn render(&self, _ctx: &RenderCtx, c: &Cursor) -> Node<Cursor> {
        let Some(root_str) = c.get(&self.term) else {
            return Node::Done;
        };
...
impl Component for CommentComponent {
    ...
    fn render(&self, _ctx: &RenderCtx, c: &Cursor) -> Node<Cursor> {
        ...
        let opens: Vec<(usize, usize)> = self
            .open
            .find_iter(&hay)
            .map(|m| (m.start(), m.end()))
            .collect();
        ...
        let regions: Vec<(usize, usize)> = match &self.close {
            ...
            Some(cre) => {
                ...
                let closes: Vec<(usize, usize)> =
                    cre.find_iter(&hay).map(|m| (m.start(), m.end())).collect();
                let mut events: Vec<(usize, bool, usize)> = Vec::new(); // (pos, is_open, end)
                for (s, e) in &opens {
                    events.push((*s, true, *e));
                }
                for (s, e) in &closes {
                    events.push((*s, false, *e));
                }
        ...
        let hits: Vec<Node<Cursor>> = regions
            .into_iter()
            .map(|(lo, hi)| {
                let mut child = c.clone();
...
impl WriteCursorComponent {
    ...
    pub fn on_capture(mut self, name: impl Into<Arc<str>>) -> Self {
        self.on_capture = Some(name.into());
        self
    }
        ...
        for c in batch {
            ...
            if let Some(name) = &self.on_capture {
                ...
                let Some(term) = c.term(name_id) else {
                    ctx.diag.emit(Diag::warn(
                        ...
                        format!(
                            "write_cursor(:on={}): cursor has no term named {}",
                            name, name
                        ),
                    ));
                    continue;
                ...
                if coord.fs == 0 {
                    ctx.diag.emit(Diag::warn(
                        ...
                        format!(
                            "write_cursor(:on={}): capture has synthetic coord (no file)",
                            name
                        ),
                    ));
                    continue;
                ...
                let Some((_hash, path)) = store.lookup_file(coord.fs) else {
                    ctx.diag.emit(Diag::warn(
                        "write/no-coord",
                        format!(
                            "write_cursor(:on={}): FileId {} not in _files",
                            name, coord.fs
                        ),
                    ));
                    continue;
                ...
                entry
                    .1
                    .push((coord.lo as usize, coord.hi as usize, c.value.clone()));
                continue;
            }
            ...
            let target = self
                ...
                .or_else(|| {
                    ...
                    let file_id_opt: Option<crate::FileId> = self
                        .store
                        .as_ref()
                        .and_then(|s| s.coord_of(c.at))
            ...
            let Some((path, file_id_opt, lo, hi)) = target else {
                continue;
            };
            let entry = by_file.entry(path).or_insert((file_id_opt, Vec::new()));
                ...
                if let Some((expected_hash, _)) = store.lookup_file(file_id) {
                    ...
                    if on_disk_hash != expected_hash {
                        ctx.diag.emit(Diag::warn(
                            "write/file-drift",
                            format!(
                                "{}: on-disk content changed since intern_file; skipping write",
                                path
                            ),
                        ));
                        continue;
...
impl WriteFileComponent {
    pub fn new(path: Option<WriteFilePath>) -> Self {
        Self { path }
    }
}
...
impl CollectComponent {
    ...
    fn emit_rows(&self, scope: BarrierScope, queue: &dyn QueueBackend<Cursor>, rows: &[Cursor]) {
        if rows.is_empty() {
            return;
        }
...
impl Component for CollectComponent {
    ...
    fn complete(&self, _ctx: &RenderCtx, scope: BarrierScope, queue: &dyn QueueBackend<Cursor>) {
        let mut states = self.states.lock().unwrap();
            ...
            let dyn_built: Option<crate::cst::dsls::json::JsonCompiled> = match &dyn_body {
                ...
                Some((body, interps, fmt)) => {
                    let pat_src =
                        crate::template::render_segments_no_diag(body.as_ref(), interps, c);
                    let built =
                        match crate::cst::dsls::json::JsonDsl::compile_typed(pat_src.as_bytes()) {
...
mod tests {
    ...
    fn run_pipe(ops: Vec<Arc<dyn Component<Next = Cursor>>>, seed: Vec<Cursor>) -> Vec<Cursor> {
