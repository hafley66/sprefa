...
pub trait DslBodyLsp: Send + Sync {
    fn hover(&self, _body: &[u8], _byte: usize) -> Option<Hover> {
        None
    }

    fn diagnostics(&self, _body: &[u8], _diags: &dyn DiagSink) {}

    fn semantic_tokens(&self, _body: &[u8]) -> Vec<SemanticToken> {
        vec![]
    }
    ...
    fn folding_ranges(&self, _body: &[u8]) -> Vec<FoldingRange> {
        vec![]
    }

    fn document_symbols(&self, _body: &[u8]) -> Vec<DocumentSymbol> {
        vec![]
    }

    fn selection_ranges(&self, _body: &[u8], _byte: usize) -> Vec<SelectionRange> {
        vec![]
    }

    fn document_highlights(&self, _body: &[u8], _byte: usize) -> Vec<DocumentHighlight> {
        vec![]
    }

    fn inlay_hints(&self, _body: &[u8], _range: Range<usize>) -> Vec<InlayHint> {
        vec![]
    }

    fn document_links(&self, _body: &[u8]) -> Vec<DocumentLink> {
        vec![]
    }

    fn formatting(&self, _body: &[u8]) -> Option<Vec<TextEdit>> {
        None
    }

    fn prepare_rename(&self, _body: &[u8], _byte: usize) -> Option<lsp_types::Range> {
        None
    }

    fn rename(&self, _body: &[u8], _byte: usize, _new_name: &str) -> Option<Vec<TextEdit>> {
        None
    }

    fn document_color(&self, _body: &[u8]) -> Vec<ColorInformation> {
        vec![]
    }
