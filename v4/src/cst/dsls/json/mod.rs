...
use data::{AnyDataNode, DataNode, JsonNode};
...
impl JsonDsl {
    ...
    pub fn compile_typed(body: &[u8]) -> Result<JsonCompiled, Diag> {
        let body_str = std::str::from_utf8(body)
            .map_err(|e| Diag::error("json.utf8", format!("body not utf-8: {e}"), 0..body.len()))?;
        ...
        let (steps, _annotations, _positions) =
            parse_body(body_str).map_err(|e| Diag::error("json.parse-body", e, 0..body.len()))?;
        let compiled =
            compile_steps(&steps).map_err(|e| Diag::error("json.compile", e, 0..body.len()))?;
        Ok(JsonCompiled {
...
fn scan_capture_names(raw: &[u8]) -> Vec<String> {
    ...
    while i < raw.len() {
        ...
        {
            let mut j = i + 1;
            while j < raw.len() && (raw[j].is_ascii_alphanumeric() || raw[j] == b'_') {
                j += 1;
            }
    ...
    fn lsp_semantic_tokens_classify_capture_string_number_braces() {
        ...
        let kinds: Vec<&str> = toks
            ...
            .map(|t| {
                ...
                match toks
                    .iter()
                    .position(|x| x as *const _ == t as *const _)
                    .map(|i| (i, t))
                {
