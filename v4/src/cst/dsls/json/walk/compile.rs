...
fn compile_one_step(step: &SelectStep) -> Result<CompiledStep, String> {
    Ok(match step {
        ...
        SelectStep::Leaf { capture } => CompiledStep::Leaf { capture: capture.clone() },
        SelectStep::CaptureAny { capture } => CompiledStep::CaptureAny { capture: capture.clone() },
        ...
        SelectStep::Array { item } => CompiledStep::Array { item: compile_steps(item)? },
    })
}
...
fn compile_key_matcher(km: &KeyMatcher) -> Result<CompiledKeyMatcher, String> {
    Ok(match km {
        ...
        KeyMatcher::Glob(pattern) => CompiledKeyMatcher::Glob(
            compile_pattern(pattern).map_err(|e| e.to_string())?,
        ),
