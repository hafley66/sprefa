...
use effect_runtime::v2::{
    expand, Component, ExpandOpts, MemQueue, Node, Pipe, PipeInstance,
...
pub fn run_once_const(p: &Pipe<Cursor>, _ctx: &LowerCtx) -> Result<String, LowerError> {
    ...
    let mut all_steps: Vec<Arc<dyn Component<Next = Cursor>>> =
    ...
    if rows.is_empty() {
        return Err(LowerError::Unknown("run_once_const: pipe emitted nothing".into()));
    }
    Ok(rows.iter().map(|c| c.value.as_ref()).collect::<Vec<_>>().join(""))
}
