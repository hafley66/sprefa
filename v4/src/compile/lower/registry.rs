...
pub fn validate_call(
    ...
    let has_variadic = spec
        .last()
        .map(|s| matches!(s.kind, ArgKind::Variadic(_)))
        .unwrap_or(false);
    let n_fixed = if has_variadic {
        spec.len() - 1
    } else {
        spec.len()
    };
    let n_required = spec
        .iter()
        .filter(|s| s.required && !matches!(s.kind, ArgKind::Variadic(_)))
        .count();
    ...
    if args.len() < n_required {
        diags.push(
            Diag::error(
                "lower/missing-slot",
                format!(
                    "op `{}` requires at least {} positional arg(s), got {}",
                    def.name(),
                    n_required,
                    args.len()
                ),
        ...
        for (extra, r) in &args[spec.len()..] {
            diags.push(
                Diag::error(
                    "lower/slot-not-allowed",
                    format!(
                        "op `{}` does not accept arg #{} ({})",
                        def.name(),
                        spec.len(),
                        extra.value.kind_str()
                    ),
                )
                .with_span(r.lo, r.hi),
            );
...
fn normalize_call_args(
    ...
    let variadic = spec
        .last()
        .filter(|s| matches!(s.kind, ArgKind::Variadic(_)));
    let fixed_len = if variadic.is_some() {
        spec.len().saturating_sub(1)
    } else {
        spec.len()
    };
    ...
    for (arg, range) in args {
        match arg.keyword {
            ...
            Some(ref keyword) => {
                saw_kw = true;
                let Some(idx) = spec[..fixed_len]
                    .iter()
                    .position(|s| s.name == keyword.as_ref())
                else {
                    diags.push(
                        Diag::error(
                            "lower/unknown-kwarg",
                ...
                if out[idx].is_some() {
                    diags.push(
                        Diag::error(
                            "lower/duplicate-arg",
                            format!(
                                "op `{}` arg `{}` assigned more than once",
                                def.name(),
                                keyword
                            ),
                        )
