...
use v4::app::{
    build_in_process, LspLocateDslReq, LspOpenReq, SprfClient, SprfError,
};
...
async fn locate_dsl_unknown_doc_errors() {
    ...
    let err = client.lsp_locate_dsl(LspLocateDslReq {
...
async fn locate_dsl_inside_re_body_uses_cache() {
    ...
    let hit = client.lsp_locate_dsl(LspLocateDslReq {
        uri:  "file:///probe.sprf".into(),
        byte: probe as u32,
    }).await.unwrap();
