...
fn init_git(path: &std::path::Path, files: &[(&str, &[u8])]) {
    let g = |args: &[&str]| {
        let st = Command::new("git").args(args).current_dir(path)
...
fn rev_head_resolves_oid_and_intern_rev() {
    ...
    assert_eq!(out[0].get("REV"), Some(oid),
...
fn rev_bare_form_defaults_to_head() {
    ...
    assert_eq!(bare_out[0].value, head_out[0].value,
