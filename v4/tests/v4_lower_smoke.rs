...
use effect_runtime::v2::{
    expand, ByteRange, Diag, ExpandOpts, FactStore, MemFactStore, MemQueue, Pipe, QueueBackend,
};

use v4::lower::op_def::{ArgKind, ArgSig};
use v4::lower::{
    default_registry, str_pipe, validate_call, BlockShape, CallArg, DslBody, DslInterp, DslShape,
    LowerCtx, OperatorDef, Value,
};
use v4::Cursor;
...
fn four_slot_lower_smoke() {
    ...
    let interp_dsl = DslBody {
        ...
        interps: vec![DslInterp {
            name: Arc::from("WHO"),
            range: br(6, 12),
            kind: v4::lower::op_def::InterpKind::Term {
                mode: v4::lower::op_def::InterpMode::Read,
                field: None,
            },
            mode: v4::lower::op_def::InterpMode::Read,
            field: None,
    ...
    let interp = str_def
        .lower(&ctx1, None, &[], None, Some(&interp_dsl))
        .unwrap();
    ...
    let diags = validate_call(
        ...
        &None,
        &[(
            CallArg::keyword("p", Value::pipe(str_pipe("x"))),
            br(10, 14),
        )],
        &None,
        &None,
    ...
    assert!(
        diags.is_empty(),
        "expected keyword arg p to validate, got: {:?}",
        diags
    );
    ...
    assert_eq!(
        reg.get("rule").unwrap().brace_block(),
