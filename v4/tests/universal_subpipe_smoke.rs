...
fn re_body_renders_subpipe() {
    // FS-less re path: matcher reads `cursor.value` via a cursor where FS
    // is not bound. ReComponent's else branch (on_term) walks `c.get(on)`
    // when `on != FS`; default is FS. To test the re subpipe path
    // directly we lower a `str` head, then `re` reads from FS (a temp
    // file containing the haystack), with the pattern `${ str`needle` }`.
    let tmp = tempfile::tempdir().unwrap();
    let path = tmp.path().join("hay.txt");
    std::fs::write(&path, b"the needle is here").unwrap();

    // Walk one fs cursor (FS bound) into re with a subpipe body that
    // drains "needle" as the pattern.
    let src = format!(
        r#"rule(:hits) {{ fs > glob`**/hay.txt` > re`${{ str`needle` }}` }};"#
    );
    let store = run_in(tmp.path(), &src);
    let rows = store.rows_of("hits");
