...
fn single_segment_capture_binds_term() {
    ...
    let mut bindings: Vec<(String, String)> = rows
        .iter()
        .map(|c| {
    ...
    assert!(
        bindings
            .iter()
            .any(|(s, e)| s.ends_with("alpha") && e == "txt"),
    ...
    assert!(
        bindings
            .iter()
            .any(|(s, e)| s.ends_with("bravo") && e == "rs"),
        "expected (bravo, rs) in {:?}",
        bindings
    );
}
...
fn multi_segment_via_double_star_and_single_capture() {
    ...
    // `**/${PARENT?}/${FILE?}.rs` matches `<anything>/deep/leaf.rs`,
