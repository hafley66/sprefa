...
use v4::store::{
    SprfStore, FILES_TABLE, PATHS_TABLE, REPOS_TABLE, REVS_TABLE, STRINGS_TABLE,
    STRING_OBSERVATIONS_TABLE, WHERE_BYTES_TABLE,
};
...
fn intern_file_is_content_addressable() {
    ...
    assert_ne!(
        id_a, id_c,
        "different content under same path → different FileId"
    );
    assert_eq!(
        s.inner().len(FILES_TABLE),
        3,
        "sentinel + 2 unique contents"
    );
...
fn intern_where_bytes_dedups_on_span_tuple() {
    ...
    let r1 = s.intern_where_bytes(WhereBytes {
        string: abc,
        repo: 0,
        rev: 0,
        file: f,
    ...
    let r2 = s.intern_where_bytes(WhereBytes {
        string: abc,
        repo: 0,
        rev: 0,
        file: f,
    ...
    let r3 = s.intern_where_bytes(WhereBytes {
        string: bc,
        repo: 0,
        rev: 0,
        file: f,
    ...
    assert_ne!(r1, r3);
    assert_eq!(
        s.inner().len(WHERE_BYTES_TABLE),
        3,
        "sentinel + 2 unique byte ranges"
    );
    assert_eq!(
        s.where_bytes_of(r1),
        Some(WhereBytes {
...
fn where_bytes_id_includes_string_id() {
    ...
    let a = s.intern_where_bytes(WhereBytes {
        string: abc,
        repo: 0,
        rev: 0,
        file: f,
    ...
    let b = s.intern_where_bytes(WhereBytes {
        string: xyz,
        repo: 0,
        rev: 0,
        file: f,
    ...
    assert_eq!(
        s.inner().len(WHERE_BYTES_TABLE),
        3,
        "sentinel + 2 located strings sharing one span"
    );
}
...
fn sentinels_pre_inserted() {
    ...
    assert_eq!(
        s.where_bytes_of(WhereBytesId::SYNTHETIC),
        Some(WhereBytes::default())
    );
...
fn norm_columns_populated() {
    ...
    assert_eq!(
        row.get("norm").as_deref(),
        Some("apiserviceapiserviceapiservice")
    );
}
...
fn find_where_bytes_covering_returns_covering_ids() {
    ...
    let r1 = s.intern_where_bytes(WhereBytes {
        string: s.intern_string("some bytes"),
        repo: 0,
        rev: 0,
        file: f,
        lo: 0,
    ...
    let r2 = s.intern_where_bytes(WhereBytes {
        string: s.intern_string("bytes for"),
        repo: 0,
        rev: 0,
        file: f,
        lo: 5,
    ...
    let r3 = s.intern_where_bytes(WhereBytes {
        string: s.intern_string("indexing"),
        repo: 0,
        rev: 0,
        file: f,
        lo: 20,
    ...
    assert!(s
        .find_where_bytes_covering(0xdead_beef_dead_beef, 7)
        .is_empty());
}
...
fn lru_eviction_invisible() {
    ...
    for (i, id) in ids.iter().enumerate() {
        ...
        assert_eq!(
            got.as_deref(),
            Some(words[i]),
            "cold-tier rehydrate after LRU eviction for {:?}",
            words[i],
        );
    }
}
