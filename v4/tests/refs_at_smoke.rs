...
async fn refs_at_returns_seeded_ref_for_covering_byte() {
    ...
    let r = store.intern_ref(Coord {
        repo: 0, rev: 0, fs: file, lo: 6, hi: 11,
    });

    let resp = client.refs_at(RefsAtReq {
...
async fn refs_at_out_of_range_byte_is_empty() {
    ...
    assert!(resp.hits.is_empty(), "byte 100 falls outside every seeded ref");
}
...
async fn refs_at_unknown_path_is_empty() {
    ...
    assert!(resp.hits.is_empty(), "unknown path resolves no FileId, no hits");
}
