...
use effect_runtime::v2::{
    expand, Component, Diag, DiagSink, ExpandOpts, FactStore, MemFactStore,
...
fn cursor_with_capture(
    ...
    let coord = Coord { repo: 0, rev: 0, fs: file_id, lo, hi };
...
fn write_cursor_on_capture_replaces_capture_bytes_only() {
    ...
    assert!(
        ...
        "no drift diag expected; got {:?}", diags,
    );
}
...
fn write_cursor_on_capture_leaves_other_captures_untouched() {
    ...
    let fn_coord  = Coord { repo: 0, rev: 0, fs: file_id, lo: 3, hi: 8 };
    let arg_coord = Coord { repo: 0, rev: 0, fs: file_id, lo: 9, hi: 13 };
