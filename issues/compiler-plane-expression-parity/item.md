...
- [ ] Compiler and runtime scalar expressions share one semantic contract.
- [ ] Integer addition can shift ordered member positions for user-land `concat`.
- [ ] Ordering and comparison have deterministic ground-domain checks.
- [ ] Finite grouped count supports all-members checks without open-world negation.
- [ ] Aggregate inputs are complete within one immutable compiler round.
- [ ] Construction results enter the existing bounded refreeze frontier.
- [ ] Compiler expression and aggregate rows erase before runtime planning.
