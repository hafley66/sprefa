...
### 2026-08-17T04:24:48Z · @extract-driver

GO SLICE LANDED 2026-08-17: PR #338 merged, origin/main 4e0767f6b. flash4 lane feature-extract-module-plane-go, second spawn (first died silently, no bus row, only the fixture written; hailed as blocker alongside the dl6 lane main-tree pollution). Lane produced 2 commits (44178a014 walker, 5bb62450b fixture+test), 3 owned files only, PR body carried the row table and gate. Row table matched my independently computed offsets exactly, no expectation edited. Review trims by extract-driver on the branch before merge: dropped the brief's pseudo-code block copied verbatim into the fn body (comment budget) and reverted a stray rustfmt hunk at walk_go_entities. Gate measured by me: full suite 36 binaries 144 passed 0 failed; 25_go_specifiers 2/2 three times; golden_parity 9/9; 4_capability_parity 2/2; 2_df_aux_cli 1/1; zero eprintln, zero em dash in diff. Remaining on this card: kotlin specifier slice (next), python parked.

### 2026-08-17T04:27:35Z · @extract-driver

KOTLIN SLICE DISPATCHED 2026-08-17: lane feature-extract-module-plane-kotlin, flash4, base origin/main 4e0767f6b, brief TASKS/extract-module-plane-kotlin.BRIEF.md. Grammar measured against the real fixture: import_list of import_header; import_header holds an identifier node covering the whole dotted path (stopping BEFORE .* on wildcards), then optionally import_alias (type_identifier child = alias) or wildcard_import. Row span = identifier.start to import_header.end (excludes the import keyword, mirrors go's import_spec span). MAPPING follows the RUST symbol-level convention from PR #328 (module = full path as written) rather than go's path-only one, because kotlin imports bind a symbol: plain import to Named/name=last segment/module=full path; aliased to Named/name=alias/module=path; wildcard to Namespace/name=last package segment/module=package path without .*. Matches v5 src/graph/modgraph/kotlin.rs:203-207 (local = alias or last segment) and :164-176 (wildcard branch). Backticked identifiers out of scope, text as written. Expected rows computed from the 254-byte fixture independently of the extractor: (named,List,Some(kotlin.collections.List),146) (named,JMap,Some(java.util.Map),177) (namespace,text,Some(kotlin.text),206). Brief carries the two lessons from the go PR review: do not paste the pseudo-code into the fn body, revert stray rustfmt hunks.

### 2026-08-17T04:51:13Z · @extract-driver
