...
for application in Applications:
    signature = resolve compiler relation application
    require first input Target:type
    invoke application with Target=current
    require exactly one return:type
    retain annotation(site, current, application, returned)
    current = returned
rewrite column type to current
...
- Parse, print, reparse equality for empty, one, and multiple annotations.
