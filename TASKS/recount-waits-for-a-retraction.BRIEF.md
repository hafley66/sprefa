...
## What the first attempt got wrong (sonnet lane, branch origin/fix/recount-waits-for-a-retraction at 7d867e309, read it, reuse tests/recount_gate.rs)
Its gate computed the 'shrank' set ONCE from the probe before the recount pass, so a loss produced BY a recount inside the pass never reached readers later in the same pass. Witness on ghcache, tick 6: both modes delete pr_due(acme/widgets, 0) and add no bucket-1 pr_due row, yet the gated engine adds pr_batch_member(1, 0, batch/0, repo_1) and removes it only at tick 7: a phantom. The naive log (tests/fixtures/ghcache_ticklog_base.txt) is RIGHT and stays. The lane then claimed 'levels run once in emitted order, not dependency order'; false: emitted order is topological (strat.pl sql_rule_order/2; dirty_repo 63 < pr_due 64 < pr_batch_member 65 in ghcache's levels).

## Deliverable
0. The shrank set is LIVE: after each reconcile with rows_changed > 0, mark that head shrank (and grew, for negated readers), so readers later in the topological pass recount; recursion groups loop until no reconcile changes rows. Pin with ONE additive conformance fixture: A loses a row in tick N; B <- A with a refcount-maintained head; C <- B; the oracle says C loses its row in the SAME tick. Ownership widened to that one fixture file and its graded.tsv row.
