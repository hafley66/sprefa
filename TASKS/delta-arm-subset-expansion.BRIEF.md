...
Issue: `issuectl show delta-arm-subset-expansion` (the measurement table and the emitted SQL sizes are there). Base: `git merge --ff-only <sha the coordinator states>` first; fail = stop and hail. Branch `fix/delta-arm-subset-expansion`. PR to main.
