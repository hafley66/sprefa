...
mod integration_tests {
    ...
    fn cargo_package_name() {
        let sprf = "rule(pkg) { fs(**/Cargo.toml) > json({ package: { name: $NAME } }) };";
        let toml = br#"
            [package]
            name = "sprefa"
            version = "0.1.0"
    ...
    fn json_nested_deps() {
        ...
        assert_eq!(
            results[0],
            vec![
                ("NAME".into(), "express".into()),
                ("VERSION".into(), "4.18.2".into())
            ]
        );
        assert_eq!(
            results[1],
            vec![
                ("NAME".into(), "lodash".into()),
                ("VERSION".into(), "4.17.21".into())
            ]
        );
    }
    ...
    fn yaml_recursive_descent() {
        let sprf = "rule(img) { fs(**/values.yaml) > json({ **: { image: { repository: $REPO, tag: $TAG } } }) };";
    ...
    fn multiple_rules() {
        let sprf = r#"
            ...
            rule(ws) { fs(**/Cargo.toml) > json({ workspace: { members: [...$M] } }) };
        "#;
        let toml = br#"
            [package]
            name = "hello"
    ...
    fn kitchen_sink_sprf_parses() {
        let sprf_src = std::fs::read_to_string(concat!(
    ...
    fn kitchen_sink_cargo_deps() {
        ...
        assert!(
            dep_names.contains(&"serde".to_string()),
            "expected serde in deps, got {:?}",
            dep_names
        );
        assert!(
            dep_names.contains(&"anyhow".to_string()),
            "expected anyhow in deps, got {:?}",
            dep_names
        );
        assert!(
            dep_names.contains(&"ast-grep-core".to_string()),
            "expected ast-grep-core in deps, got {:?}",
            dep_names
        );
    }
    ...
    fn real_workspace_cargo_toml() {
        ...
        assert!(
            results.len() > 5,
            "expected multiple workspace members, got {}",
            results.len()
        );
        let members: Vec<_> = results
            .iter()
