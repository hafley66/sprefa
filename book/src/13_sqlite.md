...
flowchart LR
  ...
  lowersql -->|fold| foldadd[int.add step, integer seed]
...
A `fold` lowers only when its step is kernel `int.add` with an integer seed (`_7_sqlite.rs:860-883`).
A rule outside that set gets `emit_sqlite_unsupported(rule(Relation, Ordinal), Reason)`, exit 1; a rule reading a failed relation gets `depends_on` (`_7_sqlite.rs:38-55`, `:106-114`, `:222-264`).

| reason (`_7_sqlite.rs:38-55`, `:228-263`) | fixture that shows it |
|---|---|
