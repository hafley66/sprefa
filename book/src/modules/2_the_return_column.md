...
The probe writes `(Wrap str)`, and `str` is the primitive. The label of `Holder`'s edge is `intern(Wrap, [primitive(str)])`:
...
(: Holder (Wrap primitive(str)) primitive(int))
```
...
step 2  input slots                      [0]                    str (the primitive) fills column 0
...
(: module(file(book/src/probes/20_full_application_bind.dl7)) WrappedText (Wrap primitive(str)))
```
