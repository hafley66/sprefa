...
On a reloaded db, a `Once` application with a matching data row is answered; an error row is not, so a restart retries it. A reloaded timer numbers past its stored ticks (`README.md:94-100`).
...
```dl7
; fixture: fixtures/store/1_continue_more.dl7
