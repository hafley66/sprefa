use super::super::{append_section_j, wire_repo_skills_j, write_starter_j,
    SetupJournal, STARTER_DL, STARTER_HOOK};
use super::*;
...
fn read_settings(dir: &Path) -> Value {
    serde_json::from_str(&std::fs::read_to_string(dir.join(".claude/settings.json")).unwrap()).unwrap()
}

fn post_cmds(value: &Value) -> Vec<String> {
    value["hooks"]["PostToolUse"].as_array().unwrap().iter()
        .flat_map(|entry| entry["hooks"].as_array().unwrap())
        .map(|hook| hook["command"].as_str().unwrap().to_string()).collect()
}
    ...
    fn setup_wiring_twice_produces_identical_tree() {
        ...
        let wire_all = |dir: &Path| {
            ...
            write_starter_j(&mut journal, dir, &dl_dir.join("dl-self-lint.dl"), STARTER_DL).unwrap();
            write_starter_j(&mut journal, dir, &dl_dir.join("hook-skill-on-test.dl"), STARTER_HOOK).unwrap();
