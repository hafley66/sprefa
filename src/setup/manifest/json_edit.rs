...
pub(super) fn append_array_value(text: &str, pointer: &str, value: &Value) -> Option<String> {
    ...
    loop {
        if let Some((start, end)) = pointer_span(&output, &keys) {
            if !matches!(output.as_bytes().get(start), Some(b'[')) { return None; }
            let close = end.checked_sub(1)?;
            let occupied = !output[start + 1..close].trim().is_empty();
            let insertion = format!("{}{}", if occupied { "," } else { "" }, serde_json::to_string(value).ok()?);
        ...
        for depth in 0..keys.len() {
            if pointer_span(&output, &keys[..=depth]).is_none() {
                let parent = if depth == 0 { root_span(&output)? } else { pointer_span(&output, &keys[..depth])? };
                if !matches!(output.as_bytes().get(parent.0), Some(b'{')) { return None; }
...
pub(super) fn remove_empty_member(text: &str, parent_pointer: &str, key: &str, empty: &str) -> Option<String> {
    let parent = if parent_pointer.is_empty() { root_span(text)? } else {
    ...
    let (index, member) = members.iter().enumerate().find(|(_, member)| member.key == key)?;
    if text[member.value_start..member.value_end].trim() != empty { return None; }
    let (delete_start, delete_end) = if index + 1 < members.len() {
        let comma = find_byte_forward(text, member.value_end, members[index + 1].member_start, b',')?;
        (member.member_start, comma + 1)
    } else if index > 0 {
        let comma = find_byte_backward(text, members[index - 1].value_end, member.member_start, b',')?;
        (comma, member.value_end)
    } else { (member.member_start, member.value_end) };
    let mut output = text.to_string();
...
fn remove_matching(text: &str, pointer: &str, matches: impl Fn(&Value) -> bool) -> Option<String> {
    ...
    let (index, &(item_start, item_end)) = items.iter().enumerate().find(|(_, (lo, hi))| {
        serde_json::from_str::<Value>(&text[*lo..*hi]).ok().is_some_and(|value| matches(&value))
    })?;
...
fn pointer_span(text: &str, keys: &[&str]) -> Option<(usize, usize)> {
    ...
    for key in keys { span = object_member(text, span, key)?; }
    Some(span)
}
...
fn object_member(text: &str, object: (usize, usize), wanted: &str) -> Option<(usize, usize)> {
    object_members(text, object)?.into_iter().find(|member| member.key == wanted)
        .map(|member| (member.value_start, member.value_end))
}

struct ObjectMember { key: String, member_start: usize, value_start: usize, value_end: usize }

fn object_members(text: &str, object: (usize, usize)) -> Option<Vec<ObjectMember>> {
    if !matches!(text.as_bytes().get(object.0), Some(b'{')) { return None; }
    ...
    while cursor < object.1 && text.as_bytes()[cursor] != b'}' {
        ...
        if text.as_bytes().get(cursor) != Some(&b':') { return None; }
        let value_start = skip_ws(text, cursor + 1);
        let value_end = scan_value(text, value_start)?;
        members.push(ObjectMember { key, member_start, value_start, value_end });
        cursor = skip_ws(text, value_end);
        match text.as_bytes().get(cursor) { Some(b',') => cursor = skip_ws(text, cursor + 1), Some(b'}') => break, _ => return None }
    }
    Some(members)
}

fn insert_object_member(text: &str, object: (usize, usize), key: &str, value: &str) -> Option<String> {
    let close = object.1.checked_sub(1)?;
    if text.as_bytes().get(close) != Some(&b'}') { return None; }
    let occupied = !text[object.0 + 1..close].trim().is_empty();
    let insertion = format!("{}{}:{value}", if occupied { "," } else { "" }, serde_json::to_string(key).ok()?);
    let mut output = text.to_string();
    output.insert_str(close, &insertion);
    Some(output)
}
...
fn array_items(text: &str, start: usize, end: usize) -> Option<Vec<(usize, usize)>> {
    if text.as_bytes().get(start) != Some(&b'[') { return None; }
...
fn scan_value(text: &str, start: usize) -> Option<usize> {
    match *text.as_bytes().get(start)? {
        ...
        b'{' | b'[' => {
            ...
            while cursor < text.len() {
                match text.as_bytes()[cursor] {
                    ...
                    byte if byte == open => { depth += 1; cursor += 1; }
                    byte if byte == close => { depth -= 1; cursor += 1; if depth == 0 { return Some(cursor); } }
                    _ => cursor += 1,
                }
            }
            None
        ...
        _ => {
            ...
            while cursor < text.len() && !matches!(text.as_bytes()[cursor], b',' | b']' | b'}' | b' ' | b'\n' | b'\r' | b'\t') { cursor += 1; }
...
fn skip_ws(text: &str, mut cursor: usize) -> usize {
    while cursor < text.len() && text.as_bytes()[cursor].is_ascii_whitespace() { cursor += 1; }
    cursor
}
...
fn find_byte_backward(text: &str, start: usize, end: usize, wanted: u8) -> Option<usize> {
    (start..end).rev().find(|&index| text.as_bytes()[index] == wanted)
}
