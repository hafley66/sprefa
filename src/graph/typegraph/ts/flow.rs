    ...
    match stmt {
        S::FunctionDeclaration(f) => {
            if let Some(body) = f.body.as_deref() {
                ...
                let mut scope: std::collections::HashMap<String, NodeIdx> =
...
fn ts_flow_body_stmt(
    ...
    match stmt {
        S::VariableDeclaration(v) => {
            for d in &v.declarations {
                ...
                let rhs_id = d
                    .init
                    .as_ref()
                    .map(|init| ts_flow_expr(init, file, starts, fn_sym, scope, out));
        ...
        S::ForStatement(f) => {
            if let Some(ts_ast::ForStatementInit::VariableDeclaration(v)) = &f.init {
                for d in &v.declarations {
                    ...
                    if let Some(name) = ts_binding_name(&d.id) {
                        let bind =
                            ts_push(out, file, starts, d.span.start, "let_bind", &name, fn_sym);
                        if let Some(rhs) = rhs_id {
                            out.edges.push(DfEdge {
                                from: rhs,
                                to: bind.clone(),
            ...
            if let Some(test) = &f.test {
                let _ = ts_flow_expr(test, file, starts, fn_sym, scope, out);
            }
            if let Some(upd) = &f.update {
                let _ = ts_flow_expr(upd, file, starts, fn_sym, scope, out);
            }
        ...
        S::ForOfStatement(f) => ts_for_in_of(
            &f.left,
            &f.right,
            &f.body,
            f.span.start,
        ...
        S::ForInStatement(f) => ts_for_in_of(
            &f.left,
            &f.right,
            &f.body,
            f.span.start,
...
fn ts_for_in_of(
    ...
    let var = match left {
        ts_ast::ForStatementLeft::VariableDeclaration(v) => v
            .declarations
            .first()
            .and_then(|d| {
...
fn ts_flow_call(
    ...
    let recv = match &c.callee {
        E::StaticMemberExpression(m) => {
            Some(ts_flow_expr(&m.object, file, starts, fn_sym, scope, out))
        }
...
fn ts_flow_expr(
    ...
    match e {
        ...
        E::StaticMemberExpression(m) => ts_flow_member(
            &m.object,
            &m.property.name,
            off,
            file,
        ...
        E::ComputedMemberExpression(m) => {
            ts_flow_member(&m.object, "", off, file, starts, fn_sym, scope, out)
        }
        ...
        E::BinaryExpression(b) if b.operator == ts_ast::BinaryOperator::Addition => {
            ...
            out.lit_spans
                .push((id.clone(), b.span.start, b.span.end, "concat"));
            id
        }
        ...
        E::ParenthesizedExpression(p) => {
            ts_flow_expr(&p.expression, file, starts, fn_sym, scope, out)
        }
        ...
        E::TSSatisfiesExpression(t) => {
            ts_flow_expr(&t.expression, file, starts, fn_sym, scope, out)
        }
        ...
        E::ChainExpression(ch) => {
            ...
            match &ch.expression {
                ...
                other => match other.member_expression() {
                    Some(ME::StaticMemberExpression(m)) => ts_flow_member(
                        &m.object,
                        &m.property.name,
                        off,
                        file,
                    ...
                    Some(ME::ComputedMemberExpression(m)) => {
                        ts_flow_member(&m.object, "", off, file, starts, fn_sym, scope, out)
                    }
                    Some(ME::PrivateFieldExpression(m)) => {
                        ts_flow_member(&m.object, "", off, file, starts, fn_sym, scope, out)
                    }
                    None => ts_push(out, file, starts, off, "expr", "", fn_sym),
                },
        ...
        E::TemplateLiteral(t) => {
            ...
            out.lit_spans
                .push((id.clone(), t.span.start, t.span.end, "template"));
            id
        }
        ...
        E::TaggedTemplateExpression(t) => {
            ...
            out.lit_spans
                .push((id.clone(), t.quasi.span.start, t.quasi.span.end, "template"));
            id
        }
    ...
    for ch in children {
        match ch {
            ts_ast::JSXChild::Element(el) => {
                filled.push((
                    "children".into(),
                    ts_flow_jsx_element(el, file, starts, fn_sym, scope, out),
                ));
            }
            ...
            ts_ast::JSXChild::ExpressionContainer(c) => {
                if let Some(e) = c.expression.as_expression() {
                    filled.push((
                        "children".into(),
                        ts_flow_expr(e, file, starts, fn_sym, scope, out),
                    ));
                }
            }
            ts_ast::JSXChild::Spread(sp) => {
                filled.push((
                    "..".into(),
                    ts_flow_expr(&sp.expression, file, starts, fn_sym, scope, out),
                ));
            }
...
mod tests {
    ...
    fn tsx_lift_jsx_elements_props_children() {
        ...
        let t_reads: Vec<&DfNode> = df
            .nodes
            .iter()
            .filter(|n| n.kind == "var_read" && n.var == "t")
            .collect();
        assert!(
            t_reads
                .iter()
                .any(|t| has_field(&df, &card, "title", &t.id)),
            "{:?}",
            df.fields
        );
        // bare boolean prop fills with a lit; spread lands under "..".
        assert!(
            df.fields.iter().any(|(i, f, _)| i == &card && f == "flag"),
            "{:?}",
            df.fields
        );
        assert!(
            df.fields.iter().any(|(i, f, _)| i == &card && f == ".."),
            "{:?}",
            df.fields
        );
        ...
        let frag = df
            .nodes
            .iter()
            .find(|n| n.kind == "new" && n.var.is_empty())
            .expect("fragment new node");
        assert!(
            df.fields
                .iter()
                .any(|(i, f, _)| i == &frag.id && f == "children"),
            "{:?}",
            df.fields
        );
    }
    ...
    fn ts_destructured_params_bind_by_prop_name() {
        ...
        let read_of = |v: &str| {
            df.nodes
                .iter()
                .find(|n| n.kind == "var_read" && n.var == v)
                .unwrap()
                .id
                .clone()
        };
        assert!(
            df.edges
                .iter()
                .any(|e| e.from == title && e.to == read_of("title")),
            "{:?}",
            df.edges
        );
        assert!(
            df.edges
                .iter()
                .any(|e| e.from == count && e.to == read_of("n")),
            "{:?}",
            df.edges
        );
        assert!(
            df.edges
                .iter()
                .any(|e| e.from == plain && e.to == read_of("plain")),
            "{:?}",
            df.edges
        );
    ...
    fn ts_lift_new_object_literal_and_members() {
        ...
        let h_reads: Vec<&DfNode> = df
            .nodes
            .iter()
            .filter(|n| n.kind == "var_read" && n.var == "h")
            .collect();
        assert!(
            h_reads.iter().any(|h| has_arg(&df, &widget, 0, &h.id)),
            "{:?}",
            df.args
        );
        ...
        let obj = df
            ...
            .iter()
            .find(|n| n.kind == "new" && n.var.is_empty())
            .expect("object literal new node");
        assert!(
            h_reads
                .iter()
                .any(|h| has_field(&df, &obj.id, "host", &h.id)),
            "{:?}",
            df.fields
        );
        assert!(
        ...
        assert!(
            df.args.iter().any(|(_, p, a)| *p == -1
                && df
                    .nodes
                    .iter()
                    .any(|n| &n.id == a && n.kind == "var_read" && n.var == "x")),
            "{:?}",
            df.args
        );
    }
    ...
    fn ts_df_lit_carries_cooked_string_and_template_holes() {
        ...
        assert!(
            df.lits
                .iter()
                .any(|(_, text, kind)| text == "plain" && *kind == "lit"),
            "{:?}",
            df.lits
        );
        assert!(
            df.lits
                .iter()
                .any(|(_, text, kind)| text == "`hi ${name}`" && *kind == "template"),
            "{:?}",
            df.lits
        );
    ...
    fn ts_concat_binop_mints_own_kind_and_edges_both_operands() {
        ...
        assert!(
            df.edges
                .iter()
                .any(|e| e.from == lit.id && e.to == concat.id),
            "{:?}",
            df.edges
        );
        ...
        assert!(
            df.lits.iter().any(|(id, text, kind)| id == &concat.id
                && text == "base + '/x'"
                && *kind == "concat"),
            "{:?}",
            df.lits
        ...
        assert!(
            other.nodes.iter().any(|n| n.kind == "binop"),
            "{:?}",
            other.nodes
        );
        assert!(
            !other.nodes.iter().any(|n| n.kind == "concat"),
            "{:?}",
            other.nodes
        );
    }
