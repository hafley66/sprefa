...
pub enum Tok {
    ...
    Star,
    Slash,
    Percent, // int arithmetic (heads + comparisons)
}
...
fn lex_scheme_body(b: &[u8], src: &str, i: &mut usize) -> Result<String> {
    if b.get(*i) == Some(&b'`') {
        ...
        *i += 1;
        ...
        if *i >= b.len() {
            bail!("unterminated fenced scheme literal");
        }
    ...
    if *i == start {
        bail!("empty scheme literal body");
    }
    ...
    while i < b.len() {
        ...
        match c {
            ...
            b'`' => {
                ...
                if i >= b.len() {
                    bail!("unterminated backtick string");
                }
                let body = src[start..i].to_string();
                i += 1;
            ...
            b'~' => {
                if b.get(i + 1) == Some(&b'~') {
                    ...
                    bail!("lone '~' (use '~~' for glob, '=~' for regex)");
                }
            }
            b'!' => {
                if b.get(i + 1) == Some(&b'=') {
            ...
            b'@' => {
                ...
                if i == start {
                    bail!("lone '@' (expected @next/@async after a rule neck)");
                }
            ...
            _ if c == b'_' || c.is_ascii_alphabetic() => {
                ...
                if word == "r" && i < b.len() && (b[i] == b'"' || b[i] == b'`' || b[i] == b'#') {
                    ...
                    let close = find_raw_close(b, i, delim, hashes).ok_or_else(|| {
                        anyhow::anyhow!(
