...
- **RCA**: code, not load. `git bisect run` over `4a0b91362..15e95de83` limited to `v6/sprefa-extract` (nine builds, one measurement each) names `7bfc8d4a4` (#567, go multi-hop receiver chains) as the first bad commit. Its `go_chain_of` walks every call site's selector chain during the per-file pass and allocates a `String` per chain step (`go.rs`, the `GoChainStep::Field/Call(... .to_string())` pushes), so a fixture that is nothing but call sites pays it 350k times with no `--resolve` asked for. Entry 104's rail (control on main) was applied by both lanes and gave the wrong answer, because main itself was red: a control run proves environment only when the control is a sha known to pass, not the tip.
- **Fail-pre-fix**: `cargo test -q --features cli --test 45_emit_throughput` at `15e95de83`: `piped emission took 5.855 s / 5.848 s / 5.805 s for 350005 lines`, rc=101 all three; at `4a0b91362`: `test result: ok` 3/3. Same machine, load averages 4.25 to 5.50 throughout. A4 (`b9ecef38a`) adds a further 0.1 s (5.96 / 5.95 / 6.02).
- **Rail**: the control run for a perf-budget failure is the budget's OWN `measured_at` commit (the commit that last touched the test, `git log -1 -- <test>`), never the tip of main. Coordinator duty: a perf-budget failure that reproduces at the control sha is environmental; one that does not is bisected before any PR merges on "accuracy receipts". The fix arc (`plans/2026-09-03-emit-throughput-fix.BRIEF.md`) also builds entry 104's unbuilt code rail: the assert reads the 1-minute load average and skips with a named note above a threshold.
- **Entry**: ARCH `extract_emit_throughput_budget` row carries the bisect; fix lane dispatched.
