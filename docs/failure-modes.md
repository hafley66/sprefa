...
## 93. A bare source filename sent an empty path to `canonicalize`

- **Incident** (2026-08-24): a resident `dl6 run` relaunch crashed in `point_at_adapters` (`v6/sprefa-engine-rs/src/bin/dl6.rs`) with `Error: read the adapters directory \n\nCaused by: No such file or directory (os error 2)`. Reproduction: `cd v6/dl/ghcache && dl6 run ghcache.dl6`, the source named as a bare filename with no directory.
- **RCA**: `Path::new("ghcache.dl6").parent()` returns `Some("")`, not `None`. The `unwrap_or_else(|| PathBuf::from("."))` arm only fires on `None`, so the empty path reached `directory.canonicalize()`, and `"".canonicalize()` fails with `os error 2`. The same shape hits `args.adapters` when the sidecar is given as a bare filename.
- **Fail-pre-fix**: `a_bare_source_filename_resolves_its_adapters_directory_to_the_cwd` copies `tests/fixtures/query_order_tail.dl6` into a tempdir and runs `dl6 run query_order_tail.dl6` with `current_dir` on that tempdir. Against HEAD it exits `Some(1)` with the same `read the adapters directory \n\nCaused by: No such file or directory (os error 2)`; on this branch it exits 0 and its stderr is free of that text.
- **Rail**: `point_at_adapters` computes the candidate directory first, then maps an empty path (a bare filename's `parent()`) to `.` before the `canonicalize`. The `DL_ADAPTERS_DIR` env var that the sidecar loader reads then points at the current directory, exactly as a bare source in the cwd implies.
- **Entry**: `dl6 run` accepts a bare source filename and resolves its adapters directory to the process cwd. Gates: the new test green pre and post fix, engine battery `cargo test --release` all ok, RUST-GRADE graded=445 byte-clean=341.
