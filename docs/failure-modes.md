...
- **Rail**: `evaluate_into` computes `demanded` once: a relation lands there only when a non-aggregate rule of its own has a kernel or negative goal whose arguments mention a head variable no earlier positive non-kernel goal binds (`needs_bound_head`). `demand` fires only for those relations. The memo is one `Memo` per round owned by the round loop and cleared with the round's inserts, so a firing reuses the round's proofs; an entry whose proof hit an in-progress key is dropped (`cycle_hits`) because a re-entrant key reads partial rows and only the cycle target's own entry is reproducible. `Trace::Demand { calls }` per round prices the whole mechanism.
