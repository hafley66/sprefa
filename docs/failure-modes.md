...
## 90. A library was picked on its data model, and nobody had compiled it

- **Incident** (2026-08-23): the `dd_oracle_crosscheck` brief named `dbsp` (Feldera) the expected winner over `differential-dataflow` on one axis, that dbsp's `step()`-per-batch model maps 1:1 onto our tick. It does. dbsp also fails to build here. `cargo build` of a crate whose only content is `RootCircuit::build(|_c| Ok(()))` ICEs `rustc 1.97.0-nightly (9eb3be26b 2026-05-18)` at `rustc_next_trait_solver/src/solve/assembly/structural_traits.rs:1005`, query `instantiate_and_check_impossible_predicates` on `operator::dynamic::multijoin::star_join::StarJoinFuncTrait`, reproduced on two independent runs, 109 s wall to the panic. That nightly is what `RUSTUP_TOOLCHAIN` pins for this repo, so `cargo test` in `sprefa-engine-rs` would have stopped compiling the moment dbsp entered `[dev-dependencies]`.
- **RCA**: the shortlist was drawn on semantics (retraction spelling, operator availability, batch-per-tick fit) and the two candidates tie on all three. dbsp has `join`, `semijoin`, `antijoin` (join.rs:374), `distinct`, `aggregate`, `average`, `count`, `recursive`, `consolidate`, `neg`; dd has `join_map`, `semijoin`, `antijoin`, `distinct`, `threshold`, `reduce`, `count`, `iterate`, `consolidate`, `negate`. A semantics-only comparison cannot separate them, and the axes that do separate them are all mechanical: dbsp is edition 2024 with MSRV 1.93.1, 65 direct runtime dependencies, and 223 new packages in this lock; dd is edition 2021 with MSRV 1.86, 7 direct dependencies, and 15 new packages. dbsp builds clean on `stable 1.95.0` in 160 s, so the failure is this toolchain and not the crate, which is exactly the kind of fact no amount of reading the docs produces.
- **Fail-pre-fix**: two throwaway crates, one per candidate, each with a probe exercising two-way join, antijoin, a `reduce` computing a float mean, a transitive-closure fixpoint, and a retraction tick. The dd probe printed the per-tick signed stream (`t0 +closure a,b` … `t1 -unused dead,unused` … `t2 +closure c,d`) and compiled in 15 s cold. The dbsp probe never got past its own dependency.
- **Rail**: half. The candidate table is in the PR body and the losing receipt is pinned at `v6/sprefa-engine-rs/Cargo.toml` beside the winning dependency, so the next agent reaching for dbsp reads why before adding it. Nothing enforces "compile the candidate before writing the comparison"; what enforces the outcome is that `cargo test` runs the crosscheck, and a dependency that does not build takes the whole crate's test target with it. The build-vs-buy law gains one clause in practice: a candidate table without a compile receipt per row is not research, it is reading.
- **Entry**: `differential-dataflow 0.25` and `timely 0.31` are `[dev-dependencies]` of `sprefa-engine-rs`; the engine's own build is unchanged and the lock went 338 -> 353 packages. `tests/dd_oracle_crosscheck.rs` hand-builds one circuit per panel program and grades dd's per-tick output batch against `conformance/dd_panel.json`: 4 passed, 0 failed, 10 of 10 programs agreeing at every tick and accumulating to the fixture finals. The comparator is pinned against reading row identity alone by `sabotage_flipped_sign_is_caught`, which negates the `mirror` head and asserts the verbatim diff `dd only : -mirror(alpha) x2, -mirror(beta) x2`; non-vacuity of the join arm was checked by hand by dropping `Caller \== Callee`, which reads `tick 2: dd only +calls(helper,helper) x1`. Gates: conformance 445/0, plunit 1,088/0, RUST-GRADE 445 graded with 341 byte-clean, cargo 176/0, ghcache ticks=14, Rust goldens 6, ARCH 7/0.

## 91. A poll asked an org-only endpoint of a user account, and its 404 fallback was statically dead

- **Incident** (2026-08-24): over 1,427 minute buckets (23.8 h) in `~/.agent/dl6.db`, `users/hafley66/events/orgs/hafley66` answered 404 1,422 times, one per poll cycle, authenticated, decrementing the REST pool. hafley66 is a USER account and that endpoint exists for orgs only. `orgs/hafley66/repos?per_page=100` added 24 more 404s on the same shape, and `ghcache_not_an_org` and `ghcache_discovered_repo` were both EMPTY, so the org-to-user repos fallback the README claimed had never once fired. Nothing backed a permanently-404 endpoint off: 404, re-demand next bucket, forever. Three `log keep(all)` telemetry rels had grown to 132,571 (`engine_tick_cost`), 31,811 (`change_log`) and 15,280 (`call_log`) rows.
- **RCA**: three independent causes. `watched_global(_, 'rest', 'org_events')` had one arm and it spelled the org form; `watched_endpoint`'s repos poll had both spellings, so the events poll was the only endpoint family with no user-account form. `not_an_org` read its 404 off `rest_response`, which carries a 200 arm and a 304 arm and no other, so the rule could never bind: a compiled, reachable, permanently unsatisfiable body. And no rel anywhere held a per-endpoint failure count, so a 404 carried no state into the next bucket's `due`.
- **Fail-pre-fix**: `ghcache.account.schedule.json`, five minute buckets over one `[[org]]` owner whose `/orgs` repos call 404s and one configured repo whose events call 404s every bucket. Folded against the HEAD program it reads `due` rows `users/hafley66/events/orgs/hafley66` 5, `repos/hafley66/ghost/events` 5, `users/hafley66/repos?per_page=100` 0. Folded against this branch: 0, 0 and 1, with `users/hafley66/events` answering 200 seven times. The gate leg asserts all seven counts.
- **Rail**: the two account planes are exclusive arms on `not_an_org`, which is now sourced off `endpoint_miss`, the backoff's own 404 rel, so the dead read cannot come back without taking the backoff with it. `miss_streak(endpoint_path, misses, at_bucket) key(1)` counts consecutive 404s on page 1 with two arms exclusive on `at_bucket`, the `page_prev_etag` shape, so a bucket's drain ticks cannot run the counter up; a non-404 answer resets it to 0; `endpoint_cooling` anti-joins into `due` for `miss_cooloff(60)` buckets past the last miss once `miss_threshold(3)` is reached. Retention is `keep(count(N))` on the three telemetry logs, and the three `call_log` edge arms folded into one `call_candidate` level rel first because `retention_head_conflict_risk` (`0_program_check.pl:666`) refuses two or more `<+` arms on a bounded log head.
- **Entry**: `call_log` keeps 16,500 rows (10.71/bucket measured, 25.7 h), `change_log` 34,000 (22.29/bucket, 25.4 h), `engine_tick_cost` 140,000 (93.23/bucket over 243 executors, 25.0 h); `pr_transition` stays `keep(all)`. The prune `DELETE ... WHERE rowid NOT IN (SELECT rowid ... LIMIT 140000) RETURNING` measures 59 ms at 150,093 rows deleting 93, against a ~13 s tick interval. The org-events 404 stream falls from 1,422 a day to 0 and the org-repos one from 24 to 1; the working `users/<owner>/repos` fallback is the one leg that ADDS traffic, 24 discovery calls a day plus 2,880 a day per repo it discovers, with `org_exclude` the config lever. `ghcache.schedule.json` needed zero edits. The cost is priced, not hidden: the 14-tick fold went 6,738 -> 6,955 statements and `recount` 3,244 -> 3,358, all of it `call_candidate`'s 9 delta rows, so `FOLD_STATEMENT_CAP` goes 6,800 -> 7,050 and `RECOUNT_STATEMENT_CAP` 3,300 -> 3,400 in `tests/ordered_statement_count.rs`. The first draft cost 7,241 because `endpoint_hit` fired on every successful answer; guarding it on a stored non-zero streak took 286 statements back and left the whole backoff plane deriving NOTHING on a healthy schedule. `tests/fixtures/ghcache_ticklog_base.txt` moves by exactly three rel names, `call_candidate` 9 rows and (before the guard) `endpoint_hit` 5 and `miss_streak` 7; with the guard the only delta is `call_candidate`, and the other 84 rels are byte-identical. New SHA-256 `bd559220096a65170fd3f1043ccf891cccb9a290a342567536d3c74b306a737f`. Gates: ghcache ticks=14 with `pr_transition_open_merged=1` and account_ticks=14, conformance 445/0, plunit 1,114/0, RUST-GRADE graded=445 byte-clean=341, cargo 176/0, Rust goldens 6, ARCH 7/0.

## 92. An aggregate re-derived a group from a base table whose retraction had not landed

- **Incident** (2026-08-24): a resident `dl6 run` restarted against the existing `~/.agent/dl6.db` panicked in its first tick at `incremental.rs:1828`, `aggregate scope batch failed: SqliteFailure(.., Some("malformed JSON"))`. A fresh db never crashed and the 14-tick gate was green. The panic named neither the head rel nor the statement, so the class was a hunt rather than a read. The stored db held zero json-invalid and zero empty bodies: the poison value did not exist on disk, it materialized mid-tick.
- **RCA**: two facts compose. `lower.pl:6480` spells a `json_object` duplicate key as `ELSE json('json_object_dup_key')`, deliberately invalid JSON, so SQLite raises and the rejection travels as a statement failure. And `apply_levels_before_edges` re-derives an aggregate's affected groups from the FULL base table while the retractions this tick staged in `apply_arrivals` have not been applied: `recompute_levels_after_edges` is where they land. A restart guarantees the collision. The host response row for `clock__tick` is replaced in place, the keyed-arrival diff stages the -1, and until the recount pass runs, `ghcache_current_clock` holds the previous process's bucket 29792977 beside the fresh 29792993. `page_fetch` then carries both buckets, `page_prev_etag`'s two arms are no longer exclusive, `request_header` derives `If-None-Match` twice for `repos/hafley66/sprefa/events` (`"196f0002..."` from the `asked_etag` arm, `W/"b2e8d748..."` from the `etag` arm), and `request_headers` rejects the group. Every aggregate computes a provisional value in that pass; `json_object` is the only one whose provisional value is fatal instead of merely wrong.
- **Fail-pre-fix**: `tests/fixtures/aggregate_restart_scope.dl6` is the shape in 20 lines, `clock` and `poll` keyed on 1 with the two `page_prev_etag` arms and a `json_object` head. `a_restart_tick_defers_a_two_valued_aggregate_group_to_the_recount_pass` folds `clock("now", 1)` to a file db, then runs the binary again against that same file with `clock("now", 2)`. Against HEAD the second run panics at `incremental.rs:1828` with the same "malformed JSON"; on this branch it reads `{"If-None-Match":"E"}` and `v_headers` holds one row. The live reproduction is the same two steps against a copy of the resident db.
- **Rail**: `apply_aggregate_level_statement` takes a `settled` flag. The two additive callers pass false and a statement failure carrying `json_object_dup_key` defers: the head is recorded in `TickWork::deferred_aggregates` and the statement abandoned. If the DELETE already ran, its rows still stage as -1, so the deferral is a retract-then-derive and never a +1 with no -1. `recompute_levels_after_edges` passes true, runs a deferred head whatever the recount gate and the `delta_maintained` early-out say, and runs at all when anything was deferred even if the retraction guard reads 0. A duplicate key with no retraction behind it therefore still fails the tick, at the settled pass, and a genuine one cannot be swallowed. Every other seam failure in the aggregate path became `BoundaryError::AggregateStatementFailed`, carrying the head rel, the phase name, the index inside the phase, and the first 200 chars of the SQL: the `.expect` calls that named none of them are gone.
- **Entry**: the live reproduction runs clean past its first resident tick and settles to one `current_clock` row per period, `page_fetch` at one bucket, and nine `request_headers` rows. `SqlRunner::batch` was a map over `execute`, so running the aggregate phases one statement at a time to name the failing index changed no ordering and no visibility. Gates: ghcache ticks=14 account_ticks=14 with `pr_transition_open_merged=1` and `due=4 200=5 304=2 304_bytes=0`, Rust goldens 6, ARCH 7/0, plunit 1,115/0.

## 93. A cross-product rule ran a statement for five minutes and nothing in the engine could stop it

- **Incident** (2026-08-24): the first draft of `v6/dl/prolog_rehome/rehome.dl6` ordered paths lexically with a hand-rolled char table: `differ_at(A, B, P) <- position(P), char_at(A, P, CA), char_at(B, P, CB), CA \== CB` and `earlier_diff(A, B, P) <- position(P), position(Q), Q < P, differ_at(A, B, Q)`. Over 187 source files and 40 positions that is a 187 x 187 x 40 x 40 join, about 56 M row visits in one level insert. `dl6 run` (debug) sat at 100% CPU for 4 min 49 s with zero output; `sample` put every frame under `sqlite3VdbeExec`/`sqlite3BtreeNext`. The lane wrapped the run in `timeout 3000`, slept in `sleep 180; ps -p` loops, killed and reran it three times, and never read the rule. The coordinator SIGKILLed the process by pid.
- **RCA**: the seam had no stop valve. `SqliteSeam` measured every statement (`trace::record`, `SEAM_TALLY`) after it returned and could do nothing while it ran; the only clock on a connection was `busy_timeout`, which bounds lock waits and never a scan. The 10-second law lived in prose and in agents' patience, and an agent's patience is exactly the wrong place for it: a lane will wait out any budget it is told to set.
- **Fail-pre-fix**: `tests/statement_budget.rs`, an unbounded `WITH RECURSIVE spin(n) AS (SELECT 1 UNION ALL SELECT n + 1 FROM spin) SELECT count(*) FROM spin` through `scalar` and through `execute`. Against HEAD both spin until killed. On this branch, with the budget set to 200 ms, both return `SqliteFailure(OperationInterrupted)` inside 5 s carrying `statement exceeded its 200 ms budget (DL_STATEMENT_BUDGET_MS) at <relation>/<verb>: <statement shape>`; a 50,000-row bounded CTE under the same budget returns 50,000 untouched, and a fresh seam reads a 10 s default.
- **Rail**: `sql.rs` installs `Connection::progress_handler` (rusqlite `hooks` feature, checked every 10,000 VM ops) on every seam connection at `wrap`. `arm_statement` writes a deadline into a shared `AtomicU64` at each of the four seam entry points (`execute`, `execute_multiple`, `scalar`, `table_shape_stands`); the handler returns true past it and SQLite aborts the statement with `SQLITE_INTERRUPT`. `budget_error` rewrites that one error to name the budget, the env override, the trace label and the statement shape, and passes every other error through. Default `DEFAULT_STATEMENT_BUDGET` is 10 s; `DL_STATEMENT_BUDGET_MS` moves it, `set_statement_budget` moves it in a test. Not covered: `source_bind/_1_runtime.rs:369`'s `ReceiptStore` connection, which runs keyed receipt writes only.
- **Entry**: the same rule shape is deleted from `rehome.dl6` (ordinal = dense rank of dependency depth, equal depth shares the number); no text ordering enters the language. Gate numbers in the PR body.

## 94. A seed re-escaped the quote it read back out of the SQL, and an id compare matched two absent rows

- **Incident** (2026-08-24): `v6/dl/prolog_graph/import_graph.dl6` spelled its quoted-atom test as `First := substr(Raw, 1, 1), First == "'"`. The arm fired for every bare atom, so `unquoted(analyze)` derived the target `nalyz`, and the `\==` arm derived nothing at all. Over `v6/prolog/**/*.pl` from `compile.pl` the program reads `reaches=29 unresolved=27` where the same program with both compares rewritten to integer `instr(...)` positions reads `reaches=30 unresolved=1`. The author took the `instr` workaround and filed the compare as the defect.
- **RCA**: two defects compose, and neither is visible alone. `atom_interned_literal/2` reads the literal seed list back out of the emitted SQL and terminated the content at the first `')`, on the claim in its own comment that `sql_literal/2` refuses a quote inside a literal. It does not: `sql_text_literal/2` DOUBLES every quote, so `'` is written `''''`, the reader took `''` back out, and `sql_literal/2` re-escaped that into a seed storing two characters. The one character the rule spells was never interned. Then `compile_comparison/4` aligned the computed text UP into id space through `aligned_pair/6` and `interned_id_sql/2`, so both operands of `==` were `(SELECT "__id" FROM "__str" WHERE "content" = ...)`, `==` lowers to `IS`, and `NULL IS NULL` is TRUE. Only a stored column's id is total. The literal's id was total only because of the seed, and the substring's only if some row happened to hold those characters, which is why `16_string_affix_tests.pl` passes: its affixes are stated as facts.
- **Fail-pre-fix**: `conformance/fixtures/text_identity_literal.pl`, four fixtures over `raw('abc')`, `plain`, `library(x)` arriving across two ticks. Against HEAD the Rust door adds `quoted(plain)` and `quoted(library(x))` the oracle never derives, derives no `unquoted` row where the oracle derives both, and drops `marked('abc', ''')` entirely because the head literal's id is NULL. The first draft of the same four fixtures carried an EMPTY schedule and graded byte-clean against HEAD: a fixture that arrives nothing writes an empty tick log, and the Rust door then compares two empty files without running a rule. Three plunit tests pin the emitted text: the character compare, the id compare two stored columns keep, and the seed statement.
- **Rail**: `content_comparison/6` sends both operands of a text `==`/`\==` to `value` demand the moment one of them is outside the dictionary, which is any encoding mismatch and the literal-against-literal pair. Two stored columns keep the id compare, both ids being total, and `both_number` is untouched: no text ordering enters the language. `quoted_body_codes/2` decodes the SQL string body by halving the doubled quotes and stopping at the first quote no second quote follows, so the seed stores what the rule spells. A guard-only literal now needs no seed at all and none is emitted for one.
- **Entry**: the same `import_graph.dl6` with both `instr` workarounds reverted to `substr`-and-compare reads `reaches=30 unresolved=1`, the `reaches` set byte-identical to the `instr` form on the same tree and root. Conformance 449/0 three runs, plunit 1,125/0, RUST-GRADE graded=449 byte-clean=345 three runs, ghcache GHCACHE_RUST_DOOR_HOLDS ticks=14 account_ticks=14. `graded.tsv` is regenerated: the four new fixtures plus seven `nested_*` rows a697649e0 renamed without regenerating the ratchet.

## 95. A dependency turned `serde_json/preserve_order` on and the tick canonicalizer trusted `Map` order

- **Incident** (2026-08-27): `bash v6/sprefa-engine-rs/grade.sh` had printed `RUST-GRADE REGRESSION` with 23 lost fixtures (`relation_depth2_*`, `struct_*`, `json_patch_fold_rfc7396_clauses`, ...) on every run since 2026-08-25, `byte-clean=322` against a `graded.tsv` ratchet of 345. Three lanes reported "322 before and after, unchanged" and the red was normalised as baseline. Bisect (`plans/2026-08-28-grade-ratchet-bisect.md`, PR #513): single-step drop at `ecf8ec5f7` (#481), a PR that touched no engine file.
- **RCA**: #481 added `oxc_resolver = "11.24"` to `v6/sprefa-extract/Cargo.toml` with default features, which enable `serde_json/preserve_order`; `sprefa-engine-rs` depends on `sprefa-extract`, so feature unification flipped `serde_json::Map` from BTreeMap to IndexMap for the engine too. `ticklog.rs:canonical_json_value` iterated the map in its own order on the comment "serde_json's Object is sorted when preserve_order is off". Every fixture whose JSON values carry more than one key then rendered in insertion order and `diff_cause.py` filed the byte diff as `number-text` (equal as parsed JSON, unequal as bytes), which hid the key-order cause. The extract side of the same PR had already met this hazard and sorted through a BTreeMap; the engine side did not.
- **Fail-pre-fix**: `ticklog.rs` `object_keys_render_sorted_regardless_of_insertion_order`: a literal with keys in non-sorted order must render sorted at every nesting depth. Under the current build graph it fails before the fix.
- **Rail**: `canonical_json_value` collects object entries into a `BTreeMap` before rendering; the canonical text no longer depends on any crate's feature choice. RUST-GRADE `graded=449 byte-clean=345`, rc=0, no REGRESSION line.
- **Entry**: a leg that is red "before and after" is not a baseline; the ratchet file is the baseline, and a lost list that no PR explains gets bisected, never carried.

## 96. A corpus run split across processes lost every cross-partition fact

- **Incident** (2026-08-29): the Fixes 3 receipt for the typescript-go crawl (PR #559) ran `xargs timeout 10 $BIN --resolve < gofiles.txt` (`plans/extract-crawl-2026-08-29/go.REPORT.md:352`). macOS xargs split the 5,096-path list into 6 extract processes, so the "single process" note on the receipt was false: the numbers were the batched shape (`go.REPORT.md:462-463`, 1 process 14,173 `resolved_import` / 4,832 reachable vs 6 processes 9,410 / 4,580). The call tuple count (60,251 unique 4-tuples) reproduced exactly under `xargs` batching (`go.REPORT.md:465-466`).
- **RCA**: every index in `sprefa-extract` is per-process. A file partition loses every fact whose target sits in another process (`go.REPORT.md:456-457`, `go.REPORT.md:470-476`): `go_modules.rs` `bindings()` emitted a `resolved_import` row only when the target dir carried a corpus file in that process; a `_test.go` sibling that arrived first took `package_of_dir` first-file-wins; and the import-qualified call leg, when it declined because the target files sat outside the invocation, emitted nothing instead of falling through (`go.REPORT.md:473-476`).
- **Fail-pre-fix**: PR #560's fail-first tests in `v6/sprefa-extract/tests/62_go_module_plane.rs` against the twin `package debug` fixture: `an_import_emits_its_row_without_the_target_package_in_the_run` (`62_go_module_plane.rs:383`, red at 0 rows from a caller-only partition run), `a_test_sibling_never_poisons_the_dir_package_name` (:436), `an_import_qualified_site_falls_back_to_the_name_match` (:456), `two_packages_sharing_a_name_bind_by_directory_not_by_name` (:405).
- **Rail**: a `resolved_import` row is emitted for every in-module import path through `go_package_dir` alone, independent of which files the invocation carries; a `foo_test` package clause never takes a directory's binding from the primary `foo`; the import leg falls through to `call_name_match` on an exported name. The receipt command for the corpus is a single `--resolve` invocation (`go.REPORT.md:551-553`).
- **Entry**: import recall 84.11% (1,810/2,152, 342 oracle-only, PR #559) -> 100% (2,152/2,152), reachable (104 roots) 4,580 -> 4,833 (PR #560 body, `go.REPORT.md:488-489`, `go.REPORT.md:548`, `go.REPORT.md:650`). Gate 498 passed / 0 failed, 93 suites (PR #560 body).

## 97. `own_blob` picked the owning file by HashMap iteration order

- **Incident** (2026-08-29, PR #555): `own_blob` scanned `DefIndex.map` (a `HashMap`) for the first `DefSite` matching one of the output's named spans, so two files sharing one `(start,end)` span pair made the owning blob depend on hash order. On the typescript-go corpus (5,097 files) five back-to-back `extract --resolve` runs read `resolved_edge` 86,070 four times and 86,061 once (PR #555 body).
- **RCA**: "first match in a HashMap's value iteration" is not a key; the function needed an identity it cannot derive from span evidence alone. The fix rides the seam: `ProjectCx` carries `own: RefCell<Option<ContentId>>` (`v6/sprefa-extract/src/types.rs:1521`), `resolve_project` sets it per file, and `own_blob` returns it outright when set (`v6/sprefa-extract/src/types.rs:1743-1746`), because the caller knows which output it handed in and any span search is a guess (comment at `types.rs:1735-1737`).
- **Fail-pre-fix**: PR #555 added `v6/sprefa-extract/tests/61_own_blob.rs` (4 tests, `tests/fixtures/own_blob/`, fixture files share one named span, `61_own_blob.rs:21-22`): `fixtures_share_one_named_span` (:76), `own_set_wins_over_span_search` (:88), `fallback_max_count_breaks_the_shared_span_tie` (:107), `fallback_exact_tie_is_none` (:125).
- **Rail**: the fallback (hand-built contexts, `own: None`, e.g. unit tests) counts, per blob, how many of the output's named spans match a `DefSite`, in ONE pass over the index with membership by `binary_search` on the deduped sorted span list (`types.rs:1766-1777`); it picks the single highest-count blob, an exact tie returns None, and blobs are scored in sorted `ContentId` order so any later tie-break stays stable (`types.rs:1782-1787`, comment `types.rs:1738-1742`).
- **Entry**: five back-to-back `extract --resolve` runs over typescript-go (`internal/` + `cmd/`, one process, `timeout 10`): `85855 85855 85855 85855 85855` (PR #555 body). Gate `cargo test --features cli --no-fail-fast` 480 pass / 0 fail (PR #555 body).

## 98. A bench compared two normal forms and reported 5.6% recall for a 45.3% fact

- **Incident** (2026-08-29): ORACLES.REPORT.md section 7 reported the go call recall as 5.6% (2,753 intersection) against the `go/callgraph` vta oracle (`plans/extract-bench-2026-08-29/ORACLES.REPORT.md:203`), and the ts module overlap as 47%. Both compared different rows of the same relation.
- **RCA**: two normal-form mismatches. Go call: the oracle writes receiver-qualified `Type.Method` where the extract emitted the bare name, confirmed by grep on the oracle side (`ORACLES.REPORT.md:218`); section 12 restates the comparison with the receiver prefix stripped on the oracle side (`ORACLES.REPORT.md:335-344`). Ts module: section 7's 47% compared binding targets (`resolved_import` through barrels) against madge/dependency-cruiser/codeql/stack-graphs/scip, which all emit file-imports-file; the file-imports-file row is `extract --deps`'s `file_edge` (`ORACLES.REPORT.md:319-324`).
- **Fail-pre-fix**: the normalized tsvs and the normalizer: `plans/extract-bench-2026-08-29/normalize.py` renders every family to the `src_path src_name dst_path dst_name` normal form (`normalize.py:1-3`), and the corrected comparisons run over the `.bare.tsv` files (`go.oracle.call.vta.bare.tsv`, `ORACLES.REPORT.md:340`) and `ts5.deps.module.tsv`.
- **Rail**: `normalize.py` and the `.bare.tsv` files: both rows stay, `file_edge` answers "what does this file import" and `resolved_import` answers "where does this name come from" (`ORACLES.REPORT.md:332-334`); the go call receipt tsv predating #558/#560/#562 is flagged for re-emission on the current binary (`ORACLES.REPORT.md:349-350`).
- **Entry**: go call recall 5.6% (PR #559 body) -> 45.3% recall / 50.9% precision, 24,980 of vta's 55,099, over `go.oracle.call.vta.bare.tsv` (`ORACLES.REPORT.md:344`, PR #561); ts module `--deps` `file_edge` vs madge: 2,010 of 2,011 rows intersecting, the one b-only row `src/../scripts/failed-tests.d.cts` outside `src/` (`ORACLES.REPORT.md:325-331`, PR #561 `ts.madge.module.tsv`).

## 99. A go method answered for the type it was named after

- **Incident** (2026-08-29): `ORACLES.REPORT.md` section 13.2 classed 5 of the 76 ours-only go `type` rows as `wrong-file`: the target sat in the REFERRING file, not the declaring one. `internal/ast/ast.go:154-157` is `type ModifierList struct { NodeList; ModifierFlags ModifierFlags }` and `internal/ast/modifierflags.go:3` declares `type ModifierFlags uint32`, but the edge pointed at `ast.go`. Same shape for `ast.go PatternAmbientModule -> Symbol`, `printer/emitcontext.go emitNode -> EmitFlags`, `project/projectcollection.go ProjectCollection -> ConfigFileRegistry`, `project/session.go Session -> Snapshot` (`ORACLES.REPORT.md:433-441`).
- **RCA**: `resolve_type_dst`'s same-file leg matched any named TypeF node (`go.rs:2593-2596` pre-fix), and go's TypeF entities include `Function`/`Method`. `internal/ast/ast.go:612` declares `func (n *Node) ModifierFlags() ModifierFlags`, so the method entity answered a TYPE reference. The corpus fallback had the same blindness the other way: `DefSite` carries `family` but no entity kind (`types.rs:1626-1630`), so a name held by one method plus one type read as ambiguous and emitted a zero dst. Neither leg encoded go's package scope either, so a bare name skipped straight from same-file to a corpus-wide uniqueness guess.
- **Fail-pre-fix**: `v6/sprefa-extract/tests/68_go_type_refs.rs` over `tests/fixtures/go_type_refs/`, red on f5fd6dbf9 with `ModifierList -> b.go ModifierFlags` and no `Session` row at all: `a_field_named_like_its_type_binds_the_type_declaration` (:74), `a_bare_type_ref_binds_inside_its_own_package` (:88).
- **Rail**: `GoModuleFacts` carries its file's `type_spec`/`type_alias` spans off the plane's existing parse (`go_modules.rs` `walk_type_decls`), `GoModuleIndex::is_type_decl` joins `(path, span)`, and every leg of `resolve_type_dst` filters through it (`go.rs` `is_go_type_decl`, `type_decl_sites`, `GoModuleIndex::resolve_type_in_dir`). The bare-name legs run same-file, then the referring file's own package directory (`resolve_type_in_own_dir`, no export gate), then corpus-unique.
- **Entry**: one `extract --resolve --family type` process over typescript-go `internal/` + `cmd/` (5,075 files, 6.2s then 7.7s, under the 10s law) against `go.oracle.type.typedecl.tsv`: recall 68.99% (4,280/6,204) -> 73.37% (4,552/6,204), precision 97.27% -> 97.43%, ours-only 120 -> 120 with all 5 `wrong-file` rows replaced by 4 function-local `type` decls and 1 `_test.go` row the oracle cannot see (`ORACLES.REPORT.md:445-450` classes `local` and `testfile`).

## 100. The scip-informed `--resolve` leg ran at 48 s and 2.2 GB RSS and nobody had a ceiling on either

- **Incident** (2026-08-30): the scip-informed resolve leg (PR #589's `--resolve` with a fresh `--scip-index`) measured ts 3.4 s / rust 5.0 s / go 24 s wall and 753 / 974 / 1,247 MB peak RSS against the 623 MB plain-resolve go number, with go at 2x the wall ceiling and 1.8x the RSS one. Two frames owned the gap: `types::containing_def_site` took 6,783 of ~14,500 on-CPU samples on go (a linear scan over every `DefIndex` name's site list per call site), and the per-occurrence `String` symbols held 952,588 duplicated strings on go (75 MB of text plus copies in the per-document caches) beside the `RandomState` cache-key defect #589 fixed.
- **RCA**: the informed leg grew two per-call joins (`site_occurrence` -> `definition_of` -> `containing_def_site`) without any ceiling asserting their cost, and the scip types carried a `String` symbol per occurrence although a go index holds ~30k distinct symbols for its 952k occurrences. Cost that no ratchet column asserts is invisible until a corpus big enough to hang the receipt shows up (`SCIP-SPEED.REPORT.md` sections 2 and 4).
- **Fail-pre-fix**: the receipt procedure of the scip-rss lane (3 runs per corpus, `nice -n 15`, `/usr/bin/time -l`, `timeout 60`): on the pre-fix binary, `containing_def_site`'s linear scan is the top self frame of a macOS `sample` of the go resolve, and the go informed RSS read 1,247 MB vs the plain leg's 623 MB with zero resolution-identical output to explain the difference (sorted jsonl identical before and after).
- **Rail**: per-corpus wall and peak-RSS ceilings (10 s / 700 MB) measured by the receipt procedure on the informed leg every time the scip seam changes, and the ratchet's wall/rss columns (`RATCHET.tsv` via `tests/ratchet_recall.rs`, +15% / +10% tolerance, `RATCHET_BUMP=1` improves-only) on the diet leg; recall/precision floors must be byte-identical (sorted jsonl `cmp` against the pre-change binary on all three corpora).
- **Entry**: interning symbols at decode (`SymbolId` into `ScipIndex::symbols`, `ScipIndex::symbol`) plus `containing_def_site` binary search over a per-blob span index plus cached def-document line tables (`byte_range_cached`): ts 3.3 -> 1.7-2.0 s / 591 -> 570 MB, rust 4.9 -> 2.2-2.4 s / 776 -> 705 MB, go 22.8 -> 7.1-8.2 s / 971 -> 890 MB, sorted jsonl IDENTICAL on all three corpora, targeted scip tests and the full gate green (commits af507ca25, a2dce88da, 267b51706 on `fix/extract-scip-rss`).

## 101. The ratchet graded diet against checker floors and called three languages broken

- **Incident** (2026-08-31): grading PR #615, `cargo test --features cli --test ratchet_recall -- --ignored` reported rust call recall 75.66% against the 93.69% floor, byte-identical on the lane tree, on clean origin/main, and on pre-#613 01f0bf022 — a "regression" in every tree including ones that had measured green hours earlier. A second protocol error in the same session: the corpus-stats checker arm ran a binary built without the `rust-checker` feature and produced diet rows labeled `arm=checker` (tokio 8,825 call rows at 0.4 s instead of 9,178 at 5.2 s), caught only because the walls matched the known checker-never-engaged signature (PR #614 round 1).
- **RCA**: the rust ratchet leg hard-wires the checker tier (`tests/bench/mod.rs` `request()`: `rust_checker: checker.then_some(corpus.root)`), so its floors are checker numbers; built without the `rust-checker` feature the field is inert and the leg silently measures diet. Two more protocol axes fail silently: a dev-profile build runs the in-process hir analysis 4-14x slower than the release floors (rust 146.5 s vs the 10.6 s ceiling, go 12.6 s vs 3.2 s), and legs sharing one test process share a `getrusage` high-water RSS, so a later leg inherits the heaviest leg's peak (ts5 read 1,698 MB against its 514 MB ceiling). The canonical protocol lives in one place, `just extract-ratchet`: `--release`, one leg per process, `cli,rust-checker` for rust only.
...
## 105. The rust checker's RSS ceiling had been red on main for six commits and only a refactor's control run found it

- **Incident** (2026-09-01 ~12:30): the tier-axis refactor measured `rust.checker` at 4,729 MB against the committed 3,756 MB ceiling, +25.9%. The obvious read was that the refactor cost 973 MB. Three measurements (one under lane load, two quiet) all landed 4,609-4,729 MB, ruling out machine load as the cause and leaving the branch as the only suspect.
- **RCA**: the branch was innocent. The control run of the OLD harness on unmodified main at `41333391a` read 4,726 MB, 3 MB from the branch, and exited 101 with all four rust rows failing on the identical rss line. `just extract-ratchet` is local-only and never runs in CI, so `CI-KNOWN-RED.md` structurally cannot list it; the last run before this one was inside the #625 lane's own worktree. Six commits touched `v6/sprefa-extract/src` between the ceiling's `measured_at_sha` (`f58cc6666`) and main's tip, and none of them re-ran the leg. The `3,756` was measured post-sysroot, so #625 is not the obvious suspect; #631 (holding the crate graph to refuse a zero-file join) is the shape worth bisecting first. The bisect is UNBUILT.
- **Fail-pre-fix**: `cargo test --release --features cli,rust-checker --test ratchet_recall -- --exact --ignored ratchet_rust` in the MAIN checkout at `41333391a`, rc=101, four rows: `rss 4726 MB above ceiling 3756 MB by 25.8% (tolerance 10%)`. Accuracy on that same run was byte-equal to the committed floors (`93.66 / 51.82`, `77.09 / 36.98`, `84.28 / 83.64`, `64.93 / 98.26`), so only memory moved.
- **Rail**: a local-only gate needs an owner and a cadence, because CI cannot hold it. Coordinator duty: `just extract-ratchet` runs on main before any PR that plants or moves a ratchet floor, and its rc is a receipt in the PR body. The `RATCHET.cost.tsv` row planted here carries the measured 4,765 MB under protest, with this entry as the reason it is not a silent normalization.
- **Entry**: cost ceiling planted at the measured value; the +970 MB bisect across #624, #626, #628, #625, #629, #631 is open and unassigned.

## 106. `extract --resolve` drops edges when the paths are relative, and says nothing

- **Incident** (2026-09-01): a sonnet agent and the coordinator ran `extract --resolve --family call,type` over the same 35 files of `v6/sprefa-engine-rs/src`, from the same cwd, and got different graphs. The agent passed absolute paths and got 1,949 `resolved_edge` + 288 `resolved_import`; the coordinator passed relative paths and got 1,936 + 263. The coordinator's answer key was the wrong one.
- **RCA**: absolute is a strict superset. 9 import pairs and 3 call pairs resolve only with absolute paths; ZERO resolve only with relative. Every dropped row has a source nested below `src/` reaching a crate-root module (`src/bin/dl6.rs` -> `src/run.rs`, `src/build_template/main.rs` -> `src/program.rs`, and 7 more). A `use crate::<module>` joins against the crate root, and the root is derived from the supplied path string, which comes out wrong when that string is relative. Files sitting directly in `src/` are unaffected, which is why the loss is 25 rows out of 288 and easy to never see.
- **Fail-pre-fix**: same cwd, same file set, only the spelling differs:
  `extract --resolve --family call,type $(find src -name '*.rs')` -> 263 resolved_import;
  `extract --resolve --family call,type $(find "$PWD/src" -name '*.rs')` -> 288.
  `comm` over the two normalized import sets: 9 absolute-only pairs, 0 relative-only.
- **Rail**: UNBUILT. `--help` says only "Pass files, never directories", never that the paths must be absolute, and the stream carries no warning that resolution was degraded. This is the silent-fallback shape entries 101 and 631's loud-checker work were stamping out. Candidate rails: canonicalize every supplied path at the door, or emit a named diagnostic row when a `crate::`-rooted import cannot join.
- **Entry**: the ratchet floors are NOT affected. `tests/bench/mod.rs:51,55,59` hold absolute corpus roots and `enumerate` builds every path from `corpus.root`, so every committed measurement used absolute paths. Ad-hoc lab runs and the 31 python scripts are unaudited.

## 107. The 350k-row emission budget went red on main for five days and every lane blamed the machine

- **Incident** (2026-09-02 to 03): `emit_throughput_350k_rows_under_budget` (`tests/45_emit_throughput.rs:89`, 5.5 s wall) failed 3/3 in the A2 and A4 TSI lanes' worktrees AND on their base `8e050ed82`. Both lanes cited entry 104 and filed it as load. The coordinator's control at 4.1 to 5.5 load on the pre-TSI sha `15e95de83` read 5.81 / 5.85 / 5.86 s; the budget-setting commit `4a0b91362` on the same machine in the same quarter hour passed 3/3.
- **RCA**: code, not load, and NOT `7bfc8d4a4`. The first bisect ran nine builds with ONE measurement each over a range whose whole spread is 0.3 s, and named a commit at random. Re-bisected with three measurements per build and the test itself as the predicate, the first bad commit is `33a2643b4` (#562, `x := f()` binds its callee's declared result type): its parent-in-range `b650feab0` (#560) passes 3/3, `33a2643b4` fails 3/3. #562 appended `go_bind_plan_store(crate::content_id_of(src), plan)` to `go_collect_receivers` — a SECOND blake3 over the whole file on the plain emission path, where the go door already hashes the same bytes for its parse cache key. blake3 is linear in file size, so on a 25 MB fixture the duplicate is a whole extra pass: 325 ms, the `go call` span reading 735 ms with it and 421 ms without. `7bfc8d4a4` (#567) is a WASH: its own parent `9220cba04` already fails 3/3, and `go_chain_of` never runs on this fixture at all — 25,000 top-level `var` declarations carry no function body and no call site. Entry 104's rail (control on main) compounded the error because main itself was red: a control run proves environment only when the control is a sha known to pass, not the tip.
- **Fail-pre-fix**: `cargo test -q --features cli --test 45_emit_throughput`, three runs per sha, one machine, load averages 2.8 to 5.5 throughout. `4a0b91362` (the budget-setting commit, whose header claims a 4.03-4.15 s band) 4.902 / 4.996 / 4.874 s, ok 3/3. `b650feab0` (#560) 5.080 / 5.178 / 5.093 s, ok 3/3. `33a2643b4` (#562) 5.506 / 5.518 / 5.512 s, rc=101 3/3. `9220cba04` 5.603 / 5.583 / 5.542 s, rc=101 3/3. `7bfc8d4a4` (#567) 5.509 / 5.526 / 5.533 s, rc=101 3/3. Tip `5b9063bef` 5.796 / 5.777 / 5.847 s, rc=101 3/3. COUNT beside the wall: `DL_TRACE_SUMMARY=1` over the fixture, `go content-id` reads `files 2`, 639,600 / 658,807 us.
- **What is NOT bisected**: only #562 crosses the 5.5 s predicate, so only #562 is bisected; every other step in the range stays under it and a pass/fail bisect cannot see inside. Three stretches remain, measured but unattributed. The header band itself is the largest: `4a0b91362` claims 4.03-4.15 s and measures 4.90 s today, so ~0.8 s of the total drift is the machine or the era, not repo code, and no commit can be blamed for it. `4a0b91362` -> `b650feab0` adds 0.19 s over 27 commits, and `33a2643b4` -> tip adds 0.29 s over 30 more. Attributing either needs a per-commit wall series, not a predicate.
- **Rail**: the control run for a perf-budget failure is the budget's OWN `measured_at` commit (the commit that last touched the test, `git log -1 -- <test>`), never the tip of main. A perf bisect measures three times per build; one measurement per build inside a range whose spread is smaller than the run-to-run noise names a commit at random, and `git bisect run` will do exactly that without saying so. Coordinator duty: a perf-budget failure that reproduces at the control sha is environmental; one that does not is bisected before any PR merges on "accuracy receipts". Entry 104's unbuilt code rail is now built: `45_emit_throughput.rs` reads `libc::getloadavg` and prints `skipped: load N > threshold` above 8.0 (12 logical CPUs, 8 of them performance) rather than measuring a saturated machine.
- **Entry**: `go_parse_shared_keyed` hands its content id to `go_collect_receivers`, so the go door hashes each file once. `go content-id` `files 2 -> 1`, 640 ms -> 325 ms; `go call` 735 ms -> 421 ms; piped through `wc -l` 4.931 / 4.870 / 4.851 -> 4.430 / 4.413 / 4.412 s; the test 5.796 / 5.777 / 5.847 (rc=101 3/3) -> eleven runs spanning 5.403 to 5.503 s, ten ok and ONE over at load 3.40. The budget is not durably met: the margin is 40 to 95 ms, and the load skip is set for a saturated machine and does not cover a load of 3.4. 5.44 s is under budget and 0.54 s above what `4a0b91362` itself measures today (4.90 s), which is the unbisected remainder above. What IS still nameable: two full hashes of the same bytes survive per file, `dispatch.rs:31` for the extract cache key and `go_parse_shared_keyed` for the parse cache key (`#583` bought the second one a 10520 -> 6680 ms resolve win, so it stays). Threading dispatch's id into `Source::extract` removes one 325 ms pass and needs the `Source` trait plus every lang arm, so it is filed rather than taken here.
- **Instrument**: the count receipt above was a span wrapping ONE call site, so a duplicate hash added anywhere else stayed invisible; #562 lived five days for exactly that reason. `content_id_of` (`v6/sprefa-extract/src/types.rs:62`) now opens the `Phase::Hash` span itself, and the phase table names every hash whatever calls it. `tests/31_tracing.rs::phase_calls_per_file_are_pinned` pins the per-file count per lang (go 2, ts 2, rust 1) over three fixtures each, load-independent. Sabotage: re-apply #562's `go_bind_plan_store(crate::content_id_of(src), plan)` and `go hash` reads `files 3` and the test FAILS with `go hashed go/sample.go 3 times, want 2`. Every run also lands a row in `extract_run` + `extract_phase` in `~/.agent/dl6.db` under `--bench` or `DL_TRACE_SUMMARY=1`, readable with `extract --trail`, so the next slow run explains itself from disk instead of from a wall number.

## 108. A 187-second checker walk was blamed on the impl enumeration, and the measurement was a debug binary

- **Incident** (2026-09-03): the coordinator's hand probe, `--rust-checker src/trace.rs` with a `cargo build --features cli,rust-checker` (debug) binary, logged `walk_ms=187159` for one 254-line file and 1129 `rust.impl` rows. The coordinator filed `rust_checker_walk_scales_with_crate` naming `Impl::all_in_crate` (`rust_checker_ra.rs:404`) as the cost and dispatched a lane on that diagnosis.
- **RCA**: two errors, both the coordinator's. (1) A row count was read as a time: 1129 impls described in a walk is a count, and nothing had measured the impl leg's wall. The lane (PR #683) removed the enumeration (1139 -> 7 `rust.impl`) and `walk_ms` did not move, 138310 ms with the item walk fully off. (2) The profile was never stated. PR #686 measured release: `walk_ms` 5036 / 5010 / 5097 ms, under the 10-second law, against 96147 / 95094 / 95875 ms debug. The debug figure was then decomposed with a span per resolution kind: 81% is `sema.parse_guess_edition` on the ONE supplied file, which fans out into `crate_def_map_query` for 508 crates, 143 of them this crate's own test-target crates (`97_ingest`, `102_tsi_...`) that share the source root; 19% is first-touch body inference, which moves between spans when the arm order changes and never shrinks; the impl leg and the fold are 0%.
- **Fail-pre-fix**: PR #686's four spans on `src/trace.rs`, debug, three runs: `checker_type_path` 605 calls 14050 ms, `checker_nav` 355 calls 3771 ms, `checker_method` 113 calls 330 ms, `checker_call_path` 46 calls 56 ms; `parse_guess_edition` 77934 / 76880 / 77600 ms. After (b) and (d): 1119 -> 437 calls, site leg 18212 -> 18751 ms. Default features + `set_test: false`: 19% cheaper and `method_unresolved` 0 -> 105 of 113.
- **Rail**: a perf row names the profile (`debug` or `release`) and the load average in its first line, and a count is never filed as a time; a row that blames a leg carries that leg's own wall, measured by a span on it, before any lane is dispatched. The 10-second law is judged on the release binary. Coordinator duty: before dispatching a perf fix, spend the lane's first commit on spans (as `plans/2026-09-03-rust-checker-site-resolve-cost.BRIEF.md` step 1 did), never on the fix.
- **Entry**: PR #683 (impls by supplied file, a correct narrowing with zero wall effect), PR #686 (`Phase::Checker*` spans, type-position-only `resolve_path`, memoized navs, `tests/109_rust_checker_site_cost.rs` pinning the call counts). The located defect, `crate_def_map_query` over 143 sibling test-target crates, is ARCH `rust_checker_def_map_over_sibling_targets`, a design fork for Chris.

## 109. `--family scip` runs every indexer a polyglot root matches, so the second one spends the whole budget

- **Incident** (2026-09-01): the go tuning control could not be measured. `run.py tuning --lang go` points at `~/projects/typescript-go`, and every attempt came back `timed_out: scip-typescript: exceeded the 900s budget; process group killed` (`plans/extract-eval-2026-08-31/heldout/SKIPS.tsv`). The go arm of the overfit question stayed open with no number, while rust and ts had theirs.
- **RCA**: `detect` (`scip_ensure.rs:653`) filters `INDEXERS` by any-of marker match and returns EVERY row that hits; `ensure_index_for_set` then loops `for indexer in detected` and runs them all, merging the parts. typescript-go carries `go.mod` and `package.json`, so it matches the go row and the typescript row. scip-go finished; scip-typescript compiled the TypeScript compiler and blew the budget, and the report the caller read was the killed one. No flag, env var or argument could reach `detect`, so there was no way to say "index this root as go".
- **Fail-pre-fix**: `tests/scip_indexer_pick.rs`. A temp root holding both `go.mod` and `package.json`, both fake indexers on PATH sleeping past a 1s budget. `a_pick_runs_only_the_named_indexer` asserted `["go"]` and got `left: ["typescript", "go"]`.
- **Rail**: `--indexer <LANG>`. `detect_picked` takes an `IndexerPick` and a named pick ignores markers entirely. An unknown name is a hard CLI error naming the roster, never a silent empty answer. A picked index is a deliberate partial, so `pick_cache_dir` keeps it in `<CACHE>/indexer-<LANG>/` and `picked_index_path` scopes reuse there: it never lands on the shared `index.scip`, and an unpicked ask never reads it.
- **Entry**: same silent-fallback shape as 101 and 106. The measurement did not report "I indexed this the wrong way"; it reported a timeout, which reads as "this repo is too big" and cost a day of the go arm. Every committed ratchet corpus is single-marker, so no floor moved.

## 110. A recall grind made `extract --resolve` hang on click/core.py, and the lane's own suite never ran a real file

- **Incident** (2026-09-03): PR #708 lifted PyCG micro-suite recall 69.49 -> 86.86 and merged green. The module-plane lane, timing its own receipt on `~/corpora/click`, found `extract --resolve --family call click/core.py` past 10 s and the 17-file run dead at 125 s with a stack overflow on a rayon worker. The pre-#708 binary at 42ca51ff6 answered the same file in 0.36 s.
- **RCA**: `python/_0_source.rs` `param_args` scans every call site and recurses through `callee_def` -> `param_target` -> `param_args` for each bound-name callee. The only guard was the active def stack, so a re-entry into an already-finished (callee, site) from a sibling branch recomputed the whole subtree: factorial in the defs on the path. Finite, not a loop; a 256 MB thread still ran past 60 s, the 2 MB rayon worker (`project.rs:1106`) overflowed. The PyCG suite is 119 files of under 30 lines each, so no case had the fan-out to show it.
- **Fail-pre-fix**: `tests/118_python_call_grind.rs` `forwarding_ring_resolves_under_the_wall` over `tests/fixtures/py_call_grind/forward_ring.py`: a ring of forwarding defs, wall asserted under 10 s and the row set pinned; at e08866c82 the test ran out the clock.
- **Rail**: `PyResolver` memoizes complete answers per (callee, site) and per site; an answer computed under a cycle cut lives in a per-site scratch map, cleared before the next top-level site, because caching a cut answer permanently dropped kwargs/call (86.02). `PY_RESOLVE_DEPTH = 12` caps the active path. click 17 files 0.15 s; suite 86.86 unchanged (PR #716).
- **Entry**: a micro-suite ratchet is not a wall receipt. Every recall lane now times one real corpus file set (click, flask, requests are on disk under `~/corpora`) under `timeout 10` beside its suite numbers, and the coordinator's gate runs the same before merging. Open: the rayon pool's 2 MB worker stack turns any deep-but-finite resolver recursion into a whole-run abort instead of one file's miss.

## 111. A product's arity read as a row count doubled every macrotime protocol arity

- **Incident** (2026-09-17, lane `feat/keys-userland-2`): moving the value-position call's arity off the `relation/3` table and onto the callable's own `:` edges turned `_21_openapi` red 4/4 with `arity_mismatch(syntax_claim, 2, 4)`, `arity_mismatch(syntax_form, 1, 2)` and `arity_mismatch(syntax_source, 8, 16)`. Declared was exactly half of observed in every row, so lowering emitted double-width calls against a correctly-declared product.
- **RCA**: `EdgeIndex::build` (`src/_2_lower/_10_index.rs:196`) is fed `concat(&promoted.edges, &imported.edges)` from `build_scope` (`_0_api.rs:96`). A unit that imports its own exports carries each of its edges TWICE in that list. The retired `constructor_relations` never saw this: it counted `declared.edges` only, and `relation_dictionary` kept the first row per callable, so a duplicate row was dropped rather than summed. A fresh per-owner row counter summed both copies. `owner_edge_count` on the check side (`_3_check/_1_graph.rs:44`) has the same row-count shape and is read by `dense_index_diagnostics`, so it could not be repurposed.
- **Fail-pre-fix**: `cargo test --test _21_openapi` at the row-counting version, 0 passed 4 failed, `dangling_ref_is_a_diagnostic_naming_the_pointer` reporting 23 `arity_mismatch` diagnostics from the macrotime bootstrap in place of the one `openapi_unresolved_ref` it asserts. `_3_lower_oracle`, `_4_check_oracle` and `_8_compile_oracle` were all GREEN at that same build: the doubling only shows where a module imports itself, and only the openapi driver path does.
- **Rail**: arity is the count of DISTINCT ordinals, never of rows. `EdgeIndex::owner_count` increments only when `by_owner_index` mints a new `(owner, index)` key, and `CheckerGraph::owner_slots` holds a `HashSet<i64>` per owner, kept separate from `owner_edge_count`. Both carry the reason in a two-line comment.
- **Entry**: a visible-row list in the lowering frame is a concatenation, not a set. Any new per-owner tally over `EdgeIndex` or `CheckerGraph` counts slots, and a test that exercises only single-module fixtures does not price it; the openapi driver is the one green-or-red signal for the self-import shape.

## 112. Every bound goal on a rule-heading relation paid for a top-down proof the fixpoint already derived

- **Incident** (2026-09-18, lane `fix/eval-demand-bleed`): the step 5 tree of `feat/prelude-macrotime` (prelude through macrotime) never returned from P1 round 1 on `fixtures/openapi/todo.dl7`. Measured on `origin/main` at `0e308ae70`: one firing of `ambiguous_return` (`macrotime/1_caret.dl7:146`, seven goals with `int.lt`) over the prelude's 16151 syntax rows costs 48353 `demand` calls and 246 ms release, and round 1 fires a twelve-goal rule once per delta position, each firing redoing its whole demand tree.
- **RCA**: `positive_solutions` (`src/_6_eval/_5_evaluate.rs:250-256`) called the top-down `demand` proof for EVERY positive goal with any bound argument on a relation that heads a rule, though bottom-up semi-naive already derives those rows in full. The proof memo was a fresh `HashMap` per firing (`:379`, and the same shape in `aggregate_proofs` and `fold_step`), and `demand` plans were `Range::All` (`:308`), so the round's delta discipline was defeated inside the demand tree as well.
- **Fail-pre-fix**: `tests/_24_demand_count.rs` over `tests/fixtures/demand_count/ambiguous_return_bleed.json` (100 source rows; one derived relation whose rule needs nothing bound, one whose kernel goal reads its head argument). At `0e308ae70` with an atomic counter on `demand` entries: 700 calls, closure rows correct. The test holds the round's `Trace::Demand` sum under 500.
- **Rail**: `evaluate_into` computes `demanded` once: a relation lands there only when a non-aggregate rule of its own cannot be derived bottom-up. `needs_bound_head` walks the rule body holding a bound set: a kernel or negative goal over a still-unbound head variable blocks. A positive kernel goal blocks only when the op cannot compute its row from the arguments bound at that point (comparisons need both sides; `cons`/`str.cons` take head and tail or the whole list; `nil`/`str.nil` need nothing; other ops take the first two arguments) and the rule reads no `intern_snapshot` goal, whose table fills across rounds while the snapshot relation never joins the delta, so the rule's round-zero fire never revisits and only demand derives its rows in place. `demand` fires only for those relations. The memo is one `Memo` per round owned by the round loop and cleared with the round's inserts, so a firing reuses the round's proofs; an entry whose proof hit an in-progress key is dropped (`cycle_hits`) because a re-entrant key reads partial rows and only the cycle target's own entry is reproducible. `Trace::Demand { calls }` per round prices the whole mechanism. On the prelude macrotime tree this emptied the phase's demanded set of `:` itself, which one `int.add` over a head variable had dragged in, and dropped P1 from never-returning to 278 ms release.
- **Entry**: a per-firing memo under a per-position firing loop multiplies any per-goal proof cost by the delta position count, and neither factor shows in a single-firing profile. Any new per-goal proof path in `_6_eval` must state which relations it serves and carry a `Trace` counter a COUNT test can hold to a bar.

## 113. Renaming a binary in another repository left this one feeding a stale build

- **Incident** (2026-09-18): `hafley-rs` renamed the `sprefa-extract` crate's `[[bin]]` target from `extract` to `ryi` (`hafley-rs` `bbd1bc3f`). Every path by which dl8 locates that binary named `extract`: `src/_9_runtime/_3_executors/extract.rs:53,55,56`, `tests/_16_extract_tsi.rs:84,86,87`, `tests/_20_hosts.rs:506,508,509`, and the `cargo build --bin extract` both tests fall back to (`_16_extract_tsi.rs:119`, `_20_hosts.rs:536`). Nothing failed, because `target/debug/extract` still sat on disk as a pre-rename artifact. Cargo does not remove the old binary when a target is renamed, so the `extract` executor kept answering rows from a build made before the rename, and would have gone on doing so until someone ran `cargo clean`.
- **RCA**: the binary is located by filename, and a filename is not a dependency. `extract_bin` (`src/_9_runtime/_3_executors/extract.rs:39`) tries `$SPREFA_EXTRACT_BIN`, then `$CARGO_TARGET_DIR/debug/<name>`, then two sites under the gitignored `hafley-rs` link, and takes the first that `exists()`. A stale file passes `exists()`. The one signal that would have fired, `_20_hosts::extract_answers_rows_for_each_family_over_the_corpus`, is listed environmentally red in `.github/CI-KNOWN-RED.md:165` for an unrelated reason (`sprefa_root` = repo parent, stale since the v8 flatten), so the rail for this class was already off when the rename landed.
- **Fail-pre-fix**: none added. The reproduction is manual and destructive to a build tree: `rm hafley-rs/crates/sprefa-extract/target/debug/extract`, then `env -u SPREFA_EXTRACT_BIN bash book/show.sh run <fixture with a real root> --serve extract`, which before the fix answers `(Failed <family> "no extract binary; set SPREFA_EXTRACT_BIN")` for every family.
- **Rail**: every site now names `ryi`. Verified with the env override unset, so the candidate search itself is what resolved: a copy of `fixtures/hosts/3_extract.dl7` with `__ROOT__` replaced by `hafley-rs/crates/sprefa-extract/tests/fixtures/ts` answers 72 `Fact` rows and zero `Failed` rows. `$SPREFA_EXTRACT_BIN` keeps its name: `README.md:50` documents it and `book/src/11_executors.md:221-224` pins its error text as generated output.
- **Entry**: a cross-repository consumer located by filename has no compiler and no linker to tell it the name changed, and a stale artifact makes the failure silent rather than loud. The executor's relation name is a separate surface: `RELATION` and `ERROR` at `src/_9_runtime/_3_executors/extract.rs:14-15` stay `extract` and `extract_error`, because `fixtures/hosts/3_extract.dl7` and `fixtures/extract/main.dl7` are dl7 programs written against those names, and renaming a served relation is a language change.

## 114. A held mailbox message never reached an omp lane mid-turn, so the wall-clock cap killed uncommitted work

- **Incident** (2026-09-19, lane `feat-eval-sqlite-step4`, preset `glm53f-omp`): the lane ran past the 30 minute cap of `plans/v8/2026-09-18-eval-on-sqlite-ivm.brief.md` Addendum 3 with 754 new lines in seven files and no commit. The coordinator's stop order (`boop beep` from `sprefa-coordinator`) landed as `held ... for the next turn boundary (lane supervisor)`; the omp harness holds one turn for the whole run, so the boundary never came. The lane was deleted at 37 minutes; the coordinator committed the tree by hand (`503304744`), fixed one scanner hit, and shipped it as PR #799.
- **RCA**: two rails assumed a turn boundary. The driver's `--wait --wait-timeout 1800` died when the driver retired between its own turns, and the mailbox rung for a lane parent delivers only at a turn boundary. The brief's 25 minute self-push rule was the one rail that did not need a boundary, and the lane skipped it.
- **Fail-pre-fix**: none added. The reproduction is a live omp lane: `boop beep --no-wait --as sprefa-coordinator <lane> "stop"` during its turn prints the `held` line and `boop db mail <lane>` shows `to_timestamp: null` until the turn ends.
- **Rail**: every implementation brief from step 4b on says the lane pushes a wip commit by its own clock at 20 minutes and after every slice, and mails field decisions to `sprefa-coordinator` directly, because the driver that spawned it is retired by then. The coordinator arms its own `Monitor` on `boop db mail <lane>` and snapshots the worktree itself when the cap passes; a lane's uncommitted tree is graded like a commit, never discarded.
- **Entry**: a message rung that waits for the recipient to yield is a no-op against a recipient that never yields. Any cap on an agent must be enforced by the agent's own clock or by a supervisor that kills and salvages, never by a message.

## 115. The eval arena guard crossed every `sql()` call and the sqlite fixpoint deadlocked on itself

- **Incident** (2026-09-19, lane `feat/eval-sqlite-step4b-r2`): `DL8_ENGINE=sqlite dl8 eval` never returned on `oracle/eval/15_aggregate_tables_only`, `4_cons_lists` or `9_edge_ref` at `720bdde96`. Every run's last log line was `dl8::sql: name="insert_seeds"`, so the hang sat after the seed insert and before any later statement could log. Three probes ran past the 10-second law and were killed by `timeout` (PIDs 96194, 8974, 15415); this row is their receipt.
- **RCA**: cause A, mutex re-entry, not unbounded SQL recursion. The sample on each fixture read 0.0% CPU with `sqlite3VdbeExec -> _pthread_mutex_firstfit_lock_wait -> __psynch_mutexwait`: the statement was mid-run and the thread was asleep on a lock it already held. `declare` takes the arena guard at `src/_6_eval/_7_sqlite_eval.rs:96` and holds it across `sql()` at `:180`, `:187` and `:190`; `read` takes it at `:216` across `:224` and `:247`. The view batch is `CREATE VIRTUAL TABLE "program" USING sqlite_ivm(...)` (`_5_reify/_7_sqlite.rs:1892`), which materializes at declare and runs the `dl_*` functions; every function locks the same arena (`src/_6_eval/_8_functions.rs:54`, `:94`, `:106`). A `std::sync::Mutex` is not reentrant, so the thread that asked sqlite for rows slept forever waiting for itself.
- **Fail-pre-fix**: `tests/_27_eval_sqlite.rs` at `720bdde96` never finished; every fixture runs through the real binary under `DL8_ENGINE=sqlite`, so the three deadlocks held the leg past 60 s. After the fix the leg finishes in about 1 s and every fixture returns in under 60 ms.
- **Rail**: `declare` scopes the guard to `flush_rows` and drops it before `flush_commit` and `declare_view` (`src/_6_eval/_7_sqlite_eval.rs`); `read` takes the guard only after the last `sql()` returns. The lowering test law stands: no arena guard crosses a `sql()` call, because every `dl_*` function locks the arena.
- **Entry**: a scalar function registered on the connection runs on the caller's thread inside the statement, so any lock the caller holds is a lock the function waits on. A 0% CPU hang under `sqlite3VdbeExec` with `__psynch_mutexwait` on the stack is this shape, not a runaway query.

## 116. Three glm53f-omp lanes in a row died at their cap on one arc with nothing shippable

- **Incident** (2026-09-19, lanes `feat-eval-sqlite-step4b`, `feat-eval-sqlite-step4b-r2`, `feat-eval-sqlite-step4c`, preset `glm53f-omp`): 4b ran 30 minutes with 358 uncommitted lines and no push; r2 pushed one receipts commit, then stopped at 25 minutes mid-fix with the guard scope still wrong; 4c ran 20 minutes and left two `eprintln!` probes and `FLOOR = 32` with 4 fixtures passing. Each brief carried the push-by-clock and push-by-event rails inline (`plans/v8/2026-09-18-eval-on-sqlite-ivm.step4b.brief.md:31-42`, `step4b-r2.brief.md:69-71`). The coordinator snapshot each tree, then implemented 4b's fix and all of 4c and 4d directly: PRs #801, #803, #804, sqlite oracles 7 to 32 of 32 in about three hours.
- **RCA**: the slices needed reading across `_5_reify/_7_sqlite.rs`, `_6_eval/_5_evaluate.rs`, `_6_eval/_4_kernel.rs` and `sqlite_ivm/src/0b_relational.rs` to find each cause (a mutex re-entry, a kernel argument position, a stored-row semantics, a sqlite_ivm grammar gap); the model spent its cap on probes and never reached a commit. A brief rail is text the model may ignore; nothing in the harness enforces it.
- **Fail-pre-fix**: none added. The receipt is the three lane rows in `boop db mail` for those routes: no `commit` row before the `result rc=129` row on 4b and 4c.
- **Rail**: the coordinator implements a slice itself after one glm lane dies on it, instead of re-briefing. A glm lane gets a slice only when the brief can name the exact function, the exact edit, and the exact test line, and the cap is 20 minutes. Rows 114 and 115 stand.
- **Entry**: a cap protects the machine, not the work. When the work needs a diagnosis before an edit, the diagnosis belongs to the agent that can hold the four files at once, and a cheap lane gets the edit after the diagnosis is written down.
