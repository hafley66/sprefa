...
| 50 | self-racing gate script (two grade.sh in one shell line clobber target/debug; the loser writes a build error as a fixture verdict) | missing | incident 2026-08-14: opus string-arc lane chained two grade.sh invocations, the second's `cargo build` rebuilt target/debug under the first, and `emit_rust_harness: No such file or directory` landed as a per-fixture verdict in graded.tsv; re-running the write pass isolated restored the truth. Rail candidates: a lockfile at the top of grade.sh (`flock` or mkdir-lock) so a concurrent run waits or bails named, plus a graded.tsv sanity check rejecting verdicts matching a build-error shape, both unowned |
