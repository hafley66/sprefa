...
- **Rail**: `run.rs:694` calls `crate::trace::force_summary()` beside `arm()`, so a resident fold always records regardless of `DL_TRACE_SUMMARY`. `tests/tick_trace.rs` pins the wall row's `sqlite_ms > 0`. Labeling every ordered-path statement (the `verb=unlabelled` half of this incident) is `ordered.rs`, owned this cycle by `@ordered-tick-recompute` (PR #423); it lands as a follow-up once that PR is on main.
- **Entry**: live `dl6 run v6/dl/ghcache/ghcache.dl6` receipt, bucket 29791045: `ddl wall_ms=95 sqlite_ms=95 calls=1638`, `boot wall_ms=13 sqlite_ms=13 calls=408`, `recompute/page_response wall_ms=7 sqlite_ms=7 calls=2`, `recompute/pull_request_seen wall_ms=2 sqlite_ms=2 calls=2`, `recompute/gql_pull wall_ms=1 sqlite_ms=1 calls=2`.
