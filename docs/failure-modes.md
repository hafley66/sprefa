...
## 107. `--family scip` runs every indexer a polyglot root matches, so the second one spends the whole budget
...
## 108. The ts checker tier did nothing on all three held-out repos, and the SCORES row said `checker`

- **Incident** (2026-09-01): `plans/extract-eval-2026-08-31/heldout/SCORES.tsv` carries byte-identical `recall`, `precision`, `matched`, `contradicted`, `unjudged`, `ours` and `oracle_rows` for `ts.call.syntax.scip` and `ts.call.checker.scip` on all three held-out repos (umami, vite, trpc). Only `wall_ms` differs. The same harness on the tuning corpus (a TypeScript checkout) shows the two tiers diverging, so the checker binary itself works; it did not run on any held-out repo.
- **RCA**: `run.py` clones a held-out repo with `git clone --depth 1` alone and never runs `npm install` (`plans/extract-eval-2026-08-31/heldout/run.py:485`), so the clone has no `node_modules`. `loadTypeScript` (`ts_checker.mjs:14-44`) tries five candidates: an env pin (unset), `require.resolve("typescript")` from the project root (no `node_modules`, throws), `lib/typescript.js` and `built/local/typescript.js` (only a TypeScript checkout carries these), and `require.resolve("typescript")` from `process.cwd()` — which `run_capped` sets to the same project root (`ts_checker.rs:315`), so it fails identically. Every candidate misses and `ts_checker.mjs:43` throws `no typescript package reachable from ${root}`. `ts_checker.rs::answer` (pre-fix) maps that to `Err(TsCheckerError::Failed(..))` with no side channel. `project.rs::load_ts_checker:481-486` catches the `Err`, calls `tracing::info!("ts checker tier off: {err}")`, and returns `None`; the syntax leg then answers every site exactly as it would with no `--ts-checker` flag at all. `trace.rs:256` defaults the subscriber's `EnvFilter` to `"off"`, so that `tracing::info!` line never reaches stderr without `RUST_LOG` set, and `run.py:454` only inspects `stderr`/`rc` when `rc != 0` — the process exits 0 because the syntax leg's answer is a legitimate success. This is outcome (a): the driver threw, and the failure was swallowed one layer at a time until a row landed in `SCORES.tsv` labeled `checker` that never engaged the checker.
- **Fail-pre-fix**: `tests/ts_checker_reachable.rs`, `a_project_with_no_node_modules_names_its_own_failure`. A temp root with a `package.json` naming `typescript` as a `devDependency`, no `node_modules`, one `.ts` file. Pre-fix: `extract --resolve --family call --project-root <root> --ts-checker <root>/src/a.ts` exited `rc=0` with `stderr=""`. The assertion (refuse loudly or fail the process) failed with `rc=Some(0) stderr=""`.
- **Rail**: `ts_checker.rs::answer` now wraps `answer_inner` and prints one `@eprintln-ok` line straight to the process's real stderr on every `Err`, unfiltered by `RUST_LOG` or any tracing subscriber: `"ts checker tier did not run for {root}: {error}; every answer below is the syntax leg alone"`. `ts_checker.mjs`'s thrown message now names the consequence too (`"...the checker tier cannot run, every answer falls back to the syntax leg"`). Neither change touches the success path; `tests/92_ts_checker.rs`'s six tests still pass unmodified.
- **Entry**: `project.rs::load_rust_checker` and `load_ts_checker` share the "every failure is a `None`, syntax answers as before" design, and that fallback is intentional, not the defect. The defect is that the ts side's failure had NO channel a default run (no `RUST_LOG`) could see, while `load_rust_checker`'s parallel `files_answered == 0` guard (`project.rs:433-440`) at least warns at `tracing::warn!`, which is still gated the same way. Neither checker's failure reaches a harness that only checks `rc`. `run.py` and `project.rs` are out of this fix's ownership (lane boundary); the loud stderr line is the one lever `ts_checker.rs` has on its own to stop a silent measurement. A harness-side fix (treating a checker-requested-but-absent tier as a hard skip, the way `run.py:429-431` already does for the oracle) is still open.
