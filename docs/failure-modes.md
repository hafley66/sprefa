...
## 93. A bare filename source made its adapters directory the empty path

- **Incident** (2026-08-24): `cd v6/dl/ghcache && dl6 run ghcache.dl6` with a bare filename (no directory) panicked in `point_at_adapters` (`src/bin/dl6.rs:434-450`) with `read the adapters directory `. `Path::new("ghcache.dl6").parent()` returns `Some("")`, so `directory` is the empty path and `"".canonicalize()` fails before the `unwrap_or_else(|| PathBuf::from("."))` arm can fire, because parent is `Some`, not `None`. The same shape held for `args.adapters` given as a bare filename.
- **RCA**: `unwrap_or_else` guards the `None` arm of `parent()`, but a bare filename's parent is `Some("")`, the empty path, not `None`. The empty path canonicalizes to the current directory, so `.canonicalize()` on `""` fails with `No such file or directory (os error 2)` and the fallback never fires.
- **Fail-pre-fix**: `tests/dl6_run.rs::a_bare_filename_source_resolves_its_adapters_directory_to_dot` copies `query_order_tail.dl6` into a tempdir, runs `dl6 run query_order_tail.dl6` with `current_dir(tempdir)`, and asserts exit success with stderr free of `read the adapters directory`. Against HEAD it reds with `Error: read the adapters directory ` and `No such file or directory (os error 2)` (run and read, 2026-08-24).
- **Rail**: `point_at_adapters` normalizes the computed candidate after the match, mapping an empty path to `.` in one place that covers both arms: `let directory = if directory.as_os_str().is_empty() { PathBuf::from(".") } else { directory };`.
- **Entry**: `cargo test --release --test dl6_run` green with the new test passing; full engine battery and grade.sh numbers in the PR body.
