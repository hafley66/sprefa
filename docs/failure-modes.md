...
## 28. Unbounded daemon logs (launchd redirect + a spawn-only cap)

- NUMBERING NOTE: this branch (`v11`) was cut before classes 25–27 landed
  elsewhere; this entry follows the number the user assigned (28) rather than
  the next free number on this branch's own sequence, to avoid a collision
  when the branches reconcile. Renumber on merge if 25–27 land first.
...
| 28 | unbounded daemon logs (launchd redirect + spawn-only cap) | enforced | numbering gap 25–27 pending merge with the branch that landed them; OS-level `newsyslog.d` complement remains a documented recommendation, not a shipped config |
