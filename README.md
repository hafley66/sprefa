...
| `comment(open)` or `comment(open, close)` | Scope downstream matchers to comment-bounded regions |
...
## comment() -- comment-bounded region scoping
...
```sprf
# single arg: flat sequential regions (each ends at next comment or EOF)
rule(section_fns) {
  fs(**/*.rs) > comment("SECTION:") > line(re:fn\s+$NAME)
};

# two args: paired open/close (nestable)
rule(region_fns) {
  fs(**/*.rs) > comment("BEGIN:", "END:") > line(re:fn\s+$NAME)
};
...
| Form | Behavior |
|---|---|
| `comment("SECTION:")` | Each matching comment starts a region. Ends at next matching comment or EOF. |
| `comment("BEGIN:", "END:")` | Paired open/close. Nestable. Unpaired opens become point matches. |

A point match is a single comment node with no corresponding close or next comment. Downstream matchers still run against it (the comment line itself is the region).

### Comment detection

comment() operates on comment nodes from whatever parser is active:
...
The regex argument matches against the full comment text, including the comment delimiter. `comment("SECTION:")` matches `// SECTION: imports` in Rust, `# SECTION: imports` in Python, `<!-- SECTION: imports -->` in HTML.
...
| Tag completions | all tags: fs, json, ast, line, comment, md, repo, rev, branch, tag, folder, file |
