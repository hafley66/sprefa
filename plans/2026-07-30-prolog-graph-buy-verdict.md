...
## The wider sweep: what moved, what did not

Every graph-shaped traversal under `v6/prolog/` was inventoried. Two moved.
The rest were inspected and left, each for a stated reason. The post-fix
execution profile is the arbiter for the "not hot" claims: the whole
`flagship-flow.dl6` compile now samples at 0.141 s total and no traversal
below appears in its top 24 predicates.

### Moved

| site | what happened |
|---|---|
| `compile/3_clock_check.pl` | `graph_reachable/3,4` deleted, `clock_scc/3` reads `graph_cyclic_components/2`, per-path SCC recomputation hoisted |
| `labs/rel_definition_hash/0_receipts.pl` | `reachable/3,4` and `mutually_reachable/3` deleted, `relation_scc/3` reads `graph_components/2` |

### Left, with reasons

| site | shape | why it stays |
|---|---|---|
| `0_type_plane.pl` `type_topological_order/2`, `type_cycle_witness/2` | round-based Kahn over the struct-type graph | Its ORDER drives the sequence of emitted DDL, intern and render statements. `top_sort/2` breaks ties differently, so swapping it changes generated bytes, and byte identity is what the whole grading rig asserts. Separately, `type_cycle_witness/2` needs the set of names that never settled, which is cyclic nodes PLUS everything downstream of them; `graph_cyclic_components/2` returns only the cyclic ones, so this would be a semantics change, not a move. Not hot. |
| `compile/strat.pl` `kahn_order/3`, `topo_order_group/2` | Kahn over one stratum group | Same ordering-bytes hazard: `select/3` over the sorted ref list is a deliberate tie-break that fixes emitted rule order. Not hot. |
| `compile/strat.pl` `relax_strata/4` | monotone numeric stratum-number fixpoint | Not reachability, closure, SCC or topological order. `0_graph.pl` offers it nothing. It also mirrors `conformance/level_eval.pl:relax_strata/4` deliberately (the file says so), so it could only move in lockstep with the oracle. |
| `conformance/level_eval.pl` `relax_strata/4`, `plain_fixpoint/5`, `agg_loop/6` | the oracle's stratifier and its bottom-up data fixpoint | Data fixpoints over rows, not walks over an edge relation. Different kind. |
| `compile/analyze.pl` `column_type_fixpoint/5` | lattice fixpoint over column types | Type inference, not a graph walk. |
| `src/kernel.pl` `grounds/1` | AND-closure over `sugar/2` (every part must ground) | AND-closure, not OR-reachability, so no predicate here computes it. Already memoized with `:- table`. |
| `ARCH.pl` `topsort/3`, `reaches_ast/1` | task roadmap order, `refines/2` closure | Self-check tooling over a handful of checked-in facts, not the compiler. No measurable cost, real churn risk. |
| `tools/arch_map.pl` `refine_chain/2`, `ground_chain/2` | single-parent chain walks | They return the PATH, not a yes/no or a component. Different question, tiny inputs. |
| `labs/generic_scan_instantiation` `step_reaches_itself/1` | `sub_term/2` self-reference check | Despite the name, one term, one level. Not a graph walk. |
| `0_body_walk.pl` `walk_body/3` | term-tree walk over one rule body | Not a graph: no edge relation, no cycle risk, no Visited list. It is the precedent for this consolidation, not a target of it. Also owned by another lane this session. |
| `0_program_check.pl` | calls `type_cycle_witness/2`, defines no traversal | Nothing to move, and owned by another lane this session. |
| `compile/lower.pl` | calls `type_topological_order/2`, defines no traversal | Nothing to move, and owned by another lane this session. |

The honest summary of the leftovers: the toposort cluster is four hand-rolled
Kahn variants and consolidating them is a real cleanup, but every one of them
fixes an emission ORDER that the byte-identity grading depends on, none is
measurably costly, and the lane that owns two of the touching files is running
concurrently. That is a separate arc with its own before/after receipts, not a
rider on a performance fix.
