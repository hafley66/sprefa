...
|---|---|---|---|
| A1 envelope | `src/types.rs` (arms), `src/schema.rs`, `src/wire.rs`, `src/bin/extract.rs` (`--witness`) | `protocol`, `run`, `witness`, `coverage` records; `fact=` on every row under the flag; `Deserialize` on every closed arm | `tests/95_witness_wire.rs`: first row is `protocol`; goldens byte-identical without the flag; decode(encode(x)) == x over every golden |
| A2 multi-witness fold | `src/project.rs` (`type_facts`, call twin), `src/lang/ts_checker.rs`, `src/lang/rust_checker.rs` | every leg's answer is a witness; `resolution_origin` = top rank | one site answered by `same_file` and `checker`: 1 fact, 2 witness rows |
| A3 registry and reverse door | `src/tsi/{mod,types,registry,sink,ingest}.rs`, `src/bin/extract.rs` (`--ingest`, `--schema --json`) | open `fact` record; registry; `--ingest` decode, validate, canonicalize, re-emit; schemars schema | `tests/96_ingest.rs`: bad arity named; dangling id named; idempotent; foreign row gains `method=foreign` |
| A4 syntax-mode TSI rows | `src/lang/ts.rs`, `src/lang/rust.rs`, `src/lang/go.rs`, `src/lang/kotlin.rs`, `src/lang/rust_type_edges.rs` | `tsi.product/sum/edge/parameter/input/output`, `ts.optional/readonly`, `rust.impl` from explicit clauses; `coverage=partial` rows | probe fixture `.ts`: `tsi.edge(_, User, id, T, 0)` with `ts.readonly`; `tsi.edge(_, User, name, string, 1)` with `ts.optional` |
...
| A7 v7 adapter | `v7/src/0_reader/6_extract_loader.pl`, `v7/prelude/5_tsi_primitives.dl7`, `v7/test/3_extract_loader.test.pl` | `accepted/1`; TSI kernel into product nodes and `:/4`; namespaced rows as comptime relations | plunit: syntax stream alone imports `User`; adding the semantic stream with `complete` for `tsi.edge` retracts the syntax edges; `Conforms` proves over the imported struct |
| A8 shared fixture and bench | `tests/fixtures/tsi/{probe.ts,probe.rs}`, `tests/97_tsi_intersection.rs`, `tests/bench/mod.rs` (`Case.mode`), `plans/extract-bench-2026-08-29/RATCHET.tsv` | criterion 8 test; bench mode axis | the `tsi.*` row set of both streams, after canonical renumbering, is equal; `ts.*` and `rust.*` sets are disjoint and non-empty |
