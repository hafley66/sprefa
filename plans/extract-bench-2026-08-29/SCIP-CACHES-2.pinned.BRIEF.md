...
## First action
```
git merge --ff-only 66ce35e0c87be77fdcfbcff02d0eb3a2dc001fea
git merge --no-edit origin/fix/extract-scip-caches
# ^ the stalled lane's checkpoint (c11f80962): 7 files, DOC_CACHES and DEF_MAPS already moved into the index; read `git diff origin/main` first and continue from it
...
## Item 1b (added after the checkpoint)
`scip.rs` still carries `static LINE_TABLES: Mutex<Option<HashMap<LineKey, Arc<LineTable>>>>`
and `static LINE_READS: AtomicU64` from #590's `byte_range_cached`. Same
treatment: the line table lives on the document (or the content-join entry),
built once; delete the statics and `LineKey`. Receipt: `grep -c '^static' src/scip.rs` prints 0.
