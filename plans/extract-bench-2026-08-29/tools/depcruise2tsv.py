...
depcruise `resolved` paths are relative to the importing file's dir and often
end in .js even for ts sources; map back to the on-disk source file.
...
with open(out, "w") as f:
    for m in data["modules"]:
        ...
        for dep in m["dependencies"]:
            dst = os.path.abspath(os.path.join(src_dir, dep["resolved"]))
            if not os.path.isfile(dst):
                base, ext = os.path.splitext(dst)
                for cand in (base + ".ts", base + ".d.ts", base + ".tsx", base + ".cjs"):
