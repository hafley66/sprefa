...
| 11 | rust type plane is blind to macro_rules-minted types | 255 of the 418 resolution-block rows name a dst with no top-level declaration (`TraitId` 25, `FunctionId` 23, `ImplId` 17, all `impl_intern!`); ceiling 3.06 pt. CALL plane measured 2026-08-31: 2,975 of the 13,764 CodeQL call misses (21.6%) are expansion-shaped (macro-minted caller 1,928, macro-arg site 450, unspelled operator sugar 324, unspelled macro callee 273) | rust.REPORT.md sec 30 (type), sec 31 (call census) | MBE expansion exists for the CALL plane (sec 14); wiring the type plane is a design arc (span of a macro-minted decl, corpus-unique eligibility, cross-crate same-name) and needs the user; the call-plane block needs proc-macro/derive expansion, a different machine than MBE |
| 12 | rust checker tier answers too rarely on the call plane | 7,145 of the 13,764 CodeQL call misses (51.9%) drop `inferred` with the checker silent; the tier's own index logs 4,185 `unjoined` references (answer names a dst def our parse never minted) and 17,402 external | rust.REPORT.md sec 31, census `rust.call_census.py` | first task: name why `call_at` has no bound for plain method sites the walk visits (`remove_underscore.rs:63` `rename_config`) |
