...
| 8 | bench protocol forks from prior-art study | BOTH LANDED (PR #618): `resolution_origin` is a closed enum on every `resolved_edge` / `resolved_type_edge`, the ratchet prints per-origin row counts per corpus, and each oracle row carries a matched / contradicted / unjudged split beside flat precision. The 4-column match protocol is unchanged and no RATCHET.tsv cell moved. | PRIOR-ART.md sec 3, `plans/extract-eval-2026-08-31/PLAN.md` sec 4, `src/types.rs` `ResolutionOrigin`, `tests/bench/mod.rs` `buckets` | closed; the open follow-up is which origins the held-out ratchet should floor separately |
