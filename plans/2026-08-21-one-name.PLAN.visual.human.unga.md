...
## 11. The fork closed itself: a cadence is a SEED, not a keyword

`bind` needed no replacement keyword. An arrival rel is demanded BY a positive
body, so the cadence is a fact the program owns, exactly like `want(Glob)`
demands `/soopy/files`:
...
Two compiler rules forced this shape, and both are the right rules:

| stop | what it means |
|---|---|
| `probe_mismatch(multiple_probes(...))` (`1_host_expand.pl:445`) | one rule body carries at most ONE arrival goal, so the turn becomes its own rel and every reader joins it |
| `level_rule_no_positive_body` on `__host_demand_clock__tick/3` | an arrival rel with nothing demanding it is not a program, it is a keyword in disguise |

Five programs moved, and all five now compile to a binary through `dl6 build`:

| program | was | is |
|---|---|---|
| `v6/dl/prwatch/prwatch.dl6` | `bind interval`, `sh pulls`, `sh cost` | `/clock/tick`, `/gh/pulls`, `/dl/tick_cost` |
| `v6/dl/fixtures/served-watch-rail.dl6` | `rel watch(...)`, inert (my regression) | `want` + `/soopy/watch` |
| `v6/dl/fixtures/files-rev-walk.dl6` | `sh files`, `sh files_at` | `/soopy/files`, `/soopy/files_at` |
| `tests/fixtures/tick_cost_beat.dl6` | `bind interval`, `sh cost` | `/clock/tick`, `/dl/tick_cost` |
| `tests/fixtures/interval_beat.dl6` | `bind interval` | `/clock/tick` |
| `tests/fixtures/watch_module_defs.dl6` | `bind watch`, `sh call_node_at` | `/soopy/watch`, `/extract/call_node_at` |

`bind` and `sh` are gone from every live program. Every `.adapters.json` the
collapse reached is deleted.

## 12. The one thing left, and it is not mine

`/clock/tick` and `/soopy/watch` have NO Rust executor. `run.rs:753` still arms
the watcher off a bindPlan, and there is no bindPlan any more, so the two
resident tests in `tests/dl6_run.rs` fail. `run.rs` and `dl6.rs` belong to the
ghcache lane under the one-server-one-db decision, so the arming change is
theirs. What they need is small and now fully specified:

| need | where |
|---|---|
| `executors/clock.rs`, `executors/watch.rs` | new files |
| two `executor_for` arms + two `arrival_executor/2` rows | `hosts.rs`, `registry.pl` |
| arm the cadence off an ARRIVAL rel, not `options.binds` | `run.rs:753` |

Until then a program naming `/clock/tick` is a named stop at construction that
prints the roster, which is the correct "not built yet" and not silence.
