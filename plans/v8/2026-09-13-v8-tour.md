...
#728 #729 #730 #731 #732 #733 merged into it. Whole corpus compiles 6.3x
faster than v7 at the median (3.5 s against 23.2 s, release binary). Each lane PR is still open against it, so you
...
flowchart LR
  ...
  C --> K[_4_comptime<br/>rounds, Sources seam]
  K --> Y[_5_reify<br/>logical rows]
...
| `_4_comptime` rounds | 2275 | 7 | 8 of 128 checked, 331 calls match | `2_comptime/2_compiler.pl:700-1591`, `1a 1d` | #732 |
| `_5_reify` | 1586 | 8 | 57 of 88 checked | `3_emit/0 0a 1`, `0_evaluator.pl:557-646` | #731 |
| `_6_eval` | 1553 | 8 | 32 | `1_libtime/0_evaluator.pl` | #725 |
| `_8_driver` + `lib.rs` | 620 | 7 | 46 compile cases, 46 match | `2_compiler.pl:75-700` | #733 |
| `tests/` | 1000 | 9 | | | |
...
cargo run -q --release -- compile ../v7/test/fixtures/2_partial.dl7 --trace   # the whole language, 110 ms
cargo run -q -- --help                                 # compile read expand lower check load comptime reify eval
...
|---|---|---|---|
| 1 | `v8/src/_6_eval/_0_term.rs` | 263 | `Term`, `Universe`: hash-consed arena, `TermId` is a `u32`, `cmp` is SWI standard order |
| 2 | `v8/src/_6_eval/_1_program.rs` | 69 | `Arg`, `Goal`, `Rule`, `Row`, `Diagnostic`, `Program`: the v7 `rule/2` term as Rust |
| 3 | `v8/src/_6_eval/_3_table.rs` | 98 | `Table`: append-only rows, frontier, per-column index, `Range::{Old,Delta,All}` |
| 4 | `v8/src/_6_eval/_4_kernel.rs` | 193 | the six kernel relations `nil cons edge_ref intern int_*` |
| 5 | `v8/src/_6_eval/_5_evaluate.rs` | 555 | `evaluate`: stratify, per level rounds, `demand`, `fire`, `Trace` sink |
| 6 | `v8/src/_2_lower/_0_api.rs` | 210 | `Unit`, `Environment`, `Lowered`, `lower_datalog` signature |
...
| 10 | `v8/src/_1_macrotime/_4_expand.rs` | 142 | one `<+` wave, `Wave` enum, the pause point comment |
...
| 13 | `v8/src/bin/dl8.rs` | 94 | the one boundary: file in, JSON out, exit code |
...
- `plans/v8/2026-09-12-v8-comptime.PLAN.visual.human.unga.md`
- `plans/v8/2026-09-12-v8-reify.PLAN.visual.human.unga.md`
...
Where a whole `dl8 compile` spends its time (`RUST_LOG=dl8=info HAFLEY_LOG_FORMAT=json`, release, warm):

| phase | `0_minimal` | `2_partial` | `5_curry` |
|---|---|---|---|
| read, 6 prelude files + the file | 11 ms | 12 ms | 12 ms |
| macrotime | 3 ms | 3 ms | 5 ms |
| lower | 2 ms | 2 ms | 4 ms |
| check | | 2 ms | 1 ms |
| comptime | | 38 ms | 413 ms |
| process total | 48 ms | 113 ms | 452 ms |

42 files at one process each is the 3.5 s. Inside comptime on `5_curry` (macOS `sample`, 331 samples): `Eval::solve` 272, `cmp` + `prolog_sort` 85, SipHash on `TermId` keys 44 (61 `HashMap` sites), prelude re-lowered per refreeze 40.
...
| comptime | none | rounds, `Sources::refreeze` is the callback seam, live impl in `_7_sources.rs` | `fx(Round)`; 32 rounds on `5_curry` |
| driver | `_8_driver/_0_read.rs` reads files, prelude via `include_str!` | `lib.rs::compile`, 130 lines | all three sinks behind `--trace` |
...
| Y1 | reify | `0_logical_program_reifier.pl:113` vs `0a:115` | `reference` target wrapped on the calls path, bare on the graph path |
| Y2 | reify | `:162` vs `0a:63` | polarity is a string on one path and an atom on the other |
| O1 | dl8 | PR #734 | done: `tracing` everywhere, 0 `eprintln!`, `hafley-observe` from the local Kellnr registry `hafley` on `127.0.0.1:8000`. Kellnr is a `nohup` process, a launchd plist is your call |
| V1 | v7 | `2_compiler.pl:833` | v7 at `f5018ad23` fails `5_curry.dl7` with `source_refreeze_limit_exhausted(16)`; three entrypoint tests red; the oracle for that fixture is missing until v7 is fixed |

## 10. Not built yet

| thing | v7 source | who |
|---|---|---|
| `1b_compiler_tracer.pl` histograms and timing | 777 lines | the `--trace` sinks exist, no aggregation |
| `emit_compiled(prolog(Callable))` arm | `1_artifact_emitter.pl:49-59` | host-Prolog callable, no Rust shape, exit 3 |
| `compile_dl7_macro_program/3` verb, `project_stream_paths/3` | `2_compiler.pl:86`, `:254` | driver lane left them |
