...
| O1 | dl8 | `bin/dl8.rs:105 :129`, `_6_cli.rs:171` | no `tracing`; 30 `eprintln!` under `@eprintln-ok`. boop uses `hafley_observe::init_with_writer` (`hafley-rs/crates/boop/src/main.rs:793`). Sinks already emit values, so wiring is `dl8.rs` only: git dep on hafley-rs, or copy `hafley-observe/src/{0_types,1_format,1_init}.rs` |
