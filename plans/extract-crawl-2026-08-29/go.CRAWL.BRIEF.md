# Brief: entrypoint crawl of typescript-go with sprefa-extract (lane `crawl-extract-typescript-go`)
...
`v6/sprefa-extract/tests/fixtures/go_findings/` with the expected fact
...
```
Failure: STOP, `boop beep --no-wait --as crawl-extract-typescript-go sprefa-coordinator "<one line>"`.
Binary: `v6/sprefa-extract/target/release/extract` in your worktree.
Corpus: `/Users/chrishafley/projects/typescript-go` (shallow clone, read-only;
never modify it, never run its build except where step 4 says).
Scratch: `/private/tmp/claude-501/-Users-chrishafley-projects-sprefa/efe90f72-55fb-4958-b159-4982236661fe/scratchpad/crawl-go`.
...
3. **Entrypoint crawl.** Entrypoints: `cmd/tsgo/main.go` and every `func main` under `cmd/`, plus every exported func in `internal/compiler/program.go`; `_test.go` funcs counted as a second root set. `scip-go` is not installed: `go install github.com/sourcegraph/scip-go/cmd/scip-go@latest` with GOBIN in scratch, then point extract at it (see `--help` for how the indexer is located); record the scip_skip row verbatim if that fails. Go interface dispatch is the expected big miss class: count call sites whose receiver is an interface type (grep `interface {` types, then sites on those method names) and report them as their own kink row.
   ...
   under `plans/extract-crawl-2026-08-29/go.crawl.py`). Record: reachable
...
4. **scip comparison** where the toolchain exists (go: see
...
5. **Kinks.** Every classified miss from steps 2-4 with a count in the
   corpus, its cause, the `src/lang/go*` site you believe owns it
   (cite the fn), and a repro fixture. Table
   `class | count | example file:line | owner fn | fixture`.

## Deliverables (commit, push, PR)
- `plans/extract-crawl-2026-08-29/go.REPORT.md`: TOC, then one table
  per step, then the kinks table, then "untested and why".
- `plans/extract-crawl-2026-08-29/go.PLAN.visual.human.unga.md`: the same
...
- `go.runs.tsv`, `go.crawl.py`, fixtures as above.
- `gh pr create --base main`, then
  `boop beep --no-wait --as crawl-extract-typescript-go sprefa-coordinator "go crawl: PR #N, files=F, resolved=R, reachable=X/Y, kinks=K"`.
