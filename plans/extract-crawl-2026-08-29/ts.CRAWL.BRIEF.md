# Brief: entrypoint crawl of TypeScript with sprefa-extract (lane `crawl-extract-typescript`)
...
`v6/sprefa-extract/tests/fixtures/ts_findings/` with the expected fact
...
```
Failure: STOP, `boop beep --no-wait --as crawl-extract-typescript sprefa-coordinator "<one line>"`.
Binary: `v6/sprefa-extract/target/release/extract` in your worktree.
Corpus: `/Users/chrishafley/projects/TypeScript` (shallow clone, read-only;
never modify it, never run its build except where step 4 says).
Scratch: `/private/tmp/claude-501/-Users-chrishafley-projects-sprefa/efe90f72-55fb-4958-b159-4982236661fe/scratchpad/crawl-ts`.
...
3. **Entrypoint crawl.** Entrypoints: the compiler and tsserver mains: `src/tsc/tsc.ts`, `src/tsserver/server.ts`, `src/typescript/typescript.ts` exports, and every `export function` in `src/compiler/program.ts` `createProgram`; `src/testRunner/**` counted as a second root set. Exclude `tests/baselines/**` and `tests/cases/**` from every step (they are not the program); include them ONLY in step 1 as a separate row (they are a huge parser stress corpus: report rc!=0/timeouts there separately). `scip-typescript` is at `~/.nvm/versions/node/v24.15.0/bin/scip-typescript`.
   ...
   under `plans/extract-crawl-2026-08-29/ts.crawl.py`). Record: reachable
...
- `plans/extract-crawl-2026-08-29/ts.REPORT.md`: TOC, then one table
  per step, then the kinks table, then "untested and why".
- `plans/extract-crawl-2026-08-29/ts.PLAN.visual.human.unga.md`: the same
...
- `ts.runs.tsv`, `ts.crawl.py`, fixtures as above.
- `gh pr create --base main`, then
  `boop beep --no-wait --as crawl-extract-typescript sprefa-coordinator "ts crawl: PR #N, files=F, resolved=R, reachable=X/Y, kinks=K"`.
