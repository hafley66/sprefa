...
//! `book/src/*.md` opens with `; fixture: <path>[:first-last]`, then
...
fn chapters() -> Vec<PathBuf> {
    let mut out: Vec<PathBuf> = std::fs::read_dir(root().join("book/src"))
        .unwrap()
...
fn marked_blocks(chapter: &Path) -> Vec<MarkedBlock> {
    ...
    while let Some(line) = lines.next() {
        ...
        let spec = marker
            ...
            .unwrap_or_else(|| panic!("{name}: dl7 block without a fixture marker: {marker}"));
...
pub fn every_block_equals_its_fixture() {
    let (code, stdout, stderr) =
        finish(Command::new("bash").arg(root().join("book/check_blocks.sh"))).unwrap();
    assert_eq!(
        code,
        0,
        "{}{}",
...
fn chapter_holds(stem: &str) {
    let chapter = root().join("book/src").join(format!("{stem}.md"));
    let blocks = marked_blocks(&chapter);
    let mut failures = in_lanes(&blocks, check_block);
    let outputs = finish(
        Command::new("bash")
            .arg(root().join("book/check_outputs.sh"))
            .arg(&chapter)
...
pub fn diagram_chain_is_continuous() {
    let (code, stdout, stderr) =
        finish(Command::new("bash").arg(root().join("book/check_chain.sh"))).unwrap();
    assert_eq!(
        code,
        0,
        "{}{}",
...
pub fn mdbook_builds_the_book() {
    ...
    assert!(
        root().join("book/book/index.html").exists(),
