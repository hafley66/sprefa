...
fn nonzero_exit_with_stdout_is_findings() {
    ...
    assert_eq!(
        code, 0,
        "diff-tool convention: exit 1 + stdout is not an error: {err}"
    );
...
fn broken_command_is_loud() {
    ...
    assert_eq!(
        code, 1,
        "exit-127 with no stdout must be a program error:\n{err}"
    );
...
fn backtick_shell_body_skips_quote_escaping() {
    ...
    assert_eq!(
        code, 0,
        "raw backtick shell body must run unescaped:\n{err}"
    );
    assert!(
        out.contains("quoted value") && out.contains("here"),
...
fn cmd_budget_caps_invocations() {
    ...
    let over = Command::new(DL)
        .arg(d.join("p.dl"))
        .current_dir(&d)
        .args(["--db", d.join("db1").to_str().unwrap(), "--cmd-budget", "2"])
        .output()
        .expect("run dl");
    ...
    let under = Command::new(DL)
        .arg(d.join("p.dl"))
        .current_dir(&d)
        .args(["--db", d.join("db2").to_str().unwrap(), "--cmd-budget", "3"])
        .output()
        .expect("run dl");
    assert_eq!(
        under.status.code(),
        Some(0),
        "3 files at budget 3 runs clean: {}",
