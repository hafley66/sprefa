...
struct Sandbox { home: PathBuf, root: PathBuf }
...
impl Sandbox {
    ...
    fn sock(&self) -> PathBuf { sprefa_v5::daemon::socket_path_for(&self.home.join("sprefa")) }
    fn spawn(&self, program: &Path) -> DaemonGuard {
        DaemonGuard(Command::new(DL)
            .args(["daemon", "start", "--foreground"]).arg(program)
            .current_dir(&self.root).env("DL_DAEMON_ROOT", &self.root).env("XDG_STATE_HOME", &self.home).env("SPREFA_CONFIG", "/nonexistent/sprefa-hermetic.toml")
    ...
    fn wait_ready(&self) -> bool {
        for _ in 0..200 {
            if self.sock().exists()
                && rpc_root(&self.sock(), 1, "ping", &self.root).is_some() { return true; }
...
fn rpc_root(sock: &Path, id: u64, method: &str, root: &Path) -> Option<serde_json::Value> {
    let body = serde_json::json!({"jsonrpc":"2.0","id":id,"method":method,
        "params":{"root": root.to_string_lossy()}}).to_string();
    std::panic::catch_unwind(|| rpc(sock, &body)).ok()
        .and_then(|r| serde_json::from_str(&r).ok())
}

fn tick_count(sock: &Path, root: &Path) -> u64 {
    rpc_root(sock, 5, "ping", root).map(|v| v["result"]["tick_count"].as_u64().unwrap_or(0)).unwrap_or(0)
}
...
fn mcp_pumps_through_daemon() {
    ...
    assert!(tick_count(&sb.sock(), &sb.root) > ticks_before,
            "daemon tick_count must advance when it pumps the request");

    let _ = rpc(&sb.sock(), r#"{"jsonrpc":"2.0","id":9,"method":"shutdown","params":{}}"#);
    let _ = daemon.wait();
}
...
fn daemon_refuses_non_port_rel() {
    ...
    let body = serde_json::json!({"jsonrpc":"2.0","id":2,"method":"mcp_request","params":{
        ...
        "in_rel":"plain","out_rel":"plain","id":"1","method":"ping","params":"null"}}).to_string();
