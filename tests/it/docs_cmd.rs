...
fn docs_lists_topics() {
    ...
    assert!(out.contains("dl docs book 3"), "addressing hint missing: {out}");
}
...
fn docs_reference_page_prints_heading() {
    ...
    assert_eq!(code, 0, "{err}");
    assert!(out.contains("# Syntax & semantics"), "syntax heading missing: {out}");
}
...
fn docs_book_index() {
    ...
    assert_eq!(code, 0, "{err}");
    assert!(out.contains("# Reactive Facts"), "book index heading missing: {out}");
}
...
fn docs_chapter_by_number_and_slug() {
    ...
    assert!(out_num.contains("# 3. The cycle problem"), "chapter-by-number missing: {out_num}");

    let (code_slug, out_slug, err_slug) = run(&["docs", "03-the-cycle-problem"]);
    assert_eq!(code_slug, 0, "{err_slug}");
    assert_eq!(out_num, out_slug, "number and slug should print the same chapter");
}
...
fn docs_unknown_topic_exits_one() {
    ...
    assert!(err.contains("unknown topic"), "no unknown-topic message: {err}");
    assert!(out.contains("REFERENCE"), "topic list not re-printed: {out}");
}
