...
fn maxby_keeps_highest_priority_row() {
    ...
    assert!(
        out.contains("slow-but-best"),
        "MaxBy(prio) must keep the prio=5 row:\n{out}"
    );
    assert!(
        !out.contains("fast"),
        "the prio=1 row must lose to prio=5:\n{out}"
    );
    assert!(
        out.contains("5"),
        "the winning prio value must surface:\n{out}"
    );
}
...
fn key_alone_is_choice_domain_first_wins() {
    ...
    assert!(
        out.contains("(1 rows)"),
        "key(id) must collapse to one row:\n{out}"
    );
}
...
fn merge_without_key_is_an_error() {
    ...
    assert!(
        err.contains("merge") || err.contains("key"),
...
fn maxby_promotes_on_warm_db() {
    ...
    assert!(
        out.contains("v2"),
        "warm db must promote the prio=9 row:\n{out}"
    );
    assert!(
        !out.contains("v1"),
        "the prio=1 incumbent must be displaced:\n{out}"
    );
}
...
fn add_key_on_warm_db_does_not_wedge() {
    ...
    assert_eq!(
        code, 0,
        "adding key(...) on a warm db must not wedge:\n{err}"
    );
    assert!(!err.contains("ON CONFLICT"), "no ON CONFLICT wedge:\n{err}");
    assert!(
        out.contains("hi"),
        "MaxBy(prio) must keep the prio=9 row:\n{out}"
    );
    assert!(
        !out.contains("lo"),
        "the prio=1 row must lose after the key is added:\n{out}"
    );
    assert!(
        out.contains("(1 rows)"),
        "key(id) must collapse to one row:\n{out}"
...
fn remove_key_on_warm_db_widens_pk() {
    ...
    assert_eq!(
        code, 0,
        "removing key(...) on a warm db must not wedge:\n{err}"
    );
    assert!(
        out.contains("(2 rows)"),
        "full-row PK keeps both rows:\n{out}"
    );
}
...
fn change_key_columns_on_warm_db() {
    ...
    assert!(
        out.contains("(1 rows)"),
        "key(id) collapses both shards to one row:\n{out}"
    );
    ...
    assert_eq!(
        code, 0,
        "changing the key column set on a warm db must not wedge:\n{err}"
    );
    assert!(
        out.contains("(2 rows)"),
        "key(id, shard) splits into two groups:\n{out}"
    );
    assert_eq!(
        out.matches("hi").count(),
        2,
        "each shard keeps its hi row:\n{out}"
    );
}
...
fn change_merge_fn_only_on_warm_db() {
    ...
    assert_eq!(
        code, 0,
        "merge-fn-only change on a warm db must not wedge:\n{err}"
    );
    assert!(
        !err.contains("ON CONFLICT"),
        "merge-fn change is schema-invariant, no wedge:\n{err}"
    );
    assert!(
        out.contains("(1 rows)"),
        "still one row per key (table not dropped):\n{out}"
    );
}
