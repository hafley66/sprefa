...
fn async_request_drains_and_lands_response() {
    ...
    let pend = rows(
        &dbp,
        "pending_effect",
        "SELECT kind, args_json, done FROM pending_effect ORDER BY args_json",
    );
    assert_eq!(pend.len(), 2, "one pending_effect per want row");
    assert!(
        pend.iter().all(|r| r[0] == "resp" && r[2] == "0"),
        "queued, not done: {pend:?}"
    );
    assert_eq!(
        rows(&dbp, "rel_resp_txt", "SELECT * FROM rel_resp_txt").len(),
        0,
        "no response before drain"
    );
    ...
    let exec = MockExec {
        table: HashMap::from([
            ...
            (
                "https://api/about".to_string(),
                vec!["404".to_string(), "NOPE".to_string()],
            ),
        ]),
    ...
    let mut got = rows(
        &dbp,
        "rel_resp_txt",
        "SELECT key, status, body FROM rel_resp_txt ORDER BY key",
    );
    got.sort();
    assert_eq!(
        got,
    ...
    assert_eq!(
        rows(&dbp, "rel_ok_txt", "SELECT key FROM rel_ok_txt"),
        vec![vec!["home".to_string()]]
    );
...
fn async_request_is_idempotent_across_ticks() {
    ...
    assert_eq!(
        rows(
            &dbp,
            "pending_effect",
            "SELECT COUNT(*) FROM pending_effect"
        ),
        vec![vec!["1".to_string()]],
...
fn shell_effect_exec_runs_real_subprocess() {
    ...
    assert_eq!(
        arity.get("resp"),
        Some(&2),
        "status + body are the two unbound slots"
    );
    ...
    assert_eq!(
        rows(
            &dbp,
            "rel_resp_txt",
            "SELECT key, status, body FROM rel_resp_txt"
        ),
        vec![vec![
            "home".to_string(),
            "200".to_string(),
            "api/home-body".to_string()
        ]]
    );
}
...
fn drain_runs_executors_in_parallel() {
    ...
    assert_eq!(
        rows(
            &dbp,
            "pending_effect",
            "SELECT done FROM pending_effect WHERE done = 0"
        )
        .len(),
        0
    );
...
fn sh_decl_supplies_effect_template() {
    ...
    assert_eq!(
        templates.get("resp").map(String::as_str),
        Some("printf '200\\n%s-body' '{url}'")
    );
    let exec = ShellEffectExec {
        templates,
...
fn body_effect_call_site_drains_with_full_env() {
    ...
    let pend = rows(
        &dbp,
        "pending_effect",
        "SELECT kind, head_rel, args_json FROM pending_effect",
    );
    ...
    assert!(
        pend[0][2].contains("\"r\":\"octo\"") && pend[0][2].contains("\"p\":\"README\""),
        "hole map keyed by sh params r/p: {}",
        pend[0][2]
    );
    ...
    assert_eq!(
        rows(
            &dbp,
            "rel_resp_txt",
            "SELECT repo, path, tag, status, body FROM rel_resp_txt"
        ),
        vec![vec![
            "octo".to_string(),
            "README".to_string(),
...
fn stream_subscription_fans_lines_and_stays_running() {
    ...
    let mut got = rows(
        &dbp,
        "rel_event_txt",
        "SELECT repo, kind, at FROM rel_event_txt ORDER BY kind",
    );
    got.sort();
    assert_eq!(
        got,
    ...
    eng.tick(&prog, true).unwrap();
    assert_eq!(
        rows(&dbp, "rel_event_txt", "SELECT COUNT(*) FROM rel_event_txt"),
        vec![vec!["2".to_string()]]
    );
    assert_eq!(
        rows(&dbp, "pending_effect", "SELECT state FROM pending_effect"),
...
fn effect_arity_and_hole_are_checked() {
    ...
    assert!(
        codes.contains(&"effect-arity"),
        "want effect-arity, got {codes:?}"
    );
    assert!(
        codes.contains(&"unused-hole"),
        "want unused-hole, got {codes:?}"
    );
}
...
fn mutating_effect_fires_exactly_once_across_a_crash() {
    ...
    // never committed (done=0). The reconcile must leave it alone.
    {
        ...
        c.exec_on(
            "pending_effect",
            "UPDATE pending_effect SET state = 'running', done = 0",
        )
        .unwrap();
    }
    assert_eq!(
...
fn collect_batches_one_request_and_fans_response() {
    ...
    assert_eq!(
        rows(
            &dbp,
            "pending_effect",
            "SELECT args_json, batch FROM pending_effect"
        ),
    ...
    assert_eq!(
        eng.drain_effects(&prog, &exec).unwrap(),
        1,
        "one batch request drained"
    );
    eng.tick(&prog, true).unwrap();
    let mut got = rows(
        &dbp,
        "rel_star_txt",
        "SELECT key, val FROM rel_star_txt ORDER BY key",
    );
    got.sort();
    assert_eq!(
        got,
...
fn collect_chunk_caps_batched_request_size() {
    ...
    assert_eq!(
        pend.len(),
        3,
        "ceil(7/3)=3 requests, not 1 (whole-set) and not 7 (per-row)"
    );
    assert!(
        pend.iter().all(|r| r[1] == "1"),
        "every request is batch=1: {pend:?}"
    );
...
fn effect_log_mirrors_the_drain_queue() {
    ...
    let log = rows(
        &dbp,
        "rel_effect_log_txt",
        "SELECT kind, head, state FROM rel_effect_log_txt",
    );
    assert_eq!(
        log.len(),
        1,
        "one effect_log row for the one request: {log:?}"
    );
    assert_eq!(
        log[0],
        vec!["resp".to_string(), "resp".to_string(), "queued".to_string()]
    );
    assert_eq!(
        rows(&dbp, "rel_queued", "SELECT id FROM rel_queued").len(),
        1,
        "queued rail fired"
    );
    assert_eq!(
        rows(&dbp, "rel_landed", "SELECT id FROM rel_landed").len(),
        0,
        "nothing done yet"
    );
    ...
    eng.drain_effects(&prog, &exec).unwrap();
    eng.tick(&prog, true).unwrap();
    let state: Vec<Vec<String>> = rows(
        &dbp,
        "rel_effect_log_txt",
        "SELECT state FROM rel_effect_log_txt",
    );
    assert_eq!(
        state,
        vec![vec!["done".to_string()]],
        "drain flipped the state"
    );
    assert_eq!(
        rows(&dbp, "rel_queued", "SELECT id FROM rel_queued").len(),
        0,
