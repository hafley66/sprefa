...
fn rust_comments_line_block_doc_and_string_safety() {
    ...
    let out = run(&d, PROG);
    // kinds
    assert!(out.contains("a line comment\tline"), "rust line kind:\n{out}");
    ...
    assert!(out.contains("trailing inline\tline"), "rust inline trailing:\n{out}");
    // string-literal safety: `// not a comment` must NOT be a row.
    assert!(!out.contains("not a comment"), "rust string leaked as comment:\n{out}");
}

#[test]
fn ts_comments_via_oxc_and_string_safety() {
    let d = sandbox("ts");
    fs::write(d.join("src/a.ts"),
        "// ts line\n\
    ...
    assert!(out.contains("inline ts\tline"), "ts inline trailing:\n{out}");
    assert!(out.contains("jsdoc block\tdoc"), "ts jsdoc doc kind:\n{out}");
    assert!(!out.contains("string not comment"), "ts string leaked:\n{out}");
}
...
fn kotlin_comments_and_string_safety() {
    ...
    assert!(out.contains("inline kt\tline"), "kotlin inline trailing:\n{out}");
...
fn generic_treesitter_lang_python_string_safety() {
    let d = sandbox("py");
    fs::write(d.join("src/util.py"),
        "# python comment\n\
         x = \"# not a comment\"  # trailing py\n").unwrap();
    let out = run(&d, PROG);
    assert!(out.contains("python comment\tline"), "python line kind:\n{out}");
    assert!(out.contains("trailing py\tline"), "python inline trailing:\n{out}");
    assert!(!out.contains("not a comment"), "python string leaked:\n{out}");
}
