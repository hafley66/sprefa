...
fn engine_over(dir: &PathBuf, program_text: &str) -> (Engine, sprefa_v5::ast::Program) {
    ...
    let errs: Vec<_> = diags.iter().filter(|d| d.severity == sprefa_v5::ast::Severity::Error).collect();
...
fn i(v: &serde_json::Value) -> i64 { v.as_i64().unwrap_or_default() }
...
fn union_carries_every_edge_common_carries_only_the_shared_spine() {
    ...
    feed(&mut eng, "take-1", 1000,
        &goto_json(None, ("src/flow.rs", CHECKOUT_START_LINE), "checkout_start"));
    feed(&mut eng, "take-1", 1001,
        &goto_json(Some(("src/flow.rs", CHECKOUT_START_LINE)), ("src/flow.rs", VALIDATE_CART_LINE), "validate_cart"));
    feed(&mut eng, "take-1", 1002,
        &goto_json(Some(("src/flow.rs", VALIDATE_CART_LINE)), ("src/flow.rs", APPLY_DISCOUNT_LINE), "apply_discount"));

    // take-2: checkout_start -> validate_cart only.
    feed(&mut eng, "take-2", 2000,
        &goto_json(None, ("src/flow.rs", CHECKOUT_START_LINE), "checkout_start"));
    feed(&mut eng, "take-2", 2001,
        &goto_json(Some(("src/flow.rs", CHECKOUT_START_LINE)), ("src/flow.rs", VALIDATE_CART_LINE), "validate_cart"));
    ...
    assert!(union_pairs.iter().any(|(a, b)| a.contains("checkout_start") && b.contains("validate_cart")),
        "union has checkout_start->validate_cart: {union_pairs:?}");
    assert!(union_pairs.iter().any(|(a, b)| a.contains("validate_cart") && b.contains("apply_discount")),
    ...
    assert!(common_pairs.iter().any(|(a, b)| a.contains("checkout_start") && b.contains("validate_cart")),
        "common has checkout_start->validate_cart: {common_pairs:?}");
    assert!(!common_pairs.iter().any(|(a, b)| a.contains("validate_cart") && b.contains("apply_discount")),
...
fn unnamed_session_defaults_to_its_own_flow() {
    ...
    feed(&mut eng, "solo", 1000,
        &goto_json(None, ("src/flow.rs", CHECKOUT_START_LINE), "checkout_start"));
    feed(&mut eng, "solo", 1001,
        &goto_json(Some(("src/flow.rs", CHECKOUT_START_LINE)), ("src/flow.rs", VALIDATE_CART_LINE), "validate_cart"));
...
fn hover_note_lands_at_zero_based_member_lines() {
    ...
    feed(&mut eng, "hovertake", 1000,
        &goto_json(None, ("src/flow.rs", CHECKOUT_START_LINE), "checkout_start"));
    feed(&mut eng, "hovertake", 1001,
        &goto_json(Some(("src/flow.rs", CHECKOUT_START_LINE)), ("src/flow.rs", VALIDATE_CART_LINE), "validate_cart"));
    ...
    assert_eq!(i(&rows[0][1]), 0, "checkout_start's note lands at 0-based line 0: {rows:?}");
    ...
    assert!(s(&rows[0][5]).contains("in flow:"), "the note text: {:?}", s(&rows[0][5]));
    assert!(s(&rows[0][5]).contains("onboarding"), "names the flow: {:?}", s(&rows[0][5]));

    // validate_cart: decl line 5 (1-based) -> hover_note line 4.
    assert_eq!(i(&rows[1][1]), 4, "validate_cart's note lands at 0-based line 4: {rows:?}");
    assert!(s(&rows[1][5]).contains("onboarding"), "names the flow: {:?}", s(&rows[1][5]));
}
...
fn jump_outside_every_span_is_a_silent_lift_miss() {
    ...
    feed(&mut eng, "misstake", 1000,
        &goto_json(None, ("src/flow.rs", CHECKOUT_START_LINE), "checkout_start"));
    feed(&mut eng, "misstake", 1001,
        &goto_json(Some(("src/flow.rs", CHECKOUT_START_LINE)), ("src/flow.rs", MISS_LINE), "nothing_here"));
