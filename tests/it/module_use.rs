...
fn diamond_import_loads_each_module_once() {
    ...
    fs::write(d.join("d.dl"), "rel shared(x: int).\nshared(1).\nshared(2).\n").unwrap();
    fs::write(d.join("b.dl"), "use \"d.dl\".\nrel b_only(x: int).\nb_only(10).\n").unwrap();
    fs::write(d.join("c.dl"), "use \"d.dl\".\nrel c_only(x: int).\nc_only(20).\n").unwrap();
    ...
    assert!(sh.contains("(2 rows)"), "shared dedups to two rows (loaded once): {out}");
...
fn same_name_same_cols_dedup() {
    ...
    assert!(out.contains("100") && out.contains("1"), "both facts present: {out}");
    assert!(out.contains("(2 rows)"), "two facts total after dedup: {out}");
}
...
fn use_requires_string_literal() {
    ...
    assert!(err.contains("in "), "parse error carries file context: {err}");
}
