...
const spread = { ...routes, extra: '/extra' };\n",
    ...
    .unwrap();
    fs::write(root.join("consts.rs"), "const HOME: &str = \"/home\";\n").unwrap();
    fs::write(
        root.join("p.dl"),
        "rel src(p: file).\n\
...
fn object_literal_const_folds_dotted_fields_and_mints_entity() {
    ...
    let routes_ent = entities
        .iter()
        .find(|r| r[2] == "routes")
        .unwrap_or_else(|| panic!("no routes entity: {entities:?}"));
    assert_eq!(
        routes_ent[3], "const",
        "routes must be a const-kind type_entity: {routes_ent:?}"
    );
    ...
    let home = consts
        .iter()
        .find(|r| r[1] == routes_sym && r[2] == "home")
        .unwrap_or_else(|| panic!("no routes.home row: {consts:?}"));
    assert_eq!(home[3], "/home");
    assert_eq!(home[4], "lit");
    let nested = consts
        .iter()
        .find(|r| r[1] == routes_sym && r[2] == "nested.a")
        .unwrap_or_else(|| panic!("no routes.nested.a row: {consts:?}"));
...
fn template_const_kind_and_string_enum_member() {
    ...
    let greeting_ent = entities
        .iter()
        .find(|r| r[2] == "greeting")
        .expect("greeting entity");
    let greeting_row = consts
        .iter()
        .find(|r| r[1] == greeting_ent[1])
        .expect("greeting const_value row");
    assert_eq!(greeting_row[4], "template");
    assert_eq!(
        greeting_row[3], "`hi ${routes.home}`",
        "holes stay intact: {greeting_row:?}"
    );

    let kind_ent = entities
        .iter()
        .find(|r| r[2] == "Kind")
        .expect("Kind enum entity");
    assert_eq!(kind_ent[3], "enum");
    let member_a = consts
        .iter()
        .find(|r| r[1] == kind_ent[1] && r[2] == "A")
        .expect("Kind.A row");
    assert_eq!(member_a[3], "alpha");
    let member_b = consts
        .iter()
        .find(|r| r[1] == kind_ent[1] && r[2] == "B")
        .expect("Kind.B row");
...
fn let_binding_excluded_spread_property_counted() {
    ...
    let consts = eng.rel_rows("const_value", 7);
    assert!(
        !entities.iter().any(|r| r[2] == "mutablePath"),
        "let binding must not mint an entity: {entities:?}"
    );
    assert!(
        !consts.iter().any(|r| r[3] == "/mut"),
        "let binding's string must never be folded: {consts:?}"
    );

    // spread: `extra` still lands (a plain property), the spread base is
    // counted loudly and never followed (no field named for it here).
    let spread_ent = entities
        .iter()
        .find(|r| r[2] == "spread")
        .expect("spread const entity");
    assert!(consts
        .iter()
        .any(|r| r[1] == spread_ent[1] && r[2] == "extra" && r[3] == "/extra"));
}
...
fn rust_const_str_and_df_lit_populate() {
    ...
    let entities = eng.rel_rows("type_entity", 7);
    let consts = eng.rel_rows("const_value", 7);
    let home_ent = entities
        .iter()
        .find(|r| r[2] == "HOME")
        .expect("Rust HOME const entity");
    assert_eq!(home_ent[3], "const");
    let home_row = consts
        .iter()
        .find(|r| r[1] == home_ent[1])
        .expect("HOME const_value row");
    ...
    assert!(
        lits.iter().any(|r| r[1] == "/mut"),
        "df_lit is unconditional on const/let: {lits:?}"
    );
    assert!(
        lits.iter()
            .any(|r| r[2] == "template" && r[1].contains("${")),
        "{lits:?}"
    );
}
...
fn const_value_rev_carries_rev_column_and_reserved_name_rejected() {
    ...
    assert!(
        rev_rows.iter().any(|r| r[7] == crate::util::NO_HEAD_REV),
        "{rev_rows:?}"
    );
        ...
        Ok((prog, diags, _)) => {
            ...
            assert!(
                tick_err.is_err()
                    || diags
                        .iter()
                        .any(|dd| dd.severity == sprefa_v5::ast::Severity::Error),
