...
fn head_var_from_triple_metavar_names_the_fix() {
    ...
    assert!(
        err.contains("not bound by any source op"),
        "keeps the base message:\n{err}"
    );
    assert!(
        err.contains("$$$B is pattern structure only"),
        "appends the $$$ note:\n{err}"
    );
...
fn unbound_constraint_var_names_the_head_fix() {
    ...
    assert_ne!(
        code, 0,
        "the unbound constraint var is a hard error:\n{err}"
    );
    assert!(
        err.contains("unbound var l2 in constraint"),
        "keeps the base message:\n{err}"
    );
    assert!(
        err.contains("put the expression in the rule head"),
        "appends the head note:\n{err}"
    );
    let _ = fs::remove_dir_all(&d);
}
...
fn lowercase_metavar_warns_under_check() {
    ...
    assert!(
        ...
        "warn code surfaces under --check:\n{err}"
    );
    assert!(
        err.contains("$body is literal text"),
        "names the offending metavar:\n{err}"
    );
