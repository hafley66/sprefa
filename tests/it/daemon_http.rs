...
fn http_json_published_then_removed_on_stop() {
    ...
    assert!(sb.wait_ready(), "daemon not ready / http.json never published");

    let info = read_http_json(&sb.http_json()).expect("http.json present");
    assert!(info["port"].as_u64().is_some_and(|p| p > 0), "http.json has a live port: {info}");
    assert_eq!(info["pid"].as_u64(), Some(child.id() as u64), "http.json pid = daemon pid: {info}");
    assert!(info["build_id"].as_str().is_some_and(|s| !s.is_empty()), "http.json build_id: {info}");
    ...
    sb.shutdown();
    let st = child.wait_timeout(std::time::Duration::from_secs(5)).expect("daemon exit");
    assert!(st.success(), "daemon exits 0 on shutdown");
    assert!(!sb.http_json().exists(), "http.json removed by shutdown_cleanup");
}
...
fn rpc_over_http_equals_uds_result() {
    ...
    assert!(http_v["result"]["results"][0]["rows"].as_array().is_some_and(|r| r.len() == 2),
...
fn health_answers_under_engine_lock_contention() {
    ...
    for _ in 0..60 {
        ...
        assert_eq!(status, 200, "health must stay 200 under engine-lock contention: {body}");
        let v: serde_json::Value = serde_json::from_str(&body).expect("health json");
        assert!(v["build_id"].as_str().is_some_and(|s| !s.is_empty()), "health build_id: {v}");
        assert!(v["pid"].as_u64().is_some(), "health pid: {v}");
        assert!(v["roots"].as_u64().is_some(), "health roots count: {v}");
        ok += 1;
        std::thread::sleep(std::time::Duration::from_millis(5));
    }
    assert_eq!(ok, 60, "all /health calls answered while the engine lock was contended");
