...
fn fn_catalog_is_self_describing_and_complete() {
    ...
    let rows = eng
        .query_sql(
            "SELECT \"name\",\"arity\",\"group\",\"doc\" FROM rel_fn_catalog_txt",
            &[],
        )
        .unwrap();
    assert!(
        rows.iter().any(|r| s(&r[0]) == "split"),
        "fn_catalog lists split"
    );
    assert!(
        rows.iter().any(|r| s(&r[0]) == "replace_re"),
        "fn_catalog lists the new replace_re"
    );
    for r in &rows {
        ...
        assert!(
            !doc.contains('|'),
            "{name} doc has a pipe, would break the md table: {doc}"
        );
    }
}
...
fn rel_catalog_is_self_describing_and_complete() {
    ...
    let rows = eng
        .query_sql(
            "SELECT \"name\",\"group\",\"cols\",\"doc\" FROM rel_rel_catalog_txt",
            &[],
        )
        .unwrap();
    ...
    for r in &rows {
        ...
        assert!(!doc.is_empty(), "{name} has no doc");
        assert!(
            cols.starts_with('(') && cols.ends_with(')'),
            "{name} cols malformed: {cols}"
        );
        assert!(
            !doc.contains('|'),
            "{name} doc has a pipe, would break the md table: {doc}"
    ...
    assert_eq!(
        imp.1, "(impl, iface)",
        "catalog cols come from the real decl"
    );
    assert!(
        imp.2.contains("is_implementation"),
        "scip_impl doc: {}",
        imp.2
    );
}
