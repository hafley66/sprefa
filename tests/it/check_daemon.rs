...
fn forced_inprocess_check_shares_the_daemon_db() {
    ...
    let _daemon = sb.spawn();
    assert!(sb.wait_ready(), "singleton daemon not ready");
    assert_eq!(sb.root_dbs().len(), 1, "daemon must have opened its root db");
    ...
    assert!(!stderr.to_lowercase().contains("database is locked"),
    ...
    assert!(!sb.cache_db_path().exists(), "no cache.db world under contention either");
...
fn hook_check_fallback_is_read_only() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(out.status.success(), "no-db hook check is a warned no-op, exit 0: {stderr}");
    assert_eq!(sb.root_dbs().len(), 0, "a hook check must never cold-build the root db: {stderr}");
    ...
    let out = run_check_with(&["--max-wall", "30"]);
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert_eq!(out.status.code(), Some(2), "read-only fallback must render the committed rail rows: {stderr}");
    assert!(stderr.contains("rail-ro"), "the committed diag row renders: {stderr}");
    assert_eq!(fs::read(&db).expect("re-read db"), bytes_before,
