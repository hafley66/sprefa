...
fn sandbox(tag: &str) -> PathBuf {
    ...
    .unwrap();
    fs::write(
        dir.join("plans/2026-01-01-alpha.md"),
        "# Alpha plan\n\n## Context\nbody text.\n\n\
...
fn dl(dir: &Path, args: &[&str]) -> (String, i32) {
    let prog = fs::read_to_string(
        Path::new(env!("CARGO_MANIFEST_DIR")).join("examples/gen-plans-index.dl"),
    )
    .unwrap();
...
fn todo_comments_round_trip_into_plans_md() {
    ...
    assert!(
        plans.contains("- `perf` plans/2026-01-01-alpha.md:6 — speed up the alpha pass"),
        "perf row:\n{plans}"
    );
    // Multi-line body flattened to one row.
    assert!(
        plans.contains(
            "- `decision` plans/2026-01-01-alpha.md:7 — multi-line body spanning two lines"
        ),
        "flattened multi-line row:\n{plans}"
    );
    // Per-plan section carries the same items keyed by plan.
    assert!(
        plans.contains("- plans/2026-01-01-alpha.md:6 `perf` — speed up the alpha pass"),
        "by-plan row:\n{plans}"
    );
    // A fenced code sample is inline content, not an html_block — never a row.
    assert!(
        !plans.contains("fenced sample"),
        "fenced sample leaked:\n{plans}"
    );
    // Hand-owned prose outside the markers survives.
    assert!(
        plans.contains("hand-owned prose survives regen"),
        "prose lost:\n{plans}"
    );
    assert!(
        plans.contains("- feature src/fixture.rs:1 — wire the fixture"),
        "code todo row:\n{plans}"
    );
    assert!(
        plans.contains("- untriaged debt: 1 bare TODO/FIXME comments in src"),
        "code debt row:\n{plans}"
    );
...
fn code_todo_drift_rail_fires_on_stale_index() {
    ...
    assert!(
        out2.contains("plans-index-drift"),
        "code drift code:\n{out2}"
    );
}
