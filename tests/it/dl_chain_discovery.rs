...
fn subdir_with_no_dl_inherits_the_ancestor_chain() {
    ...
    write(&ws.join(".dl/rails.dl"), "rel top(x: text).\ntop(\"repo-root\").\n? top(x).\n");
    let deep = ws.join("sub/nested/deep");
    fs::create_dir_all(&deep).unwrap();

    // Opened `sub/nested/deep` (no `.dl/`), yet the repo-root rail loads.
    let out = run(&deep);
    assert!(out.contains("repo-root"), "ancestor .dl loaded from a subdir: {out}");
...
fn nested_dl_dirs_merge_up_to_the_git_root() {
    ...
    fs::create_dir_all(ws.join(".git")).unwrap();
    write(&ws.join(".dl/root.dl"), "rel a(x: text).\na(\"root-layer\").\n? a(x).\n");
    write(&ws.join("sub/.dl/mid.dl"), "rel b(x: text).\nb(\"mid-layer\").\n? b(x).\n");
...
fn dl_above_the_git_root_is_the_boundary() {
    ...
    write(&outer.join(".dl/outer.dl"), "rel leaked(x: text).\nleaked(\"above-git\").\n? leaked(x).\n");
    let repo = outer.join("repo");
    fs::create_dir_all(repo.join(".git")).unwrap();
    write(&repo.join(".dl/repo.dl"), "rel inside(x: text).\ninside(\"in-repo\").\n? inside(x).\n");
    let deep = repo.join("pkg/mod");
    fs::create_dir_all(&deep).unwrap();

    let out = run(&deep);
    assert!(out.contains("in-repo"), "repo .dl loads: {out}");
    assert!(!out.contains("above-git"), "the .dl above the git root is the boundary: {out}");
