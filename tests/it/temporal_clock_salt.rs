...
fn wildcard_clock_bucket_salts_the_request_digest() {
    ...
    assert!(diags.is_empty(), "wildcard clock form must typecheck clean: {diags:?}");
    ...
    set_now(6010);
    eng.tick(&prog, true).unwrap();
    assert_eq!(count(&dbp, "pending_effect"), 1, "same bucket: digest stable, dedup holds");
    ...
    set_now(6060);
    eng.tick(&prog, true).unwrap();
    assert_eq!(count(&dbp, "pending_effect"), 2, "a fresh bucket mints a fresh request id");
    ...
    set_now(6061);
    eng.tick(&prog, true).unwrap();
    assert_eq!(count(&dbp, "rel_resp"), 1, "no bucket column: identical responses collapse");
    clear_now();
}
...
fn unused_var_bucket_is_wildcard_equivalent() {
    ...
    set_now(6060);
    eng.tick(&prog, true).unwrap();
    assert_eq!(count(&dbp, "pending_effect"), 2, "unused bucket var salts like a wildcard");
    clear_now();
}
...
fn bucket_flip_invalidates_zero_derived_rows() {
    ...
    assert_eq!(count(&dbp, "rel_resp"), 1, "identical response landed no new row");
    assert_eq!(count(&dbp, "rel_seen"), 1, "downstream derived rowset untouched");
    ...
    assert!(
        ...
        eng.last_derived_rebuilt, eng.last_derived_skipped
    );
    clear_now();
}
