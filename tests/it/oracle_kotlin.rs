...
fn kotlin_module_edge_is_subset_of_scip() {
    ...
    let truth: HashSet<_> = scip_edges(&index).into_iter()
    ...
    eprintln!("[oracle:kotlin] our edges={} scip edges={} matched={} precision={:.2} recall={:.2}",
        ours.len(), truth.len(), matched.len(),
        if ours.is_empty() { 1.0 } else { matched.len() as f64 / ours.len() as f64 },
        if truth.is_empty() { 1.0 } else { matched.len() as f64 / truth.len() as f64 });
    ...
    assert!(extra.is_empty(), "every emitted edge must be a real scip edge; extras: {extra:?}");
    ...
    assert!(ours.contains(&("src/main/kotlin/com/app/Main.kt".into(), "src/main/kotlin/com/lib/Util.kt".into())), "{ours:?}");
    assert!(ours.contains(&("src/main/kotlin/com/lib/Helper.kt".into(), "src/main/kotlin/com/lib/Util.kt".into())), "{ours:?}");
}
