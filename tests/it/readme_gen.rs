...
fn sandbox(tag: &str) -> PathBuf {
    ...
    fs::write(
        dir.join("std/hello.dl"),
        "# Standard library: a greeting relation.\nrel hello(value: text).\n",
    )
    .unwrap();
    dir
}
...
fn dl(dir: &Path, args: &[&str]) -> (String, i32) {
    let program =
        fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join("examples/gen-readme.dl"))
            .unwrap();
...
fn readme_zones_round_trip_and_detect_both_directions_of_drift() {
    ...
    assert!(
        generated.contains("Ask a real question"),
        "gallery missing:\n{generated}"
    );
    assert!(
        generated.contains("[`std/hello.dl`](std/hello.dl) | a greeting relation."),
        "stdlib missing:\n{generated}"
    );
    for prose in [
        "hand prose before",
        ...
        assert!(
            generated.contains(prose),
            "hand-owned prose changed: {prose}\n{generated}"
        );
    }
