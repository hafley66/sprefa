...
"#;
    ...
    assert!(out.contains("app/engine/01-dispatch/00-route"), "url 4:\n{out}");
    assert!(!out.contains("fake"), "string literal leaked:\n{out}");

    // arch_parent: each multi-segment url's parent is the prefix minus last seg.
    assert!(out.contains("app/cli/00-main\tapp/cli"), "parent shallow:\n{out}");
    assert!(out.contains("app/engine/01-dispatch/00-route\tapp/engine/01-dispatch"), "parent deep:\n{out}");
    ...
    assert!(out.contains("app/engine/01-dispatch/00-route\t00-route"), "last seg:\n{out}");
}
