...
fn ping_gets_pong() {
    let d = sandbox("ping");
    let (code, msgs, err) = serve(&d, ECHO, &[r#"{"jsonrpc":"2.0","id":1,"method":"ping"}"#]);
...
fn unknown_method_hits_fallback() {
    ...
    let (code, msgs, err) = serve(&d, ECHO, &[r#"{"jsonrpc":"2.0","id":3,"method":"nope"}"#]);
    assert_eq!(code, 0, "{err}");
...
fn unanswered_request_gets_error() {
    ...
    let (code, msgs, err) = serve(&d, prog, &[r#"{"jsonrpc":"2.0","id":7,"method":"nope"}"#]);
    assert_eq!(code, 0, "{err}");
...
fn answered_rows_retire_between_requests() {
    ...
    assert_eq!(
        msgs.len(),
        2,
        "one response per request, no re-answers: {msgs:?}"
    );
    assert_eq!(
        result_of(&msgs, 1).unwrap()["result"],
        serde_json::json!("pong")
    );
    assert_eq!(
        result_of(&msgs, 2).unwrap()["result"],
        serde_json::json!("two")
    );
}
...
fn rpc_envelope_checked_at_declare() {
    ...
    assert!(
        err.contains("method"),
        "message names the missing column, got: {err}"
    );
}
...
fn tools_list_advertises_builtins() {
    ...
    assert_eq!(code, 0, "{err}");
    let names: Vec<&str> = result_of(&msgs, 1).expect("tools/list response")["result"]["tools"]
        .as_array()
        .expect("tools array")
        .iter()
...
fn tool_dl_what_round_trips() {
    ...
    assert!(
        payload["total"].as_u64().is_some(),
        "count-first total missing: {payload}"
    );
    let flat = serde_json::to_string(&payload["rows"]).unwrap();
    assert!(
        flat.contains("type_entity"),
        "type family hit missing: {payload}"
    );
}
...
fn tool_dl_verb_who_calls() {
    ...
    assert!(
        flat.contains("find"),
        "who-calls tool missing caller `find`: {payload}"
    );
}
...
fn tool_dl_rows_carries_total() {
    ...
    let flat = serde_json::to_string(&payload["rows"]).unwrap();
    assert!(
        flat.contains("who-calls"),
        "rows missing who-calls: {payload}"
    );
}
...
fn initialize_gap_filled_generically() {
    ...
    let m = msgs
        .iter()
        .find(|m| m.get("id") == Some(&serde_json::json!("init-1")))
        .unwrap_or_else(|| panic!("no response for init-1: {msgs:?}"));
    assert!(
        m.get("error").is_none(),
        "generic initialize must not error: {m}"
    );
    assert_eq!(
        m["result"]["protocolVersion"],
...
fn mcp_without_ports_bails() {
    ...
    assert!(
        err.contains("@in(rpc)"),
        "guidance names the port decls, got: {err}"
    );
}
