...
fn go_interface_dispatch_finds_function_on_two_op_paths() {
    ...
    std::fs::copy(
        PathBuf::from(FLOW).join("dispatch_flow.dl"),
        root.join("dispatch_flow.dl"),
    )
    .unwrap();
    ...
    assert!(
        ops.iter().any(|r| r == &["getPet"]) && ops.iter().any(|r| r == &["createPet"]),
    ...
    assert!(
        entries.iter().any(|r| r.first() == Some(&"getPet")
            && r.get(1).is_some_and(|m| m.contains("PetAPI#getPet"))),
    ...
    assert!(
        multi
            .iter()
            .any(|r| r.first().is_some_and(|f| f.contains("httpExec"))),
