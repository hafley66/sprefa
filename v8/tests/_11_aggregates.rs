...
//! `oracle/eval/json_terms.pl`, extended with the `sum`, `min` and `max`
//! aggregate keys beside `count`.

use serde_json::{json, Value};
use std::path::{Path, PathBuf};
use std::process::Command;

/// The four aggregate head atoms, as they appear in `aggregate(Kind, Arg)`.
...
/// `arg_json/2`: a variable becomes `{"v": identity}`, an aggregate argument
/// `{"<kind>": arg}`, every other argument its own term.
fn arg_json(value: &Value) -> Value {
    match value.get("f").and_then(Value::as_str) {
        ...
        Some("aggregate") => match aggregate_subject(value) {
            Some((tag, subject)) => json!({ tag: arg_json(&subject) }),
            None => value.clone(),
        },
        _ => value.clone(),
    }
...
/// The `(name, subject)` of `aggregate(Name, Subject)` when `Name` is one of the
/// four aggregate atoms.
fn aggregate_subject(value: &Value) -> Option<(&'static str, Value)> {
    let args = value.get("args").and_then(Value::as_array)?;
    if args.len() != 2 {
        return None;
    }
    let name = args[0].get("a").and_then(Value::as_str)?;
    if !aggregate_key(name) {
        return None;
    }
    let tag = match name {
        "count" => "count",
