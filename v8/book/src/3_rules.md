...
Parity with v7 `evaluate/4` is the stated contract of `_6_eval` (`v8/README.md:15`), and the prolog oracles under `oracle/eval/` are its receipts.
...
```dl7
; fixture: v8/oracle/compile/sources/test/fixtures/12_native_runtime.dl7
(: Input
   (* (: value int)
      (: same int)
      (: tag int)))
...
```prolog
% fixture: v8/oracle/eval/0_transitive.pl
program(
