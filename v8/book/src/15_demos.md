...
- the machine has no `sprefa-extract` binary: the extract demo errors, `v8/README.md:46-59` lists where it is searched
...
```dl7
; fixture: v8/fixtures/hosts/org.dl7
...
```dl7
; fixture: v8/fixtures/extract/main.dl7
...
| `sh fetch(ep, prev, bucket) -> (status, tag, stars, full_name)` | 15-17 | `fetch_json(url, body)`: no request headers, no status on success, body as one text term | `v8/README.md:118`, `:128` |
