...
validate_integer_comparison_argument(RuleId, Name, Argument) :-
    integer_comparison_argument_diagnostic(
        Name, RuleId, Argument, Diagnostic),
    throw(sqlite_query_error(Diagnostic)).
...
lower_integer_comparison_goals(
    ...
    scalar_expression(Left, Bindings, RuleId, Name, LeftExpression),
    scalar_expression(Right, Bindings, RuleId, Name, RightExpression),
    integer_comparison_operator(Name, Polarity, Operator),
    Predicate = comparison(Operator, LeftExpression, RightExpression),
    lower_integer_comparison_goals(
        Goals, Bindings, RuleId, Predicates).

scalar_expression(var(Variable), Bindings, RuleId, Name, Expression) :-
...
scalar_expression(const(Value), _, _, _, literal(Value)).
...
render_predicate(comparison(Operator, Left, Right), Sql) :-
    render_expression(Left, LeftSql),
    render_expression(Right, RightSql),
    comparison_sql_token(Operator, Token),
    format(string(Sql), "~s ~s ~s", [LeftSql, Token, RightSql]).
render_predicate(not_null(Expression), Sql) :-
    render_expression(Expression, ExpressionSql),
    format(string(Sql), "~s IS NOT NULL", [ExpressionSql]).

comparison_sql_token(lt, "<").
comparison_sql_token(le, "<=").
comparison_sql_token(eq, "=").
comparison_sql_token(ne, "!=").
comparison_sql_token(ge, ">=").
comparison_sql_token(gt, ">").
