...
:- use_module('../src/1_libtime/00_integer_comparison',
...
test(migrated_int_lt_ordering_products_have_exact_ranks) :-
    compile_dl7('v7/test/fixtures/2_partial.dl7',
                Rows, Runtime, Diagnostics),
    rank_pairs(Rows, key_rank, 1, 2, KeyRanks),
    rank_pairs(Rows, pick_rank, 2, 3, PickRanks),
    rank_pairs(Rows, exclude_rank, 2, 3, ExcludeRanks),
    Runtime = checked_datalog(
                  _, datalog_program(_, _, RuntimeRules), _, _),
    named_owner(Rows, curry_unbound, CurryUnbound),
    named_owner(Rows, curry_predecessor, CurryPredecessor),
    named_owner(Rows, curry_has_predecessor, CurryHasPredecessor),
    named_owner(Rows, curry_rank, CurryRank),
    named_owner(Rows, intersection_edge_candidate, IntersectionEdge),
    named_owner(Rows, intersection_predecessor, IntersectionPredecessor),
    named_owner(Rows, intersection_has_predecessor,
                IntersectionHasPredecessor),
    named_owner(Rows, intersection_rank, IntersectionRank),
    include(rule_head_is_one_of(
                [ CurryPredecessor, CurryHasPredecessor, CurryRank,
                  IntersectionPredecessor, IntersectionHasPredecessor,
                  IntersectionRank
                ]),
            RuntimeRules, OrderingRules),
    OrderingSeeds =
        [ call(ref(CurryUnbound),
               [ref(source), ref(curry_result), const(a), ref(type),
                const(0)]),
          call(ref(CurryUnbound),
               [ref(source), ref(curry_result), const(c), ref(type),
                const(2)]),
          call(ref(IntersectionEdge),
               [ ref(left), ref(right), const("left"), const(a), ref(type),
                 const(0)
               ]),
          call(ref(IntersectionEdge),
               [ ref(left), ref(right), const("left"), const(c), ref(type),
                 const(2)
               ]),
          call(ref(IntersectionEdge),
               [ ref(left), ref(right), const("right"), const(b), ref(type),
                 const(1)
               ]),
          call(ref(IntersectionEdge),
               [ ref(left), ref(right), const("right"), const(d), ref(type),
                 const(3)
               ])
        ],
    evaluate(OrderingRules, OrderingSeeds,
             OrderingClosure, OrderingDiagnostics),
    findall(SourceIndex-ResultIndex,
            member(call(ref(CurryRank),
                        [ ref(curry_result), const(SourceIndex),
                          const(ResultIndex)
                        ]),
                   OrderingClosure),
            CurryRanks),
    findall(Side-SourceIndex-ResultIndex,
            member(call(ref(IntersectionRank),
                        [ ref(left), ref(right), const(Side),
                          const(SourceIndex), const(ResultIndex)
                        ]),
                   OrderingClosure),
            IntersectionRanks),
    Observed = ordering_ranks(
                   diagnostics(Diagnostics, OrderingDiagnostics),
                   key(KeyRanks), pick(PickRanks), exclude(ExcludeRanks),
                   curry(CurryRanks), intersection(IntersectionRanks)),
    Observed == ordering_ranks(
                    diagnostics([], []),
                    key([0-0, 2-1]),
                    pick([0-0, 1-1]),
...
test(checked_program_reifies_as_target_neutral_rows) :-
    ...
    memberchk(program_argument(call_id(rule(0), head), 0, HeadArgument),
              Rows1),
    memberchk(program_edge(HeadArgument, _, _, 0), Rows1),
    !.
...
test(evaluation_index_isolates_simultaneous_evaluation_ids) :-
    IdA = test_index_isolate_first,
    IdB = test_index_isolate_second,
    Relation = ref(kernel(isolated)),
    RowA = call(Relation, [const(a)]),
    RowB = call(Relation, [const(b)]),
    dl7_evaluator:install_lower_rows([RowA], IdA, [ReferenceA]),
    dl7_evaluator:install_lower_rows([RowB], IdB, [ReferenceB]),
    setup_call_cleanup(
        true,
        once(( findall(Row, dl7_evaluator:evaluation_lower(IdA, Relation, Row),
                       BeforeA),
               findall(Row, dl7_evaluator:evaluation_lower(IdB, Relation, Row),
                       BeforeB),
               erase(ReferenceA),
               findall(Row, dl7_evaluator:evaluation_lower(IdA, Relation, Row),
                       AfterA),
               findall(Row, dl7_evaluator:evaluation_lower(IdB, Relation, Row),
                       AfterB)
             )),
        ( ( catch(erase(ReferenceA), _, true) ; true ),
          ( catch(erase(ReferenceB), _, true) ; true ) )),
    BeforeA == [RowA],
    BeforeB == [RowB],
    AfterA == [],
    AfterB == [RowB].
...
evaluator_trace_probe(Rules, Seeds) :-
    ...
    Seeds = [ call(ref(seed), [const(a)]),
              call(ref(seed), [const(b)]) ].
...
trace_phase_steps(Steps, PhaseSteps) :-
    findall(Phase-Step,
            ...
            PhaseSteps0),
    msort(PhaseSteps0, PhaseSteps).

integer_metric(Metrics, Name, Value) :-
    memberchk(metric(Name, Value), Metrics),
    integer(Value).

test(evaluator_trace_is_gated_on_an_active_compile_trace) :-
    evaluator_trace_probe(Rules, Seeds),
    with_dl7_trace('steps', (
        reset_compile_trace,
        once(evaluate([], [], _, _)),
        aggregate_all(count,
                      dl7_compiler_tracer:compile_step_row(_, _, _, _, _),
                      StandaloneRows),
        with_compile_trace(
            dl7_evaluator_trace_gate_probe,
            ( evaluate(Rules, Seeds, _, _),
              collected_compile_steps(Steps) ))
    )),
    StandaloneRows == 0,
    trace_phase_steps(Steps, PhaseSteps),
    PhaseSteps == [ evaluator-evaluate_cleanup(0),
                    evaluator-evaluate_collect(0),
                    evaluator-evaluate_install(0)
                  ],
    memberchk(step(_, evaluator, evaluate_install(0), _, InstallMetrics), Steps),
    memberchk(metric(stratum_rules, 1), InstallMetrics),
    memberchk(metric(stratum_seeds, 2), InstallMetrics),
    memberchk(metric(stratum_lower_rows, 0), InstallMetrics),
    memberchk(step(_, evaluator, evaluate_cleanup(0), _, CleanupMetrics), Steps),
    memberchk(metric(erased_clauses, 3), CleanupMetrics),
    memberchk(metric(leftover_lower_rows, 0), CleanupMetrics).

test(evaluator_trace_reports_stratum_metrics) :-
    evaluator_trace_probe(Rules, Seeds),
    with_dl7_trace('collect', (
        with_compile_trace(
            dl7_evaluator_metric_probe,
            ( evaluate(Rules, Seeds, _, _),
              collected_compile_steps(Steps) ))
    )),
    memberchk(step(_, evaluator, evaluate_install(0), _, InstallMetrics), Steps),
    InstallMetrics == [ metric(stratum_rules, 1),
                        metric(stratum_seeds, 2),
                        metric(stratum_lower_rows, 0)
                      ],
    memberchk(step(_, evaluator, evaluate_collect(0), _, CollectMetrics), Steps),
    memberchk(metric(stratum_closure_rows, 5), CollectMetrics),
    integer_metric(CollectMetrics, global_table_answers, _),
    integer_metric(CollectMetrics, global_complete_calls, _),
    integer_metric(CollectMetrics, global_table_space_bytes, _),
    memberchk(step(_, evaluator, evaluate_cleanup(0), _, CleanupMetrics), Steps),
    CleanupMetrics == [ metric(erased_clauses, 3),
...
test(integer_comparison_family_positive_and_grounded_negative_are_exact) :-
    findall(Result,
            ( integer_comparison_case(Name, TrueLeft, TrueRight,
                                      FalseLeft, FalseRight),
              integer_comparison_evaluation(
                  Name, TrueLeft, TrueRight, FalseLeft, FalseRight,
                  Result)
            ),
            Observed),
    findall(Expected,
            ( integer_comparison_case(Name, _, _, _, _),
              integer_comparison_expected(Name, Expected)
            ),
            ExpectedResults),
    Observed == ExpectedResults.
...
test(integer_comparison_family_modes_and_integer_arguments_are_exact) :-
    findall(
        comparison_check(Name, UnderboundDiagnostics,
                         NonIntegerDiagnostics, NegativeDiagnostics),
    ...
    findall(
        comparison_check(
            ...
            [diagnostic(check, none,
                        underconstrained_kernel_goal(Name, [[0, 1]]))],
            [diagnostic(check, none,
                        kernel_argument_type_mismatch(
                            Name, 0, int, const("2")))],
            []),
...
test(kernel_inventory_contains_exact_integer_comparison_family) :-
    ...
    Observed == kernel_inventory(
                    ...
                    relation_rows(
                        [ relation(ref(kernel(node)), 1, []),
                          relation(ref(kernel(module)), 1, []),
                          relation(ref(kernel(product)), 1, []),
                          relation(ref(kernel(sum)), 1, []),
                          relation(ref(kernel(':')), 4,
                                   [[0, 1], [0, 3]]),
                          relation(ref(kernel(edge_snapshot)), 4,
                                   [[0, 1], [0, 3]]),
                          relation(ref(kernel(nil)), 1, [[0]]),
                          relation(ref(kernel(cons)), 3, [[0, 1], [2]]),
                          relation(ref(kernel(edge_ref)), 3, [[0, 1]]),
...
rank_pairs(Rows, Name, SourcePosition, ResultPosition, Pairs) :-
    named_owner(Rows, Name, Relation),
    findall(SourceIndex-ResultIndex,
            ( member(call(ref(Relation), Arguments), Rows),
              nth0(SourcePosition, Arguments, const(SourceIndex)),
              nth0(ResultPosition, Arguments, const(ResultIndex))
            ),
            Pairs).
...
integer_comparison_case(int_lt, 2, 5, 5, 2).
integer_comparison_case(int_le, 2, 2, 5, 2).
integer_comparison_case(int_eq, 2, 2, 2, 5).
integer_comparison_case(int_ne, 2, 5, 2, 2).
integer_comparison_case(int_ge, 5, 2, 2, 5).
integer_comparison_case(int_gt, 5, 2, 2, 5).

integer_comparison_evaluation(
    Name, TrueLeft, TrueRight, FalseLeft, FalseRight,
    comparison_result(Name, Diagnostics, Closure)) :-
    Rules =
        [ rule(call(ref(positive_true(Name)), [const(ok)]),
               [checked_goal(
                    positive,
                    call(ref(kernel(Name)),
                         [const(TrueLeft), const(TrueRight)]))]),
          rule(call(ref(positive_false(Name)), [const(no)]),
               [checked_goal(
                    positive,
                    call(ref(kernel(Name)),
                         [const(FalseLeft), const(FalseRight)]))]),
          rule(call(ref(negative_true(Name)), [const(ok)]),
               [checked_goal(
                    negative,
                    call(ref(kernel(Name)),
                         [const(FalseLeft), const(FalseRight)]))]),
          rule(call(ref(negative_false(Name)), [const(no)]),
               [checked_goal(
                    negative,
                    call(ref(kernel(Name)),
                         [const(TrueLeft), const(TrueRight)]))])
...
integer_comparison_expected(
    Name,
    comparison_result(
        Name, [],
        [ call(ref(kernel(nil)), [const([])]),
          call(ref(negative_true(Name)), [const(ok)]),
          call(ref(positive_true(Name)), [const(ok)])
        ])).
