...
test(nearest_shadow_cold_boundaries) :-
    nearest_shadow_profile(Profile),
    ColdAt = measurement(3000, 16000000, 810, 0, 0, 0, []),
    Warm = measurement(1, 2148, 810, 0, 0, 0, []),
    profile_failures(Profile, ColdAt, Warm, 0, out, out, AtFailures),
    AtFailures == [],
    ColdOver = measurement(3001, 16000001, 810, 0, 0, 0, []),
...
test(nearest_shadow_row_and_diagnostic_boundaries) :-
    nearest_shadow_profile(Profile),
    Cold = measurement(1, 1, 810, 0, 0, 0, []),
    Warm = measurement(1, 1, 810, 0, 0, 0, []),
    profile_failures(Profile, Cold, Warm, 0, out, out, []),
    WrongRows = measurement(1, 1, 811, 0, 0, 0, []),
    profile_failures(Profile, WrongRows, Warm, 0, out, out,
                     [compiler_row_checkpoint(811, 810)]),
    Diagnostic = diagnostic(evaluate, none, probe),
    BadCold = measurement(1, 1, 810, 0, 0, 0, [Diagnostic]),
    profile_failures(Profile, BadCold, Warm, 0, out, out,
                     [cold_diagnostics([Diagnostic])]),
    BadWarm = measurement(1, 1, 810, 0, 0, 0, [Diagnostic]),
...
test(nearest_shadow_warm_boundaries_and_output_parity) :-
    nearest_shadow_profile(Profile),
    Cold = measurement(1, 1, 810, 0, 0, 0, []),
    WarmAt = measurement(1, 5000, 810, 0, 0, 0, []),
    profile_failures(Profile, Cold, WarmAt, 0, out, out, []),
    WarmOver = measurement(1, 5001, 810, 0, 0, 0, []),
...
% The 2_partial budgets and order are preserved; the row checkpoint advances
% by the same exact kernel metadata delta as nearest-shadow.
test(partial_boundaries) :-
    partial_profile(Profile),
    ColdAt = measurement(1, 88000000, 15562, 0, 0, 0, []),
    WarmAt = measurement(1, 50000, 15562, 0, 0, 0, []),
    profile_failures(Profile, ColdAt, WarmAt, 8, out, out, []),
    ColdOver = measurement(1, 88000001, 15563, 0, 0, 0, []),
    profile_failures(Profile, ColdOver, WarmAt, 9, out, out,
                     [ cold_inference_budget(88000001, 88000000),
                       compiler_row_checkpoint(15563, 15562),
...
test(over_budget_measurements_exit_nonzero) :-
    nearest_shadow_profile(Profile),
    Cold = measurement(9999, 99999999, 810, 0, 0, 0, []),
    Warm = measurement(1, 2148, 810, 0, 0, 0, []),
...
test(over_budget_subprocess_exits_nonzero) :-
    ...
    format(string(Goal),
           "use_module(~q),\c
            ...
            profile_failures(P, measurement(9999,99999999,810,0,0,0,[]),\c
                             measurement(1,2148,810,0,0,0,[]), 0, out, out, F),\c
